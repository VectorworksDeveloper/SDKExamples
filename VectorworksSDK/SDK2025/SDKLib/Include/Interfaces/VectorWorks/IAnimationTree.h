...
struct Image
{
	...
	int padding = 0; // <-- unused bytes at end of row to the next row
	int width = 0;
	int height = 0;
	int channels = 0;
	bool rFirst = true; // as in RGB vs BGR
	bool dynamic = false;
...
public:
	...
	virtual void AttachGeometry(Id node, const InstanceBranch& geometryH, glm::dmat4* transform = nullptr) = 0;
	virtual void AttachLight(Id node, const InstanceBranch& lightH, glm::dmat4* transform = nullptr) = 0;

	virtual void AttachGeometry(Id node, const InstanceBranchUUID& geometryH, glm::dmat4* transform = nullptr) = 0;
	virtual void AttachLight(Id node, const InstanceBranchUUID& lightH) = 0;
