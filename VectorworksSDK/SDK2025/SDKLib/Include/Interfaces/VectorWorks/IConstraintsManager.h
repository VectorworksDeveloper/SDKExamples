...
namespace VectorWorks
{
	...
	struct SConstraintData
	{
		...
		// Used for dynamic objects only (texts, graphic legend images)
		// Set to true if you want this object to have automatic constraints with other dynamic objects
		bool		fAutoConstrain		= true;

		// Used for dynamic objects only (texts, graphic legend images) in graphic legends
		// Set to true if you want this object to align in column if it is displayed more than once with different values
		// Valid for auto constrained objects only
		bool		fAlignInColumn		= false;
	};
	...
	using TAutoConstraintCheckCallback	= std::function<bool(MCObjectHandle hObject1, MCObjectHandle hObject2, bool checkTypes)>;

	using TDoubleSTLArray				= std::vector<double>;
	...
	class DYNAMIC_ATTRIBUTE ILabelDCMSession
	{
		...
		public:
			...
			virtual void			SetHorizAutoConstraintCheckCallback(TAutoConstraintCheckCallback callback) = 0;
			virtual void			SetVertAutoConstraintCheckCallback(TAutoConstraintCheckCallback callback) = 0;
			...
			virtual void			AddLocus(MCObjectHandle hLocus, const SConstraintData& constraintData) = 0;
			...
			virtual void			SetUseCustomFrameConstraints(bool useCustomFrameConstraint) = 0;

			virtual TDoubleSTLArray	GetColumnWidths() = 0;
			virtual void			SetColumnWidths(const TDoubleSTLArray& arrWidths) = 0;

			virtual void			SetAllowVerticalChainOverlap(bool allowVerticalChainOverlap) = 0;
			virtual void			SetNonConstrainedObjectsResizeBoundary(bool nonConstrainedObjectsResizeBoundary) = 0;

			virtual WorldRect		GetBoundary() = 0;
			virtual void			EvaluateWithBoundary(double& inOutWidth, double& inOutHeight) = 0;
			virtual WorldPt			GetTopLeftAdjustment() const = 0;
	};
	typedef std::shared_ptr<ILabelDCMSession>		ILabelDCMSessionPtr;
	typedef std::vector<ILabelDCMSessionPtr>		TLabelDCMSessionPtrArr;
	...
	class DYNAMIC_ATTRIBUTE IConstraintsManager : public IVWUnknown
	{
		public:
			...
			virtual bool				VCOM_CALLTYPE AddConstraints(MCObjectHandle paramObject,
																	MCObjectHandle hGroup,
																	ILabelDCMSessionPtr inOutDcmSessionPtr,
																	MCObjectHandle hSkippedObj = nullptr,
																	bool doNotChangeText = false,
																	bool bAllowUpdateConstrainedObjectWhenNotChangingText = false) = 0;
			...
			virtual bool				VCOM_CALLTYPE HandleColumnAlignment(const TLabelDCMSessionPtrArr& arrDCMSessions) = 0;
	};
