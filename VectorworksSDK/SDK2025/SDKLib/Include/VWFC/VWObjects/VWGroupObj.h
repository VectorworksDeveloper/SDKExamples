...
namespace VWFC
{
	namespace VWObjects
	{
		...
		public:
								VWGroupObj();
								VWGroupObj(MCObjectHandle hGroup, bool allowGroupLikeObjects = true);
		...
		public:
			...
			// searches for the first object satisfying the predicate in the child list
			// will descend into chosen group-like nodes if descendIntoGrouplike is true
			MCObjectHandle		FindObject(std::function<bool(MCObjectHandle)> pred, MCObjectHandle hAfter = nullptr, bool descendIntoGrouplike = false);
		...
		public:
			...
			static bool			IsGroupObject(MCObjectConstHandle hObj);
			static bool			IsGroupLikeObject(MCObjectConstHandle hObj);
