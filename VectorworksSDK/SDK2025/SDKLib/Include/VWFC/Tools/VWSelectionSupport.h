...
namespace VWFC
{
	namespace Tools
	{
		const InternalIndex kHeterogeneousStyles = -2;

		using TValueFlags	= Uint32;
		using TFlagsArray	= std::vector<TValueFlags>;
		using THandleUSet	= std::unordered_set<MCObjectHandle>;
		...
		public:
			THandleUSet	GetSelection() const;
		...
		public:
			...
			bool		UpdateParametric(MCObjectHandle hObject, bool updateSourceObj = false) const;
		...
		// style support
		public:
			void		OnStyleChange(MCObjectHandle hSymDef);
			void		SetEditedOnStyleChange(MCObjectHandle hSymDef);
			bool		IsStyleHomogeneousMultiSel() const;
		...
		private:
			MCObjectHandle			fhHostObject	= nullptr;
			InternalIndex			fStyleID		= 0;

			THandleUSet				fSelectedObjects;
