...
//=======================================================================================
// Construct from a std u16string.
TXString::TXString(const std::u16string& src)
	: charPtr(nullptr)
	, charBufSize(0)
...
TXString::TXString(const char16_t* src)
	: charPtr(nullptr)
	, charBufSize(0)
	, stdUStr(src ? (const TXChar *)src : txu(""))
{
...
//=======================================================================================
// Construct from an untrusted char16_t buffer, which may or may not be terminated.
// maxUCChars is typically the maximum size of the buffer, but could be the string length.
// If eLengthCheck is eYes, assume the worst and search for size of the string.
// If eLengthCheck is eNo, assume the string is exactly max in length.
TXString::TXString(const char16_t* src, size_t maxUCChars, ELengthCheck eLengthCheck)
	: charPtr(nullptr)
	, charBufSize(0)
	, stdUStr(
		src && eLengthCheck == ELengthCheck::eNo ? (const TXChar *)src : txu(""),
...
//=======================================================================================
// Fill the string with "count" number of "ch"
TXString::TXString(char16_t ch, size_t count /* 1 */)
...
//=======================================================================================
// Assignment from a const char16_t buffer. If 'src' is nullptr, the string is cleared.
TXString& TXString::operator=(const char16_t* src)
{
	if (src)
	{
		stdUStr = (const TXChar *)src;
...
//=======================================================================================
// Assignment from a char16_t
TXString& TXString::operator=(const char16_t ch)
{
	stdUStr = (const TXChar)ch;
	return *this;
}
...
//=======================================================================================
// Concatenates char16_t buffer. If 'src' is nullptr, does nothing.
TXString& TXString::operator<<(const char16_t* src)
{
	return (*this += src);
}
...
//=======================================================================================
// Insert char16_t buffer at position 'pos'. If pos is larger than length, the new
// string is append to the end.
TXString& TXString::Insert(size_t pos, const char16_t* src)
{
	if (src != nullptr)
	{
		if (pos < stdUStr.length())
...
// Insert one character 'ch' at position 'pos'. If pos is larger than length, the new
// character is append to the end.
TXString& TXString::Insert(size_t pos, const char16_t ch)
{
	if (pos < stdUStr.length())
		stdUStr.insert(pos, 1, (const TXChar)ch);
	else
...
TXString& TXString::MakeCapitalized(TXChar previousChar)
{
	...
	if (cfUniStr)
	{
		...
		if (previousChar)
		{
			...
			CFStringAppendCharacters(cfUniStr, &previousChar, 1);
...
//=======================================================================================
// Returns a pointer to const char16_t buffer.
TXString::operator const char16_t*() const
{
	return (const char16_t*)stdUStr.c_str();
}
...
//=======================================================================================
// Returns a std::u16string object
std::u16string TXString::GetStdU16String() const
{
	return std::u16string((const char16_t *)stdUStr.c_str());
}
...
// the buffer. '\0' will be added to the end of string even if buffer is not big enough.
void TXString::CopyInto(char16_t* dst, size_t bufElemSize) const
{
