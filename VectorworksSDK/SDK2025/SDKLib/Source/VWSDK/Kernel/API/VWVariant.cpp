...
VWVariant::VWVariant( const char* szString )
{
	this->Init();
	fType		= eVWVariantType_String;
...
bool VWVariant::operator==(const VWVariant& v) const
{
	bool result = false;
...
double VWVariant::GetDouble() const
{
		...
		)
	{
		...
		TXString::ToDouble( this->GetTXString(), result, true );
	}
...
float VWVariant::GetFloat() const
{
		...
		)
	{
		...
		double dResult = 0.0;
		TXString::ToDouble( this->GetTXString(), dResult, true );
...
void VWVariant::SetTXString(const TXString& value)
{
	switch( fType ) {
		...
		case eVWVariantType_Double:
			{
				double	theValue	= 0.0;
				TXString::ToDouble( value, theValue, true );
		...
		case eVWVariantType_Float:
			{
				double	dValue	= 0.0;
				TXString::ToDouble( value, dValue, true );
				float	theValue	= float(dValue);
