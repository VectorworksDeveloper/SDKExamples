...
VWRectangle2D::VWRectangle2D(const WorldRect& worldRect)
	: VWRectangle2D( worldRect.TopLeft(), worldRect.BotRight() )
{}
...
VWLine2D VWRectangle2D::GetLeftEdgeCW() const
{
	return { this->GetBottomLeft(), this->GetTopLeft() };
}

VWLine2D VWRectangle2D::GetRightEdgeCW() const
{
	return { this->GetTopRight(), this->GetBottomRight() };
}

VWLine2D VWRectangle2D::GetTopEdgeCW() const
{
	return { this->GetTopLeft(), this->GetTopRight() };
}

VWLine2D VWRectangle2D::GetBottomEdgeCW() const
{
	return { this->GetBottomRight(), this->GetBottomLeft() };
}

VWLine2D VWRectangle2D::GetLeftEdgeCCW() const
{
	return { this->GetTopLeft(), this->GetBottomLeft() };
}

VWLine2D VWRectangle2D::GetRightEdgeCCW() const
{
	return { this->GetBottomRight(), this->GetTopRight() };
}

VWLine2D VWRectangle2D::GetTopEdgeCCW() const
{
	return { this->GetTopRight(), this->GetTopLeft() };
}

VWLine2D VWRectangle2D::GetBottomEdgeCCW() const
{
	return { this->GetBottomLeft(), this->GetBottomRight() };
}
...
// check if rectangles have common area.
bool VWRectangle2D::HasCommonAreaWith(const VWRectangle2D& rc) const
{
	return	( ( rc.m_bottomRight.x	>=	m_topLeft.x		) &&
			  ( m_bottomRight.x		>=	rc.m_topLeft.x	) &&
			  ( rc.m_bottomRight.y	<=	m_topLeft.y		) &&
...
// check if rectangles have common area (using epsilon).
bool VWRectangle2D::HasCommonAreaWith(const VWRectangle2D& rc, double epsilon) const
{
	return	( ( rc.m_bottomRight.x	>	m_topLeft.x		- epsilon ) &&
			  ( m_bottomRight.x		>	rc.m_topLeft.x	- epsilon ) &&
			  ( rc.m_bottomRight.y	<	m_topLeft.y		+ epsilon ) &&
