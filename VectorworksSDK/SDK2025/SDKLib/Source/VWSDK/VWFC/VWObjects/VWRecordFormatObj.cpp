...
VWRecordFormatObj::VWRecordFormatObj(MCObjectHandle hFormatHandle, ERecordIFCWrapMode wrapMode)
{
	...
	if ( VWFC_VERIFY( VWRecordFormatObj::IsRecordFormatObject( hFormatHandle ) ) )
	{
...
bool VWRecordFormatObj::IsReadOnly()
{
	bool result = false;
	if ( VWFC_VERIFY( ! this->IsIfc()) )	// IFC records and formats must be created and managed using the IFC Library
	{
		result = gSDK->IsLocked( fhObject );
	}
	return result;
}

void VWRecordFormatObj::SetReadOnly(bool bReadOnly)
{
	if ( VWFC_VERIFY( ! this->IsIfc()) )	// IFC records and formats must be created and managed using the IFC Library
	{
		gSDK->LockObject( fhObject, bReadOnly );
	}
}
...
bool VWRecordFormatObj::SetParamName( size_t paramIndex, const TXString & name )
{
	bool result = false;
	if ( ! this->IsIfc() )	// IFC records and formats must be created and managed using the IFC Library
	{
		TFormatHandler	fh( fhObject );
		if ( ! fh.IsParametric() && name.GetLength() < 64 )
		{
			fh.SetNameOfField( DemoteTo<short>( kVStanev, paramIndex+1 ), name );
			result = true;
		}
	}
...
/*static*/ bool VWRecordFormatObj::IsRecordFormatObject(MCObjectConstHandle hObj)
{
