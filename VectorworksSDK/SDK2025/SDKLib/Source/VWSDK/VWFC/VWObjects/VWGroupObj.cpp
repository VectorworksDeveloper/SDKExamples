...
VWGroupObj::VWGroupObj(MCObjectHandle hGroup, bool allowGroupLikeObjects /*= true*/)
{
	if ( VWFC_VERIFY( allowGroupLikeObjects
						? VWGroupObj::IsGroupLikeObject( hGroup )
						: VWGroupObj::IsGroupObject( hGroup ) ) )
	{
		fhObject		= hGroup;
	}
	else
...
bool VWGroupObj::IsGroupObject(MCObjectConstHandle hObj)
{
	short	type	= gSDK->GetObjectTypeN(hObj );
	return type == kGroupNode;
}

bool VWGroupObj::IsGroupLikeObject(MCObjectConstHandle hObj)
{
	short	type	= gSDK->GetObjectTypeN(hObj );
	return	setmember( type, kGroupNode, kLayerNode, kSymbolNode, kExtrudeNode, kMultiExtrudeNode,
							kParametricNode, kCSGTreeNode, kSymDefNode, kSweepNode );
}
...
void VWGroupObj::DeleteAllInnerObjects(bool useUndo/* = false*/)
{
	...
	while( *objectIterator ) {
		...
		TVariableBlock	value( Boolean( true ) );
		if ( gSDK->GetObjectVariable( objDelete, 9752, value ) ) // IsTemporaryObject?
		{ 
			value.GetBoolean( isTemporaryObject );
		}


		if( canUseUndo && !isTemporaryObject && !setmember( gSDK->GetObjectTypeN( objDelete ), kUndoPlaceholderNode, kTermNode ) ) {
			gSDK->AddBeforeSwapObject( objDelete );
		}

		::GS_DeleteObject( gCBP, objDelete, canUseUndo && !isTemporaryObject );
	}
}
...
MCObjectHandle VWGroupObj::FindObject(std::function<bool( MCObjectHandle )> pred, MCObjectHandle hAfter /*= nullptr*/, bool descendIntoGrouplike /*= false*/)
{
	MCObjectHandle	hResult	= nullptr;

	const auto	handleObjAndChildren	= [ & ] ( MCObjectHandle hObj )
		{
			MCObjectHandle	hRes	= nullptr;

			if ( pred( hObj ) )
			{
				hRes	= hObj;
			}
			else if ( descendIntoGrouplike && VWGroupObj::IsGroupLikeObject( hObj ) )
			{
				hRes	= VWGroupObj( hObj ).FindObject( pred, nullptr, descendIntoGrouplike );
			}

			return hRes;
		};

	MCObjectHandle	hCurr	= hAfter ? gSDK->NextObject( hAfter ) : gSDK->FirstMemberObj( fhObject );
	while ( !hResult && hCurr && gSDK->GetObjectTypeN( hCurr ) != kTermNode )
	{
		hResult	= handleObjAndChildren( hCurr );

		hCurr	= gSDK->NextObject( hCurr );
	}
