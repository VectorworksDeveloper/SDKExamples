...
{
	if ( VWFC_VERIFY( VWMeshObj::IsMeshObject( hMesh ) ) )
	{
		fhObject		= hMesh;
...
/*static*/ bool VWMeshObj::IsMeshObject(MCObjectConstHandle hObj)
{
...
void VWMeshCreator::GenerateMesh(VectorWorks::IMeshData& meshData)
{
	if (farrFaces.size() > 0)
	{
		...
		for (size_t iFace = 0; iFace < nFaces; ++iFace)
		{
			...
			meshData.SetFaceVertices(iFace, face.farrVertices);
...
void VWMeshCreator::GenerateMeshes(bool useUndo/*=true*/,
								   std::function<void(MCObjectHandle hMeshFace, const ObjectColorType& color)> callbackStealColors/*=nullptr*/)
{
	if( farrFaces.size() > 0 )
	{
		...
		for(size_t iFace=0; iFace < nFaces; ++iFace )
		{
			...
			{
				if ( callbackStealColors )
				{
					callbackStealColors( hMeshFace, face.fColor );
				}
				else
				{
