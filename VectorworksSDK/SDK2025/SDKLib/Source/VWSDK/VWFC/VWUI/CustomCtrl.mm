...
void VWCustomCtrl::AttachControl()
{
    ...
    @autoreleasepool
    {
		NSView* controlView = (__bridge NSView*)fControlRef;
        
		fParentWnd = (__bridge VoidPtr) [controlView window];
		...
		switch( inMessage ) {
			case msg_DrawOrPrint: {
                ...
                { // keep this block so 'VWCustomCtrlDC' could be deleted before OnAfterPaint call
                    ...
                    port = ioParam;
                    int l, t, r, b;
                    gSDK->GetComponentRect(dialogID, componentID, l, t, r, b);
                    fControlRect.left = l;
                    fControlRect.top = t;
			...
			case msg_MouseWheel: {
				int l, t, r, b;
				gSDK->GetComponentRect(dialogID, componentID, l, t, r, b);
				int	controlHeight	= b - t;
				
				SMouseEvent*			pEvent	= (SMouseEvent*) ioParam;
				
				SCustomCtrlMouseEvent	event;
				// the wherePort coordinates are such that the bottom left corner of the custom control is (0, 0)
				// but we need the top left corner to be (0, 0), that's why we transform the y using controlHeight
				event.x							= pEvent->wherePort.h;
				event.y							= controlHeight - pEvent->wherePort.v;
				event.bCtrlDown					= pEvent->macEvent.modifiers & cmdKey;
				event.bShiftDown				= pEvent->macEvent.modifiers & shiftKey;
				event.bAltDown                  = pEvent->macEvent.modifiers & optionKey;
				
				SInt16					deltaX	= pEvent->whereLocal.h;
                SInt16					deltaY	= pEvent->whereLocal.v;
				
                this->OnMouseWheel( event, deltaX, deltaY );
            } break;
