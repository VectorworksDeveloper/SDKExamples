...
//
{
	// Double values cannot always directly represent a fractional number as they are stored in binary.
	// ex. (double)0.9025 -> 0.90249999999999997
	// ex. (double)2.45   -> 2.4500000000000002
	// Adding an epsilon to bump up passed in value to improve rounding down.

	double adjustedValue = value * (1.0 + DBL_EPSILON);

    return signnum(adjustedValue) * (floor(Abs(adjustedValue)* pow(10, decimal_places))/pow(10, decimal_places));
}
...
//
{
	...
	// ex. (double)0.9025 -> 0.90249999999999997
	// ex. (double)2.45   -> 2.4500000000000002
	// Subtracting an epsilon to bump down passed in value to improve rounding up.

	double adjustedValue = value / (1.0 + DBL_EPSILON);

    return signnum(adjustedValue) * (ceil(Abs(adjustedValue)* pow(10, decimal_places))/pow(10, decimal_places));
}
