...
#include "../../../../../Include/Interfaces/VectorWorks/IGraphicLegendCoreSupport.h"
#include "../../../../../Plug-Ins/Shipping/GraphicLegend/Source/GraphicLegend/Interfaces/IGraphicLegendCoreOperationsSupport.h"
...
void VWObject::GetContainerInfo(MCObjectHandle* pOutContainerHandle, short* pOutContainerType, double* pOutContainerScale) const
{
	...
	switch( nContainerType ) {
		...
		case kGroupNode: {
			VectorWorks::IGraphicLegendCoreSupportPtr pIGLCS(VectorWorks::IID_GraphicLegendCoreSupport);

			if (VERIFYN(kEJKerr, pIGLCS) && pIGLCS->IsUserAnnotGroup(hContainer)) {
				GraphicLegend::IGraphicLegendCoreOperationsSupportPtr pIGLCOS(GraphicLegend::IID_GraphicLegendCoreOperationsSupport);

				if (pIGLCOS) {
					MCObjectHandle hGroupRegenerableUserAnnots, hVP, hImageRep, hCell, hLegend;

					if (pIGLCOS->FindRegenerableContainersForPersistentUserAnnotGroup(hContainer, hGroupRegenerableUserAnnots, hVP, hImageRep, hCell, hLegend)) {
...
bool VWObject::GetObjectTopPlanBounds(WorldRectVerts& outBoundsVerts) const
{
	return gSDK->GetObjectTopPlanBounds( fhObject, outBoundsVerts );
}
