...
CFindObjectAtPoint::CFindObjectAtPoint(MCObjectHandle hContainer, bool checkInArc/* = false*/)
	: fCheckInArc( checkInArc )
{
	fIs3DSearch = false;
...
bool CFindObjectAtPoint::CheckObject(MCObjectHandle hObject, const VWPoint3D& pt, double dRadius)
{
	...
	if( !(refId == kPlanarRefID_ScreenPlane && isPlanarObj && fIs3DSearch) )
	{
		...
		else if( VWLocus3DObj::IsLocus3DObject( hObject ) )
		{
			...
			{
				VectorWorks::IExtendedPropsSupportPtr exPropsPtr(VectorWorks::IID_ExtendedPropsSupport);
				if ( exPropsPtr && exPropsPtr->IsPluginSpecialGroup( fhContainer ) )
				{
					locPt.z = 0.0;
				}
				else
			...
			if ( worldBounds.ContainsPoint( rotPt2D ) ) {
				...
				if ( MathUtils::Equalish( dRadiusX, dRadiusY, 0.0001 ) ) {
					...
					if ( fCheckInArc )
					{
						bObjOk = arc.IsPtInOrOnArc( pt, dRadius );
					}
					else
					{
...
CFindObjectAtPoint::TObjectsArray::iterator CFindObjectAtPoint::begin()
{
	return farrFoundObjects.begin();
}

CFindObjectAtPoint::TObjectsArray::iterator CFindObjectAtPoint::end()
{
	return farrFoundObjects.end();
}

CFindObjectAtPoint::TObjectsArray::const_iterator CFindObjectAtPoint::begin() const
{
	return farrFoundObjects.begin();
}

CFindObjectAtPoint::TObjectsArray::const_iterator CFindObjectAtPoint::end() const
{
	return farrFoundObjects.end();
}
