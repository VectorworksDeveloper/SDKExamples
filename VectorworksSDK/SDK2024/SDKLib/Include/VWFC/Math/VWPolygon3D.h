...
namespace VWFC
{
	namespace Math
	{
		...
		public:
								...
								VWPolygon3D(const VWPolygon3D& src) = default;
								VWPolygon3D(VWPolygon3D&& src) = default;
			...
			virtual				~VWPolygon3D() = default;

			bool				operator==(const VWPolygon3D& poly) const;
			bool				operator!=(const VWPolygon3D& poly) const;

			VWPolygon3D&		operator=(const VWPolygon3D& src) = default;
			VWPolygon3D&		operator=(VWPolygon3D&& src) = default;
		...
		public:
			...
			bool				IsVertexCollinear(size_t at, double epsCollinear = VWPoint3D::sEpsilonCollinear) const;

			// gets part of the 3D polygon starting at 'fromVertInx', follows vertices' ascending order, and ending at 'toVertInx'.
			bool				GetPolygonPartFromTo(size_t fromVertInx, size_t toVertInx, VWPolygon3D& outPolygonPart) const;

			const VWPoint3D&	Back() const;
			VWPoint3D&			Back();

			bool				IsClosed() const;
			void				SetClosed(bool bClosed);

			// returns size_t(-1) if point is not inside polygon
			size_t				IndexOf(const VWPoint3D& pt) const;
		...
		public:
			...
			bool				GetPtOnPolyOffsetFrom(size_t vertInx, double offset, VWPoint3D& outPt, size_t& outSegInx) const;
			...
			bool				IsPtOnPolygonPart(const VWPoint3D& pt, size_t fromVertInx, size_t toVertInx, double epsilon,
			...
			bool				GetLength(size_t fromVertInx, size_t toVertInx, double& outLength) const;
		...
		private:
			...
			static void			Simplify_DouglasPeucker(const TSTLPoints3DArray& vertices3D, size_t begInx, size_t endInx, const VWTransformMatrix& rotTransf,
		...
		public:
			...
			void				SimplifyPolygon_DouglasPeucker(const double& maxDeviation);
