...
namespace VWFC
{
	namespace VWUI
	{
		...
		public:
			...
			void					EnableDragOnIndices(const std::vector<bool>& arrIndicesDragEnableState);	// arrIndicesDragEnableState is an array mirror of the items containing flag 'enableDrag' for each item
		...
		public:
			size_t					AddImage(const TXString& imageSpecifier);
														...
														)
		{
			...
			if ( VERIFYN( kVStanev, dropContextIt != sMapControlDropContext.end() ) )
			{
				...
				else if ( eventArgs.IsListBrowserDrawNDropEnd() )
				{
					...
					{
						bool	isDroppedAtDragIndex = false;

						// In multiple row selection, avoid certain issues (if the drop ID matches any dragged ID)
						for(size_t iDrag=0; iDrag<dragCnt && !isDroppedAtDragIndex; ++iDrag)
						{
							const Sint32	dragIndex = dropInfo.farrIndicesDragged.GetAt( iDrag );
							isDroppedAtDragIndex |= dropInfo.fDropIndex == dragIndex;
						}

						if ( !isDroppedAtDragIndex )
						{
							dropDataIndex++;
						}
					}
