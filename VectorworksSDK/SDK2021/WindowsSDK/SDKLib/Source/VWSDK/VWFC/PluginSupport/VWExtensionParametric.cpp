...
Sint32 VWParametric_EventSink::Execute(ParametricMessage* message)
{
	...
	switch ( message->fAction )
	{
		...
		case ParametricGetGlazingArea::kAction : {
			ParametricGetGlazingArea* msg = dynamic_cast<ParametricGetGlazingArea*>( message );

			if ( VERIFYN( kAHiguera, msg) )
				this->OnGetGlazingArea(*msg);
		} break;
...
void VWParametric_EventSink::OnGetGlazingArea(ParametricGetGlazingArea& msg) {
	// msg constructor initializes area to zero so do nothing here
}
