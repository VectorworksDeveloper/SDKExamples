...
//
{
	...
	while( record && !found){
		...
		if (format) {
			TFormatHandler currFormatHandler(format);
		...
		if (found == false)
			record = GS_NextAuxObject(cbp, GS_NextObject(cbp, record), recordNode);	// loop increment			
	}
	
	if (found) {
		recordHand = MCObjectHandle(record);
		formatHand = MCObjectHandle(format);
		return true;
	}
	else {
