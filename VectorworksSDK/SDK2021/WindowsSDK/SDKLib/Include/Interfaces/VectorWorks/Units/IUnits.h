...
namespace VectorWorks
{
	namespace Units
	{
		enum class EDefaultLenghtUnitType {
			FeetNInches,
			Inches,
			Feet,
			Miles,
		...
		enum class EDefaultAreaUnitType {
			SquareInches,
			SquareFeet,
			SquareYards,
			SquareMiles,
		...
		enum class EDefaultVolumeUnitType {
			CubicInches,
			CubicFeet,
			CubicYards,
			Gallons,
		...
		enum class EFractionDisplayStyle {
			Regular,
			Diagonal,
			Horizontal
		};

		enum class ERoundingBase {
			RoundTenth,			// 1
			RoundQuarter,		// 2.5
			RoundHalf			// 5
		};

		enum class EDefaultAngularUnitType {
			Degrees,
			Radians,
			Gradians
		};
