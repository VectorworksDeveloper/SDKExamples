...
const short kGLCameraActive			= 4; // Whether there is an active camera
const short kGLCameraBools			= 5; // Exposure enabled, dof enabled
const short kGLCameraSettings		= 6; // FStop, iso, shutter speed, focus distance, bloom
const short kGLViewportHandle		= 7; // Stores a viewort handle for reference to lighting options, etc.
const short kGLEnterTool			= 8; // Determines if we're in the tool loop. If we are, don't re enter tool code
const short kGLClickedSetFocusDist  = 9; // Returns if we clicked set focus distance
const short kOpenGLOptionsHandle	= 10; //Used for Editing the OpenGlOptionsHandle directly 

const short kGLLightOptions			= 0;
const short kGLRenderBackground		= 1;
...
struct OpenGLCustomOptions
{
	...
	short		fCreaseAngle;
	int			fEdgeThickness;
	bool		fUseEnvironmentLighting;
	bool        fUseEnvironmentReflection;
	bool		fUseScreenSpaceReflection;
	bool		fDepthOfField;
	double		fAperture;
	...
	double		fShutter;
	bool		fDepthCueing;
	double fDepthCueingParams[4] = {0.0, 1.0, 1.0, 1.0}; // depth1, opacity1, depth2, opacity2
};
