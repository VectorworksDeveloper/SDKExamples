...
#define CPPUNIT_TEST_SUITE2(PClass, PName, PEMail, PType)											\
	private:																						\
		...
		VectorWorks::Debug::ISystemTestEnvironment*		fpLocalEnv = nullptr;						\
		bool	fSuccessful = true;																	\
	public:																							\
		virtual VCOMError VCOM_CALLTYPE GetTestName(TXString& outName, VectorWorks::Debug::ESystemTestType& outType) override {	\
			outName = PName;																		\
			outType = PType;																		\
			return kVCOMError_NoError; }															\
		virtual VCOMError VCOM_CALLTYPE GetTextEMail(TXString& outEMail) override {							\
			outEMail = PEMail;																		\
			return kVCOMError_NoError; }															\
		virtual VCOMError VCOM_CALLTYPE GetSubTestsCount(size_t& outCount) override {						\
...
#define CPPUNIT_TEST_SUITE_END()																	\
		...
		virtual VCOMError VCOM_CALLTYPE GetSubTestName(size_t subTestIndex, TXString& outName) override {	\
		...
		virtual VCOMError VCOM_CALLTYPE Execute(VectorWorks::Debug::ISystemTestEnvironment* pEnv) override {						\
			fpLocalEnv			= pEnv;																\
			...
			if ( testsCnt > 0 ) {																	\
				...
				if (fSuccessful) {																	\
					...
					fpLocalEnv->Fail( TXString("Stopping after failed setUp()"), __FILE__, __LINE__ );		\
				}																					\
				this->tearDown();																	\
			}																						\
			fpLocalEnv				= NULL;															\
		...
		virtual VCOMError VCOM_CALLTYPE Execute(size_t subTestIndex, VectorWorks::Debug::ISystemTestEnvironment* pEnv) override {	\
			VCOMError	err		= kVCOMError_Failed;												\
			size_t	testsCnt	= farrSystemTest.size();												\
			this->GetSubTestsCount( testsCnt );														\
			if ( subTestIndex < farrSystemTest.size() ) {												\
...
//--------------------------------------------------------------------------------------------------
// variant on assert which, when failing, print massage and calls return 
#define CPPUNIT_ASSERT_MESSAGE_RETURN(message, condition)											\
{ 																									\
	bool expr = condition;																			\
	( this->failIf(		!expr,																\
                            TXString(	"Assertion Failed"											\
	...
	// end CPPUNIT_ASSERT_MESSAGE_RETURN definition
//--------------------------------------------------------------------------------------------------
