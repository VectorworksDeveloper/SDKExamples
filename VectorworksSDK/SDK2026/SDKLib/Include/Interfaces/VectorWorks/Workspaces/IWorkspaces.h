...
namespace VectorWorks
{
	namespace Workspaces
	{
		...
		*/
		...
		public:
			...
			/**
				...
				@param inIcon a IIcon object representing the tool set icon that needs to be set
				@return kVCOMError_NoError if the operation was successful.

				@author Alberto Antonini
				@date 16/08/2022
			 */
			virtual VCOMError VCOM_CALLTYPE SetIcon(VectorWorks::UI::IIcon* inIcon) = 0;
			/**
				@brief Gets the tool set icon.
				@param outIcon on output, if successful, an IIcon object representing the tool set icon
				@return kVCOMError_NoError if the operation was successful.

				@author Alberto Antonini
				@date 16/08/2022
			 */
			virtual VCOMError VCOM_CALLTYPE GetIcon(VectorWorks::UI::IIcon** outIcon) = 0;
			/**
		...
		*/
		...
		/**
		 @typedef IWorkspaceDMXRecordingPalette
		 @see IWorkspaceSizeablePalette
		 */
		typedef IWorkspaceSizeablePalette IWorkspaceDMXRecordingPalette;

		/**
		 @typedef IWorkspaceDMXRecordingPalette
		 @see IWorkspaceSizeablePalette
		 */
		typedef IWorkspaceSizeablePalette IWorkspaceDocumentHealthTestsPalette;
		...
		*/
		enum ESpecialKeys {
			...
			eSpecialKeys_ToggleGhostedMode,
		...
		*/
		...
		public:
			...
			/**
				@brief Creates a new Workspace ready to be saved.
			...
			*/
			...
			/**
				@func Open
				@param inWorkspaceFileToOpen is the workspace file that VectorWorks should open.
				@param inPermissions is the read/write permission bits that should be used to open the workspace file.
				@return kVCOMError_NoError if the operation was successful.

				@author Pablo Menendez Blanco
				@date 01/28/2024
			*/
			virtual VCOMError VCOM_CALLTYPE Open(const Filing::IFileIdentifierPtr inWorkspaceFileToOpen, Sint32 inPermissions) = 0;
			...
			*/
			...
			/**
				...
				@param inSavedFileNameOrPath which defaults to an empty string if the same name used to open it is to be used.
				@return kVCOMError_NoError if the operation was successful.
				@author Lyndsey Ferguson
				@date 10/02/2006
			*/
			virtual VCOMError VCOM_CALLTYPE Save(const TXString& inSavedFileNameOrPath = "") = 0;
			...
			*/
			...
			/**
			 @brief Gets the DMX Recording palette.
			 @param outDMXRecordingPalette on output, if successful, the DMX recording palette.
			 @return kVCOMError_NoError if the operation was successful.
			 
			 @author Alberto Antonini
			 */
			virtual VCOMError VCOM_CALLTYPE GetDMXRecordingPalette(IWorkspaceDMXRecordingPalette** outDMXRecordingPalette) = 0;

			/**
			 @brief Gets the Document Health Tests palette.
			 @param outDocumentHealthTestsPalette on output, if successful, the the Document Health Tests palette.
			 @return kVCOMError_NoError if the operation was successful.
			 */
			virtual VCOMError VCOM_CALLTYPE GetDocumentHealthTestsPalette(IWorkspaceDocumentHealthTestsPalette** outDocumentHealthTestsPalette) = 0;
		...
		*/
		...
		public:
			...
			*/
			...
			/**
				Gets the item's help text given the Workspace Identifier.
			 */
			virtual VCOMError VCOM_CALLTYPE GetHelpText(const TXString& inIdentifier, TXString& outHelpText) = 0;
			...
			*/
			...
			virtual VCOMError VCOM_CALLTYPE GetProtectionFlags(const TXString& inIdentifier, VectorWorks::Licensing::ModuleSet& modules, VectorWorks::Licensing::LicenseFlagSet& licenseFlags, VectorWorks::Licensing::ApplicationModeSet& appModes) = 0;
