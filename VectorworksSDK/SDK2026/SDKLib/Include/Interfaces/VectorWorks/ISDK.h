...
namespace VectorWorks
{
	...
	struct STreeControlItemStyle
	{
		EUiControlTextTint textColorTint = EUiControlTextTint::eDefaultTint;
		bool bTextBold = false;
		bool bTextHighlighted = false;
	};
	...
	struct SDateTimeData
	{
		...
		bool operator==(const SDateTimeData& other) const {
	...
	// Used for ISDK::GetPhaseStatus();
	struct SObjPhaseStatus
	{
		Uint8			fStatusID = 0;
		TXString		fStatusName;
	};
	...
	public:
		...
		virtual void			GetFaceObjects(std::vector<MCObjectHandle> & faces) const = 0;

		virtual	bool			GetFaceVertices(size_t faceIndex, TVWArray_SizeT& outArrVertexIndices) const = 0;
		virtual bool			GetFaceVertices(size_t faceIndex, TVWArray_SizeT& outArrVertexIndices, std::vector<MCObjectHandle> const & inFaceObjects) const = 0;
	...
	{
		bool fUpdateVWLibs = true;
		bool fUpdateVWOnlineLibs = true;
		bool fUpdateUserLibs = true;
		bool fUpdateWorkgroupsLibs = true;
		bool fUpdateFavorites = true;

		bool NeedsUpdate() { return (fUpdateVWLibs || fUpdateVWOnlineLibs || fUpdateUserLibs || fUpdateWorkgroupsLibs || fUpdateFavorites);}
	...
	public:
		...
		fType(kVSPluginMenu), fVersionCreated(0), fVersionModified(0), fVersionRetired(0), fIcon(VectorWorks::UI::IID_Icon)
		{
			fUniversalName[0] = 0;
			fLocalizedName[0] = 0;
			fCategory[0] = 0;
			fEnableFlags.needs = EMenuEnableFlags::None;
			fEnableFlags.needsNot = EMenuEnableFlags::None;
			fToolDataHand = 0;

			fLicenseModules = { VectorWorks::Licensing::kAnyModule };
		}
		...
		VectorWorks::Licensing::ModuleSet fLicenseModules;
		VectorWorks::Licensing::LicenseFlagSet fLicenseFlags;
		VectorWorks::Licensing::ApplicationModeSet fLicenseModes;
	};

	//-----------------------------------------------------------------------------------------------------------------
	typedef VCOMPtr<IVWUnknown>	TEditContext;

	//-----------------------------------------------------------------------------------------------------------------
	class CPopoverParameters {
	public:
		enum class PopoverPreferredLocation {
			Top, Bottom, Left, Right
		};
		
		// Shows a popover pointing to the current mouse location
		explicit CPopoverParameters(PopoverPreferredLocation location = PopoverPreferredLocation::Right): fLocation(location), fUseMousePosition(true) {};
		
		// Shows a popover pointing to the provided rect in drawing coordinates (if no dialog is open) or in dialog coordinates (if a dialog is open)
		CPopoverParameters(ViewRect drawingRect, PopoverPreferredLocation location = PopoverPreferredLocation::Right): fLocation(location), fRect(drawingRect) {};
		
		// Shows a popover pointing to the provided dialog control
		explicit CPopoverParameters(short ctrlID, PopoverPreferredLocation location = PopoverPreferredLocation::Right): fLocation(location), fControlID(ctrlID) {};

		virtual ~CPopoverParameters() = default;
		
		// uncomment/replace this when SpotlightVision, OBJExporter, Revit2022 be upgraded to C++20
		// bool operator!=(const CPopoverParameters&) const = default;
		bool operator!=(const CPopoverParameters& a) const {
			return fLocation != a.fLocation
				|| fRect != a.fRect
				|| fControlID != a.fControlID
				|| fUseMousePosition != a.fUseMousePosition;
		}
		// uncomment/replace this when SpotlightVision, OBJExporter, Revit2022 be upgraded to C++20
		// bool operator==(const CPopoverParameters&) const = default;
		bool operator==(const CPopoverParameters& a) const {
		...
		PopoverPreferredLocation GetPreferredLocation() const { return fLocation; }
		ViewRect GetRect() const { return fRect; }
		short GetControlID() const { return fControlID; }
		bool UseMousePosition() const { return fUseMousePosition; }
		virtual bool InScreenCoordinates() const { return false; }

	protected:
		void SetRect(const ViewRect& rect) { fRect = rect; }
		void SetUseMousePosition(bool useMousePosition = true) { fUseMousePosition = useMousePosition; }

	private:
		PopoverPreferredLocation fLocation = PopoverPreferredLocation::Right;
		ViewRect fRect;
		short fControlID = -1;
		bool fUseMousePosition = false;
	};

	using IPopoverParametersPtr = std::unique_ptr<VectorWorks::CPopoverParameters>;

	//-----------------------------------------------------------------------------------------------------------------
	using TCriteriaCustomCountCallback = std::function<Sint32( const TXString& criteria, bool includeSubparts )>;
	...
	public:
		...
		virtual void VCOM_CALLTYPE AddVertex(MCObjectHandle h, const WorldPt& p, VectorWorks::TEditContext editContext, VertexType t = vtCorner, WorldCoord arcRadius = 0, Boolean recalcBounds = true) = 0;
		...
		// Returns the object surface area in squared document LENGTH units (not document area units or mm^2).
		// See ObjSurfaceArea and ObjSurfaceAreaInWorldCoords.
		virtual double_gs VCOM_CALLTYPE CalcSurfaceArea(MCObjectHandle h) = 0;

		// Returns the object volume in cubed document LENGTH units (not document volume units or mm^3).
		// For a function that returns document volume units, see ObjVolume.
		// For a function that returns world coords (mm^3) units, see ObjVolumeInWorldCoords.
		...
		// Returns the object area in the current document area units.
		virtual double_gs VCOM_CALLTYPE ObjArea(MCObjectHandle h) = 0;

		// Returns the object surface area in the current document area units.
		// See ObjSurfaceAreaInWorldCoords.
		virtual double_gs VCOM_CALLTYPE ObjSurfaceArea(MCObjectHandle h) = 0;

		// Returns the object volume in the current document volume units.
		// For a function that returns world coords (mm^3) units, see ObjVolumeInWorldCoords.
		...
		virtual void VCOM_CALLTYPE SetVertex(MCObjectHandle h, short vertexNum, const WorldPt& vertex, VertexType vType, WorldCoord arcRadius, VectorWorks::TEditContext editContext, Boolean recalcBounds = true) = 0;
		...
		virtual short VCOM_CALLTYPE GetObjectType(MCObjectConstHandle h) const = 0;
		...
		// method has an input parameter to define the type of visibility (depends on the purpose of this call),
		// see definition of EVisibilityType;
		// default value is EVisibilityType::eActiveClassesLayers which includes only layer/class visibilities
		virtual Boolean VCOM_CALLTYPE IsVisible(MCObjectHandle h, EVisibilityType visType = EVisibilityType::eActiveClassesLayers) = 0;
		...
		virtual MCObjectHandle VCOM_CALLTYPE FindDataObject(MCObjectConstHandle owner, OSType tag) const = 0;
		...
		virtual MCObjectHandle VCOM_CALLTYPE CreateCustomObjectPath(const TXString& name, MCObjectHandle pathHand = NULL, MCObjectHandle profileGroupHand = NULL, bool doRegen = true) = 0;
		...
		virtual MCObjectHandle VCOM_CALLTYPE CreateGroup(bool isTemporary = false) = 0;
		...
		virtual MCObjectHandle VCOM_CALLTYPE CreateLocus(const WorldPt& location, bool isTemporary = false) = 0;
		...
		virtual Boolean VCOM_CALLTYPE AdjustComponentPixelPos(Sint32 nDialogID, Sint32 nComponentID, Sint32 nHorizontalPixels, Sint32 nVerticalPixels, bool bKeepBinding = false) = 0;
		...
		virtual Boolean VCOM_CALLTYPE HasListBrowserItemFillForeColor(Sint32 dialogID, Sint32 componentID, Sint32 iItemIndex, Sint32 iSubItemIndex, bool& hasColor) = 0;
		...
		virtual Boolean VCOM_CALLTYPE HasListBrowserItemFillBackColor(Sint32 dialogID, Sint32 componentID, Sint32 iItemIndex, Sint32 iSubItemIndex, bool& hasColor) = 0;
		...
		virtual Boolean VCOM_CALLTYPE HasListBrowserItemPenForeColor(Sint32 dialogID, Sint32 componentID, Sint32 iItemIndex, Sint32 iSubItemIndex, bool& hasColor) = 0;
		...
		virtual Boolean VCOM_CALLTYPE HasListBrowserItemPenBackColor(Sint32 dialogID, Sint32 componentID, Sint32 iItemIndex, Sint32 iSubItemIndex, bool& hasColor) = 0;
		...
		virtual Boolean VCOM_CALLTYPE HasListBrowserItemTextColor(Sint32 dialogID, Sint32 componentID, Sint32 itemIndex, Sint32 subItemIndex, bool& hasColor) = 0;
        ...
        virtual Boolean VCOM_CALLTYPE GetListBrowserItemInteractionType(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, EListBrowserItemInteractionType& interactionType) = 0;
...
//		virtual Boolean VCOM_CALLTYPE RunTemporary3DTool( Sint32& ioUserData, ToolDefProcPtr inToolProc) = 0;
		...
		virtual bool VCOM_CALLTYPE AddObjectToContainer(MCObjectHandle h, MCObjectHandle container) = 0;
		...
		virtual bool VCOM_CALLTYPE InsertObjectAfter(MCObjectHandle h, MCObjectHandle afterObject) = 0;
		virtual bool VCOM_CALLTYPE InsertObjectBefore(MCObjectHandle h, MCObjectHandle beforeObject) = 0;
		...
		// NOTE: DoubleToString function do not include thousand-separator. Use DoubleToStringN when this is a requirement
		...
		virtual short VCOM_CALLTYPE GetPerspectiveType(MCObjectHandle theLayer) = 0;
		...
		virtual void VCOM_CALLTYPE GetCurrentWorkspaceFileName(TXString& outFileName) = 0;
...
//endif
	...
	public:
        ...
        virtual bool            VCOM_CALLTYPE ExportResource(MCObjectHandle resource, IFileIdentifier* pFileID = nil, bool suppressDialogs = false, bool preserveFolders = false) = 0;
	...
	public:
		...
		virtual short			VCOM_CALLTYPE GetObjectTypeN(MCObjectConstHandle h) const = 0;
	...
	public:
		...
		virtual short			VCOM_CALLTYPE AddListBrowserImage(Sint32 dialogID, Sint32 componentID, const TXString& imageSpecifier) = 0;
		...
		virtual bool            VCOM_CALLTYPE ExportResourceN(MCObjectHandle resource, IFileIdentifier* pFileID = nullptr, TImportResourceConflictCallbackProcPtr conflictCallback = nullptr, bool preserveFolders = false) = 0;
	...
	public:
		...
		virtual void			VCOM_CALLTYPE SetPerspectiveInfo(TProjection projKind, TRenderMode renderMode, WorldCoord persDistance, const WorldPt &pt1, const WorldPt &pt2, short perspectiveType = perspectiveStandard) = 0;
	...
	public:
		...
		virtual bool				VCOM_CALLTYPE SetTreeControlItemCheckBoxState(Sint32 dialogID, Sint32 componentID, int itemID, Sint32 state, bool clickable = true) = 0;
		...
		virtual bool				VCOM_CALLTYPE ExportSymbolResource(MCObjectHandle symbolresource, const TXString& symbolFolderDest, IFileIdentifier* pFileID = nil, bool suppressDialogs = false, TImportResourceConflictCallbackProcPtr conflictCallback = nullptr, bool preserveFolders = false) = 0; // Pass empty string to insert symbol at the root level.
	...
	public:
		...
		virtual MCObjectHandle	VCOM_CALLTYPE CreateImgFromSymbol( const TXString & symbolName, SymbolImgInfo& imgInfoInout, bool supportDarkMode = false) = 0;
	...
	public:
		...
		virtual bool            VCOM_CALLTYPE ExportResourceList(const TVWArray_MCObjectHandle& inResourceList, IFileIdentifier* pFileID = nullptr, bool suppressDialogs = false, bool preserveFolders = false) = 0;
		virtual bool            VCOM_CALLTYPE ExportResourceListN(const TVWArray_MCObjectHandle& inResourceList, IFileIdentifier* pFileID = nullptr, TImportResourceConflictCallbackProcPtr conflictCallback = nullptr, bool preserveFolders = false) = 0;
		...
		// NOTE: DoubleToStringInUnits function do not include thousand-separator. Use DoubleToStringInUnitsN when this is a requirement
	...
	public:
        ...
        // linkURL can be either a URL or a contextual help ID. If you use contextual help ID, linkURL must be TXString("helpid://"+myContextualHelpID)
	...
	public:
		...
		// 'outUsedRecordNames' -- when provided contains the names of the records used by the criteria. The name might have a dotted prefix, e.g. 'IFC.', 'IDS.', 'COBie.', 'DSH.', or 'Style.'
		virtual Handle			VCOM_CALLTYPE CompileCriteriaExpression(const TXString& criteria, std::function<void(ECriteriaExpressionError err, size_t errOffset)> onErrorCallback, EExpressionContext context = eLocal, std::vector<TXString>* outUsedRecordNames=nullptr) = 0;
		...
		virtual bool			VCOM_CALLTYPE GetShowListBrowserItemMarkerByClass(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, bool& bShowByClass) = 0;
		...
		virtual bool			VCOM_CALLTYPE GetShowListBrowserItemMarkerEditList(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, bool& bShowEditList) = 0;
        ...
        // VW2023
    public:
        virtual void            VCOM_CALLTYPE CreateCheckBoxWithIcon(Sint32 dialogID, Sint32 itemID, const TXString& name, const TXString& iconRecPath) = 0;
		virtual void            VCOM_CALLTYPE CreateRadioButtonWithIcon(Sint32 dialogID, Sint32 itemID, const TXString& name, const TXString& iconRecPath) = 0;

		virtual MCObjectHandle	VCOM_CALLTYPE MirrorObjectN(MCObjectHandle h, bool dup, const WorldPt& axisStart, const WorldPt& axisEnd, bool preserveMatrix = true) = 0;

		virtual void			VCOM_CALLTYPE AddToolSearchablePullDownMenuMethod(const TXString& label) = 0;
		virtual void			VCOM_CALLTYPE AddSearchablePullDownMenuStaticChoice(short group, const TXString& idName, const TXString& text, const TXString& toolTip, const TXString& iconSpec) = 0;
		virtual void			VCOM_CALLTYPE AddSearchablePullDownMenuItem(short group, const TXString& idName, const TXString& text, const TXString& toolTip, const TXString& iconSpec) = 0;
		virtual void			VCOM_CALLTYPE AddSearchablePullDownMenuChildItem(short group, const TXString& idName, const TXString& text, const TXString& parentIdName, const TXString& toolTip, const TXString& iconSpec) = 0;
		virtual void			VCOM_CALLTYPE SetSearchablePullDownMenuSelection(short group, const TXString& id) = 0;
		virtual TXString		VCOM_CALLTYPE GetSearchablePullDownMenuSelectedID(short group) = 0;
		virtual void			VCOM_CALLTYPE ClearSearchablePullDownMenu(short group) = 0;
		virtual void			VCOM_CALLTYPE ClearSearchablePullDownMenuStaticChoices(short group) = 0;
		// NOTE: CoordToSIAString functions do not include thousand-separator. Use CoordToSIAStringN when this is a requirement
		virtual bool			VCOM_CALLTYPE CoordToSIAString(WorldCoord value, VectorWorks::Units::IVWUnits* dimUnit, bool alwaysMeters, TXString& outCentimeters, TXString& outMillimeters) = 0;
		virtual bool			VCOM_CALLTYPE CoordToSIAString(WorldCoord value, bool alwaysMeters, TXString& outCentimeters, TXString& outMillimeters) = 0;
		virtual bool			VCOM_CALLTYPE FormatTextObjectInSIA(MCObjectHandle hText, const TXString& centimeters, const TXString& millimeters, double superscriptScalePercent = 100) = 0;
		virtual bool			VCOM_CALLTYPE FormatTextObjectSegmentInSIA(MCObjectHandle hText, size_t startIndex, size_t countChars, double superscriptScalePercent = 100) = 0;

		virtual bool			VCOM_CALLTYPE SetStaticTextColorN(Sint32 dialogID, Sint32 itemID, EUiControlTextTint tint) = 0;
		virtual void			VCOM_CALLTYPE HierarchicalListBrowserItemOpened(Sint32 dialogID, Sint32 componentID, short itemIndex, Boolean recursive, short& outNumbRedisplayedItems, TXStringArray& changedItemNames) = 0;

		virtual VWIID			VCOM_CALLTYPE GetImporterIID(const TXString& fileExt) = 0;
		virtual void			VCOM_CALLTYPE SetPIOTextStyle( MCObjectHandle paramObj, const RefNumber textStyleRef, bool copyStyleSettings = true) = 0;

		virtual void			VCOM_CALLTYPE SetIsRunningLongActivity(bool isRunningLongActivity) = 0;

		virtual MCObjectHandle	VCOM_CALLTYPE CreateStoryWithUI() = 0;

		virtual TVWArray_InternalIndex	VCOM_CALLTYPE GetWorksheetSubrowObjects(MCObjectHandle hWorksheet, Sint16 row, Sint16 subrow) = 0;
		virtual void			VCOM_CALLTYPE SelectWorksheetSubrow(MCObjectHandle hWorksheet, Sint16 row, Sint16 subrow) = 0;

		virtual bool VCOM_CALLTYPE GetObjectTopPlanBounds(MCObjectHandle h, WorldRectVerts& outBoxVerts) = 0;

		virtual short						VCOM_CALLTYPE GetInsertLocationComponent(MCObjectHandle object) = 0;
		virtual void						VCOM_CALLTYPE SetInsertLocationComponent(MCObjectHandle object, short componentIndex) = 0;
		virtual EComponentInsertLocation	VCOM_CALLTYPE GetComponentInsertLocation(MCObjectHandle object) = 0;
		virtual void						VCOM_CALLTYPE SetComponentInsertLocation(MCObjectHandle object, EComponentInsertLocation insertLocation) = 0;
		virtual WorldCoord					VCOM_CALLTYPE GetComponentInsertLocationOffset(MCObjectHandle object) = 0;
		virtual void						VCOM_CALLTYPE SetComponentInsertLocationOffset(MCObjectHandle object, WorldCoord insertLocationOffset) = 0;

		virtual WorldCoord		VCOM_CALLTYPE GetWallPerpendicularOffsetFromInsertData(MCObjectHandle wall, InsertModeType insertLocation, WorldCoord insertLocationOffset) = 0;
		virtual bool			VCOM_CALLTYPE GetObjectWallPerpendicularOffset(MCObjectHandle objectHandle, MCObjectHandle wallHandle, WorldCoord &perpendicularOffset) = 0;

		virtual void			VCOM_CALLTYPE GetWallClosureInfo(MCObjectHandle wall, WallClosureInfo &wallClosureInfo) = 0;
		virtual bool			VCOM_CALLTYPE GetObjectWallClosureInfo(MCObjectHandle objectHandle, MCObjectHandle wallHandle, WallClosureInfo &wallClosureInfo) = 0;

		virtual void			VCOM_CALLTYPE GetExcludeWallClosureFromSettings(MCObjectHandle hObject, bool &left, bool &right, bool &top, bool &bottom) = 0;
		virtual bool			VCOM_CALLTYPE SetExcludeWallClosureFromSettings(MCObjectHandle hObject, bool left, bool right, bool top, bool bottom) = 0;
		virtual bool			VCOM_CALLTYPE GetExcludeWallClosureFromSettingsByStyle(MCObjectHandle hObject) = 0;
		virtual bool			VCOM_CALLTYPE SetExcludeWallClosureFromSettingsByStyle(MCObjectHandle hObject, bool byStyle) = 0;

		virtual WallClosureGeometryInfo		VCOM_CALLTYPE GetWallClosureGeometryInfo(MCObjectHandle hObject, WorldCoord planHeight, bool usePlugInObjectOptionsPlanCutHeight=false) = 0;
		
		virtual void VCOM_CALLTYPE ResetOldTextures(MCObjectHandle object, Boolean bClear2009Texture) = 0;

		virtual bool			VCOM_CALLTYPE GetIsObjectGimbalLocked(MCObjectConstHandle hObject) = 0;
		virtual void			VCOM_CALLTYPE SetIsObjectGimbalLocked(MCObjectHandle hObject, bool gimbalLocked) = 0;
        virtual MCObjectHandle    VCOM_CALLTYPE CreatePolylineWithCornerVerticesAddedToArcCurves(MCObjectHandle polyline) = 0;
		
		virtual WorldCoord VCOM_CALLTYPE ObjSurfaceAreaInWorldCoords(MCObjectHandle h) = 0;
		virtual MCObjectHandle VCOM_CALLTYPE GetCustomObjectSecondPath(MCObjectHandle objectHand) = 0;

		virtual void			VCOM_CALLTYPE GenerateSourceTracked2DRepresentation(const std::vector<MCObjectHandle>& solids, TStandardView view, WorldCoord cutPlaneCoordinate, std::unordered_map<MCObjectHandle, MCObjectHandle>& outCrossSectionFills, std::unordered_multimap<MCObjectHandle, MCObjectHandle>& outCrossSectionLinework, std::unordered_map<MCObjectHandle, MCObjectHandle>& outHiddenLineFills, std::unordered_multimap<MCObjectHandle, MCObjectHandle>& outHiddenLineLinework) = 0;
		virtual bool			VCOM_CALLTYPE GetCurrentWorldSpaceToolPoint(WorldPt3& locPos) = 0;

	// VW2024
	public:
		virtual void			VCOM_CALLTYPE TrackNodeChange(MCObjectHandle handle, const TXString& changeName = NULLSTR) = 0;
		virtual void			VCOM_CALLTYPE EnableTreeControlMultiSelection(Sint32 dialogID, Sint32 componentID, bool enabled = true) = 0;
		virtual Boolean			VCOM_CALLTYPE GetViewportStyle(MCObjectHandle h, InternalIndex &viewportStyle) = 0;
		virtual Boolean			VCOM_CALLTYPE SetViewportStyle(MCObjectHandle h, InternalIndex viewportStyle) = 0;

		virtual void            VCOM_CALLTYPE CreateBorderlessButton(Sint32 dialogID, Sint32 itemID, const TXString& name, const TXString& iconRecPath, const bool isToggleButton = false, const EButtonImagePos imagePos = EButtonImagePos::kImageAbove) = 0;
		virtual void			VCOM_CALLTYPE CreateBorderlessMenuButton(Sint32 dialogID, Sint32 itemID, const TXString& name, const TXString& iconResPath) = 0;
		...
		virtual bool 			VCOM_CALLTYPE ShowAndSelectObject( MCObjectHandle hObj ) = 0;
		virtual TEditContext	VCOM_CALLTYPE CreateEditContext(MCObjectHandle h) = 0;
		
		virtual Boolean			VCOM_CALLTYPE SetListBrowserItemResourceContent(Sint32 dialogID, Sint32 componentID, Sint32 itemIndex, Sint32 iSubItemIndex, VectorWorks::Extension::IResourceManagerContent* content) = 0;
		
		virtual Boolean			VCOM_CALLTYPE GetListBrowserItemIndentLevel(Sint32 dialogID, Sint32 componentID, Sint32 itemIndex, Sint32 iSubItemIndex, short& indentLevel) = 0;
		virtual Boolean			VCOM_CALLTYPE SetListBrowserItemIndentLevel(Sint32 dialogID, Sint32 componentID, Sint32 itemIndex, Sint32 iSubItemIndex, short indentLevel) = 0;

		virtual void			VCOM_CALLTYPE GetScreenFullSize(ViewRect& out) = 0; // return the screen size of the monitor where the cursor is

		virtual InternalIndex		VCOM_CALLTYPE GetActiveClass() = 0;
		virtual ObjectColorType		VCOM_CALLTYPE GetActiveColors() = 0;
		virtual InternalIndex		VCOM_CALLTYPE GetActiveFillPat() = 0;
		virtual OpacityRef			VCOM_CALLTYPE GetActiveOpacity() = 0;
		virtual ObjectLineType		VCOM_CALLTYPE GetActivePenFont() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveArrowByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveFColorsByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveFPatByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveLWByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActivePColorsByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActivePPatByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveOpacityByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveDropShadowByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActivePenOpacityByClass() = 0;
		virtual Boolean				VCOM_CALLTYPE GetActiveFillOpacityByClass() = 0;

		// These functions will consider the thousands-separator in 'dimUnit' where the old ones assume it's always off
		virtual bool			VCOM_CALLTYPE CoordToSIAStringN(WorldCoord value, VectorWorks::Units::IVWUnits* dimUnit, bool alwaysMeters, TXString& outCentimeters, TXString& outMillimeters) = 0;
		virtual void			VCOM_CALLTYPE DoubleToStringN(TNumStyleClass inNumStyle,  Byte inAccuracy, Byte inFormat, double_param inDouble, TXString& outString) = 0;
		virtual void			VCOM_CALLTYPE DoubleToStringInUnitsN(TNumStyleClass inNumStyle, Byte inAccuracy, Byte inFormat, double_param inDouble, VectorWorks::Units::IVWUnits* dimUnit, TXString& outString) = 0;
		
		virtual	Boolean 		VCOM_CALLTYPE CreateDataVisualizationPullDownMenu(Sint32 nDialogID, Sint32 nComponentID, int widthInStdChar, bool bShowDefaultStaticItems /*= true*/) = 0;
        
        virtual void            VCOM_CALLTYPE CreateSortedContourCurves(MCObjectHandle h, const WorldPt3 & pointOnPlane, const WorldPt3 & planeNormal, TMCObjectHandlesSTLArray    & outBoundariesArray, TMCObjectHandlesSTLArray    & outHolesArray) = 0;

		// Creates a section viewport on the given layer.
		// Class and layer visibilities are not handled here and should be handled after this call.
		// Viewport update should be done after class and layer visibilities are set up.
		virtual MCObjectHandle	VCOM_CALLTYPE CreateSectionViewport(const WorldPt& sectionLinePt1, const WorldPt& sectionLinePt2, const WorldPt& sectionLinePt3, double depth, double startHeight, double endHeight, MCObjectHandle hVPLayer) = 0;

		// return the parameter value provided by the "-prm <value>" provided at to executable
		// <value> is a system parameter, and can be wrapped in double-quotes to contain spaces
		virtual bool	VCOM_CALLTYPE GetAppParameter(size_t paramIndex, TXString& outValue) = 0;

		// arrIndicesDragEnableState is an array mirror of the items containing flag 'enableDrag' for each item
		virtual bool	VCOM_CALLTYPE EnableListBrowserDragOnIndices(Sint32 liDialogID, Sint32 liComponentID, const std::vector<bool>& arrIndicesDragEnableState) = 0;

		// Creates a new object that's offset from the given one.
		//
		// The difference between this function and CreateOffsetObjectHandle is that the latter will avoid
		// creating self-intersecting objects by creating multiple (some of which closed) objects and
		// grouping them together. In such cases, this function will just return a self-intersecting object.
		//
		// hPoly could either be kPolygonNode or kPolylineNode. Edge visibility is ignored and all edges are
		// treated as visible. Holes are ignored entirely. Self-intersecting or even entirely self-overlapping
		// objects are allowed. No modifications will be made to the source object.
		//
		// offset is the amount by which the poly should be offset. A positive offset will offset the poly
		// to its right and a negative - to its left.
		//
		// Returns a kPolygonNode or kPolylineNode on success and nullptr on error. The returned type may be
		// different from the source type. The returned object will not be inserted into the document and no undo
		// primitives will be created. No attributes will be copied from the source object.
		virtual MCObjectHandle VCOM_CALLTYPE CreateSingleOffsetPoly(MCObjectHandle hPoly, double offset) = 0;

		virtual GSColor	VCOM_CALLTYPE GetControlTextColorByTint(EUiControlTextTint tint) = 0;
		...
		virtual bool	VCOM_CALLTYPE SaveActiveFile() = 0;

	// VW2024 Update 2
	public:
		// Adds undo and redo selection behavior for the passed in handle.
		//
		// Inputs:
		//	- h: The handle to add undo and redo behavior for. If h is nullptr, then the undo and redo behavior
		//       will be applied to all currently selected objects.
		//  - undoSelection: EUndoSelectionState enum value to denote whether the passed in handle should be
		//                   selected or deselected upon undo.
		//  - redoSelection: ERedoSelectionState enum value to denote whether the passed in handle should be
		//                   selected or deselected upon redo.
		//  - clearSelectionsBeforeUndo: bool value to denote whether all selected objects should be deselected
		//                               before reapplying selection behavior on undo. Defaults to false.
		//  - clearSelectionsBeforeRedo: bool value to denote whether all selected objects should be deselected
		//                               before reapplying selection behavior on redo. Defaults to false.
		//
		// Return value:
		// - Boolean : Denotes whether or not the undo and redo behavior has been successfully
		//             added to the undo system.
		virtual Boolean	VCOM_CALLTYPE AddUndoRedoSelectObj(MCObjectHandle h, EUndoSelectionState undoSelection, ERedoSelectionState redoSelection, bool clearSelectionsBeforeUndo = false, bool clearSelectionsBeforeRedo = false) = 0;

		virtual void	VCOM_CALLTYPE GetDocumentDataVisualizations(std::vector<TXString>& outData) = 0;
		virtual bool	VCOM_CALLTYPE RenameDataVisualization(const TXString& oldName, const TXString& newName) = 0;
		virtual bool	VCOM_CALLTYPE DeleteDataVisualization(std::vector<TXString>& dataVisToDelete) = 0;

		// Based on RotateObjectN. Allows planar coord system to rotate - for 2D planar objects in their own (3D) plane.
		virtual void VCOM_CALLTYPE RotateObjectPlanar(MCObjectHandle h, const WorldPt& rotCenter, double_param angle) = 0;

		virtual TXString	VCOM_CALLTYPE GetWallClosureName(MCObjectHandle hObject) = 0;
		virtual bool		VCOM_CALLTYPE SetWallClosureName(MCObjectHandle hObject, const TXString &wallClosureName) = 0;
		virtual bool		VCOM_CALLTYPE GetWallClosureNameByStyle(MCObjectHandle hObject) = 0;
		virtual bool		VCOM_CALLTYPE SetWallClosureNameByStyle(MCObjectHandle hObject, bool byStyle) = 0;
		virtual void		VCOM_CALLTYPE GetExistingWallClosureNames(MCObjectHandle hObject, TXStringArray &existingWallClosureNames) = 0;
		virtual bool		VCOM_CALLTYPE GetObjectWallClosureInfoForName(MCObjectHandle objectHandle, MCObjectHandle wallHandle, const TXString &wallClosureName, WallClosureInfo &wallClosureInfo) = 0;

	// VW2024 Update 3
	public:
		// calls Criteria Dialog with specific context
		virtual bool	VCOM_CALLTYPE ShowCriteriaDialogContext(TXString& inOutCriteria, bool showObjectsCount, ECriteriaContext context) = 0;
		virtual bool	VCOM_CALLTYPE IsDocumentReady() = 0;

	// VW2024 Update 4
	public:
		virtual bool	VCOM_CALLTYPE IsNurbsCornerPoint(MCObjectHandle hNurbs, Sint16 pointIndex) = 0;
		virtual Boolean	VCOM_CALLTYPE GetListBrowserItemIndeterminate(Sint32 dialogID, Sint32 componentID, Sint32 itemIndex, Sint32 iSubItemIndex, bool& indeterminate) = 0;
		virtual Boolean	VCOM_CALLTYPE SetListBrowserItemIndeterminate(Sint32 dialogID, Sint32 componentID, Sint32 itemIndex, Sint32 iSubItemIndex, bool indeterminate) = 0;
		virtual Axis	VCOM_CALLTYPE SetWPHybridToolN(bool bDoingPauseAndRestore) = 0;

		// Method creates an orphaned copy of the text block, Aux list is not copied
		// linked text will be converted to a regular text with a placeholder "#Text" from string 2102, "LinkedTextPlaceholder"
		// Text attributes are preserved
		virtual MCObjectHandle VCOM_CALLTYPE CreateFormattedUnlinkedTextCopy(MCObjectHandle originalText) = 0;

	// VW2025
	public:
		virtual Boolean	VCOM_CALLTYPE IsVisibleByLayerAndClass(MCObjectHandle hObject, bool requireModifiable = false, bool allowGrayed = false) = 0;
		virtual bool	VCOM_CALLTYPE RemoveClassItems(Sint32 nDialogID, Sint32 nComponentID, const std::vector<TXString>& arrClassNames) = 0;
		
		// Runs a Layout Manager dialog as a popover.
		virtual bool 	VCOM_CALLTYPE RunPopover(Sint32 dialogID, GS_EventHandlerProcPtr handleEvent, void* env, const TXString& contextualHelpID, const CPopoverParameters&) = 0;
        virtual MCObjectHandle  VCOM_CALLTYPE ComputePolyWithPerEdgeOffsets(MCObjectHandle polyH, const std::vector<double>& offsets, std::map<size_t, size_t> & vertexMapping, bool bInsertNewOffset = false) = 0;
		virtual void UnhideObjects() = 0;
		virtual void ShowSelectedObjectsOnly() = 0;
		virtual void HideSelectedObjects() = 0;
		virtual void SetObjectsVisibility(const std::vector<MCObjectHandle>& objectHandles, bool visible) = 0;

		/**
		 * Closes all files and then gracefully quits Vectorworks.
		 * @param bAskForSave If true, the user will be prompted to save opening files before Vectorworks quits.
		 *					  If false, alll opening files will be closed without being saved (the user won't be
		 *					  prompted either), and Vectorworks will quit right after that.
		 * @param bRestart If true, Vectorworks will restart after it has quit. Otherwise, do nothing.
		 */
		virtual void	VCOM_CALLTYPE CloseAllFilesAndQuitVectorworks(bool bAskForSave = true, bool bRestart = false) = 0;
		virtual Extension::IWebPaletteFrame*	VCOM_CALLTYPE GetWebPaletteFrame(const VWIID& extensionIID) = 0;
		
		virtual Boolean VCOM_CALLTYPE GetTreeControlSelectedItems(Sint32 nDialogID, Sint32 nComponentID, std::vector<Sint32>& nItemsIDs) = 0;
		virtual Boolean VCOM_CALLTYPE GetTreeControlItemStyle(Sint32 dialogID, Sint32 componentID, Sint32 itemID, STreeControlItemStyle& itemStyle) = 0;
		virtual Boolean VCOM_CALLTYPE SetTreeControlItemStyle(Sint32 dialogID, Sint32 componentID, Sint32 itemID, const STreeControlItemStyle& itemStyle) = 0;
		virtual Boolean VCOM_CALLTYPE IsTreeControlItemExpanded(Sint32 dialogID, Sint32 componentID, Sint32 itemID) const = 0;
		
		virtual void VCOM_CALLTYPE GetCurrentWorkspaceFilePath(TXString& outFilePath) = 0;
		virtual bool	VCOM_CALLTYPE ShowCriteriaDialogContextCustomCount(TXString& inOutCriteria, bool showObjectsCount, ECriteriaContext context, const TCriteriaCustomCountCallback& customCountCallback = nullptr) = 0;

		// For Door/Window Assembly task - Handing widgets
		virtual void VCOM_CALLTYPE InitHandingWidgets() = 0;
		virtual void VCOM_CALLTYPE UpdateHandingWidgets(bool bSelectionChange) = 0;
		virtual void VCOM_CALLTYPE ClearHandingWidgets() = 0;

		// These functions are for creating and for getting and setting information about custom unit settings, which are used by dimensions.
		// The ovDimUnit selector can be used with GetObjectVariable to find out if a dimension is using a custom unit setting (0 means 
		// document units, else the 1-based index of the unit setting). The Create functions return the index of the created unit setting,
		// and the Get and Set functions are passed the index of a unit setting.
		virtual short VCOM_CALLTYPE CreateDimensionAngularUnitSetting(const TXString& settingName, EAngUnitIndex angUnitIndex, short angUnitMode, Boolean showLeading0, Boolean showTrailing0) = 0;
		virtual short VCOM_CALLTYPE CreateDimensionLinearUnitSetting(const TXString& settingName, EUnitIndex primUnitIndex, EUnitIndex secUnitIndex = EUnitIndex::Millimeters) = 0;
		virtual Boolean VCOM_CALLTYPE GetDimensionAngularUnitVariable(short customUnitIndex, short fieldSelector, TVariableBlock& value) = 0;
		virtual Boolean VCOM_CALLTYPE GetDimensionLinearUnitVariable(short customUnitIndex, short fieldSelector, TVariableBlock& value) = 0;
		virtual Boolean VCOM_CALLTYPE SetDimensionAngularUnitVariable(short customUnitIndex, short fieldSelector, const TVariableBlock& value) = 0;
		virtual Boolean VCOM_CALLTYPE SetDimensionLinearUnitVariable(short customUnitIndex, short fieldSelector, const TVariableBlock& value) = 0;

		virtual MCObjectHandle VCOM_CALLTYPE IsAssemblyEditModeActive() = 0;
		virtual MCObjectHandle VCOM_CALLTYPE IsObjectInsertedInAssembly(MCObjectHandle h) = 0;

		// Executes the callback for all reasources in side the provided resource folder identifier
		// 'resourceFolderIdentifier' should be '/' delimited path to a folder, including the .vwr file name, without extension. Example: "MyPlugin/Strings"
		virtual bool VCOM_CALLTYPE EnumerateVWResources(const char* resourceFolderIdentifier, bool recursive, std::function<void(const TXString& resourceIdentifier)> callback) = 0;
        virtual size_t VCOM_CALLTYPE PolylineCurveCountBetweenTwoCornerVertices(MCObjectHandle h, size_t index1, size_t index2) = 0;

		virtual void VCOM_CALLTYPE SetActivePlaneToLayerPlane() = 0;

		// Move or Copy Aux nodes from source obj to destination.
		// Transfer also RefNumber (object name)
		// Are transfered:
		//	- Constraint Nodes
		//	- Association Node
		//	- Record Nodes (except Parametric one)
		//	- UserData Nodes (if transferUserDataNodes is TRUE)
		//	If destination is nullptr, source aux objects are deleted
		virtual void VCOM_CALLTYPE TransferAuxList(MCObjectHandle hSource, MCObjectHandle hDestination, bool isForDuplication, bool transferUserDataNodes) = 0;
		
	// VW2025 Update 3
	public:
		virtual bool			VCOM_CALLTYPE GetListBrowserItemByClass(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, bool& bByClass) = 0;
		virtual bool			VCOM_CALLTYPE SetListBrowserItemByClass(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, bool bByClass) = 0;
		virtual bool			VCOM_CALLTYPE GetShowListBrowserItemByClassOption(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, bool& bShowByClass) = 0;
		virtual bool			VCOM_CALLTYPE ShowListBrowserItemByClassOption(Sint32 dialogID, Sint32 componentID, int iItemIndex, int iSubItemIndex, bool bShowByClass) = 0;
        
    // VW2025 Update 4
    public:
        virtual MCObjectHandle VCOM_CALLTYPE CreateNewSolidForWDAssemblyCuttingHole(MCObjectHandle wall, MCObjectHandle mainHole, MCObjectHandle insert, MCObjectHandle accessorySolid, bool bInteriorSideIsRight, short sAssemblyAccessoryId, short sillType) = 0;

	// VW2025 Update 5
	public:
		virtual bool VCOM_CALLTYPE InitInteractivePIOResizer() = 0;
		virtual bool VCOM_CALLTYPE IsInteractivePIOResizerActive() = 0;
		virtual bool VCOM_CALLTYPE MouseMoveInteractivePIOResizer() = 0;
		virtual bool VCOM_CALLTYPE CompleteInteractivePIOResizer() = 0;
		virtual void VCOM_CALLTYPE CancelInteractivePIOResizer() = 0;
        virtual void FitDWAccesories2DDispalayToClosureGeometry(MCObjectHandle pio, TStandardView view,
                                                                WorldCoord cutPlaneCoordinate) = 0;


	// VW2026
    public:
		virtual void VCOM_CALLTYPE TrackSwapChange(MCObjectHandle handle, const TXString& changeName = NULLSTR) = 0;

        virtual bool VCOM_CALLTYPE ShowCriteriaDialogExt(SCriteriaDialogParam& inOutData, bool showObjectsCount, ECriteriaContext context, const TCriteriaCustomCountCallback& customCountCallback = nullptr) = 0;
        
        // Behavior like AlertQuestion + additional hyperlink as in AlertXXXWithHyperlink. linkURL can be either a URL or a contextual help ID. If you use contextual help ID, linkURL must be TXString("helpid://"+myContextualHelpID)
		virtual short VCOM_CALLTYPE AlertQuestionWithHyperlink(const TXString& question, const TXString& adviceBeforeLink, const TXString& linkTitle, const TXString& linkURL, const TXString& adviceAfterLink, short defaultButton = 0, const TXString& OKOverrideText = "", const TXString& CancelOverrideText = "", const TXString& customButtonAText = "", const TXString& customButtonBText = "", const TXString& category = "", const TXString& item = "", const TXString& checkboxOverrideText = "") = 0;

		virtual bool VCOM_CALLTYPE SetGradientSliderFaderMode(Sint32 dialogID, Sint32 componentID, bool faderMode) = 0;
		virtual bool VCOM_CALLTYPE GetGradientSliderFaderMode(Sint32 dialogID, Sint32 componentID, bool& faderMode) = 0;
		virtual bool VCOM_CALLTYPE SetGradientSliderLiveUpdate(Sint32 dialogID, Sint32 componentID, bool liveUpdate) = 0;

		virtual bool VCOM_CALLTYPE AddObjectActionHighlight(MCObjectHandle hHighlightObject) = 0;
		virtual bool VCOM_CALLTYPE RemoveObjectActionHighlight(MCObjectHandle hHighlightObject) = 0;
		virtual void VCOM_CALLTYPE ClearActionHighlight() = 0;
		
		// Phasing
		//
		// The parameter "index" refers to the index of the phase in the document's list of phases.  The phase in the
		// first index (0) would be the first phase in chronological order, and the phase in the last index (gSDK->GetNumPhases() - 1)
		// would be the last phase in chronological order.
		virtual Sint16 		VCOM_CALLTYPE GetNumPhases() = 0;
		virtual SPhaseData 	VCOM_CALLTYPE GetPhase(Sint16 index) = 0;
		virtual void 		VCOM_CALLTYPE SetPhaseName(Sint16 index, const TXString& name) = 0;
		virtual void 		VCOM_CALLTYPE SetPhaseDescription(Sint16 index, const TXString& description) = 0;
		virtual SObjPhaseStatus	VCOM_CALLTYPE GetPhaseStatus(MCObjectHandle hObject) = 0;
		virtual SPhaseData	VCOM_CALLTYPE GetDocumentActivePhase() = 0;
		virtual SPhaseData	VCOM_CALLTYPE GetViewportActivePhase(MCObjectHandle hViewport) = 0;

		virtual WorldCoord VCOM_CALLTYPE ObjVolumeInWorldCoords(MCObjectHandle h) = 0;
	};
