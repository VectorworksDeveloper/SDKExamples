...
namespace VectorWorks
{
	...
	enum class EColorTableError {
		eNone,
		eTableFull
	};
	
	class DYNAMIC_ATTRIBUTE IDocumentColorTable : public IVWSingletonUnknown
	{
	public:
		...
		virtual VCOMError VCOM_CALLTYPE	InsertOrAdjustColor(const SColorRecord& inColorRecord, ColorRef& outColorRef, bool& cannotAdd) = 0;
		...
		virtual VCOMError VCOM_CALLTYPE	FindBestFitColor(const SColorRecord& inColorRecord, ColorRef& outColorRef) = 0;
		virtual VCOMError VCOM_CALLTYPE ReportError(EColorTableError reason) = 0;
	};
}
