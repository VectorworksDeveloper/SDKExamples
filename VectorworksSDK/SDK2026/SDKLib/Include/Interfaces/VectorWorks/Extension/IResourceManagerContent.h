...
namespace VectorWorks
{
	namespace Extension
	{
		typedef std::vector<std::pair<TXString, TXString>>	TKeyToValueArray;
		typedef std::vector<std::pair<TXString, TKeyToValueArray>> TRecordsDataArray;
		...
		struct SRMSearchMode
		{
			ERMSearchScope	fScopeOption = ERMSearchScope::Everywhere; // ERMSearchScope::Everywhere is equal to ERMSearchScope::EnabledOnly for the purpose of this method
			bool            fIncludeRecordsOption = true;
			bool			fExactPhraseOption = false;
			bool			fSearchActiveFile = true;
			bool			fSearchFavorites = true;		// these properties work as an additional filter of the search destination;
			bool			fSearchWorkgroupsLibs = true;	// if all are false, the search will be performed in the active document only;
			bool			fSearchUserLibs = true;			//  if ERMSearchScope::Everywhere and all are true, the search will be performed in all available libraries;
			bool			fSearchVWLibs = true;			// if ERMSearchScope::SelFileOnly, these properties won't have effect;
			TXString		fSearchPhrase;
		};

		struct SRMResFileInfo {
			TXString	fFilePath;
			TXString	fResName;
			short		fTDType = 0;
			std::unique_ptr<TRecordsDataArray> fRecords = nullptr;
		};
		...
		{
		public:
			virtual void		InitByTDType(TDType objectType, bool addDefaultContent = true) = 0;
			virtual void		InitByInternalID(TInternalID subType, bool addDefaultContent = true) = 0;
		...
		public:
			...
			virtual void SetSelectedResourceControlID(const SResourceControl::ControlID controlID) = 0;
			virtual SResourceControl::ControlID GetSelectedResourceControlID() const = 0;

			// Vectorworks 2025
		public:
			// Mimics the logic of the Resource manager search. Returns list of the found resources with their files' paths;
			// Returns search results from the Active file, favorites, VW Libraries, WG/User libraries 
			virtual std::vector<SRMResFileInfo>	GetSearchResults(const SRMSearchMode& searchMode) = 0;
		};
