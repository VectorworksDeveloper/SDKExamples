...
namespace VectorWorks
{
	namespace Extension
	{
		...
		public:
			...
			// A texture has been dragged from the RM to hObject (the cursor is over hSubObj and hLeafObj).
			// Return whether the object handled the drag.
			virtual bool			VCOM_CALLTYPE HandleTextureDragAndDropFromRM(MCObjectHandle hObject, MCObjectHandle hSubObj, MCObjectHandle hLeafObj, InternalIndex texture) const { return false; }

			// Return whether the texture popup in the render pane of a PIO should be enabled for a given part or face.
			// One of partID or faceID will have a valid value. An invalid partID will have a value of 0 and an invalid faceID will be empty.
			virtual bool			VCOM_CALLTYPE IsTexturePopupEnabled(MCObjectHandle hObj, size_t partID, const TXString& faceID) const { return true; }

			virtual bool			VCOM_CALLTYPE GetClassRefNumber(MCObjectHandle hObj, size_t partID, InternalIndex& outClassRefNumber) const { return false; }; 
			virtual bool			VCOM_CALLTYPE GetEnableTexturingModeChoice(MCObjectHandle hObj, Sint32 partCategoryID = 0) const { return true; };

			virtual InternalIndex	VCOM_CALLTYPE GetCurrentMaterial(MCObjectHandle hObj, size_t partID, Sint32 partCategoryID = 0) const { return 0; };
			virtual void			VCOM_CALLTYPE ChangeOIPTexture(MCObjectHandle hObj, InternalIndex newTexture, size_t partID, Sint32 partCategoryID = 0) const {  };
		};
	}
}