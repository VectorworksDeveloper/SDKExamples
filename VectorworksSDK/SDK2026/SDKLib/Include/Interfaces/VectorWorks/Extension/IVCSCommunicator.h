...
namespace VectorWorks
{
	namespace Extension
	{
		...
		struct SVCSDialogConfig
		{
			TXString	fTitle;
			TXString	fHelpID;
			ViewCoord	fDialogCX = 0;
			ViewCoord	fDialogCY = 0;
			SVCSDialogConfig() = default;
			SVCSDialogConfig( const TXString& title, const TXString& helpID, ViewCoord dialogCX, ViewCoord dialogCY )
				:fTitle( title ), fHelpID( helpID ), fDialogCX( dialogCX ), fDialogCY( dialogCY )
			{
			}
		};
		...
		struct SVCSURLKey
		{
			TXString		fMethod = "getEmbeddedDialogURL";
			TXString		fName;
			TXStringArray	fArrKeyValue;
		};
		...
		{
		public:
			virtual VCOMError VCOM_CALLTYPE LaunchDCC(bool bLocateManuallyEnabled = true) = 0;
			...
			virtual UI::EWebBrowserDlgRes VCOM_CALLTYPE RunLoadingPage( const SVCSDialogConfig& configuration, const SVCSURLKey& vcsURLKey ) = 0;
			
			virtual bool VCOM_CALLTYPE GetVCSFolderPath(TXString& outVCSPath, TXString& outDropboxPath) = 0;
			
			virtual TXString VCOM_CALLTYPE GetZMQPublisherURL() = 0;

			virtual bool VCOM_CALLTYPE IsCloudProcessingAllowed(bool bShowUI) = 0;
		};
