	...
	namespace Extension
	{
		...
		namespace ViewportVisializationOverride
		{
			...
			struct SAttrOverride
			{
				...
				bool			fLineWeightOverride = false;
				bool			fHatchLineWeightOverride = false;
				bool			fSurfHatchLineWeightOverride = false;
				bool			fSurfHatchPenColorOverride = false;
				Sint16			fHatchLineWeight = 0;
				Sint16			fSurfHatchLineWeight = 0;
				ObjectColorType	fSurfHatchPenColor;
			...
			typedef std::pair < TXString, TXString > TVisibilityEntry;
			...
			public:
				...
				virtual void VCOM_CALLTYPE	GetDataVisInfo(const UuidStorage& schemeID, ESelectionParams inParams, TXString& outSchemeName, 
					std::vector<TColorEntry>& outColorValuesVector, std::vector<TVisibilityEntry>& outHideValuesVector) const = 0;
				virtual void VCOM_CALLTYPE  GetIsAttrOperationType(const UuidStorage& schemeID, bool& isAttributeOperationType) const = 0;
				...
				// This method will return true if the object is hidden by Hide DataViz
				virtual bool VCOM_CALLTYPE GetObjectVisibilityOverride(MCObjectHandle hObject) = 0;
				// This method will return true if the object does not participate in DataViz and is hidden because this is the option for Draw Others
				virtual bool VCOM_CALLTYPE GetObjectDrawOthersHidden(MCObjectHandle hObject) = 0;
