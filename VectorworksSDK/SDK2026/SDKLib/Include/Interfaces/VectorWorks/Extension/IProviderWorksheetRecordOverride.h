...
namespace VectorWorks
{
	namespace Extension
	{
		...
		public:
			...
			//
			//	WARNGING!
			//		This function is executed many, many times. It must be very fast. std::vector::push_back for example can be too slow
			//		A for-loop is the most complexity that this function should have.
			virtual bool VCOM_CALLTYPE		GetFieldType(const TXString& univParamName, EFieldStyle& outFieldType) = 0;

		public:
			// Determine if the passed object is applicable (i.e. provides values) for this record override
			// This function is critical for the performance of the worksheet as a whole
			// It is CRITICAL to be very, very fast. It is called for all objects in the worksheet, and must determine if the GetFieldValue is appropriate to be called
			// (i.e. if it is appropriate to extact values from the object)
			virtual bool VCOM_CALLTYPE		IsApplicable(MCObjectHandle obj) = 0;
