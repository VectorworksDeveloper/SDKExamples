...
namespace VectorWorks
{
	namespace Extension
	{
		...
		public:
			...
			// Process key event
			virtual void			VCOM_CALLTYPE OnKeyDown(bool cmd, bool ctrl, bool optAlt, bool shift, char16_t key)			{ }

			// Process file dragging event
			virtual void			VCOM_CALLTYPE OnDragFiles(std::vector<TXString>)											{ }

			// Return true if you want to disable the context menu in the web-palette
			virtual bool			VCOM_CALLTYPE DisableContextMenu()															{ return false; }
		};
		...
		public:
			...
			public:
				...
				virtual void				AddExecute(const TXString& javaScriptCode) = 0;		// pre-code: execute before defining variables and functions
				virtual void				AddPostExecute(const TXString& javaScriptCode) = 0;	// post-code: execute after defining variables and functions
				virtual void				AddVariable(const TXString& name, const nlohmann::json& value) = 0;
				...
				virtual void				AddFunctionSync(const TXString& name) = 0;
				...
				virtual void				AddFunctionPromise(const TXString& name) = 0;
				virtual void				AddFunctionPromiseSync(const TXString& name) = 0;
				...
				virtual void				AddReourceAccessFunction(const TXString& integratorObj, const TXString& pluginVWRIdentifier) = 0;
			};

		public:
			virtual void		VCOM_CALLTYPE OnInit(IInitContext* context)	= 0;

			// the callback function should use the provided context to call back functions provided as paramters
			// the callback must be used to resolve the promise when added via IInitContext::AddReourceAccessFunction
			virtual void		VCOM_CALLTYPE OnFunction(const TXString& name, const std::vector<nlohmann::json>& args, VectorWorks::UI::IJSFunctionCallbackContext* context)		{ }

			// called when the palette visibility changes
			virtual void		VCOM_CALLTYPE OnPaletteVisibilityChange(bool visible, IWebPaletteFrame* frame)		{ }
  		};
	}
}