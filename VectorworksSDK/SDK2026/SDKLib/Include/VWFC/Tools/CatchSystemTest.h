...
#define CATCH_MESSAGE CPPUNIT_ASSERT_MESSAGE
#define CATCH_REQUIRE_MESSAGE CPPUNIT_ASSERT_MESSAGE_RETURN
...
void TEST_QuitVectorworksNoFileSave();
...
extern std::vector<std::function<bool(EStandardInterfaceManagerMapState, SStandardInterfaceManagerMapData&)>>& CatchHealthTests();

#define REGISTER_HEALTH_TEST(TestName, PInstanceType)																															\
	struct REGISTER##TestName {																																	\
		REGISTER##TestName() {																																	\
			VWIID Piid = GenWeakVWIID(#TestName);																												\
			CatchHealthTests().push_back([Piid](EStandardInterfaceManagerMapState state, SStandardInterfaceManagerMapData& data) {									\
				switch( state ) {																																\
					case EStandardInterfaceManagerMapState::eStandardInterfaceManagerMapState_Register:	::GS_VWRegisterInterface( Piid, VectorWorks::DocumentHealth::GROUPID_DocHealthTest, NULL );	break;						\
					case EStandardInterfaceManagerMapState::eStandardInterfaceManagerMapState_GetName:		if ( data.iid == Piid ) { data.name	= (const char*) #TestName; return true; } break;	\
					case EStandardInterfaceManagerMapState::eStandardInterfaceManagerMapState_New:			if ( data.iid == Piid ) { data.pInterface = new PInstanceType(); return true; } break;		\
					case EStandardInterfaceManagerMapState::eStandardInterfaceManagerMapState_Delete:		if ( data.iid == Piid ) { delete (PInstanceType*) data.pInterface; return true; } break;		\
					default: break;																																\
				}																																				\
				return false;																																	\
			});																																					\
...
#define	REGISTER_HEALTHTESTS		for( auto& test : CatchHealthTests() ) if( test(state, data)) return true;
...
extern std::vector<std::function<void(Sint32 action, void* moduleInfo, const VWIID& iid, IVWUnknown*& inOutInterface, CallBackPtr cbp, Sint32& reply)>>& CatchHealthTests();

#define REGISTER_HEALTH_TEST(TestName, PInstanceType)																															\
	struct REGISTER##TestName {																																				\
		REGISTER##TestName() {																																				\
			VWIID Piid = GenWeakVWIID(#TestName);																															\
			CatchHealthTests().push_back([Piid](Sint32 action, void* moduleInfo, const VWIID& iid, IVWUnknown*& inOutInterface, CallBackPtr cbp, Sint32& reply) {					\
				VWFC::PluginSupport::REGISTER_HealthTest<PInstanceType>(Piid, action, moduleInfo, iid, inOutInterface, cbp, reply);												\
			});			\
		}																																									\
	} gRegister##TestName;

//Call this in your plugin ModuleMain.cpp, in your plugin_module_main function.  
#define	REGISTER_HEALTHTESTS(action, moduleInfo, iid, inOutInterface, cbp, reply)	for( auto& test : CatchHealthTests() ){	test(action, moduleInfo, iid, inOutInterface, cbp, reply); }
