	...
	namespace Math
	{
		...
		public:
			struct SVertex
			{
				...
				// data.
				VWPoint2D			pt {};
				EVertexType			type {eVertexType_Corner};
		...
		public:
			// note: default constructor sets the polygon as closed!
			 				VWPolygon2D();
							VWPolygon2D(std::initializer_list<VWPoint2D> src, bool isClosed = false);
							VWPolygon2D(const TSTLPoints2DArray& src, bool isClosed = false);
							VWPolygon2D(const TSTLPoints2DVWArray& src, bool isClosed = false);
			explicit		VWPolygon2D(const std::vector<SVertex>& vertices, bool isClosed = false);
							VWPolygon2D(IPolyDefPtr pPolygonDef);
			explicit		VWPolygon2D(const WorldRectVerts& worldRect);
							VWPolygon2D(const VWPolygon2D& src) = default;
							VWPolygon2D(VWPolygon2D&& src) = default;
			virtual			~VWPolygon2D() = default;
			...
			VWPolygon2D&	operator=(const VWPolygon2D& src) = default;
			VWPolygon2D&	operator=(VWPolygon2D&& src) = default;
			VWPolygon2D&	operator=(IPolyDefPtr pPolygonDef);

		// operations
		public:
			// add for-each support for vertices
			TVertexArray::iterator begin();
			TVertexArray::iterator end();
			TVertexArray::const_iterator begin() const;
			TVertexArray::const_iterator end() const;
		...
		public:
			...
			// Supports only corner, radius and arc vertices.
			// Does not include visibility!
			double			GetLength() const;
			// Calculate the length of the curve formed by the polyline up to the specified point.
			// Supports only corner and radius vertices.
			// If the point is not on the poly, the function will return the total length.
			...
			bool			GetPolygonLength(size_t fromVertInx, size_t toVertInx, double& outLength) const;
			...
			struct SSegment;
			std::vector<SSegment> GetSegments() const;
		...
		public:
			...
			struct SVertexChange
			{
				enum class EType { None, Insertion, Deletion, Substitution };
				EType	fType		= EType::None;
				Sint16	fOldIndex	= -1; // Will be set to -1 for fType == EType::Insertion
				Sint16	fNewIndex	= -1; // Will be set to -1 for fType == EType::Deletion
			};
			struct SComparePolylineOptions
			{
				double fInsertionCost		= 1.0;
				double fDeletionCost		= 1.0;
				double fSubstitutionCost	= 1.0;
			};
			// Compares the two given polylines using the Levenshtein distance algorithm. Returns the changes that need to be
			// made to the vertices of oldPoly to get the vertices of newPoly, in order.
			static std::vector<SVertexChange>	ComparePolylines(const VWPolygon2D& oldPoly, const VWPolygon2D& newPoly, const SComparePolylineOptions& options);
			static std::vector<SVertexChange>	ComparePolylines(const VWPolygon2D& oldPoly, const VWPolygon2D& newPoly);

			struct SSegmentChange
			{
				enum class EType
				{
					None,			// The segment is entirely unchanged.
					ChangeBegin,	// The beginning of the segment has been changed, with its end being left located at the same place.
					ChangeEnd,		// The end of the segment has been changed, with its beginning being left located at the same place.
					Offset,			// The segment is unchanged, but is offset to a different location.
					New,			// The segment is either entirely new or has been modified such that none of the other enumerators are applicable.
				};

				EType	fType			= EType::None;

				// The index of the segment in the original polyline that corresponds to this segment.
				// Invalid if fType == EType::New.
				size_t	fSourceIndex	= -1;
			};
			static std::vector<SSegmentChange>	CompareSegments(const VWPolygon2D& oldPoly, const VWPolygon2D& newPoly, const std::vector<SVertexChange>& vertexChanges);

			// given a set of consecutive points (representing a shape), removes all collinear.
			static bool		SimplifyShape_RemoveCollinear(TSTLPoints2DArray& ioConsecPtsSet, bool isClosedShape,
		...
		public:
			...
			static double	GetPtProjectionOffsetAlongPoly(const VWPolygon2D& inPoly, const VWPoint2D& pt);
		...
		struct VWPolygon2D::SSegment
		{
			VWPolygon2D		fPoly;
			size_t			fFirstVertexIndex = -1;
		};
	}
}
