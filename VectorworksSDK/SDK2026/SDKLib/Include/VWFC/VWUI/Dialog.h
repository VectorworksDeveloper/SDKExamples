...
#include "BorderlessButtonCtrl.h"

#include "../../../Source/VWSDK/VWFC/VWUI/DialogData.h"
...
namespace VWFC
{
	namespace VWUI
	{
		...
		// ---------------------------------------------------------------------------------------------------------------
		struct SClassPopupItem
		{
			TXString	fClassName;
			TXString	fIconSpec;	// icon to be displayed for this class item

			// The first constructor is provided to allow implicting casting from TXString to this object. Should be removed eventually.
			SClassPopupItem(const TXString& className)								: fClassName(className) {}
			SClassPopupItem(const TXString& className, const TXString& iconSpec)	: fClassName(className), fIconSpec(iconSpec) {}
		};

		typedef std::vector<SClassPopupItem>	TClassPopupItemsArray;
		...
		public:
			...
			virtual bool			RunPopover(const TXString &contextualHelpID, const CPopoverParameters& popoverParams);
			
		private:
			bool 					PrepareDialogLayout();
			void					VerifyControlIsValid(VWControl* control);
			void					VerifyControlsAreValid();
		...
		public:
			...
			VWCheckButtonCtrl*			GetCheckButtonCtrByID(TControlID id) { return GetCheckButtonCtrlByID(id); }
			...
			VWImageButtonCtrl* 			GetImageButtonCtrlByID(TControlID id);
			...
			VWSearchBoxCtrl* 			GetSearchBoxCtrlByID(TControlID id);
			...
			VWBorderlessButtonCtrl* 	GetBorderlessButtonCtrlByID(TControlID id);
		...
		public:
			...
			********************************************************************/
			...
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, short* pSelected, const SClassPopupItem& standardClass);
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, short* pSelected, const SClassPopupItem& standardClass, const TClassPopupItemsArray& arrAdditionalItems);
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, short* pSelected, const TClassPopupItemsArray& arrAdditionalItems);
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected);
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected, const SClassPopupItem& standardClass);
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected, const SClassPopupItem& standardClass, const TClassPopupItemsArray& arrAdditionalItems);
			void	AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected, const TClassPopupItemsArray& arrAdditionalItems);
			void	AddDDX_ChooseLayerPopup(TControlID controlID, TXString* pValue, short* pSelected);
			void	AddDDX_ChooseLayerPopup(TControlID controlID, TXString* pValue, short* pSelected, const TClassPopupItemsArray& arrAdditionalItems);
		...
		public:			
			...
			static void				AlertInform(const TXString& text, const TXString& advice = "", bool bMinorAlert = false, const TXString& category = "", const TXString& item = "", const TXString& checkboxOverideText = "");
			...
			void					RegisterSwapControlWithListBrowser(TControlID listControlID, TControlID swapControlID, const TXString& savedSettingsName, bool autoSwap, TRegisterSwapCtrlTranslateCallback callback = nullptr);
		...
		protected:
			...
			virtual	void			OnChangeStyleListBrowserButton(TControlID controlID, VWDialogEventArgs& eventArgs);
			...
			// Context menu
			virtual void			OnContextMenu(TItemMessageData* messageData);
			virtual void			OnContextMenuItemSelected(TItemMessageData* messageData);
		...
		protected:
			...
			bool				GetDDXControlType(TControlID controlID, EDDXType& outValue);
		...
		// Style Support
		protected:
			struct SLBCellLocation {
				size_t	fRow	{};
				size_t	fColumn	{};

				bool operator==(const SLBCellLocation& rhs) const
				{
					return fRow == rhs.fRow && fColumn == rhs.fColumn;
				}
			};

			virtual void	InitializeStyleSupport();
			virtual void	InitializeStyleListBrowserColumns( TControlID controlID, const TSizeTArray& styleColumns );
			...
			virtual void	InitStyleControl( TControlID controlID, Sint32  groupID, EPluginStyleParameter styleParam, TControlID primaryControl, TControlID addedControl2 = 0, TControlID addedControl3 = 0, TControlID addedControl4 = 0 );
			virtual void	InitStyleGroupControl( TControlID controlID, Sint32 groupdID, TControlID textControlID = 0 );
			virtual void	InitStyleLBItem( TControlID controlID, size_t row, size_t column, Sint32 groupID, std::vector<TXString> parameters, const TSizeTArray& associatedColumns );
			virtual void	DeleteStyleLBItem( TControlID controlID, size_t row, size_t column, Sint32 groupID );
			virtual void	DeleteAllStyleLBItems( TControlID controlID );
			virtual void	RestrictStyleTypesMultiSel();

			virtual void	AssociateStyleControl( TControlID controlID, TControlID associatedControlID );
			virtual void	AssociateStyleControls( TControlID controlID, const std::vector<TControlID> associatedControlIDs );
			virtual void	AssociateStyleParameter( TControlID controlID, const TXString & paramName );
			virtual void	AssociateStyleParameters( TControlID controlID, const std::vector<TXString> paramNames );
			virtual void	AssociateStyleLBCells( TControlID controlID, size_t row, size_t column, std::vector<SLBCellLocation> associatedCells );

			virtual bool	GetStyleControlType(TControlID controlID, EPluginStyleParameter& styleType);
		...
		protected:
			...
			// support for automatic list browser column width storage
			void				DeleteLBColumnData(TControlID controlID, size_t columnIndex);

		private:
			void			UpdateStyleGroupButton(VWStyleGroupButtonCtrl* pGroupControl) const;
		...
		protected:
			...
			struct SStyleLBItem {
				SLBCellLocation					fLocation	{};
				Sint32							fGroupID	{};
				EPluginStyleParameter			fStyleType	{};
				std::vector<TXString>			fParameters;
				std::vector<SLBCellLocation>	fAssociatedCells;

				void EnableAssociatedCells(VWListBrowserCtrl* theListBrowser, bool enable) const;
				void UpdateAssociatedCells(VWListBrowserCtrl* theListBrowser, bool allowEnabling) const;
			};
			...
			using TStyleLBItemMap	= std::unordered_map<TControlID, std::vector<SStyleLBItem>>;
			using TStyleLBImageMap	= std::unordered_map<TControlID, std::array<size_t, 5>>;//maintain enough storage for all EPluginStyleParameter options

			TStyleControlMap	fStyleControlMap;
			TStyleLBItemMap		fStyleLBItemMap;
			TStyleLBImageMap	fStyleLBImageMap;
