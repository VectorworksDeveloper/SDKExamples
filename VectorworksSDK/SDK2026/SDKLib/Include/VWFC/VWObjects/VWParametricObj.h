...
namespace VWFC
{
	...
	namespace VWObjects
	{
		...
		public:
								...
								VWParametricObj(const TXString& parametricName, const VWPoint3D& location );
								VWParametricObj(const TXString& parametricName, const VWTransformMatrix& matrix );
		...
		public:
			...
			bool				GetIsGimbalLocked() const;
			void				SetIsGimbalLocked(bool gimbalLocked);
		...
		// styles
			...
			void				SetStyle(MCObjectHandle hStyle) const;
			void				SetStyle(RefNumber styleRefNumber) const;

			// custom control points

			using	TCustomControlPointsInitCallback	= std::function<void( Sint32 inIndex, WorldPt3& outPt, Boolean& outIs2DOnly, Boolean& outIsHidden, Sint16& outClientID )>;
			bool				CustomControlPointsCreate(size_t numPoints, TCustomControlPointsInitCallback initProc = nullptr);
			bool				CustomControlPointsRemove();
			bool				CustomControlPointsSet(const VWPoint3D& location, bool onlyIn2D, bool visible, size_t pointID, size_t pointIndex = -1);
			bool				CustomControlPointsGet(size_t pointIndex, VWPoint3D& outLocation, bool* outOnlyIn2D = nullptr, bool* outVisible = nullptr, Sint32* outPointID = nullptr) const;

		// Tools
		public:
			// Return true if the specified object handle is layer
			static bool				IsParametricObject(MCObjectConstHandle hObj);
			...
			static bool				IsPathObject(MCObjectHandle hObj);
		...
		protected:
			...
			void				CreateParametricObject(const TXString& parametricName, const VWTransformMatrix& matrix);
