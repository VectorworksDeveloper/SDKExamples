...
namespace VWFC
{
	...
	namespace VWObjects
	{
		...
		public:
			...
			// Invisible formats will be hidden from both the resource manager and the data pane of the OIP.
			bool				GetFormatVisibility() const;
			void				SetFormatVisibility(bool bVisibility);

			// Read Only records will be visible, but not editable, in the data pane of the OIP and hidden from the resource manager.
			bool				IsReadOnly();
		...
		public:
			...
			size_t				AddParamStaticString(const TXString& name, const TXString& value, bool bUpdateExistingRecords = true);

			size_t				GetParamsCount() const;
			TXString			GetParamName(size_t paramIndex) const;
			bool				SetParamName(size_t paramIndex, const TXString& name);
		...
		public:
			...
			static bool				IsRecordFormatObject(MCObjectConstHandle hObj);
