...
namespace VWFC
{
	namespace PluginSupport
	{
		...
		// ------------------------------------------------------------------------------------------------------
		class IEventSinkToolPointsProvider
		{
		public:
			virtual ~IEventSinkToolPointsProvider() = default;

			virtual size_t		GetToolPointsCount() const = 0;
			virtual void		ClearAllToolPoints() = 0;
			virtual VWPoint2D	GetToolPt2D( size_t ptIndex, bool includeCurrent = false ) const = 0; // includeCurrent=true then return current if ptIndex==GetToolPointsCount 
			virtual VWPoint3D	GetToolPt3D( size_t ptIndex, bool includeCurrent = false ) const = 0;
			virtual bool		GetToolPtCurrent2D( VWPoint2D& outPt ) const = 0;
			virtual bool		GetToolPtCurrent3D( VWPoint3D& outPt ) const = 0;
			virtual bool		PopLastToolPt() = 0;
		};
		using IEventSinkToolPointsProviderPtr = std::shared_ptr<IEventSinkToolPointsProvider>;
		...
		public:
			...
			void					SetPointsProvider( IEventSinkToolPointsProviderPtr provider );
			void					SetModeBarProvider(IToolModeBarProvider* modeBarProvider);
		...
		public:
			...
			// When right mouse button is pressed in drawing
		...
		protected:
			...
			IEventSinkToolPointsProviderPtr fpToolPtsProvider;
		};
		...
		public:
			...
			virtual const TXString&	VCOM_CALLTYPE	GetSearchSynonyms();
		...
		protected:
			...
			virtual void VCOM_CALLTYPE			SetSearchSynonyms(const TXString& searchSynonyms);
