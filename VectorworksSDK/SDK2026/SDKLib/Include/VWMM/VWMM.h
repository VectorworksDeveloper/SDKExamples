...
#ifndef Handle
typedef Ptr *                           Handle;
typedef const char *const *				ConstHandle; // ConstHandle is a pointer to a const pointer to a const char
//        |        |      |                  |
//        |        |      |            ConstHandle is...
//        |        |   a pointer to...
//        |   a const pointer to...
...
protected:
	...
	Sint32 GetMemoryCallbackFunctionIndex(void (*func)(Sint32, size_t, void*)) const;	// returns -1 if not found or index (0<=ret<NUM_MEMCALLBACK_FUNCTIONS)
	Sint32 GetFreeMemoryCallbackFunctionSlot(void) const;
	...
	void MemoryStat_GetSlotBoundsStr(char *str, size_t strMax, size_t slotNum) const; // given a slot and a buffer, fill buffer with a string such as "100-250" based on the range stored in slot
	...
	bool BlockIsResource(const MemoryBlock* block) const;
	...
	bool GetInstantiatedState(const void* mem) const;

	MemoryBlock* GetVerifiedBlockForPointer(const void* mem) const;	// return nonnull if the memory belongs to use. This is fairly expensive.
...
public:
	...
	static Partition *GetPartitionByIdentifier(const char *identifier, bool createIfNotFound);
	static Partition *GetPartitionListHead(void);
	static void		  FreeAllPartitions(void);

	void SetIdentifier(const char *newIdentifier);
	void GetIdentifier(char *buffer) const;	// Note that buffer must be at least PARTITION_IDENTIFIER_BUFFER_SIZE.

	Uint32 GetFlags(void) const {return fFlags;}
	void SetFlags(Uint32 newFlags) {fFlags = newFlags;}

	Handle  GetCurrentPool(void) const {return fPool;}
	...
	void TXStringDeallocate(char** pointer);		// For TXString

	void* GetHandle(const void* mem) const;						// given memory allocated by this library, return a handle.
	void* GetVerifiedHandle(const void* mem) const;				// slow, GetHandle plus verifies it's allocated by this library.
	size_t GetVWMMHandleSize(const void* handlein) const;				// given a handle, return its size

	size_t GetVWMMHandleMaxSize(const void* handlein) const;			// give a handle, return its max mem size (this is the actual allocated bytes not the handle size)

	bool PointerBelongsToUs(const void* mem, bool forceBlockCheck = false) const;						// return true if the memory beloings to use. This is fairly expensive.
	bool HandleBelongsToUs(const void* handle) const;
	bool PointerIsValid(const void* mem, bool checkForOwnage, bool forceBlockCheck = false) const;	// return true if the pointer is valid
	bool HandleIsValid(const void* handle, bool checkForOwnage) const;	// return true if the handle is valid
	...
	void* DuplicateHandle(const void* handlein);					// duplicate the passed handle.

	void* GetUserData(const void* handlein) const;						// return the value of user data stored in the memory block header.
	...
	Handle FindBlockWithQTResourceHandle(void** qtHandle, short homeFileRef) const;	// attempt to find a block with the given handle
	Handle GetOriginalQTResource(const void* handlein) const;			// return the QT resource handle if possible
	...
	bool		HandAndHand(ConstHandle hand1, Handle hand2);
	bool		PtrAndHand(const void *p, Handle h, GSSize byteCount);
	GSSize		GetHandleSize(ConstHandle h) const;
	GSSize		GetHandleMaxSize(ConstHandle h) const;
	GSSize		GetPtrSize(ConstPtr p) const;
	void		DisposePtr(Ptr p);
	void		DisposeHandle(Handle h);
	bool		IsHandleInUse(ConstHandle h) const;
	...
	char		HGetState(ConstHandle h) const;// this exists for backwards compatibility with handle lock/unlock mechanisms
	...
	bool		HandleIsLocked_Legacy(ConstHandle h) const;
	...
	Handle	ConvertFromQTHandleToVWMMHandle(ConstHandle qtHandle);	// given a QT handle return a VWMM handle copy
	Handle	ConvertFromVWMMHandleToQTHandle(ConstHandle qtHandle);	// given a VWMM handle return a QT handle copy
	...
	void PrintMemoryStats(Uint32 flags) const;
	static Uint32 GetMemoryBlockHash(const MemoryBlock *block);
	...
	Uint32 GetHandleOversize(ConstHandle handle) const;
	...
	size_t GetAllBlocks(const void* buffer[], size_t bufferSizeInPtrSize) const; // returns the count of blocks in buffer. Unused elements will be NULL.
	size_t GetAllBlocks(std::vector<const void*>& buffer) const; // returns the count of blocks in buffer. Unused elements will be NULL.
	void GetBlockStackTrace(const void* block, char* outBufferNullTerminated, size_t bufferSizeInChars) const; // If not collecting stack trace info, this will return an empty string in buffer
	size_t GetBlockSize(const void* block) const;
	bool BlockArrayContainsBlock(const void *const blockArray[], size_t blockArraySize, const void* block) const; // returns true if the array contains the given block.
	bool BlockArrayContainsBlock(const std::vector<const void*>& blockArray, const void* block) const; // returns true if the array contains the given block.
	...
	size_t SubtractBlockArrays(const void *const leftArray[], size_t leftArraySize, const void *const rightArray[], size_t rightArraySize, const void* resultArray[], size_t resultArraySize);
	size_t SubtractBlockArrays(const std::vector<const void*>& leftArray, const std::vector<const void*>& rightArray, std::vector<const void*>& resultArray);
...
namespace QTMM
{
	...
	MM_DLL_EXPORT Size		GetHandleSize(ConstHandle h);
