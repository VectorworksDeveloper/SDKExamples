...
enum {
	...
	varUseOccludedPickingAndSnapping		= 105,	// Boolean read-write (gets and sets whether occluded picking and snapping is used)
...
//	varUnit2DimPrecision      = 219,	// Sint32   (old name kept for compatibility below this enum)
	...
	varDebugWriteRenderworksFilePath = 243, //Pointer to TXString, read-write - sets and gets the path to save Renderworks scene load and update files - Public for VS
	varPhasingUIFeatureEnabled = 244,	// Boolean, if TRUE, phasing is enabled - Public for VS
	...
	// PDF Export
	varUseAdobePDFAuthoring		= 271,	// Boolean	[true] - For internal use only

	varReportSupportedImageFormats	= 276,	// Boolean	(value ignored) Write Only - For internal use only
	...
	varDXF_SetLineColorsBlack			= 317,	// Boolean	[false] - Public for VS, Write (set) only
		// When varDXF_SetLineColorsBlack is set to true, respectively false, then both varDXF_SetPenColorsBlack and varDXF_SetFillColorsBlack are set to true, respectively false 
	...
	// New variables for VW29...
	varDXF_ImportAllObjInActiveLayer	= 356,	// Boolean	[false] - Public for VS
	varDXF_PreserveGraphicAttributes	= 357,	// Boolean	[true]	- Public for VS
	varDXF_GraphicAttrKeepByLayer		= 358,	// Boolean	[true]	- Public for VS
	varDXF_AttachClipBoundaryToBlock	= 359,	// Boolean	[false] - Internal use only! A&A JWCAD project specific  
	
	// New variables for VW30...
	varDXF_SetPenColorsBlack			= 360,	// Boolean	[false] - Public for VS
	varDXF_SetFillColorsBlack			= 361,	// Boolean	[false] - Public for VS
...
//	varDefaultImageFillTessellate		= 525,	// bool		read/write - UNIMPLEMENTED. Indicates if image fill is tessellated.
	...
	varDarkSymPreviews							= 602,  // To get screen space reflection OpenGL option.

	varDisplayHiddenObjectsGhosted				= 604,  // Boolean

	varLowDetailExistCompObjs			= 605,	// Boolean
	varLowDetailExistSyms_Pios			= 606,	// Boolean
	varLowDetailRemovedCompObjs			= 607,	// Boolean
	varLowDetailRemovedSyms_Pios		= 608,	// Boolean
	varAltAttribRemovedObjs				= 609,	// Boolean
	...
	//
	// 3D Pane of Vectorworks Preferences Dialog
	//
	varDefaultRenderMode							= 1318,	// Default Render Mode setting in Preferences(3D Pane)	- short	read/write
	varDefaultProjection							= 1319,	// Default Projection setting in Preferences(3D Pane)	- short	read/write

	varRemember3DRenderModeAndProjection			= 1320,	// Boolean read-write (gets and sets whether render modes and projections are remembered for 3D views (i.e., true) or whether the default render mode and projection are used when the document is switched from Top/Plan to a 3D view (i.e., false))
	varRememberPrimary3DViewsSeparately				= 1321,	// Boolean read-write (gets and sets whether separate render modes and projections are remembered for primary 3D views and other 3D views (e.g., isometric and custom))
	// End 3D Pane
	...
	varPrefsInter_AppearanceProps					= 1325 + 0 + 0,	// this is used in combination with the offsets below
			...
			offset_AppearanceProps_opacity2								= 14,	// Sint32		read/write - Public for VS - [0..100]
				...
				offset_group_AppearanceProps_GeneralObjectPreviews								= 53 * 30,
				offset_group_AppearanceProps_GeneralUnhideObjectsButton							= 54 * 30,
				offset_group_AppearanceProps_OnscreenViewControlTextAndAccents					= 55 * 30,
				offset_group_AppearanceProps_OnscreenViewControlCube							= 56 * 30,
				offset_group_AppearanceProps_OnscreenViewControlWorkingPlaneCubeFace			= 57 * 30,
				offset_group_AppearanceProps_GeneralActiveViewPaneLabel							= 58 * 30,
	...
	varIsEnteredInGroup				= 6756,				// Boolean	read - Indicates whether the VW document is currently editing a group

	varCurrentLayerScale			= 6757,				// double	read	- Public for VS - The current layer scale factor (not necessarily for the active layer).
	...
	varDocPrefSaveVGMCache		= 6840,

	varSavedViewNewClassVis = 6841, //SignedByte read/write	- Visibility of newly imported classes in Saved Views
	varViewportNewClassVis = 6842, //SignedByte read/write	- Visibility of newly imported classes in Viewports
	varGraphicLegendNewClassVis = 6843,//SignedByte read/write	- Visibility of newly imported classes in Graphics Legends.
	...
	varShowItemsNotInWorkspace			= 7001,
	varShowCommandContext				= 7002,
	varActivateToolSetOnToolSelection	= 7003,
	//End of ODM Preferences variables
	...
	varIsDarkModeActive = 7013,		// Boolean	read only - True when dark mode is enabled for the app
	varDisplayToolHelpOnStatusBar = 7014,	// Boolean	read/write - True when tool help is displayed on the status bar

	// Worksheet Pane Preferences variables
	varWorksheetColumnCropMarkerDisplayAlways	= 7020,	// Boolean	read/write
	varWorksheetMenuDisplayAlways				= 7021,	// Boolean	read/write 
	//End of Worksheet Pane Preferences variables

	varEnableLegacyEightLights = 7100,
	varEnableLegacyEightShadows = 7101,
	varEnableEmitters = 7102,
	...
	varUseVGM2025 = 10100,		// short read/write
	
	// 12222 is a kludge only in VS GetPref to return gSysEnv.FarEastVersion //B054450
};
