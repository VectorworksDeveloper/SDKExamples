...
const short kUnusedNode1		= 1;	const short kUnusedNode1Prop	= 0;	// see comment after list of tdTypes if using a currently unused slot
...
const short kDimUnitSettingsNode = 7;	const short kDimUnitSettingsProp	= 0;	// Type is 'DimUnitSettingsArrayType'
...
const short kWorksheetNode		= 18;	const short kWorksheetProp		= prSubscribe;	// Type is 'WorksheetType'. Was EstmType
...
const short kUnusedNode23		= 23;	const short rotProp				= 0;	// prBox+prGroup+prGraphicObject;	// No longer used directly, although it is an abstract base class.  Type is 'RotType'. see comment after list of tdTypes if using a currently unused slot
...
const short kViewportStyleNode  = 36;	const short kViewportStyleProp	= prSubscribe + prSysNameRefs;	// Type is ViewportStyleType
const short kUnusedNode37		= 37;	const short kUnused37Prop		= 0;	// Never used; was poly3Node; see comment after list of tdTypes if using a currently unused slot
...
const short kUnusedNode46		= 46;	const short kUnused46Prop		= 0;	// Obsolete. Type is 'HatchContainerType'. see comment after list of tdTypes if using a currently unused slot
...
const short kUnusedNode50		= 50;	const short kUnused50Prop		= 0;	// view command item. see comment after list of tdTypes if using a currently unused slot
const short comPalNode			= 51;	const short comPalProp			= prGroup;	// command palette header.	Type is 'ComPalType'.
const short kUnusedNode52		= 52;	const short kUnused52Prop		= 0;	// obsolete - was worksheetNode from long ago. see comment after list of tdTypes if using a currently unused slot
const short kUnusedNode53		= 53;	const short kUnused53Prop		= 0;	// obsolete; see comment after list of tdTypes if using a currently unused slot
...
const short kColorPaletteNode	= 80;	const short kColorPaletteProp	= 0;	// For drawing's color palette. Type is 'ColorPaletteInfoType'.
const short kLightNode			= 81;	const short kLightProp			= prBox+prGraphicObject+pr3D+prColor; // Type is 'LightType'.
const short kRoofEdgeNode		= 82;	const short kRoofEdgeProp		= prBox+prGraphicObject+pr2D+pr3D+prDash+prStroke+prFill+prColor+prHandler+prOpacity;	// Type is 'RoofEdgeType'.
...
const short kRoofAdornerNode	= 87;	const short kRoofAdornerProp	= pr3D+pr2D+prHandler;	// Type is 'RoofAdornerArrayType'.
...
/////////////////////////////////////////////////////////////////////////////////////////
// When you add a new TDType by replacing a kUnusedNode constant (for example const short kUnusedNode7 = 7 was changed
// to const short kDimUnitSettingsNode = 7), there are a number of things you need to do. First the property constant for
// the TDType, which is to the right of the TDType constant should be renamed (kUnusedNode7Prop was renamed to
// kDimUnitSettingsProp), and the value for the property needs to be changed to have the needed property bits (property
// bits are defined above the list of TDTypes and have comments). Then in the properties array (in Objs.Properties.h), 
// the old property constant needs to be replaced with the new constant.
// A number of other files will not compile until you add support for the new TDType.
// In CheckObject.cpp, in TCheckObject::CheckThis, in the section for obsolete types, remove the case for the kUnusedNode
// constant being replaced, and add a case for the new TDType constant. In the case put a CheckHandleSize call, and
// add any other checks that are needed for the type. In TCheckObject::ShouldRemoveTextureTableNode, remove the old
// constant or replace it with the new one, as appropriate.
// In Drw.cpp, in LLDrwObject, replace the old constant with the new one, and add drawing code if needed.
// In FilesizeInfo.cpp, in the ADD_TDTYPE with the old constant, replace it with the new one.
// In MemoryMaintenance.cpp, in the CheckObjectSize function, replace the old constant with the new one, and add
// appropriate code to set theSize.
// In FileTran.cpp, in CheckIllegalFileTDTypes, remove the old constant.
// In DebugListView/Data/Utils.cpp, in the CUtils constructor, replace the old constant with the new one.
// In DWG Library/Source/DWG Source/Export.AD.Entities, replace the old constant with the new one, and add
// code for exporting the new tdtype to DWG if needed.
// If it has the property prGraphicObject, in VGS_DirectLink/Source/VGS_DirectLink/VGS_Handler.cpp, in
// VGSEventHandler::getTDTypeName, add a case for the new tdtype.
//
// Some other changes are needed to create an object of the new type.
// In MObject.cpp, add a case for the new tdType to GetInstantiatedDiskObjectSizeVW30.
// In MObject.Creation.cpp, add cases for the new tdType to NewInstantiatedClassHandle, to GetInstantiatedObjectNodeSize,
// and to CopyInstantiatedObjectHandle_.
// In ProjectSharingConstants.h, in the changeCategories array, replace the kUnused EChangeCategory in the slot for the new 
// tdType with a new EChangeCategory constant for the new tdType, and define the new EChagneCategory constant in the list
//  of EChangeCategory's above changeCategories with the proper behavior.
//
// If you add a field containing a RefNumber to a node type, you must add the prSysNameRefs
...
// RefNumber-containing field to the following routines:
//
...
//
// If a RefNumber reference contains the RefNumber of a drawing object, special additional
// handling is required in the Duplicate/CopyNode code.  See Mike Hartman, or whoever
// is maintaining the UMapList code. 
//
// If you are modifying a tdtype that is saved to disk, you need to change kCurrentFileVersion in 
...
const short oldDimNode				= kDimUnitSettingsNode;
...
struct ProjectionType {
			...
			if (projection < inRight.projection)
				...
				if (clipDistance < inRight.clipDistance)
					...
					if (crsPersDistance < inRight.crsPersDistance)
						...
						if (perspectiveType < inRight.perspectiveType)
							outResult = -1;
						else if (perspectiveType == inRight.perspectiveType) {
	...
	short					perspectiveType = 0; // kPerspectiveStandard
	WorldPt					frustumShift = WorldPt(0.0,0.0);
...
enum class ELevelsOfDetail : Sint16  {
	NotSet = 0,
	Low = 1,
	Medium = 2,
	High = 4,
