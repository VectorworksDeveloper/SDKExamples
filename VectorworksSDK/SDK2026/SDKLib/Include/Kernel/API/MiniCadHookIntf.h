...
const ToolPropID kToolXPropBoomerangSelection = 8;					// the tool wants alt/option key to boomerang into the cursor tool to allow selection
const ToolPropID kToolXPropPreservePointsOnPause = 9;					// the tool wants its tool points to be preserved when it gets paused and restored
const ToolPropID kToolXPropStopDataBarUpdateNotification = 10;			// the tool wants to not update the data bar when it is drawing
...
const Sint32 kToolProvideObjectGripsYES						=  1L;	// Observed by the ToolProvideObjGripsMessage::kAction action caller. The reshape Handles/Grips WILL be drawn.
#endif
...
const Sint32 kToolWantsWorkingPlaneConstrainedToView		= 21L;	// Observed by CurrentToolAllowsWorkingPlaneConstraint() in flat views.
const Sint32 kToolDoesNotWantWorkingPlaneConstrainedToView	= 22L;	// Observed by CurrentToolAllowsWorkingPlaneConstraint() in flat views.

#if ENABLE_RIGHT_MOUSE_DOWN_TOOL_MESSAGE
const Sint32 kToolOverrodeRightMouseDownContext				= 23L;	// Observed by ToolMessage::kAction_RightMouseDown event caller.
#endif
...
//		 Plug-in provides custom hide factor		 //                     // See documentation for more info. DEPRECATED, NO LONGER OBSERVED.
//---------------------------------------------------//---------------------//-------------------------------------------------------------------------------------------------
...
//													 //						// - kObjXPropEditGroupCustom - IProviderSpecialGroups::ShowOthersEditGroupForSpecialGroup is called to detirmine whether to show others;
...
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropWidgetGroupMode = 81;	//			NO			//  kObjXPropWidgetGroupDisabled = 1;	Collapsible widget group is not supported. This is the
													//						//	situation before the widget group feature is available.
//		// Sint8									//						//	kObjXPropWidgetGroupAutomatic = 2;	In this mode, widget groups can be collapsed or expanded.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
...
//		// Boolean									//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropPageBaseProfileGroup = 85; //		YES			// Object with page-base profile group.
//		// Sint8									//						// The geometry and the text in some object are always drawn in Page Units in their edit layout context.
//													//						// Without setting the paperScale value to 1.0 the text is being displayed in world scale points.
//													//						// See EObjXPropPageBaseProfileGroup for options.
//													//						// When EObjXPropPageBaseProfileGroup::eConditional VWParametric_EventSink::OnIsProfileGroupPageBased will be called.
//													//						// Return true if the profile group is currently page-based, false if world-based.
//													//						// If this extended property is used, the param object should have "World-based" parameter.
//													//						// This pamametre is used in the recurce manager.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
...
//		// Boolean									//						// specify this XProp as the purpose.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropHasConstrainedGroup = 93;//			NO			// When this property is set to true, the transaction controller will not get suspended
//		// Boolean									//						// when you receive ObjectState::kExitFromEditGroup.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropResetOnRecChange = 94;//			NO			// When this property is set to true, the object will be reset when its attached record
//		// Boolean									//						// field gets its value changed from the data pane, or when the object gets a record attached/detached
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropCustomReshapeMultiSelection = 95;//	NO			// When set to true, if reshaping in multiple selection - <IProviderReshape> and <IProviderCustomReshape> will be initialized
//		// Boolean									//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropReshapeDontShiftToOrigin = 96;//	NO			// When set to true, object matrix will not be shifted when path origin vertex is moved during reshape.
//		// Boolean									//						// The path will be adjusted without moving the object.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropSupportsWallClosure = 97;//	NO					// When set to true, walls can create a closure around the object. This is for wall insertable objects.
//		// Boolean									//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropDefaultUseWallClosureValue = 98;//	NO			//  The default Use Wall Closure value
//		// Boolean									//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropDefaultExcludeWallClosureFromBottomValue = 99;//	NO //  The default Exclude Wall Closure from Bottom value
//		// Boolean									//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropHandleGenericQTO = 100;	//			NO / YES	// When this property is set to true, the object will be notified to handle generic (no parameters) QTO functions (EQTOFunction)
//		// Boolean									//						// The handling can be done via VWWSFunctionCall_EventSink implementation or via ParametricGetQTOValue messate to the parametric
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropInteractiveSizingInsert = 101;		//	NO		//  The interactive Sizing Insert value (For Doors/Windows - Task: T02309:Interactive Sizing for Door and Window Creation and Editing)
//		// Boolean									//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropDisableCoincidentSelPrioritization = 102;//	NO	//  When a parametric object enables this property, conincident selection will be disabled. This means that if it is selected, 
//		// Boolean									//						// and nother object is coincident with any of the sides, when that side is clicked, it will change the selection to the other object
													//						// instead of leaving the selection to the parametric object, so the user can drag it for example
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropDisableVelumOpacity = 103;//		NO			//  When a parametric object enables this property, it will allow it to set different frame and opacity values.
//		// Boolean									//						// Developers will need to assign the parametric's pen and fill opacities to objects in its profile group.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropDoesNotUpdateIfRemappingIsLocked = 104;//	NO		//When a parametric object enables this property,and if TParamConstraintMgr::fIsConnectionRemappingLocked is TRUE ,
// //Boolean																//the core will not reset the object and will not send a state.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropAllowEquipmentItemAttach = 105;//		NO		// Stores the name of the object if it wants equipment items to be able to attach to it
													//						// String
													//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropMoveImportedStyles = 106;//			NO			//  When a parametric object enables this property, and if supports styles,
// //Boolean																// the core will automatically move imported styles to the default folder got from OnGetStyleFolder function.
													//						//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjPropCustomHanding = 107;		//			NO			// Allow an object to opt into custom handing which included flipping and mirroring
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropAllowedOnLayers = 108;	//			NO			// Allow an object to specify the type of layers on which it is allowed to be placed (with OIP layer widget)
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropHandleSymbolInsertion = 109;	//	YES			// Allow an object to handle the drag and drop of a symbol from the RM onto itself.
																			// Also handle the symbol insertion and selection(also removal out of the object) tools.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropAssemblyUnit = 110;		//			NO			// Allow an object to occupy Door and Window Assembly Units by implementing IAssemblyUnitObject
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------

//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjPropCustomHandingContainer = 111;//		NO			// Allow an object to opt into custom handing which includes flipping and mirroring, but with a
																			// single interior-side widget for the object overall and multiple crank widgets for child objects.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
const ObjectPropID kObjXPropInvisibleInPlan = 112;	//			NO			// Determine whether an object is invisible while in plan view when the object is vertical to the x/y plane.
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------

const ObjectPropID kLastObjXProp = kObjXPropInvisibleInPlan;// last property ID

// !!! REMEMBER to add an entry to the object property description array in file EPEModulePropsAccess.cpp for any new property IDs you add here !!!
//
//                         Property ID				//	Has Associated		//  Description:
//													//		Event(s)		//
//--------------------------------------------------//----------------------//-------------------------------------------------------------------------------------------------
...
const unsigned char kHideReadOnlyWidgets = 64;
...
// kObjXPropAllowedOnLayers
const Sint8 kObjXPropAllowedOnLayersAny			= 0;
const Sint8 kObjXPropAllowedOnLayersDesignOnly	= 1;
const Sint8 kObjXPropAllowedOnLayersSheetOnly	= 2;

enum class EObjXPropPageBaseProfileGroup : Sint8
{
	eFalse,
	eTrue,
	eConditional,
...
enum EWidgetType {
	...
	kWidgetResourceTextPopup = 109,
	kWidgetMultiSelectionSearchablePopup = 110,
};
/////////////////////////////
...
const Sint32 kObjectEditPluginStyleEnteringProfileSkipUpdate	= -13;	// This is event response constant indicating that the plugin is entering a profile group from the
														// edit style dialog and will update the styled objects and style symdef after exiting the profile, so the update
														// performed by EditPluginStyle can be skipped. (value can't be -12 because of conflict with EObjectEvent::kObjectEventStayLoaded
...
const TInternalID kInternalID_GradeLimitsContour		=	653;

const TInternalID kInternalID_CCCableRoute				=	654;
const TInternalID kInternalID_CCDropPoint				=	655;
const TInternalID kInternalID_SoftGoods2                =   656;

const TInternalID kInternalID_MagnetObject              =   657;

const TInternalID kInternalID_GraphicLegend				=	658;
const TInternalID kInternalID_GraphicLegendImage		=	659;
const TInternalID kInternalID_GraphicLegendFrame		=	660;
const TInternalID kInternalID_GraphicLegendTitleFrame	=	661;

const TInternalID kInternalID_CCAdaptor					=	662;

const TInternalID kInternalID_ElevationBenchmark2		=	663;

const TInternalID kInternalID_Hedgify					=	664;

const TInternalID kInternalID_Fence						=	665;
const TInternalID kInternalID_Railing2					=	666;
const TInternalID kInternalID_Distributor				=	667;

const TInternalID kInternalID_Cabinets					=	668;
const TInternalID kInternalID_Countertop2				=	669;

const TInternalID kInternalID_Unused					=	670;

const TInternalID kInternalID_BrxHoist					=	671;
const TInternalID kInternalID_SummaryKey				=   672;

const TInternalID kInternalID_FurnitureCW				=	673;

const TInternalID kInternalID_CCPanelLayout				=	674;
const TInternalID kInternalID_CCPanelConnector			=	675;

const TInternalID kInternalID_Curb						=	676;
const TInternalID kInternalID_ExistingTree				=	677;

const TInternalID kInternalID_CCLabelFormat				=	678;

const TInternalID kInternalID_DoorAndWindowAssembly		=	679;
const TInternalID kInternalID_AssemblyFrame				=	680;
const TInternalID kInternalID_AssemblyMullion			=	681;
const TInternalID kInternalID_AssemblyLintel			=	682;
const TInternalID kInternalID_AssemblyPanel				=	683;
const TInternalID kInternalID_AssemblyShutter			=	684;
const TInternalID kInternalID_AssemblySill				=	685;
const TInternalID kInternalID_AssemblyStool				=	686;
const TInternalID kInternalID_AssemblyTrim				=	687;

const TInternalID kInternalID_CCMarkUp					=	688;

const TInternalID kInternalID_AssemblyWallInfill		=	689;
const TInternalID kInternalID_AssemblySymbol			=	690;
const TInternalID kInternalID_RampCW					=	691;
const TInternalID kInternalID_SurveyPoint				=	692;

const TInternalID kInternalID_LEDWall					=	693;
const TInternalID kInternalID_RecessCW					=	694;
const TInternalID kInternalID_LEDPanel					=	695;
const TInternalID kInternalID_LEDAccessory				=	696;
const TInternalID kInternalID_SideArm					=	697;
const TInternalID kMAX_INTERNAL_INDEX					=	kInternalID_SideArm;
//****** DO NOT ADD INTERNAL IDS BELOW THIS LINE ***************//
