...
/// <summary>
///	Unicode-compliant interface that replaces fopen
/// </summary>
/// <remarks>
/// The Windows implementation of fopen does not support Unicode filenames, it is necessary to use _wfopen
/// </remarks>
#if GS_MAC
inline FILE* vw_fopen(const TXString& filename, char const* mode)
{
	return fopen(filename.GetCharPtr(), mode);
}
[[deprecated("Use vw_fopen for Mac and Windows")]] FILE* fopen(const char* filename, const char* mode);
#else
inline FILE* vw_fopen(const TXString& filename, char const* mode)
{
	return _wfopen(filename.GetWCharPtr(), TXString(mode).GetWCharPtr());
}
