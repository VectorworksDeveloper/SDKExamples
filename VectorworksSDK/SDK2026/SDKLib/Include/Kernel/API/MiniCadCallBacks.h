...
const GSError duplicateFilenameErr =0x03000006;	// filename alrueady exists
...
const EPluginStyleParameter kPluginStyleParameter_ByMixed = 4;
...
//
//		These selectors let you get and set properties of
...
//////////////////////////////////////////////////////////////////////////////////////////
// GetDimensionAngularUnitVariable and SetDimensionAngularUnitVariable selectors
//
//		These selectors let you get and set properties of
//		specific angular dimension unit settings.  They have the return/passed
//		type as a comment following the selector

const short dimAngUnitSettingName = 1;	// (TXString)	the name of the angular dimension unit setting
const short dimAngUnitType		= 2;	// (Sint16)		the type of angular units: degrees(0), radians(1) or gradians(2)
const short dimAngUnitMode		= 3;	// (Sint16)		angular form (degrees minutes seconds / decimal) and precision, for display
const short dimAngUnitLeading0	= 4;	// (Boolean)	whether to display a leading zero when in a decimal mode
const short dimAngUnitTrailing0	= 5;	// (Boolean)	whether to display trailing zeros when in a decimal mode

//////////////////////////////////////////////////////////////////////////////////////////
// GetDimensionLinearUnitVariable and SetDimensionLinearUnitVariable selectors
//
//		These selectors let you get and set properties of specific
//		linear dimension unit settings.  They have the return/passed
//		type as a comment following the selector. If a unit setting is
//		created with the CreateDimensionLinearUnitSetting command and a
//		standard primary unit name is passed in, such as "Feet", a number
//		of these primary properties will be set to the default value for the
//		standard primary unit name passed in.

const short dimLinUnitSettingName	= 1;	// (TXString)	the name of the linear dimension unit setting
const short dimLinUnitPrimUnitName	= 2;	// (TXString)	primary unit name ("Feet & Inches", "Meters", "Inches", ...)
const short dimLinUnitPrimUnitMark	= 3;	// (TXString)	primary unit mark (mm, ", ', cm, ...)
const short dimLinUnitPrimSUnitMark = 4;	// (TXString)	for Feet & Inches: the feet unit mark (')
const short dimLinUnitPrimSUnitDivider = 5; // (TXString)	for Feet & Inches: the dash for dividing feet and inches (1'-3")
const short dimLinUnitPrimUnitsPerInch = 6;	// (double)		primary units per inch (mm: 25.4, inches: 1, feet and inches: 1, etc.)
const short dimLinUnitPrimFormat	= 7;	// (Sint16)		2 for Feet & Inches, 0 for everything else
const short dimLinUnitPrimSMultiplier = 8;	// (Sint16)		for Feet & Inches: the number of inches in a foot (12)
const short dimLinUnitPrimDecRoundType = 9; // (Sint8)		kRoundTenth (0), kRoundQuarter (1), kRoundHalf (2)
const short dimLinUnitPrimDecPrecision = 10; // (Sint32)	n where 10^n is the decimal dimension rounding value
const short dimLinUnitPrimFracPrecision = 11; // (Sint32)	n where 2^n is the fractional dimension rounding value
const short dimLinUnitPrimRoundStyle = 12;	// (Sint8)		fractional (0), decimal (1), exact as fractions/non-exact as decimal (2)
const short dimLinUnitPrimFractDisplay = 13; // (Sint8)		horizontal (0), diagonal (1), vertical (2)
const short dimLinUnitPrimShowUnitMark = 14; // (Boolean)	whether to display the unit mark
const short dimLinUnitPrimShowThousands = 15; // (Boolean)	whether to display the thousands separator
const short dimLinUnitPrimLeading0	= 16;	// (Boolean)	whether to display a leading zero when in a decimal mode
const short dimLinUnitPrimTrailing0 = 17;	// (Boolean)	whether to display trailing zeros when in a decimal mode
const short dimLinUnitPrimIsMetric = 18;	// (Boolean)	whether the unit is metric
const short dimLinUnitPrimIsCustom = 19;	// (Boolean)	whether the unit is custom
											// secondary settings are used when the dimension standard is dual stacked or dual side by side
const short	dimLinUnitSecUnitName	= 20;	// (TXString)	secondary unit name ("Feet & Inches", "Meters", "Inches", ...)
const short dimLinUnitSecUnitMark	= 21;	// (TXString)	secondary unit mark (mm, ", ', cm, ...)
const short dimLinUnitSecSUnitMark	= 22;	// (TXString)	for Feet & Inches: the feet unit mark (')
const short dimLinUnitSecSUnitDivider = 23;	// (TXString)	for Feet & Inches: the dash for dividing feet and inches (1'-3")
const short dimLinUnitSecUnitsPerInch = 24;	// (double_gs)	seconday units per inch (mm: 25.4, inches: 1, feet and inches: 1, etc.)
const short dimLinUnitSecFormat		= 25;	// (Sint16)		2 for Feet & Inches, 0 for everything else
const short dimLinUnitSecSMultiplier = 26;	// (Sint16)		for Feet & Inches: the number of inches in a foot (12)
const short dimLinUnitSecDecRoundType = 27;	// (Sint8)		kRoundTenth (0), kRoundQuarter (1), kRoundHalf (2)
const short dimLinUnitSecDecPrecision = 28; // (Sint32)		n where 10^n is the decimal dimension rounding value
const short dimLinUnitSecFracPrecision = 29; // (Sint32)	n where 2^n is the fractional dimension rounding value
const short dimLinUnitSecRoundStyle	= 30;	// (Sint8)		fractional (0), decimal (1)
const short dimLinUnitSecShowUnitMark = 31; // (Boolean)	whether to display the unit mark
const short dimLinUnitSecShowThousands = 32; // (Boolean)	whether to display the thousands separator
const short dimLinUnitSecLeading0	= 33;	// (Boolean)	whether to display a leading zero when in a decimal mode
const short dimLinUnitSecTrailing0	= 34;	// (Boolean)	whether to display trailing zeros when in a decimal mode
const short dimLinUnitSecIsMetric	= 35;	// (Boolean)	whether the unit is metric
const short dimLinUnitSecIsCustom	= 36;	// (Boolean)	whether the unit is custom
...
const TStandardView standardViewUserDefined			= 0;
const TStandardView standardViewFront				= 3;
const TStandardView standardViewBack				= 4;
const TStandardView standardViewLeft				= 5;
const TStandardView standardViewRight				= 6;
const TStandardView standardViewTop					= 7;
const TStandardView standardViewBottom				= 8;
const TStandardView standardViewRightIso			= 9;
const TStandardView standardViewLeftIso				= 10;
const TStandardView standardViewRightRearIso		= 11;
const TStandardView standardViewLeftRearIso			= 12;
const TStandardView standardViewBottomRightIso		= 13;
const TStandardView standardViewBottomLeftIso		= 14;
const TStandardView standardViewBottomRightRearIso	= 15;
const TStandardView standardViewBottomLeftRearIso	= 16;
...
const TProjection projectionOrthogonal			= 0;
const TProjection projectionPerspective			= 1;
const TProjection projectionCavalierOblique45	= 2;	// parallel oblique projection at angle
const TProjection projectionCavalierOblique30	= 3;	// parallel oblique projection at angle
const TProjection projectionCabinetOblique45	= 4;	// parallel oblique projection at angle
const TProjection projectionCabinetOblique30	= 5;	// parallel oblique projection at angle
const TProjection projectionPlan				= 6;

const short perspectiveStandard			= 0;
const short perspectiveTwoPoint			= 1;

//////////////////////////////////////////////////////////////////////////////////////////
//Render Constants (from MObject.h)
typedef short TRenderMode;
const TRenderMode renderWireFrame				= 0;
const TRenderMode renderQuick					= 1;
const TRenderMode renderSolid					= 2;
const TRenderMode renderShadedSolid				= 3;
const TRenderMode renderShadedNoLines			= 4;
const TRenderMode renderFinalShaded				= 5;
const TRenderMode renderFinalHiddenLine			= 6;
const TRenderMode renderDashedHiddenLine		= 7;
const TRenderMode render2DSection				= 8;
const TRenderMode render3DSection				= 9;
const TRenderMode renderOpenGL					= 11;
const TRenderMode renderFastRenderWorks			= 12;
const TRenderMode renderFastShadowRenderWorks	= 13;
const TRenderMode renderFinalRenderWorks		= 14;
const TRenderMode renderCustomRenderWorks		= 15;
const TRenderMode renderOutputVRML				= 16;
const TRenderMode renderArtisticRenderWorks		= 17;
const TRenderMode renderSketch					= 18;
...
enum class EDateTimeFormat
{
	...
	DOW_Y_MMM_D_TIME			= 27,
	YMD_TIME					= 28,
	YYYYMMDD					= 29,	//year month day format for .DBF dates (no separator)
	MMDDYYYY					= 30,	//month day full year format for file creation date
	YYYY_MM_DD					= 31,	// year month day format for ifcdate (hyphen for separator)
	TIMESTAMP					= 32,   //yyyymmddhhmmss
	DDMMYY						= 33,
	DDMMYY_TIME					= 34,
	DDMMYYYY					= 35,
	DDMMYYYY_TIME				= 36,
	YYMMDD						= 37,	// no separator
	YYMMDDSys					= 38,	// separator set by system
	YYMMDDSys_TIME				= 39,
	MDYYYY						= 40,
	MDYYYY_TIME					= 41,
	MMDDYY						= 42,
	...
	DDMMMYY_TIME				= 45,
	MMDDYYYY_TIME				= 46,
	YYYYMMDDSys					= 47, //year month day format using separator set by system
	YYYYMMDDSys_TIME			= 48,
	YYYYMD						= 49,
	YYYYMD_TIME					= 50
};
...
struct TDateTimeIntlData
{
	...
	bool			fShortDateCompleteYear;
	bool			fShortDateLeadingZeros;
	bool			fShortDateUseMonthAbbrevs;
};
...
enum class EForEachMergedFileSource {
	AppFolder = 1,
	WorkgroupFolder = 2,
	UserFolder = 3,
};
	...
	struct IntersectSurfaceInformation {
 	...
 	//
	// --------------------------------------------------------
	// kludge used to raise a notification
	// --------------------------------------------------------
	//
    const Sint16 kKludgeNotifyAny = 5573;
    struct NotifyAnyStuff {
        NotifyAnyStuff(StatusID selector) { notifySelector = selector;  additionalData = 0; notifyImmediate = false; }
        StatusID	notifySelector;
        StatusData	additionalData;
		bool		notifyImmediate;
    };

	//
	// --------------------------------------------------------
	// kludge used for requesting search results from the resource manager
	// --------------------------------------------------------
	// 

	const Sint16 kGetSearchResultsRM = 5590;
	...
	struct SCatalogTypeMenu
	{
		TXString RMDB_menuitem;
		TXString JSON_menuitem;
	};
	struct SCatalogBuilderDialogData
	{
		...
		TXString output_group;
		TXString output_JSONpath_label;
		TXString output_button;
		TXString catType_label;
		TXString output_chooserDlg_label;
		SLibraryTypeMenu libMenu;
		SCatalogTypeMenu catMenu;
		...
		Sint16		fLibraryGroup;	// kVWLibraryGroupDefaultContent or kVWLibraryGroupVWLibrary
	...
	//----------------------------------------------------------------------------------
	// kludge that sends notification for Georeferencing settings change and wraps it in undo primitive
	//----------------------------------------------------------------------------------

	const Sint16 kKludgeGeorefChanged = 7561;

	//
	enum ConvertModeEnum { ConvertWireframeMode,ConvertHiddenMode,ConvertDashedMode };
	    
    //-------------------------------------------------------------------------------------
    // kludge that activates the plant tool for given symbol
    //-------------------------------------------------------------------------------------
    
    const Sint16 kActivatePlantTool = 6023;
...
//	this is sent for attributes object change
{
	...
	double			fDoubleValue = 0.0; // used only for kSizeChange
}; 
...
struct KludgeObjectResetAfterTranslateExt
{
	TXString	fObjectNameUniversal;
	short		fOldVersionNum;				// this is a Vectorworks version, e.g. 24, 25, 26, etc. (FYI 25 = Vectorworks 2020)
	short		fOldBugFixVersionNum;		// Update (or Service Pack) e.g. 2 for Update 2, 3 for Update 3.
};
...
public:
	CGSColorValueChange();
	CGSColorValueChange(const ColorRef newColor, const bool bColorByClass = false, const bool bShowByClass = false);
	virtual ~CGSColorValueChange();
	
	ColorRef fNewColor;
	bool	 fbColorByClass = false;
	bool	 fbShowByClass = false;
};
...
public:
    ...
    CGSLineWeightValueChange(const short newLineWeight, const bool bWeightByClass = false, const bool bShowByClass = false);
...
public:
    ...
    const CGSMenuItemValue* GetMenuItemAtIndex( const unsigned int menuItemIndex) const;
	bool RemoveMenuItemAtIndex(const unsigned int menuItemIndex);

	const std::vector<CGSMenuItemValue*>& GetMenuItemCollection() const { return fMenuItemCollection; }
	bool IsEmpty() const { return fMenuItemCollection.empty(); }
...
public:
    ...
    explicit CGSMenuItemValue(const bool bIsSeparator);
    virtual ~CGSMenuItemValue();

    void AddSubMenu(const CGSMenuItemCollection& subMenu);
	bool HasSubMenu() const;
	const CGSMenuItemCollection& GetSubMenuItemCollection() const;
    ...
    Sint64 fMenuItemId = 0;
    double fMenuItemIdDbl = .0;
    TXString fMenuItemIdStr;
    bool fbIsSeparator = false;
...
public:
	...
	virtual bool AddItemWithColor(const TXString& itemIDStr, const TXString& itemName, const TXString& toolTip, const TXString& imageSpec, const EUiControlTextTint itemColor) = 0;
	virtual bool AddChildItemWithColor(const TXString& itemIDStr, const TXString& itemName, const TXString& parentIDStr, const TXString& toolTip,
		const TXString& imageSpec, const EUiControlTextTint itemColor) = 0;
	virtual bool AddStaticItemWithColor(const TXString& itemIDStr, const TXString& itemName, const TXString& toolTip, const TXString& imageSpec, const EUiControlTextTint itemColor) = 0;
};
...
const Sint32 kContextMenuMessage		= -12761; // dialog should return context menu items to be created
const Sint32 kMenuItemSelectedMessage	= -12762; // dialog should process menu item selection
const Sint32 kDeveloperHelpMessage		= -12763; // developer help requested for item under cursor
...
enum EWSDialogType {
	...
	kWSSetHeaderFooterPopover		= 13,
	kWSSetMarginPopover 			= 14,
	kWSSetBordersPopover 			= 15,
	kWSSettingsPopover				= 16,
	kWSNumberFormatPopover 			= 17,
	kWSImageFormatPopover 			= 18,
};
...
enum EListBrowserOwnerDrawnType {
	kListBrowserNotOwnerDrawn,	
	kListBrowserSolidRect,		
	kListBrowserDualSolidRect,	
	kListBrowserPatternRect,		
	kListBrowserDualPatternRect,	
	kListBrowserGradientOrImage,	
	kListBrowserBlankOwnerDrawn,	
	kListBrowserTextOwnerDrawn,	
	kListBrowserDashedLine,		
	kListBrowserTile,			
	kListBrowserHatch,			
	kListBrowserLineWeight,		
	kListBrowserLineEnd,			
	kListBrowserResource,		
	kListBrowserLineStyleList	// kListBrowserLineStyleList - T02214: temporary value that supports temporary criteria dialog-only list browser line style edit control.
								// Should be removed when it is possible to select non-resource attributes via resource selector. [VML 05/12/21]
};
...
class EMenuEnableFlags
{
public:
	constexpr EMenuEnableFlags() : fBits(0) {};
	constexpr explicit EMenuEnableFlags(uint16_t bits): fBits(bits) {};
	
	const EMenuEnableFlags operator|(const EMenuEnableFlags& other) const { return EMenuEnableFlags(fBits | other.fBits); }
	EMenuEnableFlags& operator|=(const EMenuEnableFlags& other) { fBits |= other.fBits; return *this; }

	const EMenuEnableFlags operator&(const EMenuEnableFlags& other) const { return EMenuEnableFlags(fBits & other.fBits); }
	bool operator==(const EMenuEnableFlags& other) const { return fBits == other.fBits; };
	
	uint16_t GetRawValue() const { return fBits; }
	
private:
	uint16_t	fBits;
	
public:
	static const EMenuEnableFlags None;
	static const EMenuEnableFlags DocIsActive; 				  // A document is open
	static const EMenuEnableFlags ProjectFileConnected;  	  // Project Sharing is active and connected
	static const EMenuEnableFlags DocChanged;				  // The document has unsaved changes
	static const EMenuEnableFlags FilesLeft;				  // We can open more files, the max limit hasn't been reached
	static const EMenuEnableFlags DocOnDisk;				  // The document has already been saved on disk
	static const EMenuEnableFlags HomeScreen;				  // The Home Screen is open
	static const EMenuEnableFlags ObjectSelected;			  // An object is selected
	static const EMenuEnableFlags MultipleSelected;			  // More than one object is selected
	static const EMenuEnableFlags SelHas2D;					  // The selected object is 2D
	static const EMenuEnableFlags SelHas3D;					  // The selected object is 3D
	static const EMenuEnableFlags PlanProj;					  // The active projection is Top/Plan
	static const EMenuEnableFlags ActiveWorkingFile;		  // The active document is Project Sharing working file.
	static const EMenuEnableFlags SheetLayer;				  // The active layer is a Sheet layer
	static const EMenuEnableFlags GetEnableStateFromMenuItem; // This flag signals to VectorWorks that it should
															  // send a MenuItemEnabledMessage::kAction to the menu item with the current
															  // conditions in a short.
															  // Return kMenuItemIsEnabled if the menu item should be enabled.*/
};

struct MenuEnableAccess
{
	EMenuEnableFlags needs;
	EMenuEnableFlags needsNot;
};
...
struct MenuCheckHiliteMessage : public MenuChunkInfo
{
	...
	std::map<short, bool>	fmapItemID2Checked;
};
...
struct ParametricSymbolInsertionMessage : public ParametricMessage
{
	enum
	{
		kAction	= 88
	...
	ParametricSymbolInsertionMessage(MCObjectHandle hSymbol)
		: ParametricMessage( kAction )
		, fhSymbol( hSymbol )
	{}

	MCObjectHandle	fhSymbol;
};
...
struct ContainerHandingParam
{
	ContainerHandingParam()
	{
		fHandingParameterSetID		= -1;
		fOffsetFromContainerCenter	= 0.0;
		fUnitWidth = 0.0;
		fOperativeHandDirParam.reset();
		fOperativeSideParam.reset();
	}

	ContainerHandingParam(const ContainerHandingParam& inParam)
	{
		fHandingParameterSetID		= inParam.fHandingParameterSetID;
		fOffsetFromContainerCenter	= inParam.fOffsetFromContainerCenter;
		fOperativeHandDirParam		= inParam.fOperativeHandDirParam;
		fOperativeSideParam			= inParam.fOperativeSideParam;
		fUnitWidth = inParam.fUnitWidth;
	}

	ContainerHandingParam& operator=(const ContainerHandingParam& inParam)
	{
		fHandingParameterSetID		= inParam.fHandingParameterSetID;
		fOffsetFromContainerCenter	= inParam.fOffsetFromContainerCenter;
		fOperativeHandDirParam		= inParam.fOperativeHandDirParam;
	...
	Sint32 fHandingParameterSetID;
	WorldCoord fOffsetFromContainerCenter;
	WorldCoord fUnitWidth;
	SInteractiveSizingParameterInfo fOperativeHandDirParam;
	SInteractiveSizingParameterInfo fOperativeSideParam;
};

typedef std::map<short, ContainerHandingParam*> ContainerHandingParams;
...
//================================================
// 
// //================ New Parametric Message - Get Handing Information ================
...
//================ New Parametric message - Set Handing Information ================
...
//================ New Parametric message - Get Custom Scale ================
struct ParametricGetCustomScale : public ParametricMessage
{
	enum { kAction = 96 };

	ParametricGetCustomScale(const double& scale) 
	{ 
		fAction = kAction; 
		fScale = scale; 
	}
...
struct ParametricSymbolRemovalMessage : public ParametricMessage
{
	enum
	{
		kAction	= 97
	...
	ParametricSymbolRemovalMessage(MCObjectHandle hSymbol)
		: ParametricMessage( kAction )
		, fhSymbol( hSymbol )
	{
	}
...
struct ParametricSymbolConstrainMessage : public ParametricMessage
{
	enum
	{
		kAction	= 98
	...
	ParametricSymbolConstrainMessage(MCObjectHandle hSymbol, WorldPt3 currentInsertionPoint)
		: ParametricMessage( kAction )
		, fhSymbol( hSymbol )
		, fSelToolCurrInsertionPoint( currentInsertionPoint )
	{
	}

	MCObjectHandle	fhSymbol;
	WorldPt3		fSelToolCurrInsertionPoint	{};
	WorldPt3		fConstrainedInsertionPoint	{};
};

struct ParametricOverrideRotatedPathElevation : public ParametricMessage
{
	enum { kAction = 99 };

	ParametricOverrideRotatedPathElevation(const double& elevation)
		: ParametricMessage( kAction )
...
//================ Parametric message - Get Container Handing Information ================
struct ParametricGetContainerHandingInformation : public ParametricMessage
{
	enum { kAction = 100 };
	ParametricGetContainerHandingInformation(ContainerHandingParams& localContainerHandingInfo, SInteractiveSizingParameterInfo& interiorSideHandingParam, Sint32 updatedHandingParameterSetID)
	{
		fAction = kAction;
		fHandingParameterSetID	= updatedHandingParameterSetID;
		fContainerHandingInfo	= localContainerHandingInfo;
		fInteriorSideHandingParam = interiorSideHandingParam;
	}

	Sint32 fHandingParameterSetID;
	ContainerHandingParams fContainerHandingInfo;
	SInteractiveSizingParameterInfo fInteriorSideHandingParam;
};

struct ParametricSetContainerHandingInformation : public ParametricMessage
{
	enum { kAction = 102 };
	ParametricSetContainerHandingInformation(const ContainerHandingParams& localContainerHandingInfo, const SInteractiveSizingParameterInfo& interiorSideHandingParam, Sint32 updatedHandingParameterSetID)
	{
		fAction = kAction;
		fHandingParameterSetID	= updatedHandingParameterSetID;
		fContainerHandingInfo	= localContainerHandingInfo;
...
struct SDupObjInfo
{
	...
	UuidStorageMap* uuidMap = nullptr;

	SDupObjInfo(InternalIndex* pWallFakeName, UuidStorageMap* inUuidMap = nullptr) 
	{
		wallFakeName = pWallFakeName;
		uuidMap = inUuidMap;
	}
};
...
// EUndoSelectionState contains the valid values for determining selection behavior for an object on undo.
// Used in ISDK::AddUndoRedoSelectObj()
enum EUndoSelectionState {eSelectOnUndo,
						  eDeselectOnUndo};

// ERedoSelectionState contains the valid values for determining selection behavior for an object on redo.
// Used in ISDK::AddUndoRedoSelectObj()
enum ERedoSelectionState {eSelectOnRedo,
						  eDeselectOnRedo};

// used for ISDK::ShowCriteriaDialogContext() and by CriteriaDialog.cpp
enum ECriteriaContext {
	eCriteriaContextUnspecified,
	eDatabaseCriteria,			//Used for worksheet database criteria
	eVectorScriptCriteria,		//Used for VectorScript editor
	eCustomSelCriteria,			//Used for custom selection
	eCustomModificationCriteria,	//Used for custom modification menu items
	eCheckoutCriteria,           // for Custom Checkout
	eReleaseCriteria,            // for Custom Release
	eAttributeByDataCriteria,	// for Live Data Visualization
	eGraphicLegendCriteria,		// for Graphic Legend
	eCustomVisCriteria,			// Used for custom visibility
	eDataTagCriteria,			// for Data Tag
	eClassificationsCriteria,	// for Classifications Palette
    eDataManagerCriteria,       // for Data Manager

	eLastValidCriteriaContext
};

// used for ISDK::ShowCriteriaDialogExt() and by GetCustomCriteria
struct SCriteriaDialogParam
{
    SCriteriaDialogParam() {}
    SCriteriaDialogParam(const TXString& criteriaString): inOutCriteriaString(criteriaString) {}
    
    TXString inOutCriteriaString; // The criteria expression (Required)
    TXString inOutName; // The name assigned to the criteria in the dialog (Optional)
};


// used for ISDK::IsVisible
enum class EVisibilityType {
	eActiveClassesLayers = 0,	// Visible considering only layer/class;
	eHideNonPlot,				// As above, and also not hidden/ghosted by custom hide or datavis
	eShowNonPlot				// As above, but does show ghosted as visible.
};
