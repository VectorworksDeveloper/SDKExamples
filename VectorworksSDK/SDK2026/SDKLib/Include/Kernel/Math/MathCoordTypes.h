...
#pragma pack(pop)	// END 2 BYTE STUCTURE AIGNMENT <.vwx file format>
inline bool operator==(const FloatPt3& first, const FloatPt3& second)
{
	return FloatsAreNearlyEqual(first.x, second.x) && FloatsAreNearlyEqual(first.y, second.y) && FloatsAreNearlyEqual(first.z, second.z);
}
inline bool operator!=(const FloatPt3& first, const FloatPt3& second)
{
	return !(first == second);
...
public:
	...
	Degrees GS_API Get2DRotationAngle(const bool returnNegative = true) const;

	bool GS_API IsOrthogonal(bool allow2DOrtho = false) const;
...
inline bool operator==(const TransformMatrix& matrix1, const TransformMatrix& matrix2)
//
