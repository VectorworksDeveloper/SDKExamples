...
#if GS_WIN
	#define txuar(string) L##string
	#define txu(string) L##string
	#define txuc(string) L##string
#else
	#define txuar(string) u##string
	#define txu(string) (const UCChar*)u##string
	#define txuc(ch) (const UCChar)u##ch
#endif
// Files that use txu/txuc with special characters need to be saved with a BOM in MSVS (or resaved
// if editted in xcode). BomCheck assert can be added for a char in those files. I can't show an example
// because a char here even in a comment causes problems. Go find a usage in DateTime.cpp.
template<typename T, size_t Tsize>
constexpr bool BomCheck(const T(& glyph)[Tsize], size_t check, std::initializer_list<const wchar_t> bytes)
{
	return (glyph[0] == *bytes.begin() && Tsize == check);
}
...
Sint32 CStr<maxCharLen>::Format(ConstGSStr255Param format, ...)
{
	...
	result = vsnprintf(this->buf, sizeof(this->buf), szFormat, ap);
...
Sint32 CStr<maxCharLen>::Format(const char *format, ...)
{
	...
	result = vsnprintf(this->buf, sizeof(this->buf), format, ap);
...
public:
	...
	{
		gVWMM->TXStringDeallocate((char**)&p);
	}
...
// Forward declaration in order to mark as friend in TXString.
struct CaselessHasher;
...
public:
	...
	TXString(TXString&& src) noexcept;	// Move constructor
	TXString(const StdUStr& src);		// Copy constructor from StdUStr
	TXString(StdUStr&& src) noexcept;	// Move constructor from StdUStr
	...
	TXString(const UniChar* src);
	TXString(const UniChar* src, size_t len, ELengthCheck eLengthCheck = ELengthCheck::eYes);
	...
	explicit TXString(UniChar ch, size_t count = 1);
	...
	TXString& operator=( TXString&& src ) noexcept;	// Move assignment operator
	TXString& operator=(const std::string& src);	// Assignment from std::string
	TXString& operator=(const std::string_view& sv);// Assignment from std::string_view
	TXString& operator=(const std::u16string& src);	// Assignment from std::u16string
	...
	TXString& operator=(const UniChar* src);
	TXString& operator=(const wchar_t* src);
	
	// From single character
	TXString& operator=(const char ch);
	TXString& operator=(const unsigned char ch);
	TXString& operator=(const UCChar ch);
	TXString& operator=(const UniChar ch);
	TXString& operator=(const wchar_t ch);
	TXString& operator=(const int codePoint);		// For example: txString = 0x27
	...
	TXString& operator+=(const UniChar ch);
	TXString& operator+=(const wchar_t ch);
	TXString& operator+=(const int n);		// Unicode code point, for example: 0x63
	...
	TXString& Insert(size_t pos, const UniChar* src);
	TXString& Insert(size_t pos, const wchar_t* src);
	
	// Insert one character 'ch' at position 'pos'.
	TXString& Insert(size_t pos, const char ch);
	TXString& Insert(size_t pos, const unsigned char ch);
	TXString& Insert(size_t pos, const UCChar ch);
	TXString& Insert(size_t pos, const UniChar ch);
	TXString& Insert(size_t pos, const wchar_t ch);
	TXString& Insert(size_t pos, const int ch);						// e.g. 0xA5, '¥'
	...
	TXString& MakeUpper();
	TXString& MakeLower();
	TXString& MakeCapitalized(TXChar previousChar = 0);
	...
	operator const UniChar*() const;			// UTF-16
	...
	const UniChar* GetUniCharPtr() const { return (operator const UniChar*()); }
...
private:
	const std::u16string & GetConstU16String() const;	// For hashers only.
	friend std::hash<TXString>;
	friend CaselessHasher;
public:
#if GS_MAC
	...
	void CopyInto(UniChar* dst, size_t bufElemSize) const;
	...
	TXString& ftoa(Real64 value, Sint32 precision=6);
	static TXString	ToStringInt(Sint64 value);
	static TXString	ToStringInt(Sint64 value, Sint32 width, wchar_t paddingChar);
	static TXString	ToStringReal(Real64 value, Sint32 precision=6, bool fixed=true);
	template<typename T>
	...
	// Swap function
	//***********************************************************************************
	void swap(TXString& rhs) noexcept
    {
...
namespace std
{
	...
	template<>
	inline void swap<TXString>(TXString& lhs, TXString& rhs) noexcept(noexcept(lhs.swap(rhs)))
    {
        lhs.swap(rhs);
    }

	// std::hash<TXString> specialization
	// IMPORTANT: One of the requirements of a hash function is that any 2 inputs that are considered equivalent must return the same hash value.
	// Since TXString::operator== is case-insensitive, the hash function should be as well. However, it is case-sensitive because its input is not case folded.
	// Note that msvc's hash function (Windows) functions like it is case-insensitive due to its poor quality. See VB-195419
	template<>
...
/// Performs a case-insensitive hash on TXString.
struct CaselessHasher
{
	std::size_t operator()(const TXString& str) const
	{
		return std::hash<std::u16string>{}(TXString{ str }.MakeUpper().GetConstU16String());
	}
};

/// Performs a case-insensitive equality on TXString.
struct CaselessEqual {
	bool operator()(const TXString& str1, const TXString& str2) const {
		return str1.EqualNoCase(str2);
	}
};


inline bool setmemberCaseSensitive(const TXString& x, const TXString& head)
{
	return x.Equal( head );
}

template<typename... Strings>
inline bool setmemberCaseSensitive(const TXString& x, const TXString& head, const Strings&... tail)
{
	return x.Equal( head ) || setmemberCaseSensitive( x, tail... );
}
...
public:
	...
	// Comparison operator
	friend bool operator==(const TXStringArray& lhs, const TXStringArray& rhs);
...
/*static*/ TXString TXString::ToString(T value)
{
	if constexpr (std::is_floating_point_v<T>) {
		// see c++26 note in ToStringReal
		return TXString::ToStringReal(value);
	} else {
...
Uint8 PStr<maxCharLen>::Format(ConstGSStr255Param format, ...)
{
	...
	size_t max = sizeof(this->buf) - 1;
	if ( max > 255 )
		max = 255;
	int n = vsnprintf((char*) &this->buf[1], max, szFormat, ap);
	if ( n >= 0 )
		this->buf[0] = Uint8(n < max ? n : max);
...
Uint8 PStr<maxCharLen>::Format(const char *format, ...)
{
	...
	if ( max > 255 )
		max = 255;
	int n = vsnprintf((char*) &this->buf[1], max, format, ap);
	if ( n >= 0 )
		this->buf[0] = Uint8(n < max ? n : max);
