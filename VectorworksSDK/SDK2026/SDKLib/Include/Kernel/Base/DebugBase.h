...
// 4) Optionally add an entry like '#define kMyProgConst kRBerge' to your personal ExternalUser.h file
//    for a constant like '#define kRBerge TProgrammer("RBerge", ...)'. The quoted name is your os userid
//    but mac and win ids don't always agree. kMyProgConst can id you even if the string comparison fails.
...
////////////////////////////////////////////////////////////////////////////////
// Use the following macro to print TXStrings or Rect's in a printf type
// format string.  For example:
//
//		DMSG((dlAlways, "File name: %s\n", TXSTRINGPRINT(filename)));
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    // The __aarch64__ preprocessor flag is used here, instead of
    // __arm64__ - both flags should serve the same purpose in the
    // usage context of the application; however, the __aarch64__
    // flag seems to imply an Apple-specific 64-bit architecture
    // (as suggested by the following line of code from Platform.h -
    // https://opensource.apple.com/source/WTF/WTF-7601.4.2/wtf/Platform.h):
    // #if (defined(__arm64__) && defined(__APPLE__)) || defined(__aarch64__)
    #if __aarch64__
        #define Int13BreakMacro { __asm__("brk #0xF000\n" : : ); }
    #else
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	struct ValidateParameters {
		void*	thePointerOrHandle;	// keep this structure aligned with the one defined for RELEASE
		Sint32	expectedSize;		// this is very important as plugins can be compiled DEBUG[TEST] and used with RELEASE Vectorworks
									// this way the extra data can be disgarted by the RELEASE Vectorworks when passed in
...
#else  //
	struct ValidateParameters {
		void* thePointerOrHandle;	// This must be synced with the one defined for TEST
		Sint32	expectedSize;
	};
#endif // TEST
...
// Functions to change debug library behavior.
	...
	namespace GSDebug {
		...
		bool GSDEBUG_EXPORT WantBackgroundToBreak();
		bool GSDEBUG_EXPORT WantBackgroundToBreak(bool bWantToBreak);
...
{
	public:
		TProgrammer()											{ Set("Unnamed", 0, 0, 0, 0, 0, false, true); }

		TProgrammer(const char *name, Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1, Uint64 flags, bool onlyIfActiveUser, bool sendOwn=true) { Register(name, id4, id3, id2, id1, flags, onlyIfActiveUser); Set(name, id4, id3, id2, id1, flags, onlyIfActiveUser, sendOwn); }
		TProgrammer(const TProgrammer& a)						{ Set(a.fName, a.fID4, a.fID3, a.fID2, a.fID1, a.fFlags, a.fOnlyIfActiveUser, a.fSendOnOwn); }
	
		const char *GetName() const 							{ return fName; }

		bool IsEveryone() const;
		bool IsActLikeRelease() const							{ return !fID4 && !fID3 && !fID2 && !fID1; }
		...
		// at the moment, it looks like nothing needs these
		bool operator ==(const TProgrammer& a) const			{ return MatchExact(a); }
		bool operator !=(const TProgrammer& a) const			{ return !MatchExact(a); }

		bool operator ==(const char *sz) const					{ return _strcmpi(sz, fName)!=0; }
		bool MatchId(const TProgrammer& a) const				{ return fID4 == a.fID4 && fID3 == a.fID3 && fID2 == a.fID2 && fID1 == a.fID1; }
		bool MatchExact(const TProgrammer& a) const				{ return MatchId(a) && fFlags == a.fFlags && fOnlyIfActiveUser == a.fOnlyIfActiveUser; }
		bool IsIn(const TProgrammer& a) const					{ return IsIn(a.fID4, a.fID3, a.fID2, a.fID1); }

		// Return true if this "is in" the passed id pair -- which could be a single programmer or a group.
		bool IsIn(Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1) const;

		bool ContainsCurrentUser() const;

		static bool GetProgrammerName(Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1, char *name);
		static bool ExtractProgrammer(TProgrammer &who, TProgrammer &one);
	
		bool IsNoNetDebugBitSet() const							{ return (fFlags & GSDebug::kNoNetDebugFlag) != 0; }
		bool IsNoBreakBitSet() const							{ return (fFlags & GSDebug::kNoDebuggerBreakFlag) != 0; }
		bool IsNoWindowBitSet() const							{ return (fFlags & GSDebug::kNoDebugWindowFlag) != 0; }
		bool IsNoStackCrawlBitSet() const						{ return (fFlags & GSDebug::kNoStackCrawlFlag) != 0; }
		bool IsOnlyIfActiveUserBitSet() const					{ return fOnlyIfActiveUser; }

	protected:
		void Set(const char *name, Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1, Uint64 flags, bool onlyIfActiveUser, bool sendOwn)		{ strncpy(fName, name, kDebugStringLength-1); fID4 = id4; fID3 = id3; fID2 = id2; fID1 = id1; fFlags = flags; fOnlyIfActiveUser = onlyIfActiveUser; fSendOnOwn = sendOwn; }
	...
	private:
		...
		{
			public:
				TRegistered()									{ fID4 = 0; fID3 = 0; fID2 = 0; fID1 = 0; fName[0] = 0; fFlags = 0; fOnlyIfActiveUser = false; }
				char	fName[kDebugStringLength];
				Uint64	fID4, fID3, fID2, fID1;
				Uint64	fFlags;
				bool	fOnlyIfActiveUser;
		};
		static void Register(const char *name, Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1, Uint64 flags, bool onlyIfActiveUser);
		static const Sint32 fMaxProgrammers;
		static TRegistered fRegistered[];

		char	fName[kDebugStringLength];
		Uint64	fID4, fID3, fID2, fID1;	// Values in significance - so fID2 is higher significant bits than fID1.
		Uint64	fFlags;
		bool	fOnlyIfActiveUser;
		bool	fSendOnOwn;
	};
...
/////////////////////////////////////////////////////////////////////////////////////////
#if TEST
	#define MAKEWHO(name)			TProgrammer(name, 0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0, false, true)
	#define DECLAREWHO(var, name)	TProgrammer var(name, 0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL, 0, false, true)
...
//
//	Define name								File Value				fID4                   fID3                   fID2                   fID1                   flags    onlyIfActiveUser  sendOwn
	#define kJStrouse			TProgrammer("JStrouse",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000001ULL, 0,       false,            true)
	#define kMLieblein			TProgrammer("MLieblein",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000002ULL, 0,       false,            false)
	#define kMVorobyev			TProgrammer("MVorobyev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000004ULL, 0,       false,            true)
	#define kSteve				TProgrammer("Steve Johnson",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000008ULL, 0,       false,            true)
	#define kZTavor				TProgrammer("ZTavor",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000010ULL, 0,       false,            true)
	#define kPaul				TProgrammer("Paul",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000020ULL, 0,       false,            true)
    #define kAyo				TProgrammer("AOshinnaiye",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000040ULL, 0,       false,            true)
	#define kDave				TProgrammer("Dave",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000080ULL, 0,       false,            false)
	#define kDaveD				TProgrammer("DaveD",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000100ULL, 0,       false,            true)
	#define kAOwens				TProgrammer("AlanOwens", 			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000200ULL, 0,       false,            true)
	#define kSPotter			TProgrammer("SPotter",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000400ULL, 0,       false,            true)
	#define kDarick				TProgrammer("Darick",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000800ULL, 0,       false,            false)
	#define kAndrewB			TProgrammer("AndrewB",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000001000ULL, GSDebug::kNoDebugWindowFlag, false, false)
	#define kIStanchev			TProgrammer("IStanchev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000002000ULL, 0,       false,            true)
	#define kMark				TProgrammer("MFarnan",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000004000ULL, 0,       false,            false)
	#define kSPrabu				TProgrammer("Senthil Prabu",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000008000ULL, 0,       false,            true)
	#define kBruce				TProgrammer("Bruce",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000010000ULL, 0,       false,            false)
    #define kHernan				TProgrammer("Hernan",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000020000ULL, 0,       false,            true)
    #define kRobert				TProgrammer("Robert",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000040000ULL, 0,       false,            false)
	#define kKKitanov			TProgrammer("KKitanov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000080000ULL, 0,       false,            false)
//	#define XXXXX				TProgrammer("XXXXX",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000100000ULL, 0,       false,            true) empty
	#define kTShin				TProgrammer("tshin",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000200000ULL, 0,       false,            true)
	#define kKLalkovski			TProgrammer("KLalkovski",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000400000ULL, 0,       false,            true)
	#define kEJKerr				TProgrammer("JKerr",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000800000ULL, 0,       false,            true)
	#define kSasha				TProgrammer("Sasha",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000001000000ULL, 0,       false,            true)
	#define kBStanchev			TProgrammer("BStanchev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000002000000ULL, 0,       false,            true)
	#define kSKratsas			TProgrammer("Sherry Kratsas",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000004000000ULL, 0,       false,            true)
	#define kVPeichev			TProgrammer("VPeichev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000008000000ULL, 0,       false,            true)
	#define kVictor				TProgrammer("Victor",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000010000000ULL, 0,       false,            true)
	#define kAbdel				TProgrammer("Abdel",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000020000000ULL, 0,       false,            true)
	#define kAGhosh				TProgrammer("AGhosh",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000040000000ULL, 0,       false,            true)
	#define kAlex				TProgrammer("Alex",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000080000000ULL, 0,       false,            true)
	#define kBiplab				TProgrammer("Biplab",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000100000000ULL, 0,       false,            true)
	#define kFAbbasinejad		TProgrammer("FAbbasinejad",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000200000000ULL, 0,       false,            true)
	#define kNIvanova			TProgrammer("nivanova",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000400000000ULL, 0,       false,            true)
	#define kVStanev			TProgrammer("vstanev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000800000000ULL, 0,       false,            true)
	#define kMRizov				TProgrammer("MRizov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000001000000000ULL, 0,       false,            true)
	#define kNHilendarov		TProgrammer("NHilendarov",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000002000000000ULL, 0,       false,            true)
	#define kLArsenyuk			TProgrammer("LArsenyuk",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000004000000000ULL, 0,       false,            true)
	#define kMStratiev			TProgrammer("MStratiev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000008000000000ULL, 0,       false,            true)
	#define kRBerge		 		TProgrammer("RBerge",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000010000000000ULL, 0,       false,            true)
	#define kPChang				TProgrammer("PChang",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000020000000000ULL, 0,       false,            true)
	#define kKIvanov			TProgrammer("KIvanov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000040000000000ULL, 0,       false,            true)
	#define kDKasavetova		TProgrammer("DKasavetova",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000080000000000ULL, 0,       false,            true)
	#define kJHutchison			TProgrammer("JHutchison",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000100000000000ULL, 0,       false,            true)
	#define kCGraye				TProgrammer("CGraye",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000200000000000ULL, 0,       false,            true)
	#define kILane 				TProgrammer("ILane",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000400000000000ULL, 0,       false,            true)
	#define kNZhelyazkov		TProgrammer("NZhelyazkov", 			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000800000000000ULL, 0,       false,            true)
	#define kHTsafak			TProgrammer("HTsafak",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0001000000000000ULL, 0,       false,            true)
	#define kJLoy				TProgrammer("JLoy",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0002000000000000ULL, 0,       false,            true)
	#define kTolu				TProgrammer("Tolu",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0004000000000000ULL, 0,       false,            true)
	#define kDSanford			TProgrammer("DSanford",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0008000000000000ULL, 0,       false,            true)
	#define kDNguyen			TProgrammer("DNguyen",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0010000000000000ULL, 0,       false,            true)
	#define kBGeiger			TProgrammer("BGeiger",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0020000000000000ULL, 0,       false,            true)
	#define kJWarshaw			TProgrammer("JWarshaw",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0040000000000000ULL, 0,       false,            true)
	#define kBAtwood			TProgrammer("BAtwood",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0080000000000000ULL, 0,       false,            true)
	#define kTArdoin			TProgrammer("TArdoin",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0100000000000000ULL, 0,       false,            true)
	#define kMFlamer			TProgrammer("MarkFlamer",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0200000000000000ULL, 0,       false,            true)
	#define kGDixon				TProgrammer("GDixon",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0400000000000000ULL, 0,       false,            true)
	#define kVKitanov			TProgrammer("VKitanov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0800000000000000ULL, 0,       false,            true)
	#define kAGorbaty			TProgrammer("AGorbaty",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x1000000000000000ULL, 0,       false,            true)
	#define kFZafar				TProgrammer("FZafar",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x2000000000000000ULL, 0,       false,            true)
	#define kMGeorgiev			TProgrammer("MGeorgiev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x4000000000000000ULL, 0,       false,            true)
	#define kHLiatis			TProgrammer("HLiatis",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x8000000000000000ULL, 0,       false,            true)
	#define kHema				TProgrammer("Hema",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000001ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kMAydin				TProgrammer("MAydin",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000002ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kJGruendler			TProgrammer("JGruendler",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000004ULL, 0x0000000000000000ULL, 0,       false,            true)
//	#define XXXXX				TProgrammer("XXXXX",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000008ULL, 0x0000000000000000ULL, 0,       false,            true) empty
	#define kIBorisov			TProgrammer("IBorisov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000010ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kGKutleva			TProgrammer("GKutleva",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000020ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kEKolev				TProgrammer("EKolev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000040ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kNBeshevliev		TProgrammer("NBeshevliev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000080ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kKGoranov			TProgrammer("KGoranov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000100ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kBBudzon			TProgrammer("BBudzon",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000200ULL, 0x0000000000000000ULL, 0,       false,            true)
    #define kOKadry				TProgrammer("OKadry",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000400ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kVGenchev			TProgrammer("VGenchev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000800ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kAHadzhiev			TProgrammer("AHadzhiev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000001000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kBMarinov			TProgrammer("BMarinov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000002000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kGTaylor			TProgrammer("GTaylor",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000004000ULL, 0x0000000000000000ULL, 0,       false,            true)
    #define kLCusack            TProgrammer("LCusack",              0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000008000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kPBlanco			TProgrammer("PBlanco",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000010000ULL, 0x0000000000000000ULL, 0,       false,            true)
//	#define XXXXX				TProgrammer("XXXXX",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000020000ULL, 0x0000000000000000ULL, 0,       false,            true) empty
    #define kKChilaka           TProgrammer("KChilaka",             0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000040000ULL, 0x0000000000000000ULL, 0,       false,            true)
    #define kSHwang             TProgrammer("SHwang",               0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000080000ULL, 0x0000000000000000ULL, 0,       false,            true)
    #define kSWilcox            TProgrammer("SWilcox",              0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000100000ULL, 0x0000000000000000ULL, 0,       false,            true)
    #define kWZhu               TProgrammer("WZhu",                 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000200000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kTDimov				TProgrammer("TDimov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000400000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kBMerdzhanova		TProgrammer("BMerdzhanova",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000800000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kSPark				TProgrammer("SPark",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000001000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kSWilliams			TProgrammer("SWilliams",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000002000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kLMarni				TProgrammer("LMarni",			    0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000004000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kKAngelov			TProgrammer("KAngelov",			    0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000008000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kAHiguera			TProgrammer("AHiguera",			    0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000010000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kKSutton			TProgrammer("KSutton",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000020000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kTTanev				TProgrammer("TTanev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000040000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kBZarev				TProgrammer("BZarev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000080000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kTNikolov			TProgrammer("TNikolov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000100000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kMChakarov			TProgrammer("MChakarov",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000200000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kIAtovski			TProgrammer("IAtovski",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000400000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kYAleksiev			TProgrammer("YAleksiev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000800000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kMZeng				TProgrammer("MZeng",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000001000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
//	#define XXXXX			    TProgrammer("XXXXX",			    0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000002000000000ULL, 0x0000000000000000ULL, 0,       false,            true) empty
    #define kCCorbell           TProgrammer("CCorbell",             0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000004000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kTTodorov			TProgrammer("TTodorov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000008000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kADimitrov			TProgrammer("ADimitrov",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000010000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kPGeorgiev			TProgrammer("PGeorgiev",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000020000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
//	#define XXXXX				TProgrammer("XXXXX",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000040000000000ULL, 0x0000000000000000ULL, 0,       false,            true) empty
	#define kPMay				TProgrammer("PMay",					0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000080000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kJSimerly			TProgrammer("JSimerly",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000100000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kAAntonini			TProgrammer("AAntonini",		    0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000200000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kDLopezGutierrez	TProgrammer("kDLopezGutierrez",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000400000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kDPicolo			TProgrammer("DPicolo",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000800000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kRJodra				TProgrammer("RJodraBarron",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0001000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kKGAngelov			TProgrammer("KGAngelov",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0002000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kNNedev				TProgrammer("NNedev",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0004000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kBAtanasov			TProgrammer("BAtanasov",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0008000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kVMarkovski			TProgrammer("VMarkovski",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0010000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kJGlassman			TProgrammer("JGlassman",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0020000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kDMarinov			TProgrammer("DMarinov",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0040000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kJPeizer			TProgrammer("JPeizer",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0080000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kBStefanov			TProgrammer("BStefanov",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0100000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kNPark				TProgrammer("NPark",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0200000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kEBouillet			TProgrammer("EBouillet",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0400000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)

	// ComputerWorks Germany
	#define kSBender			TProgrammer("StefanBender",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0100000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kJKeser				TProgrammer("JuergenKeser",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0200000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kJStellmacher		TProgrammer("JanStellmacher",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0400000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kFDefelix			TProgrammer("Francois",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x0800000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kODaus				TProgrammer("OliverDaus",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x1000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kSLeirer			TProgrammer("SLeirer",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x2000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kTBendelic			TProgrammer("TitBendelic",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x4000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	
	#define kAGordon			TProgrammer("AGordon",				0x0000000000000000ULL, 0x0000000000000000ULL, 0x8000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kDRabiega			TProgrammer("Daniel Rabiega",       0x0000000000000000ULL, 0x0000000000000001ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kVPetrov			TProgrammer("VPetrov",				0x0000000000000000ULL, 0x0000000000000002ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kDTatarski			TProgrammer("DTatarski",			0x0000000000000000ULL, 0x0000000000000004ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)
	#define kDLigman			TProgrammer("DLigman",       		0x0000000000000000ULL, 0x0000000000000008ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0,       false,            true)

	// Special values
	#define kEveryone			MAKEWHO("Everyone")
	#define kActLikeRelease		TProgrammer("No One (null)",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0,                              false, false)
	#define kNoNetDebug			TProgrammer("No Net Debug",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, GSDebug::kNoNetDebugFlag,		false, false)
	#define kNoBreak			TProgrammer("No Debugger Break",	0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, GSDebug::kNoDebuggerBreakFlag,	false, false)
	#define kNoWindow			TProgrammer("No Window",			0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, GSDebug::kNoDebugWindowFlag,	false, false)
	#define kNoStackCrawl		TProgrammer("No Stack Crawl",		0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, GSDebug::kNoStackCrawlFlag,		false, false)
	#define kOnlyIfActiveUser	TProgrammer("Only If Active user",	0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0x0000000000000000ULL, 0,	                            true,  false)
	...
	#define kAValchev			( kNHilendarov | kBMerdzhanova | kMStratiev | kYAleksiev )
	#define kYRusev				( kNHilendarov | kBMerdzhanova | kMStratiev | kYAleksiev )
	#define kARomero			kEveryone
	#define kYKostadinov		( kNHilendarov | kBMerdzhanova | kMStratiev | kYAleksiev )
	#define kKYotova			kSMilev
	#define kZSarver			kEveryone
	#define kMao				kEveryone
	#define kCPHsiao			kEveryone
	#define kXYu				kEveryone
	#define kDShores			kEveryone
	#define kIMarinova			kSasha
	#define kSMilev				kSasha
	#define kKDyulgerov			kNZhelyazkov
...
// Release version definitions:
	...
	#define	kSKratsas			__ignore_programmer
    ...
    #define kCCorbell           __ignore_programmer
	#define kTTodorov			__ignore_programmer
	#define kADimitrov			__ignore_programmer
	#define kPGeorgiev			__ignore_programmer
	#define kPMay				__ignore_programmer
	#define kJSimerly			__ignore_programmer
	#define kAAntonini			__ignore_programmer
	#define kDLopezGutierrez	__ignore_programmer
	#define kDPicolo			__ignore_programmer
	#define kRJodra				__ignore_programmer
	#define kKGAngelov			__ignore_programmer
	#define kIMarinova			__ignore_programmer
	#define kKYotova			__ignore_programmer
	#define kKDyulgerov			__ignore_programmer
	#define kJGlassman			__ignore_programmer
	#define kDMarinov			__ignore_programmer
	#define kJPeizer			__ignore_programmer
	#define kBStefanov			__ignore_programmer
	#define kSMilev				__ignore_programmer
	#define kNPark				__ignore_programmer
	#define kEBouillet			__ignore_programmer
	#define kBAtanasov			__ignore_programmer
	#define kNNedev				__ignore_programmer
	#define kVMarkovski			__ignore_programmer

	// ComputerWorks Germany
	#define kSBender			__ignore_programmer
	#define kJKeser				__ignore_programmer
	#define kJStellmacher		__ignore_programmer
	#define kFDefelix			__ignore_programmer
	#define kODaus				__ignore_programmer
	#define kSLeirer			__ignore_programmer
	#define kTBendelic			__ignore_programmer

	#define kAGordon			__ignore_programmer
	#define kDLigman            __ignore_programmer
    #define kDRabiega           __ignore_programmer
	...
	#define kXYu				__ignore_programmer
	#define kDShores			__ignore_programmer
	#define kDRabiega			__ignore_programmer
...
#if BUG
	#if 1 || _MINICAD_EXTERNAL_ || _GSESDK_ || _GSWINSDK_
...
Sint32 GSDEBUG_EXPORT _AssertValidHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, GSConstHandle h, size_t lCount=1);
Sint32 GSDEBUG_EXPORT _AssertValidMenuHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, GSConstHandle h);
Bool8 GSDEBUG_EXPORT _VerifyValidHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, GSConstHandle h, size_t lCount=1);
...
inline void VerifyStop() { STOP(); }	// Special function since one can not put an "_asm" instruction in the middle of a "?:" expression. [DMB, 11/30/94]


#if _WINDOWS
// These are the Windows ASSERT macros.

	// why is win disabled and mac not?
	...
	// why is win ptr disabled and mac not, but string ok on both?
...
// These are the Mac ASSERT macros.
	...
	#define ASSERTHANDLE(toWhom, h, bytes) \
				((void)_AssertValidHandle(toWhom,THIS_FILE,__LINE__,#h,(GSConstHandle)h,bytes))
...
#define VERIFYHANDLE(toWhom, h, bytes)	\
			::_VerifyValidHandle(toWhom,THIS_FILE,__LINE__,#h,(GSConstHandle)h,bytes)

// All the string and pointer verifications should go away or wrap simple null checks
...
#define ASSERTMENUHANDLE(toWhom, menu) \
			::_AssertValidMenuHandle(toWhom,THIS_FILE,__LINE__,#menu,(GSConstHandle)menu)


// The concept of locked/unlocked handles no longer exists, but calls aren't all nuked
...
#else   // TEST
Bool8 GSDEBUG_EXPORT _VerifyValidHandle(GSConstHandle h, size_t lCount);
Sint32 GSDEBUG_EXPORT _AssertValidHandle(GSConstHandle h, size_t lCount);
#endif	// TEST
...
///////////////////////////////////////////////////////////////////////////////
	...
	#define VERIFYPATH(who, fullPath)				((BOOL)( (fullPath) != nullptr ))
...
//	#define ASSERTNOFPEXCEPTION()						DEBUG_DO_NOTHING
	...
	#define VERIFYSTRING(toWhom, psz, min, max)			((Bool8)( (psz) != nullptr ))
	#define VERIFYN(toWhom, exp)						((Bool8)( (exp) ? true : false ))
	#define VERIFYMSG(toWhom, exp, failMsg, ...)		((Bool8)( (exp) ? true : false ))
	#define VERIFYPOINTER(toWhom, ptr, bytes)			((Bool8)( (ptr) != nullptr ))
	#define VERIFYPOINTER_READONLY(toWhom, ptr, bytes)	((Bool8)( (ptr) != nullptr ))
	#define VERIFYHANDLE(toWhom, h, bytes)				::_VerifyValidHandle((GSConstHandle)h,bytes)
...
// DEPRECATED. Only use to address false alarms from SNYK
#define VERIFYASSERT VERIFYN
#define VERIFYISOBJ  VERIFYHANDLE
