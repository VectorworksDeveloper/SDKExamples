 ...
 ***************************************************************************************
 * 				(User Preference)
 * (Parameter)	Send nothing 	Send light		Send verbose	(Developers / Beta Users)
 * kLogLight		[v] 	 		[v]				[v]				[v]
 * kLogVerbose		[ ]		 		[ ]				[v]				[v]
 * kLogBeta			[ ]		 		[ ]				[ ]				[v]
 ***************************************************************************************
 ...
 kLogLight
 - Always logged
 - Should be used for critical information that MUST be transmitted back to
   servers. Use this sparingly.
 - Ex: Serial number validation, graphics information, version
 
 kLogVerbose [most common]
 - Should be used for details about application state
 - Ex: User preferences, errors, warnings, application interactions (eg. save, import, select tool, menu).
 
 kLogBeta
 - Should be used for tracking things we don't care too much about for typical users.
 - This is reserved for Beta users and developers. A normal user will NEVER log this message.
 - Ex: Something not that important, anything we care about as developers
...
enum ErrorReportingMode : short
{
	SendNothing = 0,
	SendCrashesOnly,
	SendCrashesAndLightLogs,
	SendCrashesAndVerboseLogs,
	SendLightLogs,
	SendVerboseLogs,
};
...
public:
	...
	TLogTimer(Uint16 level, const char* context, const char* category, double logThreshhold = 0.0);
	~TLogTimer();
private:
	Uint16 fLevel;
