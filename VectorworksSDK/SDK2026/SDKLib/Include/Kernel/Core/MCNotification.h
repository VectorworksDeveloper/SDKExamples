...
const Sint32 kNotifyDocChangingDone		= 'DOCD';		// Send after a document becomes frontmost
...
const Sint32 kNotifyDimUnitChange		= 'DIMU';
...
const Sint32 kNotifyViewStackChange		= 'VStk';
const Sint32 kNotifySavedViewChange		= 'SvVC';
const Sint32 kNotifyDynamicViewChange	= 'DyVC';		// Send with each interactive frame; !!CAUTION - check routine performance!!
...
const Sint32 kNotifyNavPalViewportModify	= 'NPVM';	// Send to update the Navigation Palette when a viewport's properties have been modified.
...
const Sint32 kNotifyRenderModeAboutToChange	= 'RndC';	// Send immediately before a render mode changes. additional data is the new render mode
const Sint32 kNotifyRenderModeChanged		= 'RnMC';	// Send immediately after  a render mode changes. additional data is the old render mode
const Sint32 kNotifyViewAboutToChange		= 'VATC';	// Send immediately before a view        changes. additional data is the new view if standard
const Sint32 kNotifyProjectionAboutToChange	= 'PATC';	// Send immediately before a projection  changes. additional data is the new projection
const Sint32 kNotifyProjectionChanged		= 'PrjC';	// Send immediately after  a projection  changes. additional data is the old projection
const Sint32 kNotifyPerspectiveDistChanged	= 'PsDC';	// Send immediately after  a perspective distance changes. no additional data
...
const Sint32 kNotifyProjectSharingStatusChanged = 'PSSC';   // Send when Project Sharing status has changed
...
const Sint32 kNotifyClassVisibilityChanged			= 'CVCH'; // Sent when a class visibility has changed so that Nav Palette can update
...
const Sint32 kNotifyToolModeFocusEdit				= 'TmFe';	// Sent to focus a control on the mode bar.
const Sint32 kNotifyInteractiveDrawCleared			= 'InOC'; // Sent when interactive or adorner objects draw lists are cleared. 
...
const Sint32 kNotifyGraphicLegendAnnotEditPalUpdate	= 'GLPU'; //Send notification that the Graphic Legend Annotation Edit Palette needs to update its values

const Sint32 kNotifyBCFManagerUpdate				= 'BCFU'; // Sent notification when loaded BCF data is changed. That must cause regeneration of the BCF Manager palette.
const Sint32 kNotifyRefreshZoom          			= 'RfZo'; // Sent when the zoom value has changed
const Sint32 kNotifyRefreshLayerScale          		= 'RfLS'; // Sent when the current layer scale value has changed

const Sint32 kNotifyUnifiedViewStateChanged         = 'UVSC'; // Sent when unified view is enabled or disabled

const Sint32 kNotifyViewBarPrefChanged				= 'VBPC'; // Sent when the view bar settings have changed

const Sint32 kNotifyGlobalTextStatusUpdated			= 'GTSU'; // Sent when the global text items status has been updated
const Sint32 kNotifySnappingShortcutsChanged		= 'SSCH'; // Sent when the snapping shortcut keys have been updated

const Sint32 kNotifyGeorefDocumentChanged			= 'GISc'; // Sent when the document/layer georeference settings are changed

const Sint32 kNotifyViewportStyleChange				= 'VPSt'; // Sent when a Viewport Style created, deleted or edited

const Sint32 kNotifyAllSnapGeometryCleared			= 'SGCL'; // Sent when all snap geometry is cleared through ToolSnapGeometry::RemoveAll()

const Sint32 kNotifyDMXRecordingPalRefresh			= 'DMXR'; // Sent when the DMX Palette needs to be refreshed

const Sint32 kNotifyAfterResourceImport				= 'ARIm'; // Sent when a resource is imported (currently only for Space styles, but can be extended)

const Sint32 kNotifyBeforeApplicationQuit			= 'BApQ'; // Sent immediately before the application quits

const Sint32 kNotifySubscripUdated					= 'SbUp'; // Send when a subscription [reference] has been updated

const Sint32 kNotifyThemeChanged					= 'ThCg'; // Sent when the theme has changed (light/dark)

const Sint32 kNotifyGenericWebPalette				= 'GWeb'; // Set by web-palette when it needs to show a dialog outside the SyncProxy callback due to eventing of sub-dialogs that need to open another web-dialog (Plant Style Manager for example). The additional data would provide unique route

const Sint32 kNotifyWorksheetImageChange			= 'WSIC'; // For updating the worksheet image widgets

const Sint32 kNotifyCloudStatusUpdate				= 'ClSt'; // Sent when the cloud status has changed, providing a nlohmann::json object with the new status
const Sint32 kNotifyCloudAction				        = 'ClAc'; // Sent when the cloud action is triggered from the UI, providing a nlohmann::json object with the action type

const Sint32 kNotifyActivePhaseChanged				= 'APCh'; // Sent when the active phase has changed
const Sint32 kNotifyPhaseListChanged				= 'PLCh'; // Sent when the phase list has changed
const Sint32 kNotifyPhaseAltAttribPrefChanged		= 'PAAP'; // Sent when the phasing alternate attribute preference changed
const Sint32 kNotifyRemovedObjectsAndMaterialsChanged = 'ROMC'; // Sent when the removed objects and materials option has changed
