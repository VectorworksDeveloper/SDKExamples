...
ViewCoord SafeAdd(ViewCoord v1, ViewCoord v2);
	...
	public:
		...
		double DegreeAngleFrom(const XViewPt& inOrigin) const	 { XViewPt p = *this - inOrigin; return p.AngleInDegrees(); }
	};
	#pragma pack(pop) // END 2 BYTE STRUCTURE ALIGNMENT (.vwx file format)
	//--------------------------------------------------------------------
	...
	public:
		...
		PagePt& operator+=(const PagePt& p)		{ x += p.x; y += p.y; return *this; }
	...
	struct GSColor {
		...
		void AssignFrom(const GSColor& src)								{ red = src.red; green = src.green; blue = src.blue; }

		Bool8 operator==(const GSColor& color) const					{ return (color.red == red && color.green == green && color.blue == blue); }
		Bool8 operator!=(const GSColor& color) const					{ return (color.red != red || color.green != green || color.blue != blue); }
	};
	...
	////////////////////////////////////////////////////////////////////////////////////////
	class ViewMatrix
	// A 2D transform is defined like this:
	// [x y 1] * [ a  b  0] = [ax+cy+tx, bx+dy+ty, 1]
	//           [ c  d  0]
	//           [tx ty  1]
	////////////////////////////////////////////////////////////////////////////////////////
	{
	public:
		...
		ViewMatrix();	// identity
		ViewMatrix(double ina, double inb, double inc, double ind, double intx, double inty);
		ViewMatrix(double inRadiansCounterClockwiseAngle, bool inbFlip, double intx, double inty);
	
		void Set(double ina, double inb, double inc, double ind, double intx, double inty);
		void Set(double inRadiansCounterClockwiseAngle, bool inbFlip, double intx, double inty);

		void Get(double& outRadiansCounterClockwiseAngle, bool& outbFlip, double& outtx, double& outty) const;

		void Concat(const ViewMatrix& inMat); // this = inMat * this;
	
		Bool8 operator==(const ViewMatrix& inMat) const;
		Bool8 operator!=(const ViewMatrix& inMat) const { return !((*this) == inMat); }

		Bool8 Inverse(ViewMatrix& outInvMat) const;

		void TransformPt(const double inX, const double inY, double& outX, double& outY) const;
		void InverseTransformPt(const double inX, const double inY, double& outX, double& outY) const;

		Bool8 TransformPtTrunc(const GSPoint32& inPt, GSPoint32& outPt) const;
		Bool8 TransformPtRound(const GSPoint32& inPt, GSPoint32& outPt) const;
		Bool8 InverseTransformPtTrunc(const GSPoint32& inPt, GSPoint32& outPt) const;
		Bool8 InverseTransformPtRound(const GSPoint32& inPt, GSPoint32& outPt) const;
		void TransformPt(const XViewPt& inPt, XViewPt& outPt) const;
		void InverseTransformPt(const XViewPt& inPt, XViewPt& outPt) const;

		Bool8 TransformRectTrunc(const GSRect32& inRect, GSRect32Verts& outVerts) const;
		Bool8 TransformRectRound(const GSRect32& inRect, GSRect32Verts& outVerts) const;
		Bool8 InverseTransformRectTrunc(const GSRect32& inRect, GSRect32Verts& outVerts) const;
		Bool8 InverseTransformRectRound(const GSRect32& inRect, GSRect32Verts& outVerts) const;
		void TransformRect(const XViewRect& inRect, XViewRectVerts& outVerts) const;
		void InverseTransformRect(const XViewRect& inRect, XViewRectVerts& outVerts) const;

		Bool8 TransformVertsTrunc(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const;
		Bool8 TransformVertsRound(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const;
		Bool8 InverseTransformVertsTrunc(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const;
		Bool8 InverseTransformVertsRound(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const;
		void TransformVerts(const XViewRectVerts& inVerts, XViewRectVerts& outVerts) const;
		void InverseTransformVerts(const XViewRectVerts& inVerts, XViewRectVerts& outVerts) const;

		// statics
		static void Multiply(const ViewMatrix& inLeft, const ViewMatrix& inRight, ViewMatrix& outResult);
	
		static ViewMatrix IdentityMatrix();
		static ViewMatrix TranslationMatrix(double tx, double ty);
		static ViewMatrix ScaleMatrix(double sx, double sy);
		static ViewMatrix RotationMatrix(double inRadiansCounterClockwiseAngle);
		static ViewMatrix RotationMatrix(double inRadiansCounterClockwiseAngle, double viewCenterX, double viewCenterY);
	};
	...
	public:
		TGSPoly();
		TGSPoly(Sint32 inNumPts);
		TGSPoly(const TGSPoly& inTGSPoly);
		TGSPoly(const PPGSPoly inppGSPoly);

		Sint32		GetNumUsed() const		{ return fNumPtsUsed; } // we support fNumPtsUsed < size, for the DrawBSPImage usage pattern
		Sint32		GetSize() const			{ return (Sint32)fPoints.size(); }
		Sint32		GetCapacity() const		{ return (Sint32)fPoints.capacity(); }
	
		bool		Reserve(size_t inNumPts);
		bool		Resize(size_t inNumPts);

		void		MakeEmpty();	// resizes to 0
		void		Restart();		// resets numPtsUsed to 0

		bool		CanAddPt() const;
		bool		AddPt(const ViewPt& inPt);
	
		ViewRect	GetBBox() const;
		void		SetBBox(const ViewRect inBox);
		void		RecalcBBox();

		void		Offset(ViewCoord dx, ViewCoord dy);

		PPGSPoly	ExtractGSPoly(Sint32 startIndex = 0, short inNumPts = -1/*all*/) const;
		// caller must delete the returned PPGSPoly handle
		...
		// assignment
		TGSPoly operator=(const TGSPoly& in);

		//
		// We support Restart(), which allows the fPoints to be larger than fNumPtsUsed.
		// This requires that we observe fNumPtsUsed when iterating.
		//
		class iterator
		// Example:
		// for (TGSPoly::iterator iter(yourTGSPolyInstance); iter.More(); iter.Next())
		//		(*iter) = ViewPt(1, 1);
		{
			public:
				friend class TGSPoly;
				
				iterator(TGSPoly& inPoly)
					: fPoly(inPoly), fIter(inPoly.fPoints.begin()), fiCurrent(0)
				{}
				
				ViewPt& operator*()		{ ASSERTN(kEJKerr, fiCurrent < fPoly.GetNumUsed()); return fiCurrent < fPoly.GetNumUsed() ? *fIter : fPoly[-1]; }
				Boolean More() const 	{ return fiCurrent < fPoly.GetNumUsed() && fIter != fPoly.fPoints.end(); }
				void Next()				{ fiCurrent++; fIter++; }
				Sint32 CurrentIndex() 	{ return fiCurrent; }
				
			private:
				TGSPoly&	fPoly;
				std::vector<ViewPt>::iterator fIter;
				Sint32		fiCurrent;
		};

		class const_iterator
		// Example:
		// for (TGSPoly::const_iterator iter(yourTGSPolyInstance); iter.More(); iter.Next())
		//		ViewPt vpt = *iter;
		{
			public:
				friend class TGSPoly;
				
				const_iterator(const TGSPoly& inPoly)
					: fPoly(inPoly), fIter(fPoly.fPoints.begin()), fiCurrent(0)
				{}
				
				const ViewPt& operator*()	{ ASSERTN(kEJKerr, fiCurrent < fPoly.GetNumUsed()); return fiCurrent < fPoly.GetNumUsed() ? *fIter : fPoly[-1]; }
				Boolean More() const 		{ return fiCurrent < fPoly.GetNumUsed() && fIter != fPoly.fPoints.end(); }
				void Next()					{ fiCurrent++; fIter++; }
				Sint32 CurrentIndex() 		{ return fiCurrent; }
			...
			private:
				const TGSPoly&	fPoly;
				std::vector<ViewPt>::const_iterator fIter;
				Sint32			fiCurrent;
		};
	...
	protected:
		Sint32				fNumPtsUsed;
		ViewRect			fBBox;
		std::vector<ViewPt>	fPoints;
		ViewPt				fvptBadValue;
	};

	//////////////////////////////////////////////////////////////////////////////
	class TGSPolyRecorder
	//////////////////////////////////////////////////////////////////////////////
	{
	public:
		TGSPolyRecorder(TGSPoly& inTGSPoly);
		~TGSPolyRecorder();
	...
	public:
		...
		void		MakeEmpty();
		void		RecalcBBox();
		void		Offset(XViewCoord dx, XViewCoord dy);

		XViewRect				fBBox;
		std::vector<XViewPt>	fPoints;
	};

	//////////////////////////////////////////////////////////////////////////////
	class TXViewPolyRecorder
	//////////////////////////////////////////////////////////////////////////////
	{
	public:
		TXViewPolyRecorder(TXViewPoly& inTXViewPoly);
		~TXViewPolyRecorder();
	
		bool AddPt(const XViewPt& inPt);

	protected:
		TXViewPoly&	fTXViewPolyRef;
	};
	...
	/////////////////////////////////////////////////////////////////////////
	class TExpressArcAsCubicCurves
	/////////////////////////////////////////////////////////////////////////
	{
	public:
		virtual ~TExpressArcAsCubicCurves() {}

		Bool8     RecurseExpressArc(const XViewPt& startPt, const XViewPt& endPt, const XViewPt& originPt, const XViewCoord radius);

	protected:	// derived classes implement ExpressArcAsCubicCurves_DoCubic
		virtual Bool8 ExpressArcAsCubicCurves_DoCubic (const XViewPt& controlPt1, const XViewPt& controlPt2, const XViewPt& endPt) = 0;
	};
