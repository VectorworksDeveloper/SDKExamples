...
void GS_API TMCPolyline::SetVertex(short vertexNum, const WorldPt& vertex, VertexType vType, WorldCoord arcRadius, VectorWorks::TEditContext editContext)
{
	gSDK->SetVertex(PolyHandle(), IndexInRange(vertexNum), vertex, vType, arcRadius, editContext);
}
...
void GS_API TMCPolyline::AddVertex(const WorldPt& p, VertexType vType, WorldCoord arcRadius, VectorWorks::TEditContext editContext)
{
	gSDK->AddVertex(PolyHandle(), p, editContext, vType, arcRadius, true);
...
void GS_API TMCPolyline::Invert()
{
	...
	if (fPolyHandle) {
		VectorWorks::TEditContext editContext = gSDK->CreateEditContext(PolyHandle());
		for (short i = 1 ; i <= fNumVerts/2 ; i++) {
			...
			SetVertex(i, setVertex, setVType, setRadius, editContext);
			SetVertex(numVerts+1-i, tempVertex, tempVType, tempRadius, editContext);
		}
		editContext = nullptr;
	}
}
	...
	if (fPolyHandle) {
		...
		for (Sint32 i = IndexInRange(index); i > 1L;i--) {
			...
			for (short j = 2; j <= fNumVerts; j++) {
				...
				SetVertex(j-1, vertex, vType, radius, editContext);
			}
			SetVertex(fNumVerts, temp, tempVType, tempRadius, editContext);
		}
		editContext = nullptr;
	}
}
...
Boolean GS_API  TMCPolyline::RemoveCrossovers(CallBackPtr fCBP, Handle poly, const WorldCoord offset)
{
	...
	VectorWorks::TEditContext editContext = gSDK->CreateEditContext(poly);
	for (ThisIndex=numVerts; ThisIndex>=1; --ThisIndex)
	{
		...
		else  // Nodes adjacent to deleted nodes should be set to be corners instead of Beziers
		{
			...
			if (Discard[next] || Discard[previous])
			{
				...
				gSDK->SetVertex(poly, ThisIndex, ThisVertex, CornerType, radius, editContext);
			}
		}
	}
	editContext = nullptr;
...
{
	short	numVerts = GS_CountVertices(fCBP, poly), ThisIndex, OtherIndex, ThisPlus2, ThisPlus1, Plugs=0, next, previous;
	WorldPt PreviousVertex(0L,0L), ThisVertex(0L,0L), Intersection(0L,0L);
	WorldPt OtherVertex(0L,0L), PreviousOtherVertex(0L,0L);
	VertexType 	vType, CornerType=vtCorner;
	WorldCoord 		radius;
	Boolean	CrossOver;
	short	forward, back, i, LastOther;

	std::vector<Boolean>Discard(numVerts + 1, false);
	std::vector<short> PlugIndices(numVerts, 0);
	std::vector<WorldPt> plugs(numVerts);
	...
	WorldCoord ColinearityThreshold)
{
	...
	{
		::GS_GetVertex(fCBP, poly, FirstIndex, FirstVertex, vType, radius);
		gSDK->SetVertex(poly, FirstIndex, FirstVertex, CornerType, radius, editContext);
	}
	editContext = nullptr;
	free(ConvertNode);
	...
	if (PolyHandle()) 
	{
		...
		{			
			...
			for  (i=1; i <= CurrentVertexCount; ++i) 
			{
				...
				gSDK->AddVertex(poly, newVert, editContext, vType, radius, true);
			}
			editContext = nullptr;
...
{
	if (fPolyHandle) {
		VectorWorks::TEditContext editContext = gSDK->CreateEditContext(fPolyHandle);
		AddVertex(vertex, vType, arcRadius, editContext);
		editContext = nullptr;
