...
bool VWPolygon2D::SVertex::operator==(const VWPolygon2D::SVertex& that) const
{
	return ( ( type			== that.type		) &&
			 ( pt			== that.pt			) &&
			 ( DoublesAreNearlyEqual( dArcRadius, that.dArcRadius ) || type != eVertexType_Arc ) &&
...
VWPolygon2D::VWPolygon2D(std::initializer_list<VWPoint2D> src, bool isClosed)
	: VWPolygon2D( TSTLPoints2DArray( src ), isClosed )
{
}

VWPolygon2D::VWPolygon2D(const TSTLPoints2DArray& src, bool isClosed)
{
...
VWPolygon2D::VWPolygon2D(const TSTLPoints2DVWArray& src, bool isClosed)
{
	// temporarily suppresses recalculating of the polygon bounding box.
	fLockedForEdit	= true;

	size_t	len		= src.size();
	farrVertices.reserve(len);
	for ( size_t i = 0 ; i < len ; ++ i )
	{
		this->AddVertex( src[ i ] );
	}
...
VWPolygon2D::VWPolygon2D( const WorldRectVerts& worldRect )
{
	// temporarily suppresses recalculating of the polygon bounding box.
	fLockedForEdit	= true;

	this->AddVertex( worldRect.topLeft );
	this->AddVertex( worldRect.topRight );
	this->AddVertex( worldRect.bottomRight );
	this->AddVertex( worldRect.bottomLeft );
...
VWPolygon2D::VWPolygon2D( const std::vector<SVertex>& vertices, bool isClosed)
	: fLockedForEdit( false )
	, farrVertices(vertices )
	, fbIsClosed( isClosed )
{
	this->RecalculateBoundingBox();	
}
...
std::vector<VWPolygon2D::SVertex>::iterator VWPolygon2D::begin()
{
	return farrVertices.begin();
}

std::vector<VWPolygon2D::SVertex>::iterator VWPolygon2D::end()
{
	return farrVertices.end();
}

std::vector<VWPolygon2D::SVertex>::const_iterator VWPolygon2D::begin() const
{
	return farrVertices.begin();
}

std::vector<VWPolygon2D::SVertex>::const_iterator VWPolygon2D::end() const
{
	return farrVertices.end();
}
...
{
	VWFC_ASSERT( at < farrVertices.size() );
	VWFC_ASSERT( farrVertices[ at ].type == eVertexType_Arc || farrVertices[at].type == eVertexType_Radius );
...
VWRectangle2D VWPolygon2D::GetPolygonBounds() const
{
	...
	return VWRectangle2D( VWPoint2D( fdMinX, fdMaxY ), VWPoint2D( fdMaxX, fdMinY ) );
}
...
double VWPolygon2D::GetLength() const
{
	...
	for(size_t i=0; i<vertsCnt;) {
		size_t prevI = (i > 0) ? (i - 1) : (vertsCnt - 1);
		EVertexType			vertType	= this->GetVertexType( i % vertsCnt );
		if ( vertType == eVertexType_Corner ) {
			if ( i > 0 || fbIsClosed ) {
				const VWPoint2D&	prevPt		= this->GetVertexAt( prevI );
			...
			ASSERTN( kPGeorgiev, ( i > 0 && i + 1 < vertsCnt ) || fbIsClosed );
			const VWPoint2D&	prevPt		= this->GetVertexAt( prevI );
			const VWPoint2D&	thisPt		= this->GetVertexAt( i );
			const VWPoint2D&	nextPt		= this->GetVertexAt( i + 1, true );
			double				vec1Len		= prevPt.DistanceTo( thisPt );
			double				vec2Len		= thisPt.DistanceTo( nextPt );
			double				r			= this->GetArcVertexRadius( i );

			const double		sweep		= VWPoint2D::CalcAngleBetween( ( thisPt - prevPt ) / vec1Len, ( nextPt - thisPt ) / vec2Len, true );
			const double		tanSweep2	= std::tan( sweep / 2 );

			// The distance between the arc vertex and the arc end points.
			double				l			= tanSweep2 * r;
			const double		maxL1		= vec1Len * ( this->GetVertexType( prevI ) == eVertexType_Corner ? 1.0 : 0.5 );
			const double		maxL2		= vec2Len * ( this->GetVertexType( ( i + 1 ) % vertsCnt ) == eVertexType_Corner ? 1.0 : 0.5 );
			const double		maxL		= std::min( maxL1, maxL2 );
			if ( l > maxL || r <= 0.0 )
			{
				l = maxL;
				r = l / tanSweep2;
			...
			// On the next iteration, the function will treat prevPt as a corner vertex, so here we compensate that by adding the
			// full arc length and subtracting `l` twice. That way it works with both corner+arc and arc+arc combinations.
			// Note that arc+radius is not allowed in polylines.
			length += sweep * r - 2 * l + prevPt.DistanceTo( thisPt );
			i++;
		}
...
double VWPolygon2D::GetLength(const VWPoint2D& pt, double dEpsilon) const
{
	...
	for(size_t i=0; i<vertsCnt;) {
		...
		if ( vertType == eVertexType_Corner ) {
			...
			ASSERTN( kPGeorgiev, i > 0 && i + 1 < vertsCnt );
			const VWPoint2D&	prevPt		= this->GetVertexAt( (i > 0) ? (i - 1) : (vertsCnt - 1) );
			const VWPoint2D&	thisPt		= this->GetVertexAt( i );
			const VWPoint2D&	nextPt		= this->GetVertexAt( i + 1, true );
			VWArc2D		arc( prevPt, thisPt, nextPt );
...
// calculates the length along the polygon between given starting and ending vertex (given by indices).
// note: works on polygon with corner vertices only; works on closed polygons (loops).
bool VWPolygon2D::GetPolygonLength(size_t fromVertInx, size_t toVertInx, double& outLength) const
{
	// init.
	bool			isOK		= false;
	outLength		= 0.0;
						...
						fromVertInx < vertCnt && toVertInx < vertCnt ) ) )	// ... valid indices.
	{
		...
		auto		lenFromTo	= [&] (size_t fromInx, size_t toInx) -> double
		{
			double	len			= 0.0;

			for ( size_t i = fromInx ; i < toInx ; ++ i )
			{
				len	+= ( farrVertices[ i ].pt ).DistanceTo( farrVertices[ i + 1 ].pt );
			}
		...
		if ( this->IsClosed() )
		{
			...
			{
				outLength		= lenFromTo( fromVertInx, toVertInx );
			}
			else //if ( fromVertInx >= toVertInx )
			{
				outLength		= ( lenFromTo( fromVertInx, ( vertCnt - 1 ) ) +
									( farrVertices[ 0 ].pt ).DistanceTo( farrVertices[ vertCnt - 1 ].pt ) +	// 'last'-'first' link.
									lenFromTo( 0, toVertInx ) );
			}
...
{
	size_t outOnSegIndex;
	return this->GetPtOnPoly( offset, outPt, outOnSegIndex );
}

// returns a point along the poly on the specified offset
bool VWPolygon2D::GetPtOnPoly(double offset, VWPoint2D& outPt, size_t& outOnSegIndex) const
{
	auto activePoly = this;

	VWPolygon2D localPolygonized;
	if ( this->HasNonCornerVertices() )
	{// This method is designed to be used only with polygonized data!
		this->GetPolygonized( localPolygonized );
		activePoly = &localPolygonized;
	}
	...
	bool		result		= false;
	double		length		= 0;
	size_t		vertsCnt	= activePoly->GetVertexCount();

	for ( size_t segmentIndex = 0 ; segmentIndex + !activePoly->IsClosed() < vertsCnt ; ++segmentIndex )
	{
		const VWPoint2D&	startPt = activePoly->GetVertexAt( segmentIndex );
		const VWPoint2D&	endPt = activePoly->GetVertexAt( segmentIndex + 1, true );

		double				newLen		= length + startPt.DistanceTo( endPt );

		if ( MathUtils::GEish( newLen, offset, VWPoint2D::sEpsilon ) )
		{
			double		dist			= offset - length;
			VWPoint2D	n				= ( endPt - startPt ).Normalize();
			outPt						= startPt + ( dist * n );
			outOnSegIndex				= segmentIndex;
			result						= true;
			// Finished, Stop!
			break;
		}
...
bool VWPolygon2D::IsPtOnPoly2D_Fast(const VWPoint2D& inPt, Sint32& outOnSegIndex, double epsilon) const
{
	...
	if ( polyPtCnt > 1 )		// poly has at least two points.
	{
		...
		if ( bboxPoly.IsPtInside( inPt, epsilon ) )
		{
...
void VWPolygon2D::GetDifferencesAgainstPolygon(const VWPolygon2D& polygon, TSTLPolygons2DArray& outDifferences, bool& outNoOverlap, double epsilon) const
{
	...
	if ( thisCnt > 1 && checkCnt > 1 )				// non-empty polygons.
	{
		...
		if ( ! thisBbox.IsOutside( checkBbox, epsilon ) )			// bounding boxes have common area.
		{
				...
				if ( overlapsCnt > 0 )				// has overlaps.
				{
					...
					for ( const auto& it : sqrDistsAndInxs )
					{
						...
						if ( hasOpenDiff )			// has a non-ended 'difference'.
						{
							...
							VWPolygon2D&			lastDiffPoly	= outDifferences.back();

							// prevent from doubling of points.
							if ( ptLast.DistanceSquaredTo( lastAdded ) > sqrEps )
							{
								lastDiffPoly.AddVertex( ptLast );	// add.
							}

							lastDiffPoly.fLockedForEdit				= false;
							lastDiffPoly.RecalculateBoundingBox();
						}
			...
			if ( diffsCnt > 0 )		// at least one 'difference'.
			{
...
std::vector<VWPolygon2D::SSegment> VWPolygon2D::GetSegments() const
{
	std::vector<SSegment> segments;

	if ( !this->IsEmpty() )
	{
		const size_t vertexCount = this->GetVertexCount();

		size_t cornerCount = 0;
		for ( size_t i = 0; i < vertexCount; ++i )
		{
			cornerCount += this->GetVertexType( i ) == eVertexType_Corner;
		}

		if ( cornerCount <= 1 )
		{
			segments.push_back( { *this, 0 } );
		}
		else
		{
			size_t firstCorner = 0;
			for ( ; firstCorner < vertexCount; ++firstCorner )
			{
				if ( this->GetVertexType( firstCorner ) == eVertexType_Corner )
				{
					break;
				}
			}
			ASSERTN( kPGeorgiev, firstCorner < vertexCount );

			VWPolygon2D currentSegment;
			currentSegment.SetClosed( false );
			size_t currentFirstVertex = firstCorner;

			SVertex vertex;

			for ( size_t i = firstCorner; i < vertexCount; ++i )
			{
				vertex = farrVertices[ i ];
				currentSegment.AddVertex( vertex.pt, vertex.type, vertex.dArcRadius );
				if ( !currentSegment.IsEmpty() && vertex.type == eVertexType_Corner )
				{
					segments.push_back( { std::move( currentSegment ), currentFirstVertex } );
					currentSegment.ClearVertices();
					currentSegment.SetClosed( false );
					currentSegment.AddVertex( vertex.pt, vertex.type, vertex.dArcRadius );
					currentFirstVertex = i;
				}
			}
			if ( VERIFYN( kPGeorgiev, currentSegment.GetVertexCount() > 0 ) && this->IsClosed() )
			{
				if ( firstCorner == 0 )
				{
					currentSegment.AddVertex( this->GetVertexAt( 0 ) );
					segments.push_back( { std::move( currentSegment ), currentFirstVertex } );
				}
				else
				{
					// Closed path with non-corner starting vertex

					VWPolygon2D polygonized;
					this->GetPolygonized( polygonized );
					const VWPoint2D firstPt = polygonized.GetVertexAt( 0 );

					for ( size_t i = 0; i <= firstCorner; ++i )
					{
						vertex = farrVertices[ i ];
						currentSegment.AddVertex( vertex.pt, vertex.type, vertex.dArcRadius );
					}

					segments.insert( segments.begin(), SSegment { std::move( currentSegment ), currentFirstVertex } );
				}
			}
		}
	}
...
/*static*/ std::vector<VWPolygon2D::SVertexChange> VWPolygon2D::ComparePolylines(const VWPolygon2D& oldPoly, const VWPolygon2D& newPoly, const SComparePolylineOptions& options)
{
	// Levenshtein distance using two matrix rows.

	struct SDistance
	{
		std::vector<SVertexChange>	fChanges;
		double						fCost {};
	};

	auto getCost = [&options](SVertexChange::EType changeType)
	{
		switch ( changeType )
		{
			case SVertexChange::EType::None:			return 0.0;
			case SVertexChange::EType::Insertion:		return options.fInsertionCost;
			case SVertexChange::EType::Deletion:		return options.fDeletionCost;
			case SVertexChange::EType::Substitution:	return options.fSubstitutionCost;
		}
		return 0.0;
	};

	auto pushChange = [&getCost](SDistance& distance, const SVertexChange& change)
	{
		distance.fChanges.push_back( change );
		distance.fCost += getCost( change.fType );
	};

	auto insertion		= [](size_t newI) { return SVertexChange { SVertexChange::EType::Insertion, -1, static_cast<Sint16>( newI ) }; };
	auto deletion		= [](size_t oldI) { return SVertexChange { SVertexChange::EType::Deletion, static_cast<Sint16>( oldI ), -1 }; };
	auto substitution	= [](size_t oldI, size_t newI) { return SVertexChange { SVertexChange::EType::Substitution, static_cast<Sint16>( oldI ), static_cast<Sint16>( newI ) }; };
	auto none			= [](size_t oldI, size_t newI) { return SVertexChange { SVertexChange::EType::None, static_cast<Sint16>( oldI ), static_cast<Sint16>( newI ) }; };

	std::vector<SDistance> previousDistances;
	std::vector<SDistance> currentDistances;
	previousDistances.resize( newPoly.farrVertices.size() + 1 );
	currentDistances.resize( newPoly.farrVertices.size() + 1 );

	for ( size_t j = 0; j < newPoly.farrVertices.size(); ++j )
	{
		currentDistances[ j + 1 ] = currentDistances[ j ];
		pushChange( currentDistances[ j + 1 ], insertion( j ) );
	}

	// These are only used in the inner loop, but are instantiated here to avoid unnecessary allocations.
	SDistance del, ins, mod;
	for ( size_t i = 0; i < oldPoly.farrVertices.size(); ++i )
	{
		std::swap( currentDistances, previousDistances );

		currentDistances[ 0 ] = previousDistances[ 0 ];
		pushChange( currentDistances[ 0 ], deletion( i ) );

		for ( size_t j = 0; j < newPoly.farrVertices.size(); ++j )
		{
			del = previousDistances[ j + 1 ];
			pushChange( del, deletion( i ) );

			ins = currentDistances[ j ];
			pushChange( ins, insertion( j ) );

			mod = previousDistances[ j ];
			if ( oldPoly.farrVertices[ i ] != newPoly.farrVertices[ j ] )
			{
				pushChange( mod, substitution( i, j ) );
			}
			else
			{
				pushChange( mod, none( i, j ) );
			}

			auto* min = std::min( { &del, &ins, &mod }, [&](auto* a, auto* b) { return a->fCost < b->fCost; } );
			currentDistances[ j + 1 ] = *min;
		}
	}

	return currentDistances.back().fChanges;
}

/*static*/ std::vector<VWPolygon2D::SVertexChange> VWPolygon2D::ComparePolylines(const VWPolygon2D& oldPoly, const VWPolygon2D& newPoly)
{
	return ComparePolylines( oldPoly, newPoly, SComparePolylineOptions {} );
}

/*static*/ std::vector<VWPolygon2D::SSegmentChange> VWPolygon2D::CompareSegments(const VWPolygon2D& oldPoly, const VWPolygon2D& newPoly, const std::vector<SVertexChange>& polyChanges)
{
	std::vector<SSegmentChange> segmentChanges;

	const auto oldSegments = oldPoly.GetSegments();
	const auto newSegments = newPoly.GetSegments();

	auto getOldSegmentIndex = [&](Sint16 oldFirstIndex, Sint16 oldLastIndex, Sint16 newFirstIndex, Sint16 newLastIndex,
									size_t& oldSegmentIndex)
	{
		if ( oldFirstIndex != -1 && oldLastIndex != -1
			&& VERIFYN( kPGeorgiev, newFirstIndex != -1 && newLastIndex != -1 )
			&& VERIFYN( kPGeorgiev, newFirstIndex < static_cast<Sint16>( newPoly.farrVertices.size() ) )
			&& VERIFYN( kPGeorgiev, oldFirstIndex < static_cast<Sint16>( oldPoly.farrVertices.size() ) )
			&& VERIFYN( kPGeorgiev, newFirstIndex <= newLastIndex || newPoly.IsClosed() ) )
		{
			for ( Sint16 i = ( newFirstIndex + 1 ) % static_cast<Sint16>( newPoly.farrVertices.size() );
				i != newLastIndex;
				i = ( i + 1 ) % static_cast<Sint16>( newPoly.farrVertices.size() ) )
			{
				if ( newPoly.farrVertices[ i ].type == eVertexType_Corner )
				{
					return false;
				}
			}
			auto it = std::find_if( oldSegments.begin(), oldSegments.end(), [&](auto&& seg) { return seg.fFirstVertexIndex == oldFirstIndex; } );
			if ( it != oldSegments.end() )
			{
				oldSegmentIndex = std::distance( oldSegments.begin(), it );
				return true;
			}
		}
		return false;
	...
	auto getOffset = [&](const SVertexChange& change, VWPoint2D& offset)
	{
		if ( change.fType == SVertexChange::EType::Substitution )
		{
			const auto o = oldPoly.farrVertices[ change.fOldIndex ], n = newPoly.farrVertices[ change.fNewIndex ];
			if ( o.type == n.type && ( o.type != eVertexType_Arc || DoublesAreNearlyEqual( o.dArcRadius, n.dArcRadius ) ) )
			{
				offset = n.pt - o.pt;
				return true;
			}
	...
	auto isOffsetChange = [&](const SVertexChange& change)
	{
		VWPoint2D unused;
		return change.fType == SVertexChange::EType::Substitution && !getOffset( change, unused );
	};

	auto areAllOffset = [&](auto begin, auto end)
	{
		VWPoint2D offset;
		if ( VERIFYN( kPGeorgiev, begin != end ) && getOffset( *begin, offset ) )
		{
			return std::all_of( std::next( begin ), end, [&](auto&& v)
			{
				VWPoint2D offset2;
				return getOffset( v, offset2 ) && offset2 == offset;
			} );
		}
		return false;
	};
	...
	{
		SSegmentChange change;

		if ( oldPoly.IsClosed() && oldSegments.size() == 1 )
		{
			// Special case where we have a single, closed segment.
			if ( std::all_of( polyChanges.begin(), polyChanges.end(), [](auto&& c) { return c.fType == SVertexChange::EType::None; } ) )
			{
				change.fType = SSegmentChange::EType::None;
				change.fSourceIndex = i;
			}
			else if ( areAllOffset( polyChanges.begin(), polyChanges.end() ) )
			{
				change.fType = SSegmentChange::EType::Offset;
				change.fSourceIndex = i;
			}
			else
			{
				change.fType = SSegmentChange::EType::New;
			}
		}
		else
		{
			const auto& seg = newSegments[ i ];
			const size_t firstIndex = seg.fFirstVertexIndex;
			const size_t lastIndex = firstIndex + seg.fPoly.GetVertexCount() - 1;

			const auto begin = std::find_if( polyChanges.begin(), polyChanges.end(),
				[&](auto&& change) { return change.fNewIndex == firstIndex; } );
			const auto last =
				lastIndex < newPoly.farrVertices.size()
				? std::find_if( begin, polyChanges.end(), [&](auto&& change) { return change.fNewIndex == lastIndex; } )
				: std::find_if( polyChanges.begin(), begin, [&](auto&& change) { return change.fNewIndex == lastIndex % newPoly.farrVertices.size(); } );

			if ( VERIFYN( kPGeorgiev, begin != polyChanges.end() && last != polyChanges.end() ) )
			{
				std::vector<SVertexChange> segmentVertexChanges;
				if ( begin < last )
				{
					segmentVertexChanges.insert( segmentVertexChanges.end(), begin, std::next( last ) );
				}
				else
				{
					segmentVertexChanges.insert( segmentVertexChanges.end(), begin, polyChanges.end() );
					segmentVertexChanges.insert( segmentVertexChanges.end(), polyChanges.begin(), std::next( last ) );
				}

				const auto begin = segmentVertexChanges.begin();
				const auto end = segmentVertexChanges.end();

				size_t oldSegmentIndex = 0;
				if ( getOldSegmentIndex( begin->fOldIndex, last->fOldIndex, begin->fNewIndex, last->fNewIndex, oldSegmentIndex ) )
				{
					if ( std::all_of( begin, end, [](auto&& c) { return c.fType == SVertexChange::EType::None; } ) )
					{
						change.fType = SSegmentChange::EType::None;
						change.fSourceIndex = oldSegmentIndex;
					}
					else if ( areAllOffset( begin, end ) )
					{
						change.fType = SSegmentChange::EType::Offset;
						change.fSourceIndex = oldSegmentIndex;
					}
					else if ( std::any_of( begin, end, [&](auto&& c) { return	setmember( c.fType,
																							SVertexChange::EType::Deletion,
																							SVertexChange::EType::Insertion )
																				|| isOffsetChange( c ); } ) )
					{
						change.fType = SSegmentChange::EType::New;
					}
					else if ( begin->fType == SVertexChange::EType::None && last->fType != SVertexChange::EType::None )
					{
						change.fType = SSegmentChange::EType::ChangeEnd;
						change.fSourceIndex = oldSegmentIndex;
					}
					else if ( begin->fType != SVertexChange::EType::None && last->fType == SVertexChange::EType::None )
					{
						change.fType = SSegmentChange::EType::ChangeBegin;
						change.fSourceIndex = oldSegmentIndex;
					}
					else
					{
		...
		segmentChanges.push_back( change );
	}

	return segmentChanges;
}

// given a set of consecutive points (representing a shape), removes all collinear.
/*static*/ bool VWPolygon2D::SimplifyShape_RemoveCollinear(TSTLPoints2DArray& ioConsecPtsSet, bool isClosedShape,
														   double epsCollinear/*=VWPoint2D::sEpsilonCollinear*/, double sqrCoordEps/*=VWPoint2D::sSqrEpsilon*/)
{
	// init.
	bool	noRemoved	= true;

	size_t	ptsCnt		= ioConsecPtsSet.size();

	if ( VERIFYN( kBStanchev, ( ptsCnt > 2 ) ) )			// good (can see inx as a shape).
	{
		size_t			lastValInx	= ( ptsCnt - 1 );		// last valid point index.

		// init.
		size_t			strtPtInx	= 1;		// second point index.
		size_t			lastPtInx	= ( ptsCnt - 2 );		// penult point index.

		if ( isClosedShape )		// closed shape.
		{
			-- strtPtInx;			// to first point index (0).
			++ lastPtInx;			// to last point index (ptsCnt - 1).
		}

		ptrdiff_t		inx			= lastPtInx;			// start from.

		while ( inx >= (ptrdiff_t)strtPtInx )	// loop the polygon points backwards.
		{
			// note: going backwards.

			size_t		prvInx		= ( ( inx < (ptrdiff_t)lastValInx ) ? ( inx + 1 ) : 0 );
			size_t		nxtInx		= ( ( inx > 0 ) ? ( inx - 1 ) : lastPtInx );

			const VWPoint2D&		curPt		= ioConsecPtsSet[ inx ];
			const VWPoint2D&		prvPt		= ioConsecPtsSet[ prvInx ];
			const VWPoint2D&		nxtPt		= ioConsecPtsSet[ nxtInx ];

			bool		removePt	= false;

			// get normalized 'previous' and 'next' vectors.

			VWPoint2D	vecPrvNrm	= ( prvPt - curPt );

			double		disEstm		= vecPrvNrm.MagnitudeSquared();

			if ( disEstm > sqrCoordEps )		// can normalize.
			{
				vecPrvNrm			/= sqrt( disEstm );		// normalize.

				VWPoint2D			vecNxtNrm	= ( nxtPt - curPt ).Normalize();

				disEstm	= vecNxtNrm.MagnitudeSquared();

				if ( disEstm > sqrCoordEps )	// can normalize.
				{
					vecNxtNrm		/= sqrt( disEstm );		// normalize.

					// check collinearity.
					if ( MathUtils::Equalish( vecPrvNrm.CrossProductMagnitude( vecNxtNrm ), 0.0, epsCollinear ) )
					{
						removePt	= true;		// mark for removing.
					}
					//else: not collinear and not too close.
				}
				else	// next point is too close (coincident).
				{
					removePt		= true;		// mark for removing.
				}
			}
			else		// previous point is too close (coincident).
			{
				removePt			= true;		// mark for removing.
			}
			...
			if ( removePt )
			{
				ioConsecPtsSet.erase( ioConsecPtsSet.begin() + inx );
				...
				if ( lastPtInx < 1 )			// single point left.
				{
					break;			// done - stop.
				}
			}

			-- inx;		// decrease counter (go to next).
		}
	}
	//else: degenerate (no-area) shape provided - bad input - check.
...
// if 'tryClosing' is true (default), then close the poly if its ending vertices (first and last) match.
void VWPolygon2D::GetPolygonized(	VWPolygon2D& outPolygon,
									...
									short	arcRecursLevel	/*=kPolygonize_MaxRecursLevel*/,
									bool	tryClosing		/*=true*/) const
{
	...
	// note:
	// in the case of matching ending vertices (first and last), the match shouldn't be removed (by removing first or last vertex) ...
	// ... if it results in consecutive non-corner vertices in the closing (L->0) segment. it may result in incorrectly polygonized ...
	// ... (parametric segment containing) closing (L->0) segment when the poly is flagged as 'closed'.

	// check for a special case: first point coincides with the last one.
	if (	tryClosing &&			// can close the poly when ending vertices (first and last) match and ...
			( ptsCnt > 2 ) )		// ... it has at least 3 vertices.
	{
		const SExtVertex&	auxFrstVtx	= arrVertices[ 0 ];
		const SExtVertex&	auxLastVtx	= arrVertices.back();

		if ( MathUtils::Equalish( auxFrstVtx.pt, auxLastVtx.pt, VWPoint2D::sEpsilon ) )
		{
			// validate: coincident points must have same vertex types.
			if( auxFrstVtx.type == auxLastVtx.type )
			{
				if ( auxLastVtx.type == eVertexType_Corner )	// coincident corner ending vertices.
				{
					// remove first point.
					arrVertices.erase( arrVertices.begin() );
					-- ptsCnt;
				}
				else		// non-corner ending vertices.
				{
					if ( arrVertices[ 1 ].type == eVertexType_Corner )	// second is corner - can remove first.
					{
						// remove first point.
						arrVertices.erase( arrVertices.begin() );
						-- ptsCnt;
					}
					else if ( arrVertices[ ptsCnt - 2 ].type == eVertexType_Corner )	// second to last vertex is corner - can remove last.
					{
						// remove last point.
						arrVertices.pop_back();
						-- ptsCnt;
					}
					//else: removing will result in consecutive non-corner vertices ...
					//		... which is undesirable for last segment polygonization.
				}
			}
			else if ( ( auxLastVtx.type == eVertexType_Corner ) &&	// last is corner while first is non-corner and ...
					  ( arrVertices[ ptsCnt - 2 ].type ) )		// ... second to last vertex is corner.
			{
				// remove last point when it is set to corner.
				arrVertices.pop_back();
				-- ptsCnt;
			}
			//else: removing last will result in consecutive non-corner vertices ...
			//		... which is undesirable for last segment polygonization.
	...
	size_t				thisVertI	= 0;
	SExtVertex			startVert	= arrVertices[ thisVertI ];	// store 'start-aux-vertex'.
	SExtVertex			thisVert	= startVert;
	...
	if ( isClosed && ( thisVert.type != eVertexType_Corner ) )	// the polyline is closed and start vertex is not corner.
	{
		if ( lastVert.type == eVertexType_Corner )				// ends with corner vertex.
		{
			// note:
			// if the starting vertex is an Arc or Radius vertex then the start (beginning point) ...
			// ... of the starting non-corner segment is included in the segment polygonization ...
			// ... and no special processing is needed.
			// special processing is only necessary for the Cubic and Bezier cases.

			if ( ( thisVert.type == eVertexType_Bezier ) ||
				 ( thisVert.type == eVertexType_Cubic ) )
			{
				outPolygon.AddVertex( lastVert.pt );			// add as corner vertex. note: it's the first added.

				startVert			= lastVert;					// update 'start-aux-vertex'.

				-- ptsCnt;			// skip last point henceforth (as it has already been added).
			}
		}
		else			// ends with non-corner vertex.
		{
			outPolygon.AddVertex( ( lastVert.pt + thisVert.pt ) * 0.5 );
		}
	...
	thisVertI						= nextVertI;
	thisVert						= nextVert;
	//nextVertI;					// not used any further.
	nextVert						= startVert;				// note: use 'start-aux-vertex'.
...
// Result is the offset along the poly to the projection of the specified point
/*static*/ double VWPolygon2D::GetPtProjectionOffsetAlongPoly(const VWPolygon2D& inPoly, const VWPoint2D& pt)
{
	double	result	= std::numeric_limits<double>::quiet_NaN();

	VWPolygon2D	polygon	= inPoly;

	if ( polygon.HasNonCornerVertices() )
	{
		// This method is designed to be used only with polygonized data!
		inPoly.GetPolygonized( polygon );
	}

	// Find the projection on the poly.
	double		outSqrDist;
	Sint32		outOnSegIndex;
	VWPoint2D	outProjPt;
	if ( VWFC_VERIFY( polygon.GetClosestPtOnPolygon( pt, outSqrDist, outOnSegIndex, outProjPt ) ) )
	{
		result	= polygon.GetLength( 0, outOnSegIndex ) + outProjPt.DistanceTo( polygon.GetVertexAt( outOnSegIndex ) );
	}
