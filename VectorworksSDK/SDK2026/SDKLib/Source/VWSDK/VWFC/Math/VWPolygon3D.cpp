...
// checks whether the vertex is collinear in the 3D polygon context (i.e. is collinear with respect to its neighboring vertices).
bool VWPolygon3D::IsVertexCollinear(size_t at, double epsCollinear/*=VWPoint3D::sEpsilonCollinear*/) const
{
	// init.
	...
	VWPoint3D		prevPt, nextPt;

	// get the previous and the next 'apposite' polygon points (not too close).
	if ( this->GetPrevAppositePt( at, prevPt, VWPoint3D::sEpsilon ) &&
		 this->GetNextAppositePt( at, nextPt, VWPoint3D::sEpsilon ) )
	{
		isCollinear	= VWPoint3D::ArePointsCollinear( prevPt, farrVertices[ at ], nextPt, epsCollinear );
	}
...
// gets part of the 3D polygon starting at 'fromVertInx', follows vertices' ascending order, and ending at 'toVertInx'.
bool VWPolygon3D::GetPolygonPartFromTo(size_t fromVertInx, size_t toVertInx, VWPolygon3D& outPolygonPart) const
{
	// init.
	outPolygonPart.ClearVertices();

	size_t		vertsCnt		= this->GetVertexCount();

	if ( VWFC_VERIFY( ( vertsCnt > 1 &&					// at least two points (has formed segment) and ...
						fromVertInx	< vertsCnt &&
						toVertInx	< vertsCnt ) ) )	// ... valid input.
	{
		if ( fromVertInx < toVertInx )
		{
			for ( size_t i = fromVertInx ; i < ( toVertInx + 1 ) ; ++ i )
			{
				// add.
				outPolygonPart.AddVertex( farrVertices[ i ] );
			}
		}
		else if ( this->IsClosed() )			// closed - looping allowed.
		{
			for ( size_t i = fromVertInx ; i < vertsCnt ; ++ i )
			{
				// add.
				outPolygonPart.AddVertex( farrVertices[ i ] );
			}
			...
			// add first.
			outPolygonPart.AddVertex( farrVertices[ 0 ] );

			for ( size_t i = 1 ; i < ( toVertInx + 1 ) ; ++ i )
			{
				// add.
				outPolygonPart.AddVertex( farrVertices[ i ] );
			}
	...
	// result.
	return	( outPolygonPart.GetVertexCount() > 0 );
}

const VWPoint3D& VWPolygon3D::Back() const
{
	return farrVertices.back();
}

VWPoint3D& VWPolygon3D::Back()
{
	return farrVertices.back();
}
...
void VWPolygon3D::CleanUpPolygon(double dEpsilon)
{
	...
	while( it != farrVertices.end() )
	{
		...
		{
			// note: calculate and compare squared distance - faster.
			double_gs	sqrDist		= ( lastPt - (*it) ).MagnitudeSquared();
...
bool VWPolygon3D::GetPtOnPolyOffsetFrom(size_t vertInx, double offset, VWPoint3D& outPt, size_t& outSegInx) const
{
	// init.
	bool		notFound		= true;
	outSegInx	= -1;
						...
						offset >= 0.0 ) ) )		// ... non-negative offset.
	{
		...
		auto	getPtOnPolySeg	= [&] (size_t segInx, bool loopNeeded) -> bool
		{
			...
			const VWPoint3D&	thisPt			= farrVertices[ segInx ];

			// get valid next index.
			size_t				nextInx			= ( loopNeeded ? ( ( segInx + 1 ) % vertsCnt ) : ( segInx + 1 ) );

			VWPoint3D			segVec			= ( farrVertices[ nextInx ] - thisPt );
			...
			if ( segLen > 0.0 )	// check segment length - prevent division by zero.
			{
				...
				if ( curOffset >= offset )		// the 'right' segment is reached.
				{
					...
					outPt		= thisPt + ( ( offset - prevLen ) * segVec );
		...
		if ( this->IsClosed() &&				// closed polygon and ...
...
bool VWPolygon3D::IsPtOnPolygonPart(const VWPoint3D& pt, size_t fromVertInx, size_t toVertInx, double epsilon,
						...
						fromVertInx < vertCnt && toVertInx < vertCnt ) ) )	// ... valid indices.
	{
		...
		auto		IsPtOnPart		= [&] (size_t fromInx, size_t toInx, bool doLoop) -> bool
		{
			...
			do
			{
				...
				// segment ends.
				const VWPoint3D&	segPt0			= farrVertices[ inx ];
				const VWPoint3D&	segPt1			= farrVertices[ nextInx ];

				// calculate the segment length.
				double				segLength		= segPt0.DistanceTo( segPt1 );

				// check whether point projects on the inx-th segment.
				if ( VWLine3D::IsPtOnLine(	pt,
...
// Bozho: test it more !
// calculates the length along the polygon between starting and ending vertices (given by indices).
bool VWPolygon3D::GetLength(size_t fromVertInx, size_t toVertInx, double& outLength) const
{
	// init.
	bool			isOK		= false;
	outLength		= 0.0;
						...
						fromVertInx < vertCnt && toVertInx < vertCnt ) ) )	// ... valid indices.
	{
		...
		auto		lenFromTo	= [&] (size_t fromInx, size_t toInx) -> double
		{
			...
			{
				len	+= ( farrVertices[ i ] ).DistanceTo( farrVertices[ i + 1 ] );
			}
		...
		if ( this->IsClosed() )
		{
			...
			else //if ( fromVertInx >= toVertInx )
			{
				outLength		= ( lenFromTo( fromVertInx, ( vertCnt - 1 ) ) +
									( farrVertices[ 0 ] ).DistanceTo( farrVertices[ vertCnt - 1 ] ) +	// 'last'-'first' link.
...
size_t VWPolygon3D::IndexOf( const VWPoint3D& pt ) const
{
	auto it = std::find( farrVertices.begin(), farrVertices.end(), pt );

	return it == farrVertices.end() ? -1 : ( it - farrVertices.begin() );
}
...
// private tool, recursive: used by the Douglas-Peucker simplification algorithm: analyses a sub-set of the given vertices array, updates the 'vertices-to-keep' flags.
/*static*/ void VWPolygon3D::Simplify_DouglasPeucker(const TSTLPoints3DArray& vertices3D, size_t begInx, size_t endInx, const VWTransformMatrix& rotTransf,
													 const double& maxDeviation, bool needsRotToPlane, std::vector<Uint8>& ioKeep3DPts)
{
	if ( endInx - begInx > 1 )			// at least three vertices required.
	{
		const VWPoint3D&	begPt3D		= vertices3D[ begInx ];
		const VWPoint3D&	endPt3D		= vertices3D[ endInx ];

		VWPoint2D			begPt2D, endPt2D;
		// go into a 2D context.
		if ( needsRotToPlane )			// has rotation transform to 'normalize' points so they lie on xy plane.
		{
			VWPoint3D		rotBegPt3D	= rotTransf.PointTransform( begPt3D );
			VWPoint3D		rotEndPt3D	= rotTransf.PointTransform( endPt3D );

			// validate.
			VWFC_ASSERT( (	MathUtils::Equalish( rotBegPt3D.z, 0.0, VWPoint3D::sEpsilon ) &&
							MathUtils::Equalish( rotEndPt3D.z, 0.0, VWPoint3D::sEpsilon ) ) );	// not expected - check.

			begPt2D.SetPoint( rotBegPt3D.x, rotBegPt3D.y );
			endPt2D.SetPoint( rotEndPt3D.x, rotEndPt3D.y );
		}
		else				// no rotation transform (sub-set already lying on a plane parallel to xy plane).
		{
			begPt2D.SetPoint( begPt3D.x, begPt3D.y );
			endPt2D.SetPoint( endPt3D.x, endPt3D.y );
		}

		// init.
		double	maxSqrDist	= -1.0;
		size_t	maxInx		= begInx;

		// calculate the maximum (squared) deviation of the vertices' sub-set.

		for ( size_t i = ( begInx + 1 ) ; i < endInx ; ++ i )
		{
			const VWPoint3D&			pt3D		= vertices3D[ i ];

			VWPoint2D		pt2D;
			// go into a 2D context.
			if ( needsRotToPlane )		// has rotation transform to 'normalize' points so they lie on xy plane.
			{
				VWPoint3D	rotPt3D		= rotTransf.PointTransform( pt3D );

				// validate.
				VWFC_ASSERT( MathUtils::Equalish( rotPt3D.z, 0.0, VWPoint3D::sEpsilon ) );

				pt2D.SetPoint( rotPt3D.x, rotPt3D.y );
			}
			else			// no rotation transform (sub-set already lying on a plane parallel to xy plane).
			{
				pt2D.SetPoint( pt3D.x, pt3D.y );
			}

			VWPoint2D		prjPt;
			// get the projection on (infinite) line formed by 'begPt2D' and 'endPt2D'.
			VWLine2D::PtPerpLine( pt2D.x, pt2D.y, begPt2D.x, begPt2D.y, endPt2D.x, endPt2D.y, prjPt.x, prjPt.y );

			// check the squared distance to the projection.
			double			sqrDist		= pt2D.DistanceSquaredTo( prjPt );
		...
		if ( maxSqrDist < sqr( maxDeviation + VWPoint2D::sEpsilon ) )
		{
			for (size_t i = ( begInx + 1 ) ; i < endInx ; ++ i )
			{
				ioKeep3DPts[ i ]		= false;
		...
		{
			// recursive calls for the two sub-sets.

			VWPolygon3D::Simplify_DouglasPeucker(	vertices3D, begInx, maxInx, rotTransf,
													maxDeviation, needsRotToPlane, ioKeep3DPts );

			VWPolygon3D::Simplify_DouglasPeucker(	vertices3D, maxInx, endInx, rotTransf,
													maxDeviation, needsRotToPlane, ioKeep3DPts );
		}
	}
}
...
// Douglas-Peucker 3D polygon simplification.
void VWPolygon3D::SimplifyPolygon_DouglasPeucker(const double& maxDeviation)
{
	const	double			kSqrEpsColinear	= 0.0000000001;	// collinearity check squared epsilon. 1e-10.

	size_t	vertsCnt		= farrVertices.size();

	if ( VWFC_VERIFY( vertsCnt > 2 ) )		// at least three vertices expected.
	{
		std::vector<Uint8>	keep3DPts;		// flags.
		TSTLPoints3DArray	vertices3D;		// temp 3D vertices.

		// starting vertices.
		VWPoint3D			frstPt3D		= farrVertices[ 0 ];
		VWPoint3D			secPt3D			= farrVertices[ 1 ];

		keep3DPts.push_back( true );
		keep3DPts.push_back( true );

		vertices3D.push_back( frstPt3D );
		vertices3D.push_back( secPt3D );
		...
		bool				needsRotToPlane	= true;

		VWTransformMatrix	rotTransf;

		double				zValue			= 0.0; 			// for checking vertex against the 'found plane'.

		WorldPt3			rotatedPt3D;

		for ( size_t i = 2 ; i < vertsCnt ; ++ i )			// loop.
		{
			const VWPoint3D& curPt3D		= farrVertices[ i ];

			vertices3D.push_back( curPt3D );
			keep3DPts.push_back( true );	// init.

			if ( foundPlane )				// has plane fit.
			{
				bool	doSimplifyPart		= true;			// init.

				if ( needsRotToPlane )		// has rotation transform for 'normalizing' the points so they lie on xy plane.
				{
					// rotate.
					VWPoint3D				rotatedPt3D		= rotTransf.PointTransform( curPt3D );

					if ( MathUtils::Equalish( rotatedPt3D.z, 0.0, VWPoint3D::sEpsilon ) )	// current point is lying on the plane.
					{
						endInx				= i;			// update.
						doSimplifyPart		= false;		// do not simplify at this step.
					}
				}
				else		// no rotation transform.
				{
					if ( MathUtils::Equalish( curPt3D.z, zValue, VWPoint3D::sEpsilon ) )	// current point is lying on the plane.
					{
						endInx				= i;			// update.
						doSimplifyPart		= false;		// do not simplify at this step.
					}
				...
				if ( doSimplifyPart )		// should simplify.
				{
					// this point is not on the plane, simplify the 3D polygons from 'begInx' to 'endInx'.
					VWPolygon3D::Simplify_DouglasPeucker(	vertices3D, begInx, endInx, rotTransf,
															maxDeviation, needsRotToPlane, keep3DPts );

					// update 'begInx' and 'endInx'.
					begInx	= endInx;
					endInx	= i;

					frstPt3D				= vertices3D[ begInx ];
					secPt3D					= vertices3D[ endInx ];

					foundPlane				= false;
				}
				//else: go on.
			}
			else			// need to find new plane.
			{
				endInx	= i;				// update the 'endInx'.

				// get two vectors to check for collinearity.
				// note: since this is a direction test the vectors must be normalized. otherwise, the result becomes unstable if they differ greatly in length.

				double		fsLen, fcLen;	// for the vectors' lengths.

				VWPoint3D	vecFS			= ( secPt3D - frstPt3D );
				vecFS.Normalize( fsLen );	// normalize and get 'vecFS' vector original length.

				VWPoint3D	vecTmp			= ( curPt3D - frstPt3D );
				vecTmp.Normalize( fcLen );	// normalize and get 'vecTmp' vector original length.

				bool		arePtsCollinear	= true;			// init.

				if ( ( fsLen > VWPoint3D::sEpsilon ) &&
					 ( fcLen > VWPoint3D::sEpsilon ) )		// non-zero length vectors.
				{
					// get cross product: normal vector to plane defined by the two vectors above.
					VWPoint3D				vecCP			= vecFS.Cross( vecTmp );

					if ( vecCP.MagnitudeSquared() > kSqrEpsColinear )		// the three vertices are not collinear - plane is found.
					{
						foundPlane			= true;
						arePtsCollinear		= false;

						vecCP.Normalize();

						// check whether vecCP is same as (0,0,+-1) - plane points are parallel to xy_plane.
						if ( ( VWPoint2D::DistanceSqare( vecCP.x, vecCP.y, 0.0, 0.0 ) < VWPoint2D::sSqrEpsilon ) &&
							 ( ( 1.0 - abs( vecCP.z ) ) < VWPoint3D::sSqrEpsilon ) )
						{
							needsRotToPlane	= false;
							zValue			= frstPt3D.z;
						}
						else				// the new plane is not parallel to xy plane.
						{
							// prepare rotation transform for 'normalizing' the points so they lie on xy plane.

							vecTmp			= vecCP.Cross( vecFS );
							vecTmp.Normalize();

							rotTransf.SetMatrix( vecFS, vecTmp, vecCP, frstPt3D );
							rotTransf.Invert();
						}
					}
				}
				//else: the three 3D vertices are (assumed) collinear.

				if ( arePtsCollinear )		// still not found new plane.
				{
					// mark that previous should be removed.
					keep3DPts[ i - 1 ]		= false;
		...
		if ( foundPlane )	// has ending plane fit.
		{
			// this point is not on the plane, simplify the 3D polygons from 'begInx' to 'endInx'.
			VWPolygon3D::Simplify_DouglasPeucker(	vertices3D, begInx, endInx, rotTransf,
													maxDeviation, needsRotToPlane, keep3DPts );
		...
		TSTLPoints3DArray	simplArrVertices;

		// simplify the polygon - remove the flagged vertices.
		for ( size_t i = 0 ; i < vertsCnt ; ++ i )
		{
			if ( keep3DPts[ i ] )
			{
				simplArrVertices.push_back( farrVertices[ i ] );
			}
		}

		// update to simplified.
		farrVertices		= simplArrVertices;
	}
	//else: bad input - no need to simplify.
}