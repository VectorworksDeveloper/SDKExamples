...
// returns a point on this line segment which is closest to the input point.
VWPoint3D VWLine3D::GetClosestPointOnLine( const VWPoint3D& pt ) const
{
	VWPoint3D lineDistance = this->GetEnd() - this->GetStart();
	VWPoint3D objDistance = pt - this->GetStart();
	...
	if ( !DoubleIsNearlyZero( squaredMagnitude ) )
	{
		transposition = ( objDistance.DotProduct( lineDistance ) / squaredMagnitude );
	}

	transposition = std::max( 0.0, std::min( transposition, 1.0 ) );

	return ( this->GetStart() + ( transposition * lineDistance ) );
}
											 ...
											 double coordEpsilon, double radEpsilon)
{
	...
	// check the line.
	if ( lineLen <= coordEpsilon )		// degenerate zero-length line.
	{
