...
void Graph::CollectGraph(Node *current_node,BOOL_OP operation, bool detecthole,Sint32 graphnumber, bool& foundholes )
{
	...
	if (!currentlink)
   {
      char buf[100];
      if (detecthole)
	      snprintf(buf, sizeof(buf), "no NON flat link Collectgraph for operation at %15.3lf , %15.3lf",
         				(double) current_node->GetX(), (double) current_node->GetY());
      else
	      snprintf(buf, sizeof(buf), "no NON flat link Collectgraph at %15.3lf , %15.3lf",
         				(double) current_node->GetX(), (double) current_node->GetY());
...
bool Graph::MakeClockWise()
{
   ...
   {
      char buf[100];
	   snprintf(buf, sizeof(buf), "no NON flat link MakeClockWise at %15.3lf , %15.3lf",
