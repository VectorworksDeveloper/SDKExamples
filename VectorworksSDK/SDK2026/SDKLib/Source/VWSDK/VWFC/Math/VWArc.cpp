...
double VWArc2D::GetLength(const VWPoint2D& pt) const
{
	...
	// Check the direction of the angle. This handles cases where the angle of the given point is >180 degrees.
	const bool isCW = fSweepAngleDeg < 0.0;
	if ( vecPt.CrossProductMagnitude( vecStart ) < 0 == isCW && ang * fRadius > VWPoint2D::sRawEpsilon )
	{
		ang = 2 * PI - ang;
	}
