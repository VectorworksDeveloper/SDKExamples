...
class DL_Iter
{
	public:
      ...
      void    takeover        (DL_Iter* otheriter, Sint32 maxcount);
      ...
      //!iterate through all items of the list
		...
		void mergesort(Sint32 (*fcmp) (Dtype, Dtype));

		//!sort list with cocktailsort
		void cocktailsort          (Sint32 (*)(Dtype,Dtype), void (*)(Dtype,Dtype)=NULL);

	protected:

		//!sort list with mergesort
		void mergesort_rec(Sint32 (*fcmp)(Dtype,Dtype), DL_Node<Dtype> *RT1,Sint32 n);

		//!sort list with mergesort
		void mergetwo(Sint32 (*fcmp)(Dtype,Dtype), DL_Node<Dtype> *RT1,DL_Node<Dtype> *RT2);
...
template <class DType> class DL_SortIter :public DL_Iter<DType>
{
	public:
	   //!Constructor of sort iterator for given list and sort function
		DL_SortIter(DL_List<DType>* nw_list, Sint32 (*new_func)(DType ,DType ));

	   //!Constructor of sort iterator with sort function and no list attached
		DL_SortIter(Sint32 (*newfunc)(DType,DType));
      ...
      void   insbefore (bool n){sortitererror();};
	   ...
	   //!Not allowed
		void   takeover (DL_Iter<DType>* otheriter, Sint32 maxcount){sortitererror();};
	   //!Not allowed
...
\code
	DL_List<Sint32> * a_list = new DL_List<Sint32>();
\endcode
*/
template <class Dtype>
...
\par Example:
    How to construct a list of type integer:
\code
   DL_List<Sint32> * a_list = new DL_List<Sint32>(); # declaration and allocation
...
{
	if (_iterlevel != 0)
		   ACTIVE_BoolEng->error("DL_List::~DL_List()\n_iterlevel > 0 ","list error");
...
{
   char buf[100];
   snprintf(buf, 100, "DL_List<Dtype>::%s", function);
   size_t max_cat_len = 100 - strlen(buf) - 1;
   switch (a_error)
   {
		case NO_MES:             break;
		case EMPTY:              strncat(buf,"list is empty",max_cat_len); break;
		case ITER_GT_0:          strncat(buf,"more then zero iter",max_cat_len); break;
		case NO_LIST:            strncat(buf,"no list attached",max_cat_len); break;
		case SAME_LIST:          strncat(buf,"same list not allowed",max_cat_len); break;
		case AC_ITER_LIST_OTHER: strncat(buf,"iter not allowed on other list",max_cat_len); break;
		default:						 strncat(buf,"unhandled error",max_cat_len); break;
   }

   ACTIVE_BoolEng->error(buf,"list error");
}
...
\code
DL_List<Sint32> _intlist; #create a list of integers
...
*/
template <class Dtype>
void DL_List<Dtype>::remove_all()
{
	if (_iterlevel > 0 )
		Error("remove_all()",ITER_GT_0);

	DL_Node<Dtype> *temp;
	for (Sint32 i=0; i<_nbitems; i++)
	{
		temp = _root->_next;
		_root->_next = temp->_next;
		delete temp;
	}
...
*/
template <class Dtype>
void DL_List<Dtype>::insend(Dtype newitem)
{
	if (_iterlevel > 0)
		Error("insend()",ITER_GT_0);

	DL_Node<Dtype>* newnode = SDK_NEW DL_Node<Dtype>(newitem);
...
*/
template <class Dtype>
void DL_List<Dtype>::insbegin(Dtype newitem)
{
	if (_iterlevel > 0)
...
\code
...

     {
             ...
             DL_Iter<Sint32>*  a_listiter=new DL_Iter<Sint32>(a_list);
...
{
   char buf[100];
   snprintf(buf, 100, "DL_Iter<Dtype>::%s", function);
   size_t max_cat_len = 100 - strlen(buf) - 1;
   switch (a_error)
   {
		case NO_MES:             break;
		case NO_LIST:            strncat(buf,"no list attached",max_cat_len); break;
		case NO_LIST_OTHER:      strncat(buf,"no list on other iter",max_cat_len); break;
		case AC_ITER_LIST_OTHER: strncat(buf,"iter not allowed on other list",max_cat_len); break;
		case SAME_LIST:          strncat(buf,"same list not allowed",max_cat_len); break;
		case NOT_SAME_LIST:      strncat(buf,"must be same list",max_cat_len); break;
		case ITER_GT_1:          strncat(buf,"more then one iter",max_cat_len); break;
		case ITER_HITROOT:          strncat(buf,"iter at root",max_cat_len); break;
		case NO_ITEM:            strncat(buf,"no item at current",max_cat_len); break;
		case NO_NEXT:            strncat(buf,"no next after current",max_cat_len); break;
		case NO_PREV:            strncat(buf,"no prev before current",max_cat_len); break;
		case EMPTY:              strncat(buf,"list is empty",max_cat_len); break;
		case NOT_ALLOW:          strncat(buf,"not allowed",max_cat_len); break;
		case ITER_NEG:           strncat(buf,"to much iters deleted",max_cat_len); break;
		default:						 strncat(buf,"unhandled error",max_cat_len); break;
   }
   ACTIVE_BoolEng->error(buf,"list error");
}
...
\code
    DL_List<Sint32>* IntegerList;
    IntegerList = new DL_List<Sint32>();
    DL_Iter<Sint32>*  a_listiter=new DL_Iter<Sint32>(IntegerList);
\endcode
...
\code
 ...
 DL_Iter<Sint32>*  a_secondlistiter=new DL_Iter<Sint32>(a_listiter);
\endcode
...
\code
   DL_Iter<Sint32>*  a_iter=new DL_Iter<Sint32>();
...
template <class Dtype> void DL_Iter<Dtype>::merge(DL_List<Dtype>* otherlist)
{
	...
	for(Sint32 i=0; i<otherlist->NB; i++)
	{
...
DL_List<Sint32> _intlist2; #create a list of integers

Sint32 a=123;

DL_Iter<Sint32>*  a_listiter2=new DL_Iter<Sint32>(&_intlist2);
...
DL_Iter<Sint32>*  a_listiter1=new DL_Iter<Sint32>(&_intlist);
...
*/
template <class Dtype>
void DL_Iter<Dtype>::takeover(DL_Iter* otheriter, Sint32 maxcount)
{
	if (otheriter->_current==0)
		Error("takeover(DL_Iter*,Sint32)",NO_LIST_OTHER);
	if (_current==0)
		Error("takeover(DL_Iter*,Sint32)",NO_LIST);

	if (otheriter->_list->_iterlevel > 1)
		Error("takeover(DL_Iter*,Sint32)",AC_ITER_LIST_OTHER);
	else if (otheriter->_list == _list)
		Error("takeover(DL_Iter*,Sint32)",SAME_LIST);

	if (maxcount<0)
		Error("takeover(DL_Iter*,Sint32), maxcount < 0",NO_MES);
	...
	{  //take maxcount elements from otheriter
      ...
      {	// this is faster (1st half)
			node=otheriter->HD;
         for(Sint32 i=1; i<maxcount; i++)
           node=node->_next;
		}
		else
		{	// no, this is faster (2nd half)
			node=otheriter->TL;
         for(Sint32 i=NB; i>maxcount+1; i--)
           node=node->_prev;
		}
...
\code
DL_List <Sint32> _intlist; #create a list of integers
DL_Iter<Sint32>*  a_listiter=new DL_Iter<Sint32>(&_intlist);
...
*/
template <class Dtype>
void DL_Iter<Dtype>::operator++(Sint32)
{
	if (_current==0)
		Error("operator++(Sint32)",NO_LIST);
...
*/
template <class Dtype>
void DL_Iter<Dtype>::operator>>(Sint32 n)
{
	if (_current==0)
		Error("operator>>()",NO_LIST);
...
DL_List <Sint32> _intlist; #create a list of integers
DL_Iter<Sint32>*  a_listiter=new DL_Iter<Sint32>(&_intlist);
DL_Iter<Sint32>*  a_listiter2=new DL_Iter<Sint32>(&_intlist);
...
a_listiter->insend(1234);
a_listiter->tohead();
Sint32 theitem=a_listiter->item();
\endcode
*/
template <class Dtype>
...
Sint32 count=2*a_listiter->count();
while (count)
{
      cout << a_listiter->item();
      next_wrap();
...
*/
template <class Dtype>
void DL_Iter<Dtype>::insend(Dtype newitem)
{
	if (_current==0)
...
*/
template <class Dtype>
void DL_Iter<Dtype>::insbegin(Dtype newitem)
{
	if (_current==0)
...
*/
template <class Dtype>
void DL_Iter<Dtype>::insbefore(Dtype newitem)
{
	if (_current==0)
...
*/
template <class Dtype>
void DL_Iter<Dtype>::insafter(Dtype newitem)
{
	if (_current==0)
...
\verbatim
          ...
          Declaration: Sint32 (*fcmp) (Dtype,Dtype)
...
\code
Sint32 numbersorter(Sint32 a,Sint32 b)
{
      if(a < b) return(1);
      else
      if(a == b) return(0);
...
*/
template <class Dtype>
void DL_Iter<Dtype>::cocktailsort(Sint32 (*fcmp) (Dtype, Dtype), void (*fswap)(Dtype, Dtype))
{
	if (_current==0)
	...
	// ga door zolang er geswapped is  & og en bg elkaar niet raken
	while (swapped && (og!=bg))
	{
		swapped = false;

		// BUBBELSLAG  ondergrens--->> bovengrens
		cursor=og;
		while(!(cursor == bgold))
		{
			...
			if( fcmp(cursor->_next->_item, cursor->_item)==1)
			{
				// user function
				if ( fswap != NULL )
					fswap(cursor->_item, cursor->_next->_item);
				// update swap-flag en bovengrens
				swapped = true;
				bg = cursor;
		...
		// BRICKSLAG   ondergrens <<---bovengrens
		cursor=bg;
		while(!(cursor == ogold))
		{
			...
			if( fcmp(cursor->_item, cursor->_prev->_item)==1)
			{
				// user function
				if ( fswap != NULL )
					fswap(cursor->_item, cursor->_prev->_item);
				// update swap-flag en ondergrens
				swapped = true;
				og = cursor;
				// swap de items van de nodes
				swap = cursor->_item;
...
*/
template <class Dtype>
void DL_Iter<Dtype>::mergesort(Sint32 (*fcmp) (Dtype, Dtype))
{
	if (_current==0)
...
template <class Dtype>
void DL_Iter<Dtype>::mergesort_rec(Sint32 (*fcmp)(Dtype,Dtype), DL_Node<Dtype> *RT1, Sint32 n1)
{
...
template <class Dtype>
void DL_Iter<Dtype>::mergetwo(Sint32 (*fcmp)(Dtype,Dtype), DL_Node<Dtype> *RT1,DL_Node<Dtype> *RT2)
{
	DL_Node<Dtype>       *c,*a,*b;
   ...
   do
   {
      ...
      if (a == RT1)
      { 	c->_next=b;b->_prev=c; //connect list b to the list made sofar
         RT1->_prev=RT2->_prev;
         RT1->_prev->_next=RT1;
      	break;
      }
...
*
*\code     DL_List<Sint32>* a_list = new DL_List<Sint32>();# declaration and allocation
*
*     Sint32 a=123;
*
*     Sint32 b=345;
...
*     {
*
*             DL_StackIter<Sint32>*  a_listiter=new DL_StackIter<Sint32>(a_list);
*
*             a_listiter->push(a)
*
*             a_listiter->push(b)
...
template <class Dtype>
Sint32  DL_StackIter<Dtype>::count()
{
...
// constructor
template <class DType>
DL_SortIter<DType>::DL_SortIter(DL_List<DType>* nw_list, Sint32 (*new_func)(DType ,DType ))
