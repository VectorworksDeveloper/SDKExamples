...
#ifdef _WINDOWS
	float scaleFactor = DPIDisplayAdjustment::DPIScaleFactor(reinterpret_cast<HWND>(fDevHandle));
...
void VWCustomCtrlDC::SelectFont(const VWCustomCtrlFont& font)
{
	if ( VERIFYN( kVStanev, fpDrawPad ) ) {
		...
		textFont.ptSize = DPIDisplayAdjustment::DPIScale(reinterpret_cast<HWND>(fDevHandle), font.GetFontSize());
#else
...
float VWCustomCtrlDC::GetDPIScaleFactor() const
{
#ifdef _WINDOWS
	return DPIDisplayAdjustment::DPIScaleFactor(reinterpret_cast<HWND>(fDevHandle));
#else
	return 1.0;
#endif
}
...
void VWCustomCtrlDC::DrawDashedLine(const ViewPt& fromPt, const ViewPt& toPt, InternalIndex lineStyle)
{
	if ( VERIFYN( kVStanev, fpDrawPad ) ) {
		this->PrepForFrame();
		
		ViewPt offset(fXOffset, fYOffset);
		fpDrawPad->DrawDashedLineN(fromPt + offset, toPt + offset, lineStyle, static_cast<short>(fCurrentPen.GetSize()));
	}
}
...
void VWCustomCtrlDC::DrawString(const ViewPt& pt, const TXString& string)
{
	if ( VERIFYN( kVStanev, fpDrawPad ) ) {
		...
		fpDrawPad->TextMoveTo( pt.x + fXOffset, pt.y + fYOffset );
...
void VWCustomCtrlDC::DrawIcon(const ViewPt& pt, VectorWorks::UI::IIcon* pIcon)
{
	...
	if ( isSet ) 
	{
		VectorWorks::UI::IconStream imgStreamData;

		if ( VERIFYN( kAAntonini, VCOM_SUCCEEDED( pIcon->GetPngImageStream( imgStreamData, VectorWorks::UI::ImageVariant::Auto ) ) ) ) {
...
void VWCustomCtrlDC::DrawIcon(const ViewRect& rect, VectorWorks::UI::IIcon* pIcon)
{
	...
	if ( isSet )
	{
		...
		if ( VERIFYN( kAAntonini, VCOM_SUCCEEDED( pIcon->GetPngImageStream( imgStreamData, VectorWorks::UI::ImageVariant::Auto ) ) ) ) {
			...
			VectorWorks::Imaging::ICompressedImagePtr pICompImage(VectorWorks::Imaging::IID_CompressedImage);
			if ( VCOM_SUCCEEDED( pICompImage->CreateFromStreamN( imgStreamData) ) ) {
