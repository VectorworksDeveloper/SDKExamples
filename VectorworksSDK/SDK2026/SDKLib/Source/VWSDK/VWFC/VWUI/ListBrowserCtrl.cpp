...
VWListBrowserCtrl::VWListBrowserCtrl(TControlID id)
	: VWControl( id, {eCompListBrowserControl} )
...
void VWListBrowserCtrl::EnableDragOnIndices(const std::vector<bool>& arrIndicesDragEnableState)
{
	VERIFYN( kVStanev, gSDK->EnableListBrowserDragOnIndices( fpParentDlg->GetControlID(), fControlID, arrIndicesDragEnableState ) );
}
...
{
	if ( VERIFYN( kIAtovski, fpParentDlg ) )
	{
		fpParentDlg->DeleteLBColumnData( fControlID, columnIndex );
	}
...
size_t VWListBrowserCtrl::AddRow(const TXString& text, Sint32 marker /*=0*/)
{
	...
	return this->InsertRow( rowIndex, text, marker );
}

size_t VWListBrowserCtrl::InsertRow(size_t rowIndex, const TXString& text, Sint32 marker /*=0*/)
{
    size_t	newRowIndex	= size_t(-1);
    if ( rowIndex != size_t(-1) )
	{
		newRowIndex = ::GS_InsertListBrowserItem( gCBP, fpParentDlg->GetControlID(), fControlID, DemoteTo<short> ( kVStanev, rowIndex ), text );
		
		if ( fIsHierarchical == false && newRowIndex != -1 )
		{
			// set marker for this item
			if ( newRowIndex >= farrIndexToMarker.size() )
			{
			...
			{
				farrIndexToMarker.insert( farrIndexToMarker.begin() + newRowIndex, marker );
			}
		}
	}
...
// ------------------------------------------------------------------------
// images support
size_t	VWListBrowserCtrl::AddImage(const TXString& imageSpecifier)
{
...
static const char* GetStandardImageSpecifier(EStandardImageID imageID)
{
	...
	switch(imageID) {
		...
		case kStandardImageID_CheckMarkWhite:
			DSTOP((kAAntonini, "kStandardImageID_CheckMarkWhite is deprecated. Neither platform uses white checkmarks instead of regular ones for selected rows anymore. Please remove this usage and its associated logic."));
...
size_t VWListBrowserCtrl::AddImageStandard(EStandardImageID image)
{
	const char* imageSpecifier = GetStandardImageSpecifier(image);
	return this->AddImage(imageSpecifier);
}
