...
VWDialog::VWDialog()
: VWControlsContainer( 0, {eCompUndefined} )
{
	fbIsSetup				= false;
...
void VWDialog::VerifyControlIsValid(VWControl* pControl)
{
	// Cannot check because the dialog hasn't been created yet. A later pass will be done by VWDialog::CreateDialog to check the existing controls.
	if (fControlID == 0)
		return;
		
	bool valid = false;
	for (const auto compType: pControl->fAllowedComponentTypes) {
		auto kludgeTuple = std::make_tuple(fControlID, pControl->GetControlID(), compType);
		if ((valid = gSDK->Kludge(7554/*kKludgeCheckDialogComponentType*/, &kludgeTuple, nullptr))) {
			break;
		}
	}
	VERIFYMSG(kAAntonini, valid, "Trying to tie a VWControl instance (ID = %d) to a Layout Manager control with a different component type", pControl->GetControlID());
}

void VWDialog::VerifyControlsAreValid()
{
#if BUG
	if (VERIFYN(kAAntonini, fpArrControls)) {
		TControlsArray&		arrControls		= * ((TControlsArray*) fpArrControls);
		
		for (const auto control: arrControls) {
			VerifyControlIsValid(control.pControl);
		}
	}
#endif
}
...
bool VWDialog::PrepareDialogLayout()
{
	...
	return VERIFYN(kVStanev, ::GS_VerifyLayout( gCBP, fControlID ));
}
...
EDialogButton VWDialog::RunDialogLayout(const TXString &contextualHelpID, const bool bEnableContextualHelp)
{
    ...
    if (PrepareDialogLayout()) {
		...
		result = static_cast<EDialogButton>(gSDK->RunLayoutDialog(fControlID, VWDialog::DialogEventHandler, this, contextualHelpID, bEnableContextualHelp));
    }
    
    return result;
}

bool VWDialog::RunPopover(const TXString &contextualHelpID, const CPopoverParameters& popoverParams)
{
	bool success = false;
	
	if (PrepareDialogLayout()) {
		// Display the popover. Will call the Event handling routine specified.
		//FIXME: What's the return value? Do we need a new enumeration for popover exit modes?
		
		success = gSDK->RunPopover(fControlID, VWDialog::DialogEventHandler, this, contextualHelpID, popoverParams);
	}
...
void VWDialog::LocalDialogEventHandler(Sint32& itemHit, void* pData)
{
	...
	case kContextMenuMessage:
		{
			// return list of menu items
			TItemMessageData* messageData = (TItemMessageData*)pData;
			if (messageData)
				this->OnContextMenu(messageData);
			
			break;
		}
	case kMenuItemSelectedMessage:
		{
			TItemMessageData* messageData = (TItemMessageData*)pData;
			if (messageData)
				this->OnContextMenuItemSelected(messageData);
				...
				for(const SSavedColumnData& data : farrSavedSwapTabData)
				{
					if ( data.fControlID == controlID )
					{
						if ( data.fColumnIndex == 0 ) // swap
						{
							...
							{
								TXString	savedValue;	this->GetSavedValue( data.fSavedSettingsColumnName, savedValue );
								overviewPaneList->SelectIndex( (size_t) savedValue.atoi() );
							}
						}
						...
						else if ( data.fColumnIndex == 2 ) // swap with list browser
						{
							VWListBrowserCtrl*	overviewPaneList	= this->GetListBrowserCtrlByID( data.fControlID );
							size_t	selIndex = overviewPaneList->GetFirstSelected();
							if ( selIndex != size_t(-1) )
							{
								if ( data.fTranslateCallback )
								{
							...
							else
							{
								...
								overviewPaneList->DeselectAll();
								// the list browser could have hidden rows if it is hierarchical
								// so selListIndex could be a valid index for the total list browser rows
								// but it could be bigger than the visible rows count
								if ( selListIndex < overviewPaneList->GetRowsCount() )
								{
									overviewPaneList->SelectRow( selListIndex, true );
								}
							}
						}
					}
...
void VWDialog::OnContextMenu(TItemMessageData* messageData)
{
}

void VWDialog::OnContextMenuItemSelected(TItemMessageData* messageData)
{
}
...
VWImageButtonCtrl* VWDialog::GetImageButtonCtrlByID(TControlID id)
{
	VWControl*	pControl	= this->GetControl( id );
	if ( pControl )
		return dynamic_cast<VWImageButtonCtrl*>( pControl );
	pControl				= new VWImageButtonCtrl( id );
	pControl->fpParentDlg	= this;
	this->AddControl( pControl, true );
	return (VWImageButtonCtrl*) pControl;
}
...
VWSearchBoxCtrl* VWDialog::GetSearchBoxCtrlByID(TControlID id)
{
	VWControl*	pControl	= this->GetControl( id );
	if ( pControl )
		return dynamic_cast<VWSearchBoxCtrl*>( pControl );
	pControl				= new VWSearchBoxCtrl( id );
	pControl->fpParentDlg	= this;
	this->AddControl( pControl, true );
	return (VWSearchBoxCtrl*) pControl;
}
...
VWBorderlessButtonCtrl* VWDialog::GetBorderlessButtonCtrlByID(TControlID id)
{
	VWControl*	pControl	= this->GetControl( id );
	if ( pControl )
		return dynamic_cast<VWBorderlessButtonCtrl*>( pControl );
	pControl				= new VWBorderlessButtonCtrl( id );
	pControl->fpParentDlg	= this;
	this->AddControl( pControl, true );
	return (VWBorderlessButtonCtrl*) pControl;
}
...
void VWDialog::UpdateSelectionInfo(TControlID controlID, VWDialogEventArgs& eventArg)
{
	...
	if ( bFound ) {
		...
		switch( data.type ) {
			...
			{
				const VWChooseClassPopupCtrl*	pCtrl	= this->GetChooseClassPopupCtrlByID( data.controlID );

				bool			isEdited;
				TXString		value;
				if ( pCtrl && pCtrl->GetSelectedIndex() != 0
					&& fpSelectionSupport->IsEdited( data.key, isEdited, data.flag ) && !isEdited
					&& fpSelectionSupport->GetValue( data.key, value ) )
				{
					bool		isDeterminate;	fpSelectionSupport->IsDeterminate( data.key, isDeterminate, data.flag );
					TXString	selectedClass;	pCtrl->GetSelectedClass( selectedClass );
					const bool	isNewValue		= value != selectedClass;

					fpSelectionSupport->SetEdited( data.key, isNewValue || !isDeterminate, data.flag );
				}
...
void VWDialog::UpdateDataInternal(bool bSafeToUI, TControlID id, bool bOnlyThisOne, bool bDoBlackBackgroundInvert)
{
	...
	for(size_t i=0; i<len; i++) {
		...
		switch( data.type ) {
			...
			case kDDXTypeSearchEditBox: {
					VWSearchBoxCtrl*		pCtrl	= this->GetSearchBoxCtrlByID( data.controlID );
					TXString*				pData	= (TXString*) data.pData;
					...
					if ( pCtrl && pData && bSafeToUI ) {
						if ( !bDeterminate && !bEdited ) {
							...
							pCtrl->SetSearchText( *pData );
						}
					}
					if ( pCtrl && pData && ! bSafeToUI  && bEdited ) {
						*pData = pCtrl->GetSearchText();
					}
				} break;
			case kDDXTypeEditPassword: {
...
void VWDialog::AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, short* pSelected, const SClassPopupItem& standardClass)
{
	ASSERTN( kJWarshaw, fpArrDDXData );
	if ( fpArrDDXData == NULL )
		return;
	...
	data.pData				= (void*) pValue;
	data.pData1				= (void*) pSelected;
	data.strData			= standardClass.fClassName;
	arrDDX.push_back( data );
...
#else
		TXString	userClass	= VWFC::Tools::VWAutoClassing::GetUserClass( standardClass.fClassName );
		thePopup->InsertProposedClass( userClass, standardClass.fIconSpec );
#endif
	}
}

void VWDialog::AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, short* pSelected, const SClassPopupItem& standardClass, const TClassPopupItemsArray& arrAdditionalItems )
{
	ASSERTN( kJWarshaw, fpArrDDXData );
	if ( fpArrDDXData == NULL )
		return;
	...
	VWChooseClassPopupCtrl*		thePopup	= this->GetChooseClassPopupCtrlByID( controlID );
	if ( thePopup ) {
		size_t	itemsCnt	= arrAdditionalItems.size();
		for(size_t i=0; i<itemsCnt; i++) {
			const SClassPopupItem& item	= arrAdditionalItems[ i ];
			thePopup->InsertProposedClass( item.fClassName, item.fIconSpec );
		}
...
void VWDialog::AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, short* pSelected, const TClassPopupItemsArray& arrAdditionalItems )
{
	ASSERTN( kJWarshaw, fpArrDDXData );
	if ( fpArrDDXData == NULL )
		return;
...
void VWDialog::AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected, const SClassPopupItem& standardClass)
{
	ASSERTN( kJWarshaw, fpArrDDXData );
	if ( fpArrDDXData == NULL )
		return;
...
void VWDialog::AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected, const SClassPopupItem& standardClass, const TClassPopupItemsArray& arrAdditionalItems )
{
	ASSERTN( kJWarshaw, fpArrDDXData );
	if ( fpArrDDXData == NULL )
		return;
...
void VWDialog::AddDDX_ChooseClassPopup(TControlID controlID, TXString* pValue, size_t* pSelected, const TClassPopupItemsArray& arrAdditionalItems )
{
	ASSERTN( kJWarshaw, fpArrDDXData );
	if ( fpArrDDXData == NULL )
		return;
...
void VWDialog::AddDDX_ChooseLayerPopup(TControlID controlID, TXString* pValue, short* pSelected, const TClassPopupItemsArray& arrAdditionalItems)
{
	ASSERTN(kILane, fpArrDDXData);
	if (fpArrDDXData == NULL)
		return;
	...
	if (thePopup) {
		...
		for (size_t i = 0; i < itemsCnt; i++) {
			const SClassPopupItem& item = arrAdditionalItems[ i ];
			thePopup->InsertProposedLayer(item.fClassName, item.fIconSpec);
		}
	}
}
...
bool VWDialog::GetDDXControlType(TControlID controlID, EDDXType& outValue)
{
	if ( !VERIFYN( kVStanev, fpArrDDXData ) )
		return false;
	...
	for ( size_t i = 0, len = arrDDX.size(); !found && i < len; i++ )
	{
		SDDXData&	data	= arrDDX[ i ];
...
// Specify a category and item to show an optional "Do not show this dialog again" checkbox.
/*static*/ void VWDialog::AlertInform(	const TXString& text,
...
{
	::GS_AlertInform( gCBP, text, advice, bMinorAlert, category, item, checkboxOverideText );
}
...
{
	//update the indexes for each column when a column is inserted to the list
	for ( SSavedColumnData& data : farrSavedLSColumnData )
	{
		if ( data.fControlID == controlID &&
...
void VWDialog::RegisterSwapControlWithListBrowser( TControlID listControlID, TControlID swapControlID, const TXString& savedSettingsName, bool autoSwap, TRegisterSwapCtrlTranslateCallback callback )
{
	SSavedColumnData	data;
	data.fControlID					= listControlID;
	data.fSupportControlID			= swapControlID;
...
void VWDialog::InitializeStyleSupport()
{
	...
	fByMixedButtonHelp = TXResource( "Vectorworks/Strings/PluginStyles.vwstrings", "StyleButtonMixed" );
}

void VWFC::VWUI::VWDialog::InitializeStyleListBrowserColumns( TControlID controlID, const TSizeTArray& styleColumns )
{
	// should be called after the list browser's columns have been created but before adding any rows

	if ( VWListBrowserCtrl* listBrowser = this->GetListBrowserCtrlByID( controlID );
		VERIFYN( kTDimov, listBrowser && !styleColumns.empty() ) )
	{
		fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByInstance			]	= listBrowser->AddImage( "Vectorworks/Standard Images/LBByInstance.png"	);
		fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByStyle			]	= listBrowser->AddImage( "Vectorworks/Standard Images/LBByStyle.png"	);
		fStyleLBImageMap[ controlID ][ kPluginStyleParameter_AllwaysByInstance	]	= fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByInstance		];
		fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByCatalog			]	= listBrowser->AddImage( "Vectorworks/Standard Images/LBByCatalog.png"	);
		fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByMixed			]	= listBrowser->AddImage( "Vectorworks/Standard Images/LBByMixed.png"	);

		for ( size_t columnIndex : styleColumns )
		{
			VWListBrowserColumn	column	= listBrowser->GetColumn( columnIndex );

			column.SetColumnType( fbStyleMode ? kListBrowserControlMultiState : kListBrowserControlNoClick );
			column.SetItemDisplayType( kListBrowserDisplayImageOnly );

			column.InsertColumnChoice( TXResource( "Vectorworks/Strings/PluginStyles.vwstrings", "Instance"	), fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByInstance	] );
			column.InsertColumnChoice( TXResource( "Vectorworks/Strings/PluginStyles.vwstrings", "Style"	), fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByStyle		] );
			column.InsertColumnChoice( TXResource( "Vectorworks/Strings/PluginStyles.vwstrings", "Catalog"	), fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByCatalog	] );
			column.InsertColumnChoice( TXResource( "Vectorworks/Strings/PluginStyles.vwstrings", "Mixed"	), fStyleLBImageMap[ controlID ][ kPluginStyleParameter_ByMixed		] );
		}
	}
}
...
void VWFC::VWUI::VWDialog::InitStyleControl( TControlID controlID, Sint32 groupID, EPluginStyleParameter styleParam, TControlID primaryControl, TControlID addedControl2, TControlID addedControl3, TControlID addedControl4 )
{
	VWStyleButtonCtrl * pControl = NULL;
	pControl = GetStyleButtonCtrlByID( controlID );
	if ( pControl ) {
		...
		style.paramName	= "";
		style.groupID = groupID;
		style.styleType	= styleParam;
		style.isLinkedWithParam = false;

		pControl->SetParameterName( "" );
		pControl->SetStyleType( style.styleType );
		if ( fbStyleMode ) {
			if ( style.styleType == kPluginStyleParameter_ByCatalog ) {
				pControl->SetHelpText( fByCatalogButtonHelp );
...
void VWDialog::InitStyleGroupControl( TControlID controlID, Sint32 groupID, TControlID textControlID )
{
	...
	if ( pControl ) {
		...
		this->UpdateStyleGroupButton( pControl );
	}
}

void VWDialog::InitStyleLBItem( TControlID controlID, size_t row, size_t column, Sint32 groupID, std::vector<TXString> parameters, const TSizeTArray& associatedColumns )
{
	// should be called after you've already added the specified row

	if ( VERIFYN( kTDimov, !parameters.empty() && !associatedColumns.empty() ) )
	{
		EPluginStyleParameter	styleType	= kPluginStyleParameter_ByInstance;
		if ( fpStyleSupport && fpStyleSupport->GetStyleType( parameters.front(), styleType) )
		{
			if ( fpStyleSupport->IsCatalogParam( parameters.front() ) )
			{
		...
		{
			for ( const TXString& parameter : parameters )
			{
				fpStyleSupport->AddItem( parameter, kPluginStyleParameter_ByInstance );
			}
		}

		if ( VWListBrowserCtrl* listBrowser = this->GetListBrowserCtrlByID( controlID ); VERIFYN( kTDimov, listBrowser ) )
		{
			VWListBrowserItem	item	= listBrowser->GetItem( row, column );
			item.SetItemImage( fStyleLBImageMap[ controlID ][ styleType ] );
		}

		fStyleLBItemMap[ controlID ].push_back( SStyleLBItem { SLBCellLocation { row, column }, groupID, styleType, std::move( parameters ) } );
		auto& associatedCells	= fStyleLBItemMap[ controlID ].back().fAssociatedCells;
		const auto	cells	= associatedColumns | std::views::transform( [ row ] ( size_t col ) { return SLBCellLocation { row, col }; } );
		associatedCells.insert( associatedCells.end(), cells.begin(), cells.end() );
		this->UpdateStyleGroupButton( groupID );
	}
}

void VWDialog::DeleteStyleLBItem( TControlID controlID, size_t row, size_t column, Sint32 groupID )
{
	const auto LBItems = fStyleLBItemMap.find( controlID );
	if ( LBItems != fStyleLBItemMap.end() )
	{
		auto& itemsVector = LBItems->second;
		const auto LBITem = std::ranges::find_if( itemsVector, [ & ]( const SStyleLBItem& item )
			{ return item.fLocation == SLBCellLocation{ row, column }; } );

		if ( LBITem != itemsVector.end() )
		{
			itemsVector.erase( LBITem );

			int newRow = 0;
			std::ranges::for_each( itemsVector, [ & ]( SStyleLBItem& item )
				{
					item.fLocation.fRow = newRow;
					std::ranges::for_each( item.fAssociatedCells, [ & ]( SLBCellLocation& assocCell )
						{
							assocCell.fRow = newRow;
						} );
					++newRow;
				} );
...
void VWDialog::DeleteAllStyleLBItems( TControlID controlID )
{
	const auto LBItems = fStyleLBItemMap.find( controlID );
	if ( LBItems != fStyleLBItemMap.end() ) {
		LBItems->second.clear();
	}
}

void VWDialog::RestrictStyleTypesMultiSel()
{
	// Should be called at the end of plug-in's ::OnInitializeContent() - after style data has been initialized for standard controls and LB items.

	if ( !fpSelectionSupport || fpSelectionSupport->IsStyleHomogeneousMultiSel() )
		return;

	std::unordered_set<MCObjectHandle>	arrSymDef;
	std::vector<VWStyleSupport>			arrStyleSupport;
	std::vector<EPluginStyleParameter>	arrStyleTypes;

	// Initialize containers for unique style instances. Allow max 1 unstyled entry.
	for ( auto h : fpSelectionSupport->GetSelection() )
	{
		if ( MCObjectHandle iterSymDef = VWParametricObj( h ).GetStyleHandle() )
		{
			if ( !arrSymDef.contains( iterSymDef ) )
			{
				arrSymDef		.insert( iterSymDef );
				arrStyleSupport	.emplace_back( iterSymDef );
				arrStyleTypes	.emplace_back( kPluginStyleParameter_ByInstance );
			}
		}
		else if ( !arrSymDef.contains( nullptr ) )
		{
			arrSymDef			.insert( nullptr );
			arrStyleSupport		.emplace_back( h );
			arrStyleTypes		.emplace_back( kPluginStyleParameter_ByInstance );
		}
	}

	for ( auto& [_, control] : fStyleControlMap )
	{
		const TXString&	key = control.paramName;

		// Collect all style type variations for the iterated parameter
		for ( size_t i = 0, styleSupportCount = arrStyleSupport.size(); i < styleSupportCount; i++ )
		{
			arrStyleSupport[i].GetStyleType( key, arrStyleTypes[i] );
		}

		// Check if there are different style types for the iterated parameter
		if ( std::ranges::any_of( arrStyleTypes, [&](const auto& type) { return type != arrStyleTypes.front(); } ) )
		{
			control.styleType = kPluginStyleParameter_ByMixed;
			control.pControl->SetStyleType( kPluginStyleParameter_ByMixed );
		}
	}

	for ( auto& [_, items] : fStyleLBItemMap )
	{
		for ( SStyleLBItem& item : items )
		{
			const TXString& key = item.fParameters.front();
			...
			if ( std::ranges::any_of( arrStyleTypes, [&](const auto& type) { return type != arrStyleTypes.front(); } ) )
			{
				item.fStyleType = kPluginStyleParameter_ByMixed;
			}
		}
	}
}
...
void VWDialog::AssociateStyleControls( TControlID controlID, const std::vector<TControlID> associatedControlIDs )
{
    TStyleControl_Iterator iter = fStyleControlMap.find( controlID );
    if ( iter != fStyleControlMap.end() )
    {
        for (const TControlID& associatedControlID : associatedControlIDs)
        {
            VWStyleButtonCtrl * pCtrl = iter->second.pControl;
            pCtrl->AddAssociatedControl( associatedControlID );
        }
...
void VWDialog::AssociateStyleParameters( TControlID controlID, const std::vector<TXString> paramNames )
{
    TStyleControl_Iterator iter = fStyleControlMap.find( controlID );
    if ( iter != fStyleControlMap.end() )
    {
        for (const TXString& paramName : paramNames)
        {
            VWStyleButtonCtrl * pCtrl = iter->second.pControl;
            pCtrl->AddAssociatedParameter( paramName );
        }
...
void VWDialog::AssociateStyleLBCells( TControlID controlID, size_t row, size_t column, std::vector<SLBCellLocation> associatedCells )
{
	const auto	LBItems	= fStyleLBItemMap.find( controlID );
	if ( VERIFYN( kTDimov, LBItems != fStyleLBItemMap.end() ) )
	{
		const auto	LBITem	= std::ranges::find_if( LBItems->second, [ & ] ( const SStyleLBItem& item )
			{ return item.fLocation == SLBCellLocation { row, column }; } );

		if ( VERIFYN( kTDimov, LBITem != LBItems->second.end() ) )
			std::ranges::move( associatedCells, std::back_inserter( LBITem->fAssociatedCells ) );
	}
}

bool VWDialog::GetStyleControlType(TControlID controlID, EPluginStyleParameter& styleType)
{
	const auto	iter	= fStyleControlMap.find( controlID );
	const bool	found	= iter != fStyleControlMap.end();
	if ( found )
	{
...
{
	for ( VWStyleGroupButtonCtrl* pGroupControl : fStyleGroupControls )
		this->UpdateStyleGroupButton( pGroupControl );
}

void VWDialog::UpdateStyleGroupButton(Sint32 groupID)
{
	auto iter	= std::ranges::find_if( fStyleGroupControls, [ groupID ] ( VWStyleGroupButtonCtrl* pGroupControl ) { return pGroupControl->GetGroupID() == groupID; } );

	this->UpdateStyleGroupButton( iter != fStyleGroupControls.end() ? *iter : nullptr );
}

void VWDialog::UpdateStyleGroupButton(VWStyleGroupButtonCtrl* pGroupControl) const
{
	if ( pGroupControl )
	{
		const Sint32	groupID	= pGroupControl->GetGroupID();

		bool			byStyle	= false;
		bool			byInst	= false;

		for ( const auto& [_, control] : fStyleControlMap )
		{
			if ( control.groupID == groupID )
			{
				byStyle	= byStyle || control.styleType == kPluginStyleParameter_ByStyle;
				byInst	= byInst || control.styleType == kPluginStyleParameter_ByInstance;
			}
		...
		if ( !( byStyle && byInst ) )
		{
			const auto	isInGroup	= [ groupID ] ( const SStyleLBItem& item ) { return item.fGroupID == groupID; };
			for ( const auto& [_, items] : fStyleLBItemMap )
			{
				for ( const SStyleLBItem& item : items | std::views::filter( isInGroup ) )
				{
					byStyle	= byStyle || item.fStyleType == kPluginStyleParameter_ByStyle;
					byInst	= byInst || item.fStyleType == kPluginStyleParameter_ByInstance;
...
void VWDialog::UpdateStyleSupport()
{
	if ( fbStyleMode && fpStyleSupport ) {
		...
		for ( const auto& [_, control] : fStyleControlMap ) {
			pStyleButtonControl = control.pControl;
			paramName = control.paramName;
			if ( control.styleType != kPluginStyleParameter_ByCatalog ) {
				if( control.isLinkedWithParam )
				{
					fpStyleSupport->SetStyleType( paramName, control.styleType );
				}
				...
				if (  numAddedParams > 0 ) {
					for ( size_t i = 0; i < numAddedParams; i++ ) {
						...
						if ( paramName.IsEmpty() == false && control.styleType != kPluginStyleParameter_ByCatalog ) {
							fpStyleSupport->SetStyleType( paramName, control.styleType );
						}
					}
				}
		...
		for ( const auto& [_, items] : fStyleLBItemMap )
		{
			for ( const SStyleLBItem& item : items )
			{
				if ( item.fStyleType != kPluginStyleParameter_ByCatalog )
				{
					for ( const TXString& parameter : item.fParameters )
					{
						fpStyleSupport->SetStyleType( parameter, item.fStyleType );
					}
				}
			}
		}
...
void VWDialog::UpdateStyleControlTypes()
{
	...
	VWCatalogSupport	catSupport;
	bool				checkCatalog = false;
	if ( fpStyleSupport->fhHostSymbolDefiniiton )
	{
		catSupport.LoadCatalogList( fpStyleSupport->fhHostSymbolDefiniiton );
		checkCatalog = !catSupport.GetCatalogName().IsEmpty();
	}

	for ( auto& [_, control] : fStyleControlMap )
	{
		EPluginStyleParameter	styleType;
		if ( fpStyleSupport->GetStyleType( control.paramName, styleType ) )
		{
			if ( checkCatalog && catSupport.IsCatalogParam( control.paramName ) )
			{
				styleType = kPluginStyleParameter_ByCatalog;
			}

			control.styleType	= styleType;
			control.pControl->SetStyleType( styleType );
			control.pControl->UpdateAssociatedControls();
		}
	}

	for ( auto& [LBID, items] : fStyleLBItemMap )
	{
		if ( VWListBrowserCtrl* listBrowser = this->GetListBrowserCtrlByID( LBID ); VERIFYN( kTDimov, listBrowser ) )
		{
			for ( SStyleLBItem& styleItem : items )
			{
				if ( EPluginStyleParameter styleType; VERIFYN( kTDimov, !styleItem.fParameters.empty() ) && fpStyleSupport->GetStyleType( styleItem.fParameters.front(), styleType ) )
				{
					styleItem.fStyleType	= styleType;
					styleItem.UpdateAssociatedCells( listBrowser, true );

					VWListBrowserItem	item	= listBrowser->GetItem( styleItem.fLocation.fRow, styleItem.fLocation.fColumn );
					item.SetItemImage( fStyleLBImageMap[ LBID ][ styleType ] );
				}
			}
		}
	}
...
void VWDialog::UpdateStyleUI()
{
	...
	if ( fpStyleSupport ) {
		for ( const auto& [_, control] : fStyleControlMap ) {
			...
			if ( fpStyleSupport->IsCatalogParam( control.paramName ) ) {
		...
		for ( const auto& [LBID, items] : fStyleLBItemMap )
		{
			if ( VWListBrowserCtrl* listBrowser = this->GetListBrowserCtrlByID( LBID ); VERIFYN( kTDimov, listBrowser ) )
			{
				for ( const SStyleLBItem& styleItem : items )
				{
					if ( fbStyleMode )
					{
						if ( fpStyleSupport->IsCatalogParam( styleItem.fParameters.front() ) )
						{
							styleItem.EnableAssociatedCells( listBrowser, false );
							VWListBrowserItem	item	= listBrowser->GetItem( styleItem.fLocation.fRow, styleItem.fLocation.fColumn );
							item.SetItemImage( fStyleLBImageMap[ LBID ][ kPluginStyleParameter_ByCatalog ] );
							item.EnableItemDirectEdit( false );

						}
						else
						{
							VWListBrowserItem	item	= listBrowser->GetItem( styleItem.fLocation.fRow, styleItem.fLocation.fColumn );
							item.SetItemImage( fStyleLBImageMap[ LBID ][ styleItem.fStyleType ] );
						}
					}
					else
					{
						VWListBrowserItem	item	= listBrowser->GetItem( styleItem.fLocation.fRow, styleItem.fLocation.fColumn );
						item.SetItemImage( fStyleLBImageMap[ LBID ][ styleItem.fStyleType ] );
						styleItem.UpdateAssociatedCells( listBrowser, false );
					}
				}
			}
		}
		...
		for ( VWStyleGroupButtonCtrl* pGroupControl : fStyleGroupControls )
			pGroupControl->Enable( fbStyleMode );
	}
}
...
void VWDialog::OnChangeStyleButton( TControlID controlID, VWDialogEventArgs& eventArgs )
{
	...
	if ( fbStyleMode ) {
		...
		TStyleControl_Iterator iter = fStyleControlMap.find( controlID );
		if ( ( paramName.IsEmpty() == false || iter->second.isLinkedWithParam == false )  && iter != fStyleControlMap.end() ) {
...
{
	auto iter	= std::ranges::find_if( fStyleGroupControls, [ controlID ] ( const VWStyleGroupButtonCtrl* pGroupControl ) { return pGroupControl->GetControlID() == controlID; } );
	VWStyleGroupButtonCtrl* pGroupControl = iter != fStyleGroupControls.end() ? *iter : nullptr;

	if ( VERIFYN( kJWarshaw, pGroupControl ) ) {
		const Sint32				groupID				= pGroupControl->GetGroupID();
		const EPluginStyleGroup		groupType			= pGroupControl->GetStyleGroupType();

		const EPluginStyleGroup		newStyleGroupType	= groupType == kPluginStyleGroup_ByStyle ? kPluginStyleGroup_ByInstance : kPluginStyleGroup_ByStyle;

		const EPluginStyleParameter	newType				= newStyleGroupType == kPluginStyleGroup_ByStyle ? kPluginStyleParameter_ByStyle : kPluginStyleParameter_ByInstance;

		for ( auto& [_, control] : fStyleControlMap ) {
			if ( control.groupID == groupID ) {
				control.styleType = newType;
				control.pControl->SetStyleType( newType );
			}
		}

		for ( auto& [LBID, items] : fStyleLBItemMap )
		{
			for ( SStyleLBItem& styleItem : std::ranges::filter_view( items, [ groupID ] ( const SStyleLBItem& item ) { return item.fGroupID == groupID; } ) )
			{
				styleItem.fStyleType	= newType;
				if ( VWListBrowserCtrl* listBrowser = this->GetListBrowserCtrlByID( LBID ); VERIFYN( kTDimov, listBrowser ) )
				{
					VWListBrowserItem	item	= listBrowser->GetItem( styleItem.fLocation.fRow, styleItem.fLocation.fColumn );
					item.SetItemImage( fStyleLBImageMap[ LBID ][ newType ] );
				}
			}
		}
...
void VWDialog::OnChangeStyleListBrowserButton(TControlID controlID, VWDialogEventArgs& eventArgs)
{
	VWListBrowserCtrl*	listBrowser	= this->GetListBrowserCtrlByID( controlID );

	size_t	clickedRow;
	size_t	col;

	const auto	findItemAtRow	= [ & ] ( size_t row )
		{
			SStyleLBItem*	itemAtRow	= nullptr;

			const auto	LBItems	= fStyleLBItemMap.find( controlID );
			if ( LBItems != fStyleLBItemMap.end() )
			{
				const auto	LBITem	= std::ranges::find_if( LBItems->second, [ & ] ( const SStyleLBItem& item )
					{ return item.fLocation == SLBCellLocation { row, col }; } );

				if ( LBITem != LBItems->second.end() )
					itemAtRow	= std::to_address( LBITem );
			}

			return itemAtRow;
		};

	if ( VERIFYN( kTDimov, listBrowser ) && fbStyleMode && eventArgs.GetListBrowserEvent( clickedRow, col ) == kListBrowserEventType_DataChangeClick )
	{
		TSizeTArray	selectedRows;
		auto	clickedItem	= findItemAtRow( clickedRow );
		if ( VERIFYN( kTDimov, clickedItem ) && VERIFYN( kTDimov, listBrowser->GetSelectedItemIndices( selectedRows ) ) )
		{
			EPluginStyleParameter	newStyleType	= clickedItem->fStyleType == kPluginStyleParameter_ByInstance
													? kPluginStyleParameter_ByStyle
													: kPluginStyleParameter_ByInstance;

			for ( size_t row : selectedRows )
			{
				if ( auto selectedItem = findItemAtRow( row ); VERIFYN( kTDimov, selectedItem ) )
				{
					selectedItem->fStyleType	= newStyleType;
					VWListBrowserItem	item	= listBrowser->GetItem( row, col );
					item.SetItemImage( fStyleLBImageMap[ controlID ][ selectedItem->fStyleType ] );
					this->UpdateStyleGroupButton( selectedItem->fGroupID );
				}
			}
		}
	}
...
void VWDialog::DeleteLBColumnData( TControlID controlID, size_t columnIndex )
{
	//update the indexes for each column when a column is deleted
	for ( size_t index = farrSavedLSColumnData.size(); index > 0; index-- )
	{
		SSavedColumnData& data = farrSavedLSColumnData[ index - 1 ];

		if ( data.fControlID == controlID )
		{
			if ( data.fColumnIndex == columnIndex )
			{
				farrSavedLSColumnData.erase( farrSavedLSColumnData.begin() + index - 1 );
			}
			else if ( data.fColumnIndex > columnIndex )
			{
				data.fColumnIndex--;
			}
		}
...
void VWDialog::SStyleLBItem::EnableAssociatedCells(VWListBrowserCtrl* theListBrowser, bool enable) const
{
	if ( VERIFYN( kTDimov, theListBrowser && !fAssociatedCells.empty() ) )
	{
		for ( const auto& associatedCell : fAssociatedCells )
		{
			VWListBrowserItem	item = theListBrowser->GetItem( associatedCell.fRow, associatedCell.fColumn );
			item.EnableItemDirectEdit( enable );
			item.SetItemTextColorN( enable ? EUiControlTextTint::eDefaultTint : EUiControlTextTint::eDisabledTint );
		}
	}
}

void VWDialog::SStyleLBItem::UpdateAssociatedCells(VWListBrowserCtrl* theListBrowser, bool allowEnabling) const
{
	if ( VERIFYN( kTDimov, theListBrowser && !fAssociatedCells.empty() ) )
	{
