...
// FIXME: We should really create a VWPulldownMenuGroupBox class. Until that time, silence eCompPullDownMenuGroupBox.
VWPullDownMenuCtrl::VWPullDownMenuCtrl(TControlID id)
: VWControl( id, {eCompPullDownMenu, eCompEnhancedComboBox, eCompEditableEnhancedComboBox, eCompPullDownMenuGroupBox} )
{
}
...
void VWPullDownMenuCtrl::AddItem(const TXString& itemText, size_t insertAt /* = (size_t)-1*/, Sint32 marker /*= 0*/ )
{
	...
	if ( VWFC_VERIFY( insertAt <= farrIndexToMarker.size() ) )
	{
