...
const VWTreeCtrlItem	VWTreeCtrlContainer::kEmptyTreeItem;
...
void VWTreeCtrlItem::EnableMultiSelection(bool enabled /*= true*/)
{
	if (this->IsValid()) {
		gSDK->EnableTreeControlMultiSelection(fDialogID, fThisControlID, enabled);
	}
}
...
void VWTreeCtrlItem::SetItemCheck(ECheckButtonCtrlState state, bool clickable /*= true*/)
{
	if ( this->IsValid() ) {
		VERIFYN( kVStanev, gSDK->SetTreeControlItemCheckBoxState( fDialogID, fThisControlID, fThisItemID, state, clickable ) );
	}
}
