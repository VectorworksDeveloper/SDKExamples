...
VWTreeCtrlItem VWTreeCtrl::GetSelectedItems(std::vector<Sint32>& outItemIDs)
{
	if (fpParentDlg && gSDK->GetTreeControlSelectedItems(fpParentDlg->GetControlID(), fControlID, outItemIDs))
	{
		if (outItemIDs.size() > 0)
			return VWTreeCtrlItem(fDialogID, fControlID, outItemIDs[0]);
		else
			return VWTreeCtrlContainer::kEmptyTreeItem;
	}
	else
