...
namespace VWFC
{
	namespace VWUI
	{
		class VWCustomCtrlDCAccessor : public VWCustomCtrlDC
		{
			friend class VWCustomCtrlMemDC;
		public:
			SintptrT GetDevHandle() const { return VWCustomCtrlDC::GetDevHandle(); };
		};

		// ----------------------------------------------------------------------------------------
		class VWCustomCtrlMemDC final
		{
		public:
			VWCustomCtrlMemDC(VWCustomCtrlDC&, const ViewRect&);
			virtual ~VWCustomCtrlMemDC();

			VWCustomCtrlDC& GetDC() { return fbMemDC ? fdcMem : fdc; }
			bool			IsMemDC() const { return fbMemDC; }

			// Delete copy/move so extra instances can't be created/moved.
			VWCustomCtrlMemDC(const VWCustomCtrlMemDC&) = delete;
			VWCustomCtrlMemDC& operator=(const VWCustomCtrlMemDC&) = delete;
			VWCustomCtrlMemDC(VWCustomCtrlMemDC&&) = delete;
			VWCustomCtrlMemDC& operator=(VWCustomCtrlMemDC&&) = delete;
		private:
			VWCustomCtrlDC& fdc;
			bool			fbMemDC = false;
			VWCustomCtrlDC	fdcMem;
			ViewRect		fRect;
		};
		...
		// ----------------------------------------------------------------------------------------
		// VWCustomCtrlMemDC
		VWCustomCtrlMemDC::VWCustomCtrlMemDC(VWCustomCtrlDC& dc, const ViewRect& rect)
			: fdc(dc)
			, fdcMem(rect.Width(), rect.Height())
			, fRect(rect)
		{
			VERIFYN(kMVorobyev, !fRect.IsEmpty());

			fdc.SetClipRect(fRect);
			static_cast<VWCustomCtrlDCAccessor&>(fdcMem).SetDevHandle(static_cast<const VWCustomCtrlDCAccessor&>(fdc).GetDevHandle());
			fbMemDC = static_cast<const VWCustomCtrlDCAccessor&>(fdc).GetDevHandle() != NULL;
		}
		// ----------------------------------------------------------------------------------------
		VWCustomCtrlMemDC::~VWCustomCtrlMemDC()
		{
			fdc.BitBlt(ViewRect(0, 0, fRect.Width(), fRect.Height()), fdcMem, ViewPt(0, 0));
		}
	}
}
...
void VWCustomCtrl::DettachControl()
{
	...
	{
		VERIFYN(kMVorobyev, ::SetWindowLongPtr(it->first, GWLP_WNDPROC, (LPARAM)(WNDPROC)fpDefaultWndProc));
	...
	if ( this->OnEvent( event ) ) {
		...
		switch( nMsg ) {
			case WM_PAINT: {
					if ( fbDefaultPaint ) {
						...
						{ // keep this block so 'VWCustomCtrlDC' could be deleted before OnAfterPaint call
							...
							VWCustomCtrlMemDC memDC(dc, fCustomCtrlRect);
							VWCustomCtrlDC* pDC = &memDC.GetDC();
						...
						if ((event.x != oldPt.x) || (event.y != oldPt.y)) {
							...
							if ( fpActiveToolTip ) {
								...
								if ( this->OnToolTip( fpActiveToolTip->fTipID, fpActiveToolTip->fToolTip ) ) {
									...
									if ( fpActiveToolTip->fToolTip.fAdjustPositionForScroll )
									{
										Sint32		leftStart	= 0;
										Sint32		topStart	= 0;
										this->GetScrollArea( leftStart, topStart );
			...
			case WM_MOUSEWHEEL: {
					...
					POINT					localPt	= { ( (Sint32) (short) LOWORD( lParam ) ), ( (Sint32) (short) HIWORD( lParam ) ) };
					ScreenToClient( fhCustomWnd, &localPt );
