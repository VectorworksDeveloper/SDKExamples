...
{
	if ( VWFC_VERIFY( VWMaterialObj::IsMaterialObject( hMaterial ) ) )
	{
		fhObject = hMaterial;
	}
	else
...
struct Submaterial
{
	VWMaterialObj	fObject;
	double			fFraction;
};
static bool GetSubmaterials( VWMaterialObj materialObj, std::vector<Submaterial>& outSubmaterials )
{
	outSubmaterials.clear();

	CompoundSubmaterialDataVector submaterials;
	if ( !materialObj.IsSimpleMaterial() && gSDK->GetSubmaterialsFromMaterial( materialObj, submaterials ) )
	{
		outSubmaterials.reserve( submaterials.size() );
		for ( const auto& submaterial : submaterials )
		{
			InternalIndex index;
			MCObjectHandle h;
			const bool isSubmaterialValid =	gSDK->NameToInternalIndexN( submaterial.fSubmaterialName, index )
											&& ( h = gSDK->InternalIndexToHandle( index ) )
											&& VWMaterialObj::IsMaterialObject( h );
			...
			outSubmaterials.push_back( { VWMaterialObj( h ), submaterial.fFraction } );
		}
	}
...
static bool CalculateCompoundDensity( VWMaterialObj materialObj, double& outDensity )
{
	double totalDensity		= 0.0;
	double totalFraction	= 0.0;

	std::vector<Submaterial> submaterials;
	if ( !::GetSubmaterials( materialObj, submaterials ) || submaterials.empty() )
	{
		return false;
	}
	...
	{
		double submaterialDensity;
		if ( !submaterial.fObject.GetAverageDensity( submaterialDensity ) )
		{
			return false;
		}

		totalDensity += submaterialDensity * submaterial.fFraction;
		totalFraction += submaterial.fFraction;
	}

	if ( DoubleIsNearlyZero( totalFraction ) )
	{
		outDensity = 0.0;
	}
	else
	{
		outDensity = totalDensity / totalFraction;
	}

	return true;
}

static bool CalculateCompoundPhysicalProperty(const VWMaterialObj& materialObj, EMaterialPhysicalProperty property, double& outValue)
{
	...
	for ( auto& submaterial : submaterials )
	{
		double submaterialValue;
		if ( !submaterial.fObject.GetAveragePhysicalProperty( property, submaterialValue ) )
		{
			return false;
		}

		totalValue += submaterialValue * submaterial.fFraction;
		totalFraction += submaterial.fFraction;
	}
	...
	{
		outValue = totalValue / totalFraction;
	}

	return true;
}

bool VWMaterialObj::GetAverageDensity( double& outDensity )
{
	bool ok = false;

	if ( !this->IsSimpleMaterial() )
	{
		ok = CalculateCompoundDensity( *this, outDensity );
	}
	else if ( this->GetOvUsesDensity() )
	{
		outDensity = this->GetOvDensity();
		ok = true;
	}
...
TXString VWMaterialObj::GetOvDescription()
{
    ...
    TXString data;
    return var.GetTXString(data) ? data : TXString();
}
...
bool VWMaterialObj::GetPhysicalProperty(EMaterialPhysicalProperty property, double& outValue)
{
	return IExtendedPropsSupportPtr( IID_ExtendedPropsSupport )->GetMaterialPhysicalProperty( fhObject, property, outValue );
}

bool VWMaterialObj::GetAveragePhysicalProperty(EMaterialPhysicalProperty property, double& outValue)
{
    if ( this->IsSimpleMaterial() )
    {
        return this->GetPhysicalProperty( property, outValue );
    }
    else
    {
        return ::CalculateCompoundPhysicalProperty( *this, property, outValue );
    }
}

void VWMaterialObj::SetPhysicalProperty(EMaterialPhysicalProperty property, double value, bool updateUse)
{
	switch ( property )
	{
		case EMaterialPhysicalProperty::SpecificGravity:		this->SetOvSpecificGravity( value, updateUse ); break;
		case EMaterialPhysicalProperty::ModulusOfElasticity:	this->SetOvModulusOfElasticity( value, updateUse ); break;
		case EMaterialPhysicalProperty::YieldStrength:			this->SetOvYieldStrength( value, updateUse ); break;
		case EMaterialPhysicalProperty::TensileStrength:		this->SetOvTensileStrength( value, updateUse ); break;
		case EMaterialPhysicalProperty::SpecificHeat:			this->SetOvSpecificHeat( value, updateUse ); break;
		case EMaterialPhysicalProperty::Emissivity:				this->SetOvEmissivity( value, updateUse ); break;
		case EMaterialPhysicalProperty::Albedo:					this->SetOvAlbedo( value, updateUse ); break;
		case EMaterialPhysicalProperty::ThermalExpCoeff:		this->SetOvThermalExpCoeff( value, updateUse ); break;
		case EMaterialPhysicalProperty::Lambda:					this->SetOvLambda( value, updateUse ); break;
		case EMaterialPhysicalProperty::EmbodiedCarbon:			this->SetOvEmbodiedCarbon( value, updateUse ); break;
		case EMaterialPhysicalProperty::SoundVelocity:			this->SetOvSoundVelocity( value, updateUse ); break;
		case EMaterialPhysicalProperty::Density:				this->SetOvDensity( value, updateUse ); break;
		case EMaterialPhysicalProperty::AcousticImpedance:		this->SetOvAcousticImpedance( value, updateUse ); break;
		case EMaterialPhysicalProperty::SlipResistance:			this->SetOvSlipResistance( value, updateUse ); break;
	}
}
...
VWFC::VWObjects::EEmbodiedCarbonUnitType VWMaterialObj::GetOvEmbodiedCarbonUnitType()
{
    VWFC::VWObjects::EEmbodiedCarbonUnitType  data;
	auto extProps = IExtendedPropsSupportPtr(IID_ExtendedPropsSupport);
	if (extProps == nullptr)
	{
		return VWFC::VWObjects::EEmbodiedCarbonUnitType::CO2ePerWeight;
	}
    bool gotValue = extProps->GetEmbodiedCarbonUnitType(fhObject, data);
...
void VWMaterialObj::SetOvEmbodiedCarbonUnitType(VWFC::VWObjects::EEmbodiedCarbonUnitType embodiedCarbon)
{
    auto extProps = IExtendedPropsSupportPtr(IID_ExtendedPropsSupport);
    if (extProps != nullptr)
        extProps->SetEmbodiedCarbonUnitType(fhObject, embodiedCarbon);
}

bool VWMaterialObj::GetOvEmbodiedCarbonManufFlag()
{
	TVariableBlock var;
	gSDK->GetObjectVariable(fhObject, ovMaterialEmbodiedCarbonManufFlag, var);
...
void VWMaterialObj::SetOvEmbodiedCarbonManufFlag(bool embodiedCarbonManufFlag)
{
	TVariableBlock var(embodiedCarbonManufFlag);
	gSDK->SetObjectVariable(fhObject, ovMaterialEmbodiedCarbonManufFlag, var);
}
...
bool VWMaterialObj::GetOvUsesDensity()
{
	...
	bool data;
    return var.GetBoolean(data) ? data : false;  // default value: false
}
...
bool VWMaterialObj::IsMaterialObject(MCObjectConstHandle hObject)
{
