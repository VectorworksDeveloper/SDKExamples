...
// Variables originally defined in EditShaderDialog to save tag data for metallic reflectivity shader
const OSType kReflectivityMetallicOptionsNodeID = 'rmop';
#define FLOAT_TO_PERCENT( x ) ( (Sint32)floor( ( (x) * 100 ) + 0.5 ) )
#define FLOATPERCENT_TO_FLOAT( x ) ( (double)(x) / (double)100 )

enum {
	kReflectivityMetallicBooleanTag = 1,
	kReflectivityMetallicIntTag = 2,
	kReflectivityMetallicDoubleTag = 3
};

enum ReflectivityMetallicParamIntTags
{
	kNumReflectivityMetallicParamsInt = 1,

	kPatternID = 0
};

enum ReflectivityMetallicParamDoubleTags
{
	kNumReflectivityMetallicParamsDouble = 14,
	...
	kSize2ID = 8,
	kLength2ID = 9,
	kMetalReflectionID = 10,
	kMetalRoughnessID = 11,
	kClearCoatReflectionID = 12,
	kClearCoatRoughnessID = 13,
};

class ReflectivityMetallicParameters
{
	public:
		ReflectivityMetallicParameters();
		void Set(MCObjectHandle materialHandle, double val, ReflectivityMetallicParamDoubleTags param);
		double Get(MCObjectHandle materialHandle, ReflectivityMetallicParamDoubleTags param);
	private:
		short       fMetalPresets;
		RGBColor    fColor;
		double      fReflection;
		double      fBlurriness;
		double      fMetalReflection;
		double      fMetalRoughness;
		double      fClearCoatReflection;
		double      fClearCoatRoughness;
		short       fPattern;
		Boolean     fIsBrushed;
		double      fScale;
		double      fMetalOrientation;
		double      fOffsetU;
		double      fOffsetV;
		Boolean     fHasPrimaryScratches;
		double      fDepth1;
		double      fSize1;
		double      fLength1;
		Boolean     fHasSecondaryScratches;
		double      fDepth2;
		double      fSize2;
		double      fLength2;

		void RetrieveTagData(MCObjectHandle materialHandle);
		void SaveTagData(MCObjectHandle materialHandle, ReflectivityMetallicParamDoubleTags param);
};

ReflectivityMetallicParameters::ReflectivityMetallicParameters()
{
	fMetalPresets = 0;
	...
	// converted to float when save to shader record
	fReflection = 80;//0.8f;
	fBlurriness = 50;//0.5f;

	fMetalReflection = 80;//0.8f;
	fMetalRoughness = 40;//0.4f;

	fClearCoatReflection = 0;//0.0f;
	fClearCoatRoughness = 0;//.0;

	fIsBrushed = true;
	fPattern = 0;
	fScale = 100;//1.0f;
	fMetalOrientation = 90.0;
	fOffsetU = 0;//0.0f;
	fOffsetV = 0;//0.0f;
	fHasPrimaryScratches = true;
	fDepth1 = 100;//1.0f;
	fSize1 = 7;//0.07f;
	fLength1 = 1000;//10.0f;
	fHasSecondaryScratches = false;
	fDepth2 = 100;//1.0f;
	fSize2 = 7;//0.07f;
	fLength2 = 1000;//10.0f;
}

double ReflectivityMetallicParameters::Get(MCObjectHandle materialHandle, ReflectivityMetallicParamDoubleTags param)
{
	switch (param)
	{
		case kScaleID:
		...
		case kLength2ID:
			return fLength2;
		case kMetalReflectionID:
			return fMetalReflection;
		case kMetalRoughnessID:
			return fMetalRoughness;
		case kClearCoatReflectionID:
			return fClearCoatReflection;
		case kClearCoatRoughnessID:
			return fClearCoatRoughness;
		default:
			return 0.0;
	};
...
void ReflectivityMetallicParameters::Set(MCObjectHandle materialHandle, double val, ReflectivityMetallicParamDoubleTags param)
{
	//recover saved tag data
	RetrieveTagData(materialHandle);
	...
	//write the values back to the tag data
	SaveTagData(materialHandle, param);
}

// Function Get: if tag exists it reads the values that are modified by VWTextureShaderReflectivityAnisotropic
// all other values are set to default
void ReflectivityMetallicParameters::RetrieveTagData(MCObjectHandle materialHandle)
{
	Sint32 numDoubles = 0;
	gSDK->TaggedDataGetNumElements((MCObjectHandle)materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, &numDoubles);
	if (numDoubles == kNumReflectivityMetallicParamsDouble)
	{
		double* metalReflection = nullptr;
		double* metalRoughtness = nullptr;
		Boolean success = gSDK->TaggedDataGet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kMetalReflectionID, &metalReflection);
		if (success)
			fMetalReflection = FLOAT_TO_PERCENT(*metalReflection);
		success = gSDK->TaggedDataGet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kMetalRoughnessID, &metalRoughtness);
		if (success)
			fMetalRoughness = FLOAT_TO_PERCENT(*metalRoughtness);
	}
}

// Function SaveParamsToTag
// This function saves the values of the metallic reflectivity shader parameters to the render style tag data
// When tag is created it saves all the values of type double to prevent reading incorrect data
// when updating the value we only add the variables that are modified by VWTextureShaderReflectivityAnisotropic
void ReflectivityMetallicParameters::SaveTagData(MCObjectHandle materialHandle, ReflectivityMetallicParamDoubleTags param)
{
	...
	switch (param)
	{
		case kMetalReflectionID:
		{
			percent = FLOATPERCENT_TO_FLOAT(fMetalReflection);
			break;
		}
		case kMetalRoughnessID:
		{
			percent = FLOATPERCENT_TO_FLOAT(fMetalRoughness);
			break;
		}
		default:
			// this is wrong must return to prevent saving incorrect data
			return;
	};
	
	success = gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, param, &percent);
	
	if (!success)
	{
		gSDK->TaggedDataCreate(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kNumReflectivityMetallicParamsDouble);
		//Save all the doubles to pevent reading incorrect data
		percent = FLOATPERCENT_TO_FLOAT(fScale);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kScaleID, &percent);
		percent = fMetalOrientation;
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kOrientationID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fOffsetU);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kOffsetUID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fOffsetV);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kOffsetVID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fDepth1);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kDepth1ID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fSize1);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kSize1ID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fLength1);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kLength1ID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fDepth2);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kDepth2ID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fSize2);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kSize2ID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fLength2);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kLength2ID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fMetalReflection);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kMetalReflectionID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fMetalRoughness);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kMetalRoughnessID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fClearCoatReflection);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kClearCoatReflectionID, &percent);
		percent = FLOATPERCENT_TO_FLOAT(fClearCoatRoughness);
		gSDK->TaggedDataSet(materialHandle, kReflectivityMetallicOptionsNodeID, kTaggedDataDoubleArrayTypeID, kReflectivityMetallicDoubleTag, kClearCoatRoughnessID, &percent);
	}
}
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityImage::IsShaderRecordObject( hShaderRecord ) ) )
	{
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityMirror::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityFresnel::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityNoise::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityTiles::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityPlastic::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityBricks::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityPavement::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityGlow::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityBacklit::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	if ( VWFC_VERIFY( VWTextureShaderReflectivityAnisotropic::IsShaderRecordObject( hShaderRecord ) ) )
	{
		fhObject	= hShaderRecord;
	}
	else
...
{
	ReflectivityMetallicParameters flaggedParams;
	flaggedParams.Set(fhObject, value, kMetalReflectionID);
	this->SetShaderRecordDouble( 8, value );
}
...
{
	ReflectivityMetallicParameters flaggedParams;
	flaggedParams.Set(fhObject, value, kMetalRoughnessID);
	this->SetShaderRecordDouble( 9, value );
}
