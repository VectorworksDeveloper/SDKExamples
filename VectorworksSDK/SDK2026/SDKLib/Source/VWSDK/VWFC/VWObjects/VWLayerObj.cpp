...
: VWGroupObj( 0 )	// call nothing doing constructor -- we dont want a new group created
{
	if ( VWFC_VERIFY( VWLayerObj::IsLayerObject( hLayer ) ) )
	{
		fhObject	= hLayer;
	}
	else
...
bool VWLayerObj::IsLayerObject(MCObjectConstHandle hObj)
{
...
{
	VWFC_ASSERT( fhObject );
	TVariableBlock		value( DemoteTo<Sint16>( kTDimov, ELayerVisibility::kLayerVisibilityInvisible ) );
