...
{
	if ( VWFC_VERIFY( VWTextBlockObj::IsTextBlockObject( hTextBlock ) ) )
	{
		fhObject	= hTextBlock;
	}
	else
...
bool VWTextBlockObj::IsTextBlockObject(MCObjectConstHandle hObj)
{
...
bool VWTextBlockObj::InsertText(const TXString& strAdd, size_t beforeIndex /*= 0*/)
{
	...
	if ( textLen == 0 ) {
		...
		if ( strLen > 0 ) {
			...
			// create a new text block, place it before fhObject (preserving its current container)
			MCObjectHandle	hObject = ::GS_CreateTextBlock( gCBP, strAdd, fOriginPt, false, 0 );
			::GS_InsertObjectBefore( gCBP, hObject, fhObject );

			// delete current text block, reassign fhObject
...
bool VWTextBlockObj::SetTightFill(bool tightFill) const
{
	return gSDK->SetObjectVariable( fhObject, ovTextTightFill, TVariableBlock( tightFill ) );
}
...
bool VWTextBlockObj::FormatAsSIA(const TXString& centimeters, const TXString& millimeters, double superscriptScalePercent /*= 100*/)
{
	return gSDK->FormatTextObjectInSIA( fhObject, centimeters, millimeters, superscriptScalePercent );
}

bool VWTextBlockObj::FormatAsSIA(WorldCoord value, bool alwaysMeters, double superscriptScalePercent /*= 100*/)
{
	TXString	centimeters;
	TXString	millimeters;
	return gSDK->CoordToSIAString( value, alwaysMeters, centimeters, millimeters ) && gSDK->FormatTextObjectInSIA( fhObject, centimeters, millimeters, superscriptScalePercent );
}

bool VWTextBlockObj::FormatSegmentAsSIA(size_t startIndex, size_t countChars, double superscriptScalePercent /*= 100*/)
{
	return gSDK->FormatTextObjectSegmentInSIA( fhObject, startIndex, countChars, superscriptScalePercent );
}
