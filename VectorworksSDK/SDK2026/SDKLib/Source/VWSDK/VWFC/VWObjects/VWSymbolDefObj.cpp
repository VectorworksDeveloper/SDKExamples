...
: VWGroupObj( 0 )	// call nothing doing constructor -- we dont want a new group created
{
	if ( VWFC_VERIFY( VWSymbolDefObj::IsSymbolDefObject( hSymbolDef ) ) )
	{
		fhObject	= hSymbolDef;
	}
	else
...
bool VWSymbolDefObj::IsSymbolDefObject(MCObjectConstHandle hObj)
{
...
bool VWSymbolDefObj::CanInsertIntoWalls() const
{
	TVariableBlock	value;
	VERIFYN( kVStanev, ::GS_GetObjectVariable( gCBP, fhObject, ovSymDefInsertIntoWalls, value ) );
...
bool VWSymbolDefObj::GetPageBased() const
{
	TVariableBlock		value;
	VERIFYN( kVStanev, ::GS_GetObjectVariable( gCBP, fhObject, ovSymDefPageBased, value ) );
...
void VWSymbolDefObj::SetPageBased(bool bPageBased)
{
	TVariableBlock		value( (Boolean) bPageBased );
	VERIFYN( kVStanev, ::GS_SetObjectVariable( gCBP, fhObject, ovSymDefPageBased, value ) );
}
