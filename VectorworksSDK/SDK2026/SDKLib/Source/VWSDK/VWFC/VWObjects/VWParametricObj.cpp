...
, fhParamRecord( nullptr )
, fhParamFormat( nullptr )
{
	if VWFC_VERIFY( VWParametricObj::IsParametricObject( hParametric ) )
	{
...
VWParametricObj::VWParametricObj(const TXString& parametricName, const VWPoint3D& location )
	: VWGroupObj( 0 )	// call nothing doing constructor -- we dont want a new group created
{
	this->CreateParametricObject( parametricName, VWTransformMatrix( location ) );
}

VWParametricObj::VWParametricObj( const TXString& parametricName, const VWTransformMatrix& matrix )
	: VWGroupObj( 0 )	// call nothing doing constructor -- we dont want a new group created
{
	this->CreateParametricObject( parametricName, matrix );
}
...
bool VWParametricObj::IsParametricObject(MCObjectConstHandle hObj)
{
...
/*static*/ bool VWParametricObj::IsParametricObject(MCObjectHandle hObj, const TXString& name)
{
	...
	TInternalID officialID = 0;
	TInternalID pioID = GetInternalID(hObj);
	gSDK->Kludge(7105/*kKludgeGetInternalIDByName*/, (void*)&name, &officialID);

	if (pioID != 0 || officialID != 0) {
		// Fast handling of known plugins. These should be calling IsParametricObject with an ID, not just a name.
		match = (pioID == officialID);
	}
	else {
		// slow lookup of format's name for third party plugins
		bool	bFound	= false;
...
/*static*/ bool VWParametricObj::IsPathObject(MCObjectHandle hObj)
{
	return IsParametricObject( hObj ) && ::GS_GetCustomObjectPath( gCBP, hObj );
}
...
void VWParametricObj::SetParametricHandle(MCObjectHandle hParametric)
{
	...
	while( hRecord && ! bFound ){
		hFormat = ::GS_GetDefinition( gCBP, (MCObjectHandle) hRecord );
		if (hFormat && VERIFYN(kVictor, gSDK->GetObjectTypeN(hFormat) == kFormatNode)) {
			TFormatHandler		currFormatHandler( hFormat );
			if ( currFormatHandler.IsParametric() ) {
				// If we find a recordNode on the aux list of the kParametricNode, and its formatNode has the parametric bit set,
				// then we can assume it is "the one".  No name checking is necessary.  JAK 9/27/99 Fix for VW 8.5.0.
...
void VWParametricObj::CreateParametricObject(const TXString& parametricName, const VWTransformMatrix& matrix)
{
	::GS_DefineCustomObject( gCBP, parametricName, kCustomObjectPrefNever );
	MCObjectHandle hParametric = ::GS_CreateCustomObjectByMatrix( gCBP, parametricName, matrix );
	VWFC_ASSERT( hParametric );
	//	if ( hParametric == NULL )
	//		THROW_VWFC_EXCEPTION( VWCannotCreateObjectException, 0, "cannot create parametric" );
	this->SetParametricHandle( hParametric );
...
bool VWParametricObj::GetIsGimbalLocked() const
{
	return gSDK->GetIsObjectGimbalLocked( fhObject );
}

void VWParametricObj::SetIsGimbalLocked( bool gimbalLocked )
{
	gSDK->SetIsObjectGimbalLocked( fhObject, gimbalLocked );
}
...
void VWParametricObj::SetStyle(MCObjectHandle hStyle) const
{
	if(fhObject != nullptr)
	{
		if(hStyle != nullptr && gSDK->IsPluginStyle( hStyle ))
		{
			RefNumber styleRefNumber = gSDK->GetObjectInternalIndex( hStyle );
			if(styleRefNumber > 0)
			{
				gSDK->SetPluginObjectStyle( fhObject, styleRefNumber );
			}
...
bool VWParametricObj::CustomControlPointsCreate(size_t numPoints, TCustomControlPointsInitCallback initProc /*= nullptr*/)
{
	const auto	adaptor	= [] (CallBackPtr cbp, Sint32 inIndex, WorldPt3& outPt, Boolean& outIs2DOnly, Boolean& outIsHidden, short& outClientID, void* env)
	{
		return ( *(TCustomControlPointsInitCallback*) env )( inIndex, outPt, outIs2DOnly, outIsHidden, outClientID );
	};

	return initProc
			? gSDK->CustomObjectControlPtsCreate( fhObject, DemoteTo<Sint32>( kTDimov, numPoints ), adaptor, &initProc )
			: gSDK->CustomObjectControlPtsCreate( fhObject, DemoteTo<Sint32>( kTDimov, numPoints ), nullptr, nullptr );
}

bool VWParametricObj::CustomControlPointsRemove()
{
	return gSDK->CustomObjectControlPtsRemove( fhObject );
}

bool VWParametricObj::CustomControlPointsSet(const VWPoint3D& location, bool onlyIn2D, bool visible, size_t pointID, size_t pointIndex /*= -1*/)
{
	const Sint32	ID	= DemoteTo<Sint32>( kTDimov, pointID );
	return gSDK->CustomObjectControlPtSet( fhObject, pointIndex == -1 ? ID - 1 : DemoteTo<Sint32>( kTDimov, pointIndex ), location, onlyIn2D, !visible, ID );
}

bool VWParametricObj::CustomControlPointsGet(size_t pointIndex, VWPoint3D& outLocation, bool* outOnlyIn2D /*= nullptr*/, bool* outVisible /*= nullptr*/, Sint32* outPointID /*= nullptr*/) const
{
	WorldPt3	tempPt;
	Boolean		temp2DOnly;
	Boolean		tempHidden;
	...
	const bool	success	= gSDK->CustomObjectControlPtGet( fhObject, DemoteTo<Sint32>( kTDimov, pointIndex ), tempPt, &temp2DOnly, &tempHidden, &tempClientID );

	if ( success )
	{
							outLocation		= tempPt;
		if ( outOnlyIn2D )	*outOnlyIn2D	= temp2DOnly;
		if ( outVisible )	*outVisible		= !tempHidden;
		if ( outPointID )	*outPointID		= tempClientID;
	}
