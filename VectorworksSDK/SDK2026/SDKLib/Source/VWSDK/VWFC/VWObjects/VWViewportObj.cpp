...
{
	if ( VWFC_VERIFY( VWViewportObj::IsViewportObject( hViewport ) || VWViewportObj::IsDesignLayerViewportObject( hViewport ) ) )
	{
		fhObject = hViewport;
	}
	else
...
/*static*/ bool VWViewportObj::IsViewportObject(MCObjectConstHandle hObj)
{
...
//----------------------------------------------------------------------------
bool VWViewportObj::SetDataVizSchemes(const UuidStorageList& inSchemes)
{
	
	TXString strSchemes;
	for (size_t i = 0; i < inSchemes.size(); i++) {
		strSchemes << inSchemes[i].ToString() << " ";
	}
	
	TVariableBlock varBlock(strSchemes);
	return gSDK->SetObjectVariable(fhObject, ovViewportDataVizSchemes, varBlock);
}

//----------------------------------------------------------------------------
bool VWViewportObj::GetDataVizSchemes(UuidStorageList& outSchemes) const
{
	bool outbSuccess = false;
	TVariableBlock varBlock;
	TXString strSchemes;

	if (gSDK->GetObjectVariable(fhObject, ovViewportDataVizSchemes, varBlock)) {
		varBlock.GetTXString(strSchemes);
		
		TXStringSTLArray schemeTokens;
		TXStringExt::Tokenize( strSchemes, schemeTokens, " ");
		std::copy(schemeTokens.begin(), schemeTokens.end(), std::back_inserter(outSchemes));
...
//----------------------------------------------------------------------------
TXString VWViewportObj::GetActivePhaseName() const
{
	SPhaseData vpActivePhaseData = gSDK->GetViewportActivePhase(fhObject);
	return vpActivePhaseData.fName;
}
