...
template<class T> T EnsureSwappedBytes(const T& value, VWDataSerializerContext& context, EDataSerializerSwapContext callContext)
{
	...
	else if ( callContext == eDataSerializerSwapContext_Write )
	{
...
void VWDataSerializerValue::SetValue(const TXString& str)
{
	...
	if ( m_context.fSaveStringsInMacEncoding )
	{
		...
		m_nBytesCnt		= (len + 1) * sizeof(char);	// 1 extra char reserved for the 0-term
		m_pBytes		= SDK_NEW Uint8[ m_nBytesCnt ];
...
TXString VWDataSerializerValue::GetDefaultStringRepresentation() const // used for the error cases
{
	...
	else if ( m_valueAsStringType == EValueAsStringType::Real )
	{
		...
		{
			result.Format( "%0.14f", m_valueAsString.fReal );
		}
	}
	else if ( m_valueAsStringType == EValueAsStringType::String )
	{
		...
		{
			if (m_nBytesCnt >= 4 && ((UCChar*)m_pBytes)[0] == 0xFEFF)
				result = TXString( ((const UCChar*)m_pBytes) + 1, m_nBytesCnt / sizeof(UCChar) - 2 );
			else
...
VWDataSerializerValue::operator double() const
{
	...
	{
		if (m_valueAsStringType != EValueAsStringType::Int) // Used to default missing values, do not assert
...
VWDataSerializerValue::operator TXString() const
{
	...
	else if (m_nBytesCnt >= 4 && ((UCChar*)m_pBytes)[0] == 0xFEFF)
	{
		return TXString( ((const UCChar*)m_pBytes) + 1, m_nBytesCnt / sizeof(UCChar) - 2 );
	}
	else
	{
...
Sint32 VWDataSerializerValue::GetSIntValue() const
{
	...
	else
	{
		if ( m_arrEnumValues.size() > 0 )
		{
			...
			TXString	strValue;
			if (m_nBytesCnt >= 4 && ((UCChar*)m_pBytes)[0] == 0xFEFF)
				strValue= TXString( ((const UCChar*)m_pBytes) + 1, m_nBytesCnt / sizeof(UCChar) - 2 );
			else
...
bool VWDataSerializerGroup::IsGroupIdentifier(TDataIdentifier id, const char*)
{
...
void VWDataSerializer::SaveDataInObjectHandle(MCObjectHandle userDataNodeHandle, const TXString& changeName /*= NULLSTR*/, bool bTrackChange /*= true*/) const
{
	if (VERIFYN(kTolu, userDataNodeHandle && gSDK->GetObjectTypeN(userDataNodeHandle) == kUserDataNode)) {
		CWriteFromMemoryContext context(m_pMainGroup);
		if (bTrackChange)
			gSDK->TrackSwapChange(userDataNodeHandle, changeName);
		this->SaveDataInMemoryReq(&context, (Uint8*)(*userDataNodeHandle) + kUserDataTypeSize);
	}
}

void VWDataSerializer::SaveDataInObjectHandle(MCObjectHandle userDataNodeHandle, const std::vector<TXString>& changeNames, bool bTrackChange /*= true*/) const
{
	if (VERIFYN(kTolu, userDataNodeHandle && gSDK->GetObjectTypeN(userDataNodeHandle) == kUserDataNode)) {
		CWriteFromMemoryContext context(m_pMainGroup);
		if (bTrackChange) {
			for (const TXString& changeName : changeNames) {
				gSDK->TrackSwapChange(userDataNodeHandle, changeName);
			}
		}
...
void VWDataSerializer::SaveDataInObjectHandle(MCObjectHandle userDataNodeHandle, IWriteContext* writeContext, const TXString& changeName /*= NULLSTR*/, bool bTrackChange /*= true*/) const
{
	if (VERIFYN(kTolu, userDataNodeHandle && gSDK->GetObjectTypeN(userDataNodeHandle) == kUserDataNode)) {
		if (bTrackChange)
			gSDK->TrackSwapChange(userDataNodeHandle, changeName);
		this->SaveDataInMemoryReq(writeContext, (Uint8*)(*userDataNodeHandle) + kUserDataTypeSize);
	}
}

// This method calls SaveDataInMemoryReq with NO CTS TRACKING.  This should only be used in cases when
// data is not saved to a UserData node or file translation.  Otherwise SaveDataInObjectHandle() should be
// used.
void VWDataSerializer::UntrackedSaveDataInMemory(void* pData) const
{
	CWriteFromMemoryContext context( m_pMainGroup );
	...
	if ( rawFile && VCOM_SUCCEEDED( rawFile->Open( fileID, false, true, false, true ) ) )
	{
		...
		if ( fileSize > 0 )
		{
			try
			{
				...
				this->UntrackedSaveDataInMemory( fileBuffer );
