...
#include "Objs.UserDataType.Tags.h"
...
// kPluginCatalogListDataTag = 'CTLG'


// ------------------------------------------------------------------------------------------------
VWStyleSupport::CLockForEdit::CLockForEdit(VWStyleSupport& styleSupport, bool mustVerifySuccess /*= false*/)
	: fStyleSupport( styleSupport )
	, fEditSuccess( !mustVerifySuccess )
{
	fStyleSupport.fLockedForEdit = true;
}

VWStyleSupport::CLockForEdit::~CLockForEdit()
{
	fStyleSupport.fLockedForEdit = false;

	if ( fEditSuccess )
	{
		fStyleSupport.UpdateSymbolDefinition();
	}
}
...
VWStyleSupport::VWStyleSupport(MCObjectHandle hObject)
{
	if ( VWSymbolDefObj::IsSymbolDefObject( hObject ) )
	{
		this->InitFromSymbolDefinition( hObject );
	}
	else if ( VWParametricObj::IsParametricObject( hObject ) )
	{
		this->CreateUnstyledMap( hObject );
	}
	else
	{
		DSTOP( ( kTDimov, "Invalid object type for style support construction!" ) );
	}
}
...
bool VWStyleSupport::InitFromSymbolDefinition( MCObjectHandle hObject )
{
	...
	if ( bReturn ) {
		...
		VWDataSerializerSimple ser( fhHostSymbolDefiniiton, kPluginStyleMapDataTag);
		ser.Load();
...
void VWStyleSupport::LoadEditList()
{
	...
	VWDataSerializerSimple serEditList( fhHostSymbolDefiniiton, kPluginStyleEditListDataTag );
...
bool VWStyleSupport::AddDisplayNamesToEditList(const TEditListDataArray& arrEditListData)
{
	bool result = this->GetStyleRefNumber() != 0;
	if ( result )
	{
		std::for_each( arrEditListData.begin(), arrEditListData.end(), [&] ( const SEditListData& data )
		{
			result = result && this->AddItemToEditList( data.fParamName, kPluginSytleEditList_NewDisplayName, data.fDisplayName );
		} );
	}
...
void VWStyleSupport::UpdateParamsByStyle(MCObjectHandle hObject, bool updateAllParams /*= false*/, bool usedUndo /*= true*/)
{
	if ( hObject )
	{
		VWParametricObj			styleParamObj( this->GetStyleSourceObject() );
		CGenericParamProvider	destObject( hObject );

		if ( usedUndo )
		{
			gSDK->AddBothSwapObject( hObject );
		}

		for ( size_t i = 0, numParams = styleParamObj.GetParamsCount(); i < numParams; i++ )
		{
			const TXString& paramName = styleParamObj.GetParamName( i );

			EPluginStyleParameter styleType;
			this->GetStyleType( paramName, styleType );
			if ( updateAllParams || styleType == kPluginStyleParameter_ByStyle )
			{
				EFieldStyle paramType = styleParamObj.GetParamStyle( i );
				if ( setmember( paramType, kFieldReal, kFieldCoordDisp, kFieldCoordLocX, kFieldCoordLocY, kFieldAngle, kFieldArea, kFieldVolume ) )
				{
					double value = styleParamObj.GetParamReal( i );
					destObject.SetParamReal( paramName, value );
				}
				else
				{
					TXString value = styleParamObj.GetParamAsString( i );
					destObject.SetParamValue( paramName, value );
				}
			}
		}
	}
...
bool VWStyleSupport::SetParametersByStyle(const TXStringSTLArray& parameters, bool isSymbolDefinitionForUpdating /*= true*/)
{
	return this->SetParametersStyleType( parameters, kPluginStyleParameter_ByStyle, isSymbolDefinitionForUpdating );
}

bool VWStyleSupport::SetParametersByInstance(const TXStringSTLArray& parameters, bool isSymbolDefinitionForUpdating /*= true*/)
{
	return this->SetParametersStyleType( parameters, kPluginStyleParameter_ByInstance, isSymbolDefinitionForUpdating );
}

bool VWStyleSupport::SetParametersByInstanceAlways(const TXStringSTLArray& parameters, bool isSymbolDefinitionForUpdating /*= true*/)
{
	return this->SetParametersStyleType( parameters, kPluginStyleParameter_AllwaysByInstance, isSymbolDefinitionForUpdating );
}

bool VWStyleSupport::SetAllParametersByInstanceAlwaysExceptProvided(const TXStringSTLArray& parametersByStyle, bool isSymbolDefinitionForUpdating)
{
	if ( this->GetStyleRefNumber() != 0 )
	{
		// force all by instance always
		for(auto it : fmapStyleInfo)
		{
			it.second.styleType = kPluginStyleParameter_AllwaysByInstance;
		}
	}

	return this->SetParametersByStyle( parametersByStyle, isSymbolDefinitionForUpdating );
}

/*static*/ bool VWStyleSupport::IsParamInStyle(MCObjectHandle hObject)
{
	MCObjectHandle	hParent = gSDK->ParentObject(hObject);

	return hParent && VWSymbolDefObj::IsSymbolDefObject(hParent) && VWSymbolDefObj(hParent).HasPluginStyleSupport();
}

bool VWStyleSupport::IsParamByStyle(const TXString& paramName) const
{
	EPluginStyleParameter styleType;
	this->GetStyleType( paramName, styleType );
	return styleType == kPluginStyleParameter_ByStyle ;
}

bool VWStyleSupport::IsParamByInstance(const TXString& paramName) const
{
	EPluginStyleParameter styleType;
	this->GetStyleType( paramName, styleType );
	return styleType == kPluginStyleParameter_ByInstance;
}

bool VWStyleSupport::HasStyle()
{
	return this->GetStyleRefNumber() != 0;
}

/*static*/ bool VWStyleSupport::CompareWithToolPluginStyle(MCObjectHandle symDefHandle, const TXString& objectName)
{
	RefNumber	refNum;
	return gSDK->GetPluginStyleForTool( objectName, refNum ) && refNum == gSDK->GetObjectInternalIndex( symDefHandle );
}

/*static*/ bool VWStyleSupport::CompareWithObjectPluginStyle(MCObjectHandle symDefHandle, MCObjectHandle hObject)
{
	RefNumber	refNum;
	return gSDK->GetPluginObjectStyle( hObject, refNum ) && refNum == gSDK->GetObjectInternalIndex( symDefHandle );
}

/*static*/ MCObjectHandle VWStyleSupport::GetStylesFolder(const TXString& styleFolderName)
{
	MCObjectHandle	hStyleFolder	= gSDK->GetNamedObject( styleFolderName );

	if ( !hStyleFolder )
	{
		hStyleFolder	= gSDK->CreateSymbolFolder( styleFolderName );
		gSDK->AddAfterSwapObject( hStyleFolder );
	}

	return hStyleFolder;
}

/*static*/ void VWStyleSupport::MoveStyleToFolder(MCObjectHandle hStyle, MCObjectHandle hFolder)
{
	if ( VWSymbolDefObj::IsSymbolDefObject( hStyle ) && hFolder && gSDK->GetObjectTypeN( hFolder ) == kFolderNode && gSDK->ParentObject( hStyle ) != hFolder )
	{
		gSDK->AddBeforeMoveObject( hStyle );
		gSDK->AddObjectToContainer( hStyle, hFolder );
		gSDK->AddAfterMoveObject( hStyle );
	}
}

/*static*/void VWStyleSupport::MoveStyleToFolder(MCObjectHandle hStyle, const TXString& styleFolderName)
{
	VWStyleSupport::MoveStyleToFolder( hStyle, VWStyleSupport::GetStylesFolder( styleFolderName ) );
}

/*static*/ InternalIndex VWStyleSupport::GetClassFromStyle(MCObjectHandle hStyleSym, bool resolveActiveClass /*= true*/)
{
	...
	if ( hStyleSym )
	{
		...
		gSDK->GetObjectVariable( hStyleSym, ovSymDefUseClass, value );

		bool	hasAssocClass;

		result	= value.GetBoolean( hasAssocClass ) && hasAssocClass
			? gSDK->GetObjectClass( hStyleSym )
			: -1;

		if (resolveActiveClass && result == -1)
		{
			result = gSDK->GetDefaultClass();
		}
	}
...
/*static*/ void VWStyleSupport::SetClassByStyle(MCObjectHandle hObject)
{
	if ( VWParametricObj::IsParametricObject( hObject ) )
	{
		const VWParametricObj	paramObj( hObject );
		const MCObjectHandle	hStyle = paramObj.GetStyleHandle();
		if ( hStyle && gSDK->GetClassByStyle( hStyle ) )
		{
			InternalIndex classRefNum = gSDK->GetObjectClass( hStyle );
			if ( VWClass::IsValidClassIndex( classRefNum ) )
			{
				gSDK->SetObjectClass( hObject, classRefNum );
				gSDK->SetClassByStyle( hObject, true );
			}
		}
	}
}

bool VWStyleSupport::SetParametersStyleType(const TXStringSTLArray& parameters, EPluginStyleParameter styleType, bool isSymbolDefinitionForUpdating)
{
	bool	result = this->GetStyleRefNumber() != 0;

	if (result)
	{
		auto setStyleType = [&](const TXString& paramName)
		{
			if ( result && !this->SetStyleType( paramName, styleType ) )
			{
				result = result && VERIFYN( kKYotova, this->AddItem( paramName, styleType ) );
			}
		};

		std::for_each( parameters.begin(), parameters.end(), setStyleType );

		if ( result && isSymbolDefinitionForUpdating )
		{
			this->UpdateSymbolDefinition();
		}
	}
...
bool VWStyleSupport::UpdateSymbolDefinition()
{
	...
	VWDataSerializerSimple serEdit( fhHostSymbolDefiniiton, kPluginStyleEditListDataTag );
...
EPluginStyleParameter VWStyleSupport::GetStyleType(const TXString& key) const
{
	EPluginStyleParameter styleType = kPluginStyleParameter_ByInstance;

	if ( !key.IsEmpty() ) 
	{
		const auto iter = fmapStyleInfo.find( key );
		if ( iter != fmapStyleInfo.end() ) 
		{
			styleType = iter->second.styleType;
		}
...
bool VWStyleSupport::CanEditParameter( const TXString& key ) const
{
	bool bReturn = true;
	EPluginStyleParameter styleType = kPluginStyleParameter_ByInstance;

	bool bFound = this->GetStyleType( key, styleType );
	if ( bFound ) {
		if ( styleType == kPluginStyleGroup_ByStyle ) {
			bReturn = false;
		}
		else if ( this->IsCatalogParam( key ) ) {
			bReturn = false;
		}
	}
...
bool VWStyleSupport::IsCatalogParam( const TXString& key ) const
{
	bool bReturn = false;
	
	if ( key.IsEmpty() == false ) {
		TStyleInfo_ConstIterator iter = fmapStyleInfo.find( key );
		if ( iter != fmapStyleInfo.end() ) {
...
bool VWStyleSupport::AddItem( const TXString& key, EPluginStyleParameter styleType, bool saveStyle /*= true*/ )
{
	if ( fhHostSymbolDefiniiton == NULL ) {
		return false;
	}
...
bool VWStyleSupport::AddItemToEditList( const TXString& key, EPluginStyleEditList editListType, const TXString& displayName )
{
	...
	VWDataSerializerSimple ser( fhHostSymbolDefiniiton, kPluginStyleEditListDataTag );
	ser.Load();
