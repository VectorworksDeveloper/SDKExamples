...
std::vector<std::function<bool(EStandardInterfaceManagerMapState, SStandardInterfaceManagerMapData&)>>& CatchHealthTests()
{
	static std::vector<std::function<bool(EStandardInterfaceManagerMapState, SStandardInterfaceManagerMapData&)>> gCatchHealthTests;
	return gCatchHealthTests;
}
...
std::vector<std::function<void(Sint32 action, void* moduleInfo, const VWIID& iid, IVWUnknown*& inOutInterface, CallBackPtr cbp, Sint32& reply)>>& CatchHealthTests()
{
	static std::vector<std::function<void(Sint32 action, void* moduleInfo, const VWIID& iid, IVWUnknown*& inOutInterface, CallBackPtr cbp, Sint32& reply)>> gCatchHealthTests;
	return gCatchHealthTests;
}
...
{
	gSDK->CloseAllFilesAndQuitVectorworks(false);
}
