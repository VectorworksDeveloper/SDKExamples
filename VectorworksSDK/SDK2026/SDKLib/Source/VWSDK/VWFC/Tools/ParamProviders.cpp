...
// ---------------------------------------------------------------------------------------------------
// Process Color parameter (kFieldClass)
// ---------------------------------------------------------------------------------------------------
void CParamProviderAbstract::ProcessParameterClass( bool bRead, InternalIndex& inOutValue, const InternalIndex& oldValue, const TXString& szParamName )
{
	if ( bRead ) {
		inOutValue = this->GetParamClass( szParamName );
	}
	else if ( oldValue != inOutValue ) {
		this->SetParamClass( szParamName, inOutValue );
	}
	
}

// ---------------------------------------------------------------------------------------------------
// Process Building Material parameter (kFieldBuildingMaterial)
// ---------------------------------------------------------------------------------------------------
void CParamProviderAbstract::ProcessParameterBuildingMaterial( bool bRead, MaterialRefNumber& inOutValue, MaterialRefNumber oldValue, const TXString& szParamName )
{
	if ( bRead ) {
		inOutValue = this->GetParamBuildingMaterial( szParamName );
	}
	else if ( oldValue != inOutValue ) {
		this->SetParamBuildingMaterial( szParamName, inOutValue );
	}
	
}

// ---------------------------------------------------------------------------------------------------
// Process Pen Style parameter (kFieldPenStyle)
// ---------------------------------------------------------------------------------------------------

void CParamProviderAbstract::ProcessParameterPenStyle(bool bRead, VWPattern& inOutValue, const VWPattern& oldValue, const TXString& szParamName)
{
	if (bRead) {
		inOutValue = GetParamPenStyle(szParamName);
	}
	else if (oldValue != inOutValue) {
		this->SetParamPenStyle( szParamName, inOutValue);
	}
}

// ---------------------------------------------------------------------------------------------------
// Process Pen Style parameter (kFieldFill)
// ---------------------------------------------------------------------------------------------------

void CParamProviderAbstract::ProcessParameterFill(bool bRead, VWPattern& inOutValue, const VWPattern& oldValue, const TXString& szParamName)
{
	if (bRead) {
		inOutValue = GetParamFill(szParamName);
	}
	else if (oldValue != inOutValue) {
		this->SetParamFill( szParamName, inOutValue);
	}
}

// ---------------------------------------------------------------------------------------------------
// Process Pen Weight parameter (kFieldPenWeight)
// ---------------------------------------------------------------------------------------------------

void CParamProviderAbstract::ProcessParameterPenWeight(bool bRead, Uint8& inOutValue, const Uint8& oldValue, const TXString& szParamName)
{
	if (bRead) {
		inOutValue = GetParamPenWeight(szParamName);
	}
	else if (oldValue != inOutValue) {
		this->SetParamPenWeight( szParamName, inOutValue);
	}
}

// ---------------------------------------------------------------------------------------------------
// Process Color parameter (kFieldColor)
// ---------------------------------------------------------------------------------------------------
void CParamProviderAbstract::ProcessParameterColor( bool bRead, ColorRef& inOutValue, const ColorRef& oldValue, const TXString& szParamName )
{
	if ( bRead ) {
		inOutValue = this->GetParamColor( szParamName );
	}
	else if ( oldValue != inOutValue ) {
		this->SetParamColor( szParamName, inOutValue );
	}
	
}

// ---------------------------------------------------------------------------------------------------
// Process Color parameter (kFieldTexture)
// ---------------------------------------------------------------------------------------------------
void CParamProviderAbstract::ProcessParameterTexture( bool bRead, InternalIndex& inOutValue, const InternalIndex& oldValue, const TXString& szParamName )
{
	if ( bRead ) {
		inOutValue = this->GetParamTexture( szParamName );
	}
	else if ( oldValue != inOutValue ) {
		this->SetParamTexture( szParamName, inOutValue );
	}
}

// ---------------------------------------------------------------------------------------------------
// Process Color parameter (kFieldSymDef)
// ---------------------------------------------------------------------------------------------------
void CParamProviderAbstract::ProcessParameterSymDef( bool bRead, InternalIndex& inOutValue, const InternalIndex& oldValue, const TXString& szParamName )
{
	if ( bRead ) {
		inOutValue = this->GetParamSymDef( szParamName );
	}
	else if ( oldValue != inOutValue ) {
		this->SetParamSymDef( szParamName, inOutValue );
	}
}
...
{
	EFieldStyle fieldStyle = GetParamStyle( paramIndex );
	switch ( fieldStyle ) {
		case kFieldBuildingMaterial: {
			if ( bRead ) {
				inOutValue = this->GetParamBuildingMaterial( paramIndex );
			}
			else if ( oldValue != inOutValue ) {
				this->SetParamBuildingMaterial( paramIndex, inOutValue );
			}
			break;
		}
		...
		case kFieldTexture: {
			if ( bRead ) {
				inOutValue = this->GetParamTexture( paramIndex );
			}
			else if ( oldValue != inOutValue ) {
				this->SetParamTexture( paramIndex, inOutValue );
			}			
			break;
		}
...
void CGenericParamProvider::Set(MCObjectHandle hGenericObject)
{
	...
	{
		fProvider	= std::make_shared<CObjectParamProvider>( hGenericObject );
	}
	else if ( VWFC::VWObjects::VWRecordFormatObj::IsRecordFormatObject( hGenericObject ) )
	{
		fProvider	= std::make_shared<CFormatParamProvider>( hGenericObject );
	}
	else if ( VWFC::VWObjects::VWRecordObj::IsRecordObject( hGenericObject ) )
	{
		fProvider	= std::make_shared<CRecordParamProvider>( hGenericObject );
	}
...
void CGenericParamProvider::Set(MCObjectHandle hParametricObject, const TXString& szFormatName)
{
	...
	{
		fProvider	= std::make_shared<CObjectParamProvider>( hParametricObject );
	}
	else
	{
		fProvider	= std::make_shared<CFormatParamProvider>( VWRecordFormatObj::GetRecordFormat( szFormatName ) );
	}
	VWFC_ASSERT( fProvider != NULL );
}
...
void CGenericParamProvider::Set(MCObjectHandle hParametricObject, MCObjectHandle hFormat)
{
	...
	{
		fProvider	= std::make_shared<CFormatParamProvider>( hFormat );
	}
	VWFC_ASSERT( fProvider != NULL );
}
...
TNumStyleClass CGenericParamProvider::GetParamNumStyle(const TXString& szParamName) const
{
	if ( fProvider )
		return fProvider->GetParamNumStyle( szParamName );
	else 
		return EFieldStyle::kFieldReal;
}
...
bool CGenericParamProvider::GetParamChoices(const TXString& univParamName, TXStringSTLArray& outArray) const
{
	bool result = false;
	if ( fProvider )
		result = fProvider->GetParamChoices( univParamName, outArray );
	return result;
}

bool CGenericParamProvider::GetParamLocalizedChoices(const TXString& univParamName, TXStringSTLArray& outArray)
{
	bool result = false;
	if ( fProvider )
		result = fProvider->GetParamLocalizedChoices( univParamName, outArray );
	return result;
}
...
bool CGenericParamProvider::GetParamLocalizedChoices(size_t paramIndex, TXStringSTLArray& outArray)
{
	bool result = false;
	if ( fProvider )
		result = fProvider->GetParamLocalizedChoices( paramIndex, outArray );
	return result;
}
...
TNumStyleClass CGenericParamProvider::GetParamNumStyle(size_t paramIndex) const
{
	if ( fProvider )
		return fProvider->GetParamNumStyle( paramIndex );
	else 
		return nsGeneral;
}
...
TXString CGenericParamProvider::PopupGetLocalizedChoice(const TXString& univParamName, const TXString& key)
{
	TXString	result;
...
TNumStyleClass CObjectParamProvider::GetParamNumStyle(const TXString& szParamName) const
{
	return fObj.GetRecordFormat().GetParamNumStyle( szParamName );
}

size_t CObjectParamProvider::GetParamIndex(const TXString& szParamName) const
{
	// use the dedicated VWParametricObj funciton so it caches and works faster than just calling GetParamIndex (a.k.a. GetFieldIndex)
	return fObj.GetParamIndexByUnivName( szParamName );
}
...
bool CObjectParamProvider::GetParamChoices(const TXString& univParamName, TXStringSTLArray& outArray) const
{
	size_t paramIndex = this->GetParamIndex( univParamName );
	return this->GetParamChoices( paramIndex, outArray );
}

bool CObjectParamProvider::GetParamLocalizedChoices(const TXString& univParamName, TXStringSTLArray& outArray)
{
	size_t paramIndex = fObj.GetParamIndexByUnivName( univParamName );
	return fObj.GetParamLocalizedChoices( paramIndex, outArray );
}
...
bool CObjectParamProvider::GetParamLocalizedChoices(size_t paramIndex, TXStringSTLArray& outArray)
{
	return fObj.GetParamLocalizedChoices( paramIndex, outArray );
}
...
TNumStyleClass CObjectParamProvider::GetParamNumStyle(size_t paramIndex) const
{
	return fObj.GetRecordFormat().GetParamNumStyle( paramIndex );
}
...
TXString CObjectParamProvider::PopupGetLocalizedChoice(const TXString& univParamName, const TXString& key)
{
	TXString	result;
...
TNumStyleClass CFormatParamProvider::GetParamNumStyle(const TXString& szParamName) const
{
	return fObj.GetParamNumStyle( szParamName );
}
...
bool CFormatParamProvider::GetParamChoices(const TXString& univParamName, TXStringSTLArray& outArray) const
{
	size_t paramIndex = this->GetParamIndex( univParamName );
	return this->GetParamChoices( paramIndex, outArray );
}

bool CFormatParamProvider::GetParamLocalizedChoices(const TXString& univParamName, TXStringSTLArray& outArray)
{
	std::vector<std::pair<TXString, TXString>>	keyValuePairs;
	fObj.PopupGetChoices( univParamName, keyValuePairs );

	for(const auto& it : keyValuePairs)
	{
		outArray.push_back( it.second );
	}
...
bool CFormatParamProvider::GetParamLocalizedChoices(size_t paramIndex, TXStringSTLArray& outArray)
{
	std::vector<std::pair<TXString, TXString>>	keyValuePairs;
	fObj.PopupGetChoices( paramIndex, keyValuePairs );
...
TNumStyleClass CFormatParamProvider::GetParamNumStyle(size_t paramIndex) const
{
	return fObj.GetParamNumStyle( paramIndex );
}
...
TXString CFormatParamProvider::PopupGetLocalizedChoice(const TXString& univParamName, const TXString& key)
{
	TXString	result;
...
TNumStyleClass CRecordParamProvider::GetParamNumStyle(const TXString& szParamName) const
{
	return fObj.GetParamNumStyle( szParamName );
}
...
bool CRecordParamProvider::GetParamChoices(const TXString& univParamName, TXStringSTLArray& outArray) const
{
	size_t paramIndex = this->GetParamIndex( univParamName );
	return this->GetParamChoices( paramIndex, outArray );
}

bool CRecordParamProvider::GetParamLocalizedChoices(const TXString& univParamName, TXStringSTLArray& outArray)
{
	std::vector<std::pair<TXString, TXString>>	keyValuePairs;
	fObj.PopupGetChoices( univParamName, keyValuePairs );
...
bool CRecordParamProvider::GetParamLocalizedChoices(size_t paramIndex, TXStringSTLArray& outArray)
{
	std::vector<std::pair<TXString, TXString>>	keyValuePairs;
	fObj.PopupGetChoices( paramIndex, keyValuePairs );
...
TNumStyleClass CRecordParamProvider::GetParamNumStyle(size_t paramIndex) const
{
	return fObj.GetParamNumStyle( paramIndex );
}
...
TXString CRecordParamProvider::PopupGetLocalizedChoice(const TXString& univParamName, const TXString& key)
{
	TXString	result;
