...
// Don't use VWPluginLibraryArgument::SetArgString when the argType is kCharDynarrayVarArgType or kCharDynarrayArgType,
// because SetArgString will change argType to kStringArgType, and then when 
// vs_CallDynVSFunction calls vs_CallDynVSFunction_TransferValueToPy(arg, true/*doOnlyReturnParams*/)
// to prepare variables for python support it will skip the param, and setup a crash when the python
// code tries to unpack the PyTuple.
// Remember that kCharDynarrayVarArgType supports >255 chars, so it's preferred over kStringArgType, kStringVarArgType.
{
	VWFC_ASSERT( fpArgument );
	VWFC_ASSERT( fpArgument && (fpArgument->argType == kNullArgType || fpArgument->argType == kStringArgType || fpArgument->argType == kStringVarArgType || fpArgument->argType == kStringInOutArgType) );
	DSTOPIF( fpArgument->argType == kCharDynarrayArgType, ( kVStanev, "Potential crash in python. You should be using SetArgDynArrayChar instead.\n" ));
	DSTOPIF( fpArgument->argType == kCharDynarrayVarArgType, ( kVStanev, "Potential crash in python. You should be using SetArgDynArrayCharVar instead.\n" ));
