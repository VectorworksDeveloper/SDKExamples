...
void CParametricCustomBar::SetFieldEditable(size_t fieldIndex, bool isEditable)
{
	if ( this->IsValid() )
	{
		CustomBarFieldInfo info;
		gSDK->CustomBarGetFieldInfo( fCustomBarID, DemoteTo<short>( kKIvanov, fieldIndex ), info );
		info.fIsEditable = isEditable;
		gSDK->CustomBarSetFieldInfo( fCustomBarID, DemoteTo<short>( kKIvanov, fieldIndex ), info );
	}
}
