...
const TXString&	VWExtensionTool::GetSearchSynonyms()
{
	if (fstrSearchSynonyms.IsEmpty()) {
		TXString universalName = GetUniversalName();
		if (!universalName.IsEmpty() && universalName[0] == '-')
		{
			gSDK->Kludge(4137 /*kKludgeGetToolItemUserFriendlyName*/, &universalName, &universalName);
		}
		
		SetSearchSynonyms(TXResStr("SearchSynonyms", universalName, eAllowEmptyResult));
	}
	return fstrSearchSynonyms;
}
...
void VWExtensionTool::SetSearchSynonyms( const TXString &searchSynonyms )
{
	fstrSearchSynonyms = searchSynonyms;
	
	if (!VERIFYMSG(kAAntonini, fstrSearchSynonyms.GetLength() < 50, "Please keep the search synonyms under 50 characters"))
		fstrSearchSynonyms.Truncate(50);
}
...
void VWTool_EventSink::SetPointsProvider( IEventSinkToolPointsProviderPtr provider )
{
	fpToolPtsProvider = provider;
}

void VWTool_EventSink::SetModeBarProvider(IToolModeBarProvider* modeBarProvider)
{
...
//				break;
		...
		case ToolMessage::kAction_OnGetToolWantsWPConstrainedToView:
			reply = this->HandleOnGetToolWantsWPConstrainedToView();
			break;

#if ENABLE_RIGHT_MOUSE_DOWN_TOOL_MESSAGE
		case ToolMessage::kAction_RightMouseDown:
		{
			this->RightMouseDown();
			reply = this->GetResult();
			break;
...
size_t VWTool_EventSink::GetToolPointsCount() const
{
	...
	if ( !fpToolPtsProvider )
	{
		toolPtsCnt = GS_GetNumToolPts( gCBP );
	}
	else
	{
		toolPtsCnt = fpToolPtsProvider->GetToolPointsCount();
	}
...
void VWTool_EventSink::ClearAllToolPoints()
{
	...
	{
		fpToolPtsProvider->ClearAllToolPoints();
	}
}
...
VWPoint2D VWTool_EventSink::GetToolPt2D(size_t ptIndex, bool includeCurrent) const
{
	...
	{
		pt = fpToolPtsProvider->GetToolPt2D( ptIndex, includeCurrent );
	}
...
VWPoint3D VWTool_EventSink::GetToolPt3D(size_t ptIndex, bool includeCurrent) const
{
	...
	{
		pt = fpToolPtsProvider->GetToolPt3D( ptIndex, includeCurrent );
	}
...
bool VWTool_EventSink::GetToolPtCurren2D(VWPoint2D& outPt) const
{
	...
	if ( !fpToolPtsProvider )
	{
		ok = ::GS_GetToolPtCurrent2D( gCBP, pt );
		outPt = pt;
	}
	else
	{
		ok = fpToolPtsProvider->GetToolPtCurrent2D( outPt );
	}
...
bool VWTool_EventSink::GetToolPtCurren3D(VWPoint3D& outPt) const
{
	...
	if ( !fpToolPtsProvider )
	{
		ok = ::GS_GetToolPtCurrent3D( gCBP, pt ) ? true : false;
		outPt = pt;
	}
	else
	{
		ok = fpToolPtsProvider->GetToolPtCurrent3D( outPt );
	}
...
bool VWTool_EventSink::PopLastToolPt()
{
	...
	if ( !fpToolPtsProvider )
	{
		result = GS_PopLastToolPt( gCBP ) ? true : false;
	}
	else
	{
		result = fpToolPtsProvider->PopLastToolPt();
	}
...
/*static*/ void	VWTool_EventSink::DrawCrosshairHighlight(const WorldPt3& centerPt) 
{
	...
	Boolean	isPlanRotated = false;

	if ( gSDK->GetProgramVariable( varPlanRotated, &isPlanRotated ) )
	{
		if ( isPlanRotated )
		{
			screenCenter.x = centerPt.x;
			screenCenter.y = centerPt.y;
		}
		else
		{
...
Sint32 VWTool_EventSink::HandleOnGetToolWantsWPConstrainedToView() 
{
	this->Default();
...
#if ENABLE_RIGHT_MOUSE_DOWN_TOOL_MESSAGE
void VWTool_EventSink::RightMouseDown()
{
	this->Default();
}
#endif
...
void VWToolDefaultPoint_EventSink::SetToolInfo(const SDefaultPointToolInfo& info)
{
	...
	if ( fVWToolInfo.fInteractiveSizingGroup >= 0 )		fVWToolInfo.fInteractiveSizingGroup++;
	...
	if ( fVWToolInfo.fInteractiveSizingIndex >= 0 )		fVWToolInfo.fInteractiveSizingIndex++; 
	if ( fVWToolInfo.fIntSizingCornerToCornerIndex >= 0 ) fVWToolInfo.fIntSizingCornerToCornerIndex++;
	if ( fVWToolInfo.fIntSizingCenterToCornerIndex >= 0 ) fVWToolInfo.fIntSizingCenterToCornerIndex++;
