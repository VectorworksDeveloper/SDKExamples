...
VWExtensionMenu::VWExtensionMenu(	CallBackPtr cbp,
	...
	, fVersionModified	( menuDef.fVersionModified	 )
	, fVersionRetired	( menuDef.fVersionRetired	 )
...
const TXString& VWExtensionMenu::GetSearchSynonyms()
{
	if (fstrSearchSynonyms.IsEmpty()) {
		TXString universalName = GetUniversalName();
		if (!universalName.IsEmpty() && universalName[0] == '-')
		{
			gSDK->Kludge(4136 /*kKludgeGetMenuItemUserFriendlyName*/, &universalName, &universalName);
		}
...
void VWExtensionMenu::GetNeedsFlags(EMenuEnableFlags& outNeeds, EMenuEnableFlags& outNeedsNot)
{
	outNeeds	= fNeeds;
...
void VWExtensionMenu::GetVersionInfo(Sint16& outCreated, Sint16& outModified, Sint16& outRetired)
{
	...
	outModified	= fVersionModified;
	outRetired	= fVersionRetired;
}
...
void VWExtensionMenu::SetSearchSynonyms(const TXString &synonyms)
{
	fstrSearchSynonyms = synonyms;
...
Sint32 VWMenu_EventSink::Execute(MenuMessage* message)
{
	...
	switch( message->fAction )
	{
		...
		case MenuCheckHiliteMessage::kAction: {
			...
			CCheckedStatusContext context( dynamic_cast<MenuCheckHiliteMessage*>( message ) );
			this->GetItemsCheckedStatus( & context );
			break;
		}
...
void VWMenu_EventSink::GetItemsCheckedStatus(ICheckedStatusContext* context)
{
}
...
// --------------------------------------------------------------------------------------------------------
VWMenu_EventSink::CCheckedStatusContext::CCheckedStatusContext(MenuCheckHiliteMessage* msg)
	: fMessage( msg )
{
}

void VWMenu_EventSink::CCheckedStatusContext::Clear()
{
	if ( fMessage )
		fMessage->fmapItemID2Checked.clear();
}

void VWMenu_EventSink::CCheckedStatusContext::SetChecked(bool value)
{
	if ( fMessage )
	{
		fMessage->fmapItemID2Checked.clear();
		fMessage->fmapItemID2Checked[0] = value;
	}
}

void VWMenu_EventSink::CCheckedStatusContext::SetChecked(Sint16 chunkIndex, bool value)
{
	if ( fMessage )
		fMessage->fmapItemID2Checked[chunkIndex] = value;
}
