...
#include "../../SDKLib/Include/Interfaces/VectorWorks/Extension/IAssemblyPluginSupport.h"
...
void VWExtensionParametric::SetLocalizedParamDef( const TXString& univParamName, const TXString& localizedParamName )
{
	...
	{
		const SResString* paramLocalName = nullptr;
		if ( farrParam2Defs_ )
		{
			for(const SParametricParam2Def* thisParam = farrParam2Defs_; thisParam && !thisParam->fUniversalName.IsEmpty(); ++thisParam)
			{
				...
				{
					paramLocalName = & thisParam->fLocalizedName;
					break;
				}
			}
		}
		...
		if ( paramLocalName )
		{
			auto	resIt		= fResStringChache_.find( *paramLocalName );
			if ( resIt != fResStringChache_.end() )
			{
...
nlohmann::json VWExtensionParametric::GetParamDefault(const TXString& univParamName) const
{
	try
	{
		nlohmann::json jsonParams = fParamDefaults["params"];
		return jsonParams[ univParamName.operator const char * () ];
	}
	catch(const std::exception& ex)
	{
		const char* what = ex.what();
		DSTOP((kVStanev, "VWExtensionParametric: Unexpected parameter defaults JSON error! [err: %s]", what));
		return nlohmann::json();
	}
}

const nlohmann::json& VWExtensionParametric::GetParamDefaults() const
{
	return fParamDefaults;
}
...
const VWExtensionParametric::CParametersProvider::SParamDefaults& VWExtensionParametric::CParametersProvider::GetParamDefaults(size_t paramIndex)
{
	if (fmapParamDefaults.size() == 0 && fParent->fParamsCount )
	{
		// initialize the param defaults from the resources
		if ( fParent->fUniversalName.IsEmpty() )
			fParent->fUniversalName		= fParent->GetUniversalName();

		char resourceIdentifier[256];
		snprintf(resourceIdentifier, sizeof(resourceIdentifier), "%s/Defaults/%s.json", DEFAULTVWRIDENTIFIER, fParent->fUniversalName.operator const char * () );

		TXResource	resource( resourceIdentifier );
		if ( resource.GetHandle() )
		{
			const TXString&	jsonStr = resource.operator const TXString & ();
			try
			{
				fParent->fParamDefaults = nlohmann::json::parse( jsonStr.operator const char*() );
				nlohmann::json jsonParams = fParent->fParamDefaults["params"];

				for(size_t paramIndex=0; paramIndex<fParent->fParamsCount; paramIndex++)
				{
					SParamDefaults	paramDef;

					const SParametricParam2Def&	thisParam	= fParent->farrParam2Defs_[ paramIndex ];
					const char* thisParamUniv = thisParam.fUniversalName.operator const char * ();
					if ( jsonParams.contains( thisParamUniv ) )
					{
						try
						{
							nlohmann::json jsonParamDef = jsonParams[thisParamUniv]["default"];
							if ( jsonParamDef.is_string() )
							{
								paramDef.fValueMetric	= jsonParamDef.get<std::string>();
								paramDef.fValueImerial	= paramDef.fValueMetric;
							}
							else
							{
								paramDef.fValueMetric	= jsonParamDef["metric"].get<std::string>();
								paramDef.fValueImerial	= jsonParamDef["imperial"].get<std::string>();
							}
						}
						catch(const std::exception& ex)
						{
							const char* what = ex.what();
							DSTOP((kVStanev, "VWExtensionParametric: Unexpected defaults JSON error for parameter: %s | Err: %s", thisParamUniv, what));
						}
					}

					fmapParamDefaults[paramIndex] = paramDef;
				}
			}
			catch(const std::exception& ex)
			{
				const char* what = ex.what();
				DSTOP((kVStanev, "VWExtensionParametric: Unexpected parameter defaults JSON error! %s", what));

				for(size_t paramIndex=0; paramIndex<fParent->fParamsCount; paramIndex++)
				{
					fmapParamDefaults[paramIndex] = fEmptyDefaults;
				}
			}
		}
		else
		{
			DSTOP((kVStanev, "\"%s\" parametric missing JSON defaults.\nExpecting resource file: %s", fParent->fUniversalName.operator const char * (), resourceIdentifier ));
		}
	}

	return paramIndex < fmapParamDefaults.size() ? fmapParamDefaults[paramIndex] : fEmptyDefaults;
}
...
void VWExtensionParametric::CParametersProvider::GetParamDefValueImperial(size_t paramIndex, IRecordItem* outValue)
{
	if ( VERIFYN( kVStanev, paramIndex < fParent->fParamsCount ) )
	{
		if ( fParent->farrParam2Defs_ )
		{
			const SParamDefaults&	paramDefaults = this->GetParamDefaults( paramIndex );
			TXString theValue = paramDefaults.fValueImerial;
...
void VWExtensionParametric::CParametersProvider::GetParamDefValueMetric(size_t paramIndex, IRecordItem* outValue)
{
	if ( VERIFYN( kVStanev, paramIndex < fParent->fParamsCount ) )
	{
		...
		{
			const SParamDefaults&	paramDefaults = this->GetParamDefaults( paramIndex );
			TXString theValue = paramDefaults.fValueMetric;
...
void VWExtensionParametric::CParametersProvider::GetParamChoices(size_t paramIndex, TXStringArray& outArrUniversalNames, TXStringArray& outArrDisplayNames)
{
	...
	if ( VERIFYN( kVStanev, paramIndex < fParent->fParamsCount ) )
	{
		...
		{
			short choicesChcID = fParent->farrParamDefs_ ? fParent->farrParamDefs_[ paramIndex ].fChoicesChcID : fParent->farrParam2Defs_[ paramIndex ].fChoicesChcID;

			if ( choicesChcID > 0 )
			{
				TChcCache_::const_iterator	chcIt	= fParent->fParamChcCache_.find( choicesChcID );
				if ( chcIt != fParent->fParamChcCache_.end() )
				{
					const SParametricParamChc*	currChc	= chcIt->second;
					while ( currChc && choicesChcID == currChc->fChcID && !currChc->fUniversalChoice.IsEmpty() )
					{
						...
						outArrDisplayNames.Append( ( VERIFYN( kVStanev, resIt != fParent->fResStringChache_.end() ) ) ? resIt->second : TXString() );
			...
			if ( thisParam.fChoicesChcID > 0 )
			{
				...
				if ( chcIt != fParent->fParamChcCache.end() )
				{
					...
					while ( currChc && thisParam.fChoicesChcID == currChc->fChcID && !currChc->fUniversalChoice.IsEmpty() )
					{
						...
						outArrDisplayNames.Append( ( VERIFYN( kVStanev, resIt != fParent->fResStringChache.end() ) ) ? resIt->second : TXString() );
...
Sint32 VWParametric_EventSink::Execute(ParametricMessage* message)
{
	...
	switch ( message->fAction )
	{
		...
		case ParametricGetQTOValue::kAction : {
			...
			{
				if ( this->OnQTOFunction( msg->fFunction, msg->fOption, msg->farrArguments, msg->fSubpart, msg->fValue ) )
					reply = kObjectEventHandled;
			}
		} break;
		...
		case ParametricStyleMessage::kAction_BeginStyledObjectsUpdate: {
			...
			if ( VERIFYN( kJWarshaw, theMessage ) ) {
				...
				if (fhObject && gSDK->GetObjectProperty(fhObject, kObjXPropAssemblyUnit, isUnitObj) && isUnitObj)
				{
					IAssemblyPluginSupportPtr assemblySupport( IID_AssemblyPluginSupport );
					if ( VERIFYN( kGDixon, assemblySupport))
					{
						assemblySupport->BeginStyleUpdate(theMessage->fSymDefHandle);
					}
				}
			}
		} break;
		...
		case ParametricStyleMessage::kAction_EndStyledObjectsUpdate: {
			...
			if ( VERIFYN( kJWarshaw, theMessage ) ) {
				...
				if (fhObject && gSDK->GetObjectProperty(fhObject, kObjXPropAssemblyUnit, isUnitObj) && isUnitObj)
				{
					...
					if ( VERIFYN( kGDixon, assemblySupport))
					{
						assemblySupport->EndStyleUpdate(theMessage->fSymDefHandle);
					}
				}
			}
		} break;
		...
		case ParametricUpdatePluginStyleObject::kAction : {
				...
				if (fhObject && gSDK->GetObjectProperty(fhObject, kObjXPropAssemblyUnit, isUnitObj) && isUnitObj)
				{
					...
					if ( VERIFYN( kGDixon, assemblySupport))
					{
						assemblySupport->AddUnitObjectToUpdateList(fhObject);
					}
				}
			} break;
		...
		case ParametricIsProfileGroupPageBasedMessage::kAction:
		{
			ParametricIsProfileGroupPageBasedMessage* theMessage	= dynamic_cast<ParametricIsProfileGroupPageBasedMessage*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				theMessage->fIsPageBased	= this->OnIsProfileGroupPageBased( theMessage->fContext );
			}
			break;
		}

		case ParametricAutoDimMessage_GetDisplayCategoryName::kAction:
		{
			ParametricAutoDimMessage_GetDisplayCategoryName* theMessage	= dynamic_cast<ParametricAutoDimMessage_GetDisplayCategoryName*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				if ( this->OnAutoDimMessage_GetDisplayCategoryName( theMessage->foutstrCategory_UniversalName, theMessage->foutstrCategory_LocalizedName) )
					reply = kObjectEventHandled;
				else
					reply = kObjectEventNotHandled;
			}
		...
		case ParametricAutoDimMessage_GetLocalizedTypeName::kAction:
		{
			ParametricAutoDimMessage_GetLocalizedTypeName* theMessage	= dynamic_cast<ParametricAutoDimMessage_GetLocalizedTypeName*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				if ( this->OnAutoDimMessage_GetLocalizedTypeName( theMessage->finstrDimID_UniversalName, theMessage->foutstrDimID_LocalizedName ) )
					reply = kObjectEventHandled;
				else
					reply = kObjectEventNotHandled;
			}
		...
		case ParametricGetInteractiveSizingInformation::kAction: {
			ParametricGetInteractiveSizingInformation * theMessage = dynamic_cast<ParametricGetInteractiveSizingInformation*>(message);
			if (VERIFYN(kJWarshaw, theMessage)) {
				reply = this->OnGetInteractiveSizingParams(theMessage->fInteractiveSizingInfo /* vector of SInteractiveSizingParameterInfo */);
			}
		} break;
		case ParametricSetInteractiveSizing::kAction: {
			ParametricSetInteractiveSizing * theMessage = dynamic_cast<ParametricSetInteractiveSizing*>(message);
			if (VERIFYN(kJWarshaw, theMessage)) {
				reply = this->OnSetInteractiveSizingParams(theMessage->fInteractiveSizingInfo/* vector of SInteractiveSizingParameterInfo */);
			}
		} break;

		case ParametricAutoDimMessage_GetSupportedTypes::kAction:
		{
			ParametricAutoDimMessage_GetSupportedTypes* theMessage	= dynamic_cast<ParametricAutoDimMessage_GetSupportedTypes*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				if ( this->OnAutoDimMessage_GetSupportedTypes( theMessage->finView, theMessage->foutvecTypes ) )
					reply = kObjectEventHandled;
				else
					reply = kObjectEventNotHandled;
			}
		...
		case ParametricAutoDimMessage_GetDimensionDefinitions::kAction:
		{
			ParametricAutoDimMessage_GetDimensionDefinitions* theMessage	= dynamic_cast<ParametricAutoDimMessage_GetDimensionDefinitions*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				if ( this->OnAutoDimMessage_GetDimensionDefinitions( theMessage->finView, theMessage->finstrDimID_UniversalName, theMessage->finPlace, theMessage->foutvecDimensions ) )
					reply = kObjectEventHandled;
				else
					reply = kObjectEventNotHandled;
			}
		...
		case ParametricPrepareForContextMessage::kAction:
		{
			ParametricPrepareForContextMessage* theMessage	= dynamic_cast<ParametricPrepareForContextMessage*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				if ( this->OnPrepareForContext( theMessage->finContext, theMessage->finView, theMessage->finBgRenderMode, theMessage->finFgRenderMode, theMessage->foutbChanged ) )
					reply = kObjectEventHandled;
				else
					reply = kObjectEventNotHandled;
			}
		...
		case ParametricProvidesClosureGapsMessage::kAction:
		{
			ParametricProvidesClosureGapsMessage *theMessage = dynamic_cast<ParametricProvidesClosureGapsMessage*>( message );
			if ( VERIFYN( kAHiguera, theMessage ) )
			{
				this->OnClosureGapQuery( *theMessage );
				reply = kObjectEventHandled;
			}
			break;
		}

		case ParametricEquipmentItemDataMessage::kAction:
		{
			ParametricEquipmentItemDataMessage* theMessage	= dynamic_cast<ParametricEquipmentItemDataMessage*>( message );
			if ( VERIFYN( kNZhelyazkov, theMessage != nullptr ) )
			{
				this->OnEquipmentItemDataMessage( *theMessage );
			}
			break;
		}

		case ParametricPreModificationMessage::kAction: {
			ParametricPreModificationMessage* theMessage = dynamic_cast<ParametricPreModificationMessage*>(message);
			if (VERIFYN(kJWarshaw, theMessage)) {
				reply = this->OnParamPreModification(theMessage->fModificationID);
			}
		} break;
		case ParametricGetHandingInformation::kAction: {
			ParametricGetHandingInformation* theMessage = dynamic_cast<ParametricGetHandingInformation*>(message);
			if (VERIFYN(kJWarshaw, theMessage)) {
				reply = this->OnParamGetHandingParams(theMessage->fHandingInfo);
			}
		} break;
		case ParametricSetHandingInformation::kAction: {
			ParametricSetHandingInformation* theMessage = dynamic_cast<ParametricSetHandingInformation*>(message);
			if (VERIFYN(kJWarshaw, theMessage)) {
				reply = this->OnParamSetHandingParams(theMessage->fHandingInfo);
			}
		} break;
		case ParametricGetCustomScale::kAction:
		{
			ParametricGetCustomScale* theMessage = dynamic_cast<ParametricGetCustomScale*>(message);
			if (VERIFYN(kIBorisov, theMessage))
			{
				if (this->OnGetCustomScale(theMessage->fScale))
				{
					reply = kObjectEventHandled;
				}
				else
		...
		case ParametricSymbolInsertionMessage::kAction:
		{
			auto* theMessage	= dynamic_cast<ParametricSymbolInsertionMessage*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				reply	= this->OnSymbolInsertion( theMessage->fhSymbol );
			}
			break;
		}

		case ParametricSymbolRemovalMessage::kAction:
		{
			auto* theMessage	= dynamic_cast<ParametricSymbolRemovalMessage*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				reply	= this->OnSymbolRemoval( theMessage->fhSymbol );
			}
			break;
		}

		case ParametricSymbolConstrainMessage::kAction:
		{
			auto* theMessage	= dynamic_cast<ParametricSymbolConstrainMessage*>( message );
			if ( VERIFYN( kTDimov, theMessage ) )
			{
				theMessage->fConstrainedInsertionPoint	= this->OnSymbolMoveConstraint( theMessage->fhSymbol, theMessage->fSelToolCurrInsertionPoint );
				reply	= kObjectEventHandled;
			}
			break;
		}

		case ParametricOverrideRotatedPathElevation::kAction:
		{
			ParametricOverrideRotatedPathElevation* theMessage = dynamic_cast<ParametricOverrideRotatedPathElevation*>(message);
			if ( VERIFYN( kKIvanov, theMessage ) )
			{
				if ( this->OnGetOverrideRotatedPathElevation( theMessage->fElevation ) )
				{
					reply = kObjectEventHandled;
				}
				else
		...
		case ParametricGetContainerHandingInformation::kAction: {
			ParametricGetContainerHandingInformation* theMessage = dynamic_cast<ParametricGetContainerHandingInformation*>(message);
			if (VERIFYN(kHema, theMessage)) {
				reply = this->OnParamGetContainerHandingParams( theMessage->fContainerHandingInfo, theMessage->fInteriorSideHandingParam, theMessage->fHandingParameterSetID);
			}
		} break;
		case ParametricSetContainerHandingInformation::kAction: {
			ParametricSetContainerHandingInformation* theMessage = dynamic_cast<ParametricSetContainerHandingInformation*>(message);
			if (VERIFYN(kHema, theMessage)) {
				reply = this->OnParamSetContainerHandingParams(theMessage->fContainerHandingInfo, theMessage->fInteriorSideHandingParam, theMessage->fHandingParameterSetID);
			}
		} break;
...
Sint32 VWParametric_EventSink::HandleOnGetToolWantsWPConstrainedToView()
{
	return(kObjectEventNotHandled);
}

Sint32 VWParametric_EventSink::OnSymbolInsertion(MCObjectHandle hSymbol)
{
	return kObjectEventNotHandled;
}

Sint32 VWParametric_EventSink::OnSymbolRemoval(MCObjectHandle hSymbol)
{
	return kObjectEventNotHandled;
}

WorldPt3 VWParametric_EventSink::OnSymbolMoveConstraint(MCObjectHandle hSymbol, WorldPt3 interactiveInsertionPoint)
{
	return interactiveInsertionPoint;
}
...
bool VWParametric_EventSink::OnQTOFunction(EQTOFunction function, const TXString& option, const std::vector<VWVariant>& args, MCObjectHandle hSubPart, VWVariant& outResult) 
{
	// not handled
	return false;
}
...
EObjectEvent VWParametric_EventSink::OnGetInteractiveSizingParams(InteractiveSizingInformation& fInteractiveSizingInfo /* vector of SInteractiveSizingParameterInfo */)
{
	return kObjectEventNotImplemented;
}

EObjectEvent VWParametric_EventSink::OnSetInteractiveSizingParams(const InteractiveSizingInformation& fInteractiveSizingInfo /* vector of SInteractiveSizingParameterInfo */)
{
	return kObjectEventNotImplemented;
}

EObjectEvent  VWParametric_EventSink::OnParamPreModification(const EParamModification& modificationID)
{
	return kObjectEventNotImplemented;
}

void VWParametric_EventSink::OnClosureGapQuery( ParametricProvidesClosureGapsMessage& msg ) {
	// Do nothing - default message construction is sufficient
}

EObjectEvent	VWParametric_EventSink::OnParamGetHandingParams(HandingInformation& handingInfo /* vector of SInteractiveSizingParameterInfo */)
{
	return kObjectEventNotImplemented;
}

EObjectEvent	VWParametric_EventSink::OnParamSetHandingParams(const HandingInformation& handingInfo /* vector of SInteractiveSizingParameterInfo */)
{
	return kObjectEventNotImplemented;
}

EObjectEvent	VWParametric_EventSink::OnParamGetContainerHandingParams(ContainerHandingParams& localContainerHandingParams, SInteractiveSizingParameterInfo& newInteriorSide, Sint32 updatedHandingParamsSetID)
{
	return kObjectEventNotImplemented;
}

EObjectEvent	VWParametric_EventSink::OnParamSetContainerHandingParams(const ContainerHandingParams& localContainerHandingParams, const SInteractiveSizingParameterInfo& newInteriorSide, Sint32 updatedHandingParamsSetID)
{
	return kObjectEventNotImplemented;
}
