...
//----------------------------------------------------------------------------
Boolean XViewPt::IsZero(void) const
{
	return DoubleIsNearlyZero(x) && DoubleIsNearlyZero(y);
}

//----------------------------------------------------------------------------
double XViewPt::AngleInDegrees(void) const
{
	double outAngle = 0;

	if ( ! this->IsZero())
		outAngle = kDegreesPerRadian * atan2(-y, x);
...
//----------------------------------------------------------------------------
ViewMatrix::ViewMatrix(double ina, double inb, double inc, double ind, double intx, double inty)
: a(ina), b(inb), c(inc), d(ind), tx(intx), ty(inty)
{}

//----------------------------------------------------------------------------
ViewMatrix::ViewMatrix(double inRadiansCounterClockwiseAngle, bool inbFlip, double intx, double inty)
{	this->Set(inRadiansCounterClockwiseAngle, inbFlip, intx, inty);	}

//----------------------------------------------------------------------------
void ViewMatrix::Set(double ina, double inb, double inc, double ind, double intx, double inty)
{	a = ina; b = inb; c = inc; d = ind; tx = intx; ty = inty;	}

//----------------------------------------------------------------------------
void ViewMatrix::Set(double inRadiansCounterClockwiseAngle, bool inbFlip, double intx, double inty)
{
	// The angle for view space must be reversed.
	inRadiansCounterClockwiseAngle = -inRadiansCounterClockwiseAngle;

	double sinTheta = sin(inRadiansCounterClockwiseAngle);
	double cosTheta = cos(inRadiansCounterClockwiseAngle);
...
//----------------------------------------------------------------------------
void ViewMatrix::Get(double& outRadiansCounterClockwiseAngle, bool& outbFlip, double& outtx, double& outty) const
{
	outbFlip = (a*d - b*c < 0);

	outRadiansCounterClockwiseAngle = -atan2(c, a);
...
//----------------------------------------------------------------------------
void ViewMatrix::Concat(const ViewMatrix& inMat)
{
	ViewMatrix result;
	ViewMatrix::Multiply(inMat, *this, result);
	*this = result;
}

//----------------------------------------------------------------------------
Bool8 ViewMatrix::operator==(const ViewMatrix& inMat) const
{
	return DoublesAreNearlyEqual(a, inMat.a)
			&& DoublesAreNearlyEqual(b, inMat.b)
			&& DoublesAreNearlyEqual(c, inMat.c)
			&& DoublesAreNearlyEqual(d, inMat.d)
			&& DoublesAreNearlyEqual(tx, inMat.tx)
			&& DoublesAreNearlyEqual(ty, inMat.ty);
}

//----------------------------------------------------------------------------
Bool8 ViewMatrix::Inverse(ViewMatrix& outMat) const
{
	Bool8 bSuccess = false;

	double det, temp, pos, neg, mag;

	pos = 0.0;
	neg = 0.0;

	temp = a*d;		(temp >= 0 ? pos : neg) += temp;
	temp = -(c*b);	(temp >= 0 ? pos : neg) += temp;
	
	det = pos+neg;
	mag = pos-neg;
	
	if (VERIFYN(kEJKerr, mag != 0.0)) {
		temp = det/mag;

		if (VERIFYN(kEJKerr, !DoubleIsNearlyZero(temp))) { 
			outMat.a	= d/det;
			outMat.b	= -b/det;
			outMat.c	= -c/det;
			outMat.d	= a/det;
			outMat.tx	= -(tx*outMat.a + ty*outMat.c);
			outMat.ty	= -(tx*outMat.b + ty*outMat.d);
...
//----------------------------------------------------------------------------
void ViewMatrix::TransformPt(const double inX, const double inY, double& outX, double& outY) const
{
	double x = inX, y = inY;
	outX = x * a + y * c + tx;
	outY = x * b + y * d + ty;
...
//----------------------------------------------------------------------------
void ViewMatrix::InverseTransformPt(const double inX, const double inY, double& outX, double& outY) const
{
	double x = inX - tx, y = inY - ty;
	outX = x * a + y * b;
	outY = x * c + y * d;
}

//----------------------------------------------------------------------------
Bool8 ViewMatrix::TransformPtTrunc(const GSPoint32& inPt, GSPoint32& outPt) const
{
	Bool8 bSuccess = true;

	double rotX=0, rotY=0, temp=0;
	this->TransformPt(inPt.x, inPt.y, rotX, rotY);

	temp = Truncate(rotX);
	outPt.x = DemoteTo<GSCoord32>(kEJKerr, PinTo<GSCoord32>(kEJKerr, temp));
	bSuccess = bSuccess && (temp == outPt.x);

	temp = Truncate(rotY);
	outPt.y = DemoteTo<GSCoord32>(kEJKerr, PinTo<GSCoord32>(kEJKerr, temp));
	bSuccess = bSuccess && (temp == outPt.y);

	return bSuccess;
}

//----------------------------------------------------------------------------
Bool8 ViewMatrix::TransformPtRound(const GSPoint32& inPt, GSPoint32& outPt) const
{
	Bool8 bSuccess = true;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::InverseTransformPtTrunc(const GSPoint32& inPt, GSPoint32& outPt) const
{
	Bool8 bSuccess = true;

	double rotX=0, rotY=0, temp=0;
	this->InverseTransformPt(inPt.x, inPt.y, rotX, rotY);
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::InverseTransformPtRound(const GSPoint32& inPt, GSPoint32& outPt) const
{
	Bool8 bSuccess = true;
...
//----------------------------------------------------------------------------
void ViewMatrix::TransformPt(const XViewPt& inPt, XViewPt& outPt) const
{
	this->TransformPt(inPt.x, inPt.y, outPt.x, outPt.y);
}

//----------------------------------------------------------------------------
void ViewMatrix::InverseTransformPt(const XViewPt& inPt, XViewPt& outPt) const
{
	this->InverseTransformPt(inPt.x, inPt.y, outPt.x, outPt.y);
}

//----------------------------------------------------------------------------
Bool8 ViewMatrix::TransformRectTrunc(const GSRect32& inRect, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = TransformPtTrunc(GSPoint32(inRect.left, inRect.top), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtTrunc(GSPoint32(inRect.right, inRect.top), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtTrunc(GSPoint32(inRect.right, inRect.bottom), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtTrunc(GSPoint32(inRect.left, inRect.bottom), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::TransformRectRound(const GSRect32& inRect, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = TransformPtRound(GSPoint32(inRect.left, inRect.top), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtRound(GSPoint32(inRect.right, inRect.top), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtRound(GSPoint32(inRect.right, inRect.bottom), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtRound(GSPoint32(inRect.left, inRect.bottom), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::InverseTransformRectTrunc(const GSRect32& inRect, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = InverseTransformPtTrunc(GSPoint32(inRect.left, inRect.top), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtTrunc(GSPoint32(inRect.right, inRect.top), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtTrunc(GSPoint32(inRect.right, inRect.bottom), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtTrunc(GSPoint32(inRect.left, inRect.bottom), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::InverseTransformRectRound(const GSRect32& inRect, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = InverseTransformPtRound(GSPoint32(inRect.left, inRect.top), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtRound(GSPoint32(inRect.right, inRect.top), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtRound(GSPoint32(inRect.right, inRect.bottom), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtRound(GSPoint32(inRect.left, inRect.bottom), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
void ViewMatrix::TransformRect(const XViewRect& inRect, XViewRectVerts& outVerts) const
{
	this->TransformPt(inRect.TopLeft(), outVerts.SetTopLeft());
	this->TransformPt(inRect.TopRight(), outVerts.SetTopRight());
	this->TransformPt(inRect.BottomRight(), outVerts.SetBottomRight());
	this->TransformPt(inRect.BottomLeft(), outVerts.SetBottomLeft());
}

//----------------------------------------------------------------------------
void ViewMatrix::InverseTransformRect(const XViewRect& inRect, XViewRectVerts& outVerts) const
{
	this->InverseTransformPt(inRect.TopLeft(), outVerts.SetTopLeft());
	this->InverseTransformPt(inRect.TopRight(), outVerts.SetTopRight());
	this->InverseTransformPt(inRect.BottomRight(), outVerts.SetBottomRight());
	this->InverseTransformPt(inRect.BottomLeft(), outVerts.SetBottomLeft());
}

//----------------------------------------------------------------------------
Bool8 ViewMatrix::TransformVertsTrunc(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = TransformPtTrunc(inVerts.TopLeft(), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtTrunc(inVerts.TopRight(), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtTrunc(inVerts.BottomRight(), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtTrunc(inVerts.BottomLeft(), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::TransformVertsRound(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = TransformPtRound(inVerts.TopLeft(), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtRound(inVerts.TopRight(), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtRound(inVerts.BottomRight(), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = TransformPtRound(inVerts.BottomLeft(), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::InverseTransformVertsTrunc(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = InverseTransformPtTrunc(inVerts.TopLeft(), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtTrunc(inVerts.TopRight(), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtTrunc(inVerts.BottomRight(), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtTrunc(inVerts.BottomLeft(), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
Bool8 ViewMatrix::InverseTransformVertsRound(const GSRect32Verts& inVerts, GSRect32Verts& outVerts) const
{
	Bool8 bSuccess = true;

	Bool8 bTemp;

	bTemp = InverseTransformPtRound(inVerts.TopLeft(), outVerts.SetTopLeft());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtRound(inVerts.TopRight(), outVerts.SetTopRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtRound(inVerts.BottomRight(), outVerts.SetBottomRight());
	bSuccess = bSuccess && bTemp;

	bTemp = InverseTransformPtRound(inVerts.BottomLeft(), outVerts.SetBottomLeft());
	bSuccess = bSuccess && bTemp;
...
//----------------------------------------------------------------------------
void ViewMatrix::TransformVerts(const XViewRectVerts& inVerts, XViewRectVerts& outVerts) const
{
	this->TransformPt(inVerts.TopLeft(), outVerts.SetTopLeft());
	this->TransformPt(inVerts.TopRight(), outVerts.SetTopRight());
	this->TransformPt(inVerts.BottomRight(), outVerts.SetBottomRight());
	this->TransformPt(inVerts.BottomLeft(), outVerts.SetBottomLeft());
}

//----------------------------------------------------------------------------
void ViewMatrix::InverseTransformVerts(const XViewRectVerts& inVerts, XViewRectVerts& outVerts) const
{
	this->InverseTransformPt(inVerts.TopLeft(), outVerts.SetTopLeft());
	this->InverseTransformPt(inVerts.TopRight(), outVerts.SetTopRight());
	this->InverseTransformPt(inVerts.BottomRight(), outVerts.SetBottomRight());
	this->InverseTransformPt(inVerts.BottomLeft(), outVerts.SetBottomLeft());
}

//----------------------------------------------------------------------------
void ViewMatrix::Multiply(const ViewMatrix& inmatL, const ViewMatrix& inmatR, ViewMatrix& outmatResult)
// static
{
	// copy inmatL,inmatR to locals, to allow outmatResult to be inmatL or inmatR
	ViewMatrix l = inmatL;
	ViewMatrix r = inmatR;

	outmatResult.a	= l.a * r.a + l.b * r.c;
	outmatResult.b	= l.a * r.b + l.b * r.d;
	outmatResult.c	= l.c * r.a + l.d * r.c;
	outmatResult.d	= l.c * r.b + l.d * r.d;
	outmatResult.tx	= l.tx * r.a + l.ty * r.c + r.tx;
	outmatResult.ty	= l.tx * r.b + l.ty * r.d + r.ty;
}

//----------------------------------------------------------------------------
ViewMatrix ViewMatrix::IdentityMatrix()
// static
{	return ViewMatrix(1, 0, 0, 1, 0, 0);	}

//----------------------------------------------------------------------------
ViewMatrix ViewMatrix::TranslationMatrix(double tx, double ty)
// static
{	return ViewMatrix(1, 0, 0, 1, tx, ty);	}

//----------------------------------------------------------------------------
ViewMatrix ViewMatrix::ScaleMatrix(double sx, double sy)
// static
{	return ViewMatrix(sx, 0, 0, sy, 0, 0);	}

//----------------------------------------------------------------------------
ViewMatrix ViewMatrix::RotationMatrix(double inRadiansCounterClockwiseAngle)
// static
{
	return ViewMatrix(inRadiansCounterClockwiseAngle, false/*inbFlip*/, 0/*intx*/, 0/*inty*/);
}

//----------------------------------------------------------------------------
ViewMatrix ViewMatrix::RotationMatrix(double inRadiansCounterClockwiseAngle, double viewCenterX, double viewCenterY)
// static
{
	ViewMatrix resultMat;

	ViewMatrix transMat(1, 0, 0, 1, -viewCenterX, -viewCenterY);
	ViewMatrix rotMat(inRadiansCounterClockwiseAngle, false/*inbFlip*/, viewCenterX, viewCenterY);

	ViewMatrix::Multiply(transMat, rotMat, resultMat);
...
//////////////////////////////////////////////////////////////////////////////
// TGSPoly Implementation
//////////////////////////////////////////////////////////////////////////////
const Sint32 kMaxTGSPolyPts = kGSMaxSint32;

//----------------------------------------------------------------------------
TGSPoly::TGSPoly()
	: fNumPtsUsed(0), fvptBadValue(kGSMinSint16, kGSMinSint16)
{
	fBBox.MakeEmpty();
}

//----------------------------------------------------------------------------
TGSPoly::TGSPoly(Sint32 inNumPts)
	: fNumPtsUsed(0), fvptBadValue(kGSMinSint16, kGSMinSint16)
{
	this->Resize(inNumPts);
	fBBox.MakeEmpty();
}

//----------------------------------------------------------------------------
TGSPoly::TGSPoly(const TGSPoly& inTGSPoly)
	: fvptBadValue(kGSMinSint16, kGSMinSint16)
{
	fNumPtsUsed = inTGSPoly.fNumPtsUsed;
	fBBox		= inTGSPoly.fBBox;
	fPoints		= inTGSPoly.fPoints;
}

//----------------------------------------------------------------------------
TGSPoly::TGSPoly(const PPGSPoly inppGSPoly)
	: fNumPtsUsed(0), fvptBadValue(kGSMinSint16, kGSMinSint16)
{
	if (VERIFYN(kEJKerr, inppGSPoly)
		&& VERIFYPOINTER(kEJKerr, *inppGSPoly, sizeof(GSPoly))) {
		
		fBBox = (*inppGSPoly)->polyBBox;
		
		Sint16 numPts = (*inppGSPoly)->GetNumPts();
		this->Resize(numPts);
		
		if (numPts > 0) {
			std::vector<ViewPt>::iterator iter = fPoints.begin();
			
			for (Sint16 i = 0; i < numPts && VERIFYN(kEJKerr, iter != fPoints.end()); i++, iter++) {
				*iter = (*inppGSPoly)->polyPoints[i];
			}
...
//----------------------------------------------------------------------------
bool TGSPoly::Reserve(size_t inNumPts)
{
	bool outbSuccess = false;

	if (VERIFYN(kEJKerr, inNumPts >= 0)) {
		try {
			fPoints.reserve(inNumPts);
			outbSuccess = true;
		}
		catch (std::bad_alloc) {
			outbSuccess = false;
...
//----------------------------------------------------------------------------
bool TGSPoly::Resize(size_t inNumPts)
{
	bool outbSuccess = false;

	if (VERIFYN(kEJKerr, inNumPts >= 0)) {
		try {
			fPoints.resize(inNumPts);
			outbSuccess = true;
		}
		catch (std::bad_alloc) {
			outbSuccess = false;
		...
		if (outbSuccess) {
			...
			if (fPoints.size() != inNumPts)
				outbSuccess = false;

			if (fPoints.size() < (size_t)fNumPtsUsed)
				fNumPtsUsed = DemoteTo<Sint32>(kDan, fPoints.size());
		}
	}
...
//----------------------------------------------------------------------------
bool TGSPoly::CanAddPt() const
{
	return (fNumPtsUsed < kMaxTGSPolyPts);
}

//----------------------------------------------------------------------------
bool TGSPoly::AddPt(const ViewPt& inPt)
{
	if (fNumPtsUsed < fPoints.size()) {
		fPoints[fNumPtsUsed] = inPt;
	}
	else {
		fPoints.push_back(inPt);
	}
...
//----------------------------------------------------------------------------
void TGSPoly::Restart()
// This leaves the memory allocation as is, and restarts the point counter so
// that we have no points added.
{
	fBBox.MakeEmpty();
	fNumPtsUsed = 0;
}
...
//----------------------------------------------------------------------------
ViewRect TGSPoly::GetBBox() const
{
	return fBBox;
}

//----------------------------------------------------------------------------
void TGSPoly::SetBBox(const ViewRect inBox)
{
	fBBox = inBox;
}

//----------------------------------------------------------------------------
void TGSPoly::RecalcBBox()
{
	if (fNumPtsUsed > 0) {
		std::vector<ViewPt>::const_iterator iter = fPoints.begin();

		fBBox.left = fBBox.right = (*iter).x;
		fBBox.top = fBBox.bottom = (*iter).y;
		
		for (Sint32 i=0; iter != fPoints.end() && i<fNumPtsUsed; iter++, i++) {
			ViewPt pt = (*iter);
			
			if (pt.x < fBBox.left)
				fBBox.left = pt.x;
			else if (pt.x > fBBox.right)
				fBBox.right = pt.x;
				
			if (pt.y < fBBox.top)
				fBBox.top = pt.y;
			else if (pt.y > fBBox.bottom)
				fBBox.bottom = pt.y;
		}
	}
	else {
...
//----------------------------------------------------------------------------
void TGSPoly::Offset(ViewCoord dx, ViewCoord dy)
{
	ViewPt pt(dx, dy);
...
//----------------------------------------------------------------------------
PPGSPoly TGSPoly::ExtractGSPoly(Sint32 startIndex, short inNumPts) const
{
	PPGSPoly hPoly = NULL;

	if (inNumPts == -1) {
		inNumPts = DemoteTo<short>(kEJKerr, min(this->GetNumUsed(), (Sint32) kMaxPolyVertices));
	}
	
	if (VERIFYN(kEJKerr, inNumPts > 0 && inNumPts <= kMaxPolyVertices && startIndex >= 0)
		&& VERIFYN(kEJKerr, this->GetNumUsed() >= startIndex+inNumPts)) {
		
		short theSize = GSPoly::CalcSize(inNumPts);
		
		if (theSize) {
			hPoly = (PPGSPoly) gVWMM->NewHandle(theSize);

			if (VERIFYN(kEJKerr, hPoly)) {
				(*hPoly)->polySize = theSize;
				...
				for (Sint32 i=0; iter != fPoints.end() && i < inNumPts; iter++,i++) {
					(*hPoly)->polyPoints[i] = *iter;
				}
				
				if (startIndex == 0 && inNumPts == this->GetNumUsed())
					(*hPoly)->polyBBox = fBBox;
				else
					(*hPoly)->CalcBBox();
			}
		}
...
//----------------------------------------------------------------------------
const ViewPt& TGSPoly::operator[](ptrdiff_t i) const
{
	ASSERTN(kEJKerr, i >= 0 && (size_t)i < fPoints.size());
	ASSERTN(kEJKerr, fPoints.size() > 0);
	
	return i >= 0 && (size_t)i < fPoints.size() ? fPoints[i] : fvptBadValue;
}

//----------------------------------------------------------------------------
ViewPt& TGSPoly::operator[](ptrdiff_t i)
{
	ASSERTN(kEJKerr, i >= 0 && (size_t)i < fPoints.size());
	ASSERTN(kEJKerr, fPoints.size() > 0);

	// since they can get a ref to it, they might modify it, so keep resetting it here.
	fvptBadValue.Set(kGSMinSint16, kGSMinSint16);

	return i >= 0 && (size_t)i < fPoints.size() ? fPoints[i] : fvptBadValue;
}

//----------------------------------------------------------------------------
TGSPoly TGSPoly::operator=(const TGSPoly& in)
{
	fNumPtsUsed = in.fNumPtsUsed;
	fBBox		= in.fBBox;
	fPoints		= in.fPoints;
...
//////////////////////////////////////////////////////////////////////////////
// TGSPolyRecorder Implementation
//////////////////////////////////////////////////////////////////////////////

//----------------------------------------------------------------------------
TGSPolyRecorder::TGSPolyRecorder(TGSPoly& inTGSPoly)
	: fTGSPolyRef(inTGSPoly)
{
	const Sint32 kPRInitialNumPts	= 40;
	
	if (fTGSPolyRef.GetCapacity() < kPRInitialNumPts)
		fTGSPolyRef.Reserve(kPRInitialNumPts);
}

//----------------------------------------------------------------------------
TGSPolyRecorder::~TGSPolyRecorder()
{
	fTGSPolyRef.RecalcBBox();
}

//----------------------------------------------------------------------------
bool TGSPolyRecorder::CanAddPt() const
{
	return fTGSPolyRef.CanAddPt();
}

//----------------------------------------------------------------------------
bool TGSPolyRecorder::AddPt(const ViewPt& inPt)
{
	bool bSuccess = false;

	const Sint32 kPRGrowthFactor	= 8;
	
	Sint32 numPtsUsed = fTGSPolyRef.GetNumUsed();

	if (numPtsUsed < fTGSPolyRef.GetCapacity()) {
		bSuccess = fTGSPolyRef.AddPt(inPt);
	}
	else {

		if (VERIFYN(kEJKerr, numPtsUsed < kMaxTGSPolyPts)) {
			double newNumPts = (double)numPtsUsed * (double)kPRGrowthFactor;

			if (newNumPts > kMaxTGSPolyPts)
				newNumPts = kMaxTGSPolyPts;
			
			bSuccess = fTGSPolyRef.Reserve(DemoteTo<Sint32>(kEJKerr, Truncate(newNumPts)));
			
			if (VERIFYN(kEJKerr, bSuccess)) {
				bSuccess = fTGSPolyRef.AddPt(inPt);
			}
		}
	}
...
//////////////////////////////////////////////////////////////////////////////
// TXViewPoly implementation
//////////////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------------
bool TXViewPoly::IsEmpty() const
{
	return fPoints.size() == 0;
}

//----------------------------------------------------------------------------
void TXViewPoly::MakeEmpty()
{
	fBBox.MakeEmpty();
	fPoints.clear();
}
...
//----------------------------------------------------------------------------
void TXViewPoly::RecalcBBox()
{
	if (fPoints.size() > 0) {
		fBBox.left = fBBox.right = fPoints[0].x;
		fBBox.top = fBBox.bottom = fPoints[0].y;
		
		for (auto& it : fPoints) {

			if (it.x < fBBox.left)
				fBBox.left = it.x;
			else if (it.x > fBBox.right)
				fBBox.right = it.x;
				
			if (it.y < fBBox.top)
				fBBox.top = it.y;
			else if (it.y > fBBox.bottom)
				fBBox.bottom = it.y;
		}
	}
	else {
...
//----------------------------------------------------------------------------
void TXViewPoly::Offset(XViewCoord dx, XViewCoord dy)
{
	for (auto& it : fPoints) {
		it.x += dx;
		it.y += dy;
...
//////////////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------------
TXViewPolyRecorder::TXViewPolyRecorder(TXViewPoly& inTXViewPoly)
	: fTXViewPolyRef(inTXViewPoly)
{
	const size_t kPRInitialNumPts	= 40;
	
	if (fTXViewPolyRef.fPoints.capacity() < kPRInitialNumPts)
		fTXViewPolyRef.fPoints.reserve(kPRInitialNumPts);
}

//----------------------------------------------------------------------------
TXViewPolyRecorder::~TXViewPolyRecorder()
{
	fTXViewPolyRef.fPoints.shrink_to_fit();
	fTXViewPolyRef.RecalcBBox();
}

//----------------------------------------------------------------------------
bool TXViewPolyRecorder::AddPt(const XViewPt& inPt)
{
	bool outbSuccess = false;

	const Sint32 kPRGrowthFactor	= 8;
	
	size_t numPtsUsed = fTXViewPolyRef.fPoints.size();

	if (numPtsUsed < fTXViewPolyRef.fPoints.capacity()) {
		fTXViewPolyRef.fPoints.push_back(inPt);
		outbSuccess = true;
	}
	else {

		double newNumPts = (double)numPtsUsed * (numPtsUsed < kGSMaxUint16 ? (double)kPRGrowthFactor : 2);

		try {
			fTXViewPolyRef.fPoints.reserve(DemoteTo<size_t>(kEJKerr, Truncate(newNumPts)));
			fTXViewPolyRef.fPoints.push_back(inPt);
			outbSuccess = true;
		}
		catch (std::bad_alloc) {
			outbSuccess = false;
			ASSERTN(kEJKerr, outbSuccess);
		}
	}
...
//----------------------------------------------------------------------------
bool TXViewPolyToTGSPoly(const TXViewPoly& inTXViewPoly, TGSPoly& outPly)
{
	bool outbSuccess = false;

	if (VERIFYN(kEJKerr, inTXViewPoly.fPoints.size() < kMaxTGSPolyPts)) {
		outPly.Resize(inTXViewPoly.fPoints.size());
		outPly.Restart();

		ViewPt vpt;

		for (size_t i=0; i<inTXViewPoly.fPoints.size(); i++) {
			XViewPtToViewPt(inTXViewPoly.fPoints[i], vpt);
			outPly.AddPt(vpt);
		}

		ViewRect vr;
		outbSuccess = XViewRectToViewRect(inTXViewPoly.fBBox, vr);
		outPly.SetBBox(vr);
	}
...
//----------------------------------------------------------------------------
void PPGSPolyToTXViewPoly(const PPGSPoly inhPoly, TXViewPoly& outXPoly)
{
	if (inhPoly) {
		Sint16 numpts = (*inhPoly)->GetNumPts();

		if (numpts > 0) {

			if (outXPoly.fPoints.size() == 0) {
				outXPoly.fPoints.reserve(numpts);

				for (Sint16 i=0; i<numpts; i++)
					outXPoly.fPoints.emplace_back((*inhPoly)->polyPoints[i]);
			}
			else {
				outXPoly.fPoints.resize(numpts);

				for (Sint16 i=0; i<numpts; i++)
					outXPoly.fPoints[i] = (*inhPoly)->polyPoints[i];
			}

			outXPoly.fBBox = (*inhPoly)->polyBBox;
		}
		else {
			outXPoly.fPoints.clear();
			outXPoly.fBBox.MakeEmpty();
		}
	}
}

//----------------------------------------------------------------------------
void TGSPolyToTXViewPoly(const TGSPoly& inPoly, TXViewPoly& outXPoly)
{
	Sint32 numpts = inPoly.GetSize();
	...
	if (numpts > 0) {
		...
		if (outXPoly.fPoints.size() == 0) {
			...
			for (Sint32 i=0; i<numpts; i++)
				outXPoly.fPoints.emplace_back(inPoly[i]);
		}
		else {
			outXPoly.fPoints.resize(numpts);

			for (Sint32 i=0; i<numpts; i++)
				outXPoly.fPoints[i] = inPoly[i];
		}

		outXPoly.fBBox = inPoly.GetBBox();
	}
	else {
		outXPoly.fPoints.clear();
		outXPoly.fBBox.MakeEmpty();
...
//----------------------------------------------------------------------------
void XViewRectVertsToTXViewPoly(const XViewRectVerts& in, TXViewPoly& outXPoly)
{
	if (outXPoly.fPoints.size() == 0) {
		outXPoly.fPoints.reserve(4);

		for (int i=0; i<4; i++)
			outXPoly.fPoints.emplace_back(in[i]);
	}
	else {
		outXPoly.fPoints.resize(4);

		for (int i=0; i<4; i++)
			outXPoly.fPoints[i] = in[i];
	}

	in.GetBoundingBox(outXPoly.fBBox);
}

//----------------------------------------------------------------------------
static Bool8 CalcArcCenterPoint(const XViewPt& startPt, const XViewPt& endPt,
								const XViewPt& originPt, const double radius,
								XViewPt& outCenterPt)
// supports TExpressArcAsCubicCurves::RecurseExpressArc
{
	Bool8 bSuccess = false;

	XViewPt bisectorPt = startPt + endPt;
	bisectorPt.Scale(0.5);

	XViewPt v1 = bisectorPt - originPt;
...
//----------------------------------------------------------------------------
static Radians AngleFromPoint(const XViewPt& pt)
// supports TExpressArcAsCubicCurves::RecurseExpressArc
{
	Radians angle = kRadiansPerDegree * pt.AngleInDegrees();
...
//----------------------------------------------------------------------------
static Bool8 CalcArcBezierControlPoints(const XViewPt& startPt, const XViewPt& endPt, const XViewPt& originPt, const XViewCoord radius,
								XViewPt& outControlPt1, XViewPt& outControlPt2)
//
// supports TExpressArcAsCubicCurves::RecurseExpressArc
//
//            2
...
//
{
	...
	if (startPt != originPt && originPt != endPt && startPt != endPt && radius > 2) {
		XViewPt v1 = startPt-originPt;
		XViewPt v2 = endPt-originPt;

		Radians angle1 = AngleFromPoint(v1);
		Radians angle2 = AngleFromPoint(v2);

		Radians angleBetween = angle2-angle1;
		...
		if (angleBetween > PI) {
			ASSERTN(kEJKerr, "How did this happen?");
			angleBetween = 2*PI - angleBetween;
		}

		Radians halfAngle = angleBetween/2;

		if ( ! DoublesAreNearlyEqual(halfAngle, PI/2, 1e-8)) {
			double tanHalfAng = tan(halfAngle);

			double crossProdZ = (v1.x * v2.y) - (v1.y * v2.x);

			XViewPt v1Perp(-v1.y, v1.x);

			double factor = radius*tanHalfAng/v1Perp.Magnitude();
			...
			XViewPt cornerPt = startPt + v1Perp;

			XViewPt pt0(cos(halfAngle), sin(halfAngle));
			XViewPt pt1((4 - pt0.x)/3, ((1-pt0.x) * (3-pt0.x))/(3*pt0.y));
			XViewPt vDiff = pt1-pt0;
			double alpha = vDiff.Magnitude()/tanHalfAng;
			...
			outControlPt1 = startPt + v1;
			outControlPt2 = endPt + v2;
...
/////////////////////////////////////////////////////////////////////////
Bool8	TExpressArcAsCubicCurves::RecurseExpressArc(const XViewPt& startPt, const XViewPt& endPt, const XViewPt& originPt, const XViewCoord radius)
/////////////////////////////////////////////////////////////////////////
{
	Bool8 bSuccess = false;
	...
	const double kMaxArcAngle = PI/2;

	if (angleBetween > kMaxArcAngle) {
		XViewPt centerPt;

		if (CalcArcCenterPoint(startPt, endPt, originPt, radius, centerPt)) {
			Bool8 bFirst = this->RecurseExpressArc(startPt, centerPt, originPt, radius);
			Bool8 bNext = this->RecurseExpressArc(centerPt, endPt, originPt, radius);
			bSuccess = (bFirst && bNext);
		}
	}
	else {
		XViewPt controlPt1, controlPt2;

		if (CalcArcBezierControlPoints(startPt, endPt, originPt, radius, controlPt1, controlPt2)) {
			bSuccess = ExpressArcAsCubicCurves_DoCubic(controlPt1, controlPt2, endPt);
		}
	}
...
/////////////////////////////////////////////////////////////////////////
Bool8 CalcArcFilletPoints(const XViewPt& startPt, const XViewPt& cornerPt, const XViewPt& endPt, const XViewCoord radius,
							XViewPt& outStartFilletPt, XViewPt& outEndFilletPt)
//            
//	1_______2  ,3
//          \
//           \
...
/////////////////////////////////////////////////////////////////////////
{
	...
	if (startPt != cornerPt && cornerPt != endPt && startPt != endPt && radius > 2) {
		XViewPt v1 = startPt-cornerPt;
		XViewPt v2 = endPt-cornerPt;

		XViewPt v1ForAngle = cornerPt-startPt;
		Radians angle1 = AngleFromPoint(v1ForAngle);
		Radians angle2 = AngleFromPoint(v2);
		...
		if ( ! DoublesAreNearlyEqual(halfAngle, PI/2, 1e-8)) {
			...
			XViewCoord distanceFromCorner = radius * tanHalfAng;

			outStartFilletPt	= v1 * (distanceFromCorner/v1.Magnitude());
			outEndFilletPt		= v2 * (distanceFromCorner/v2.Magnitude());
			
			outStartFilletPt	+= cornerPt;
			outEndFilletPt		+= cornerPt;
...
/////////////////////////////////////////////////////////////////////////
Bool8 CalcArcOriginPoint(const XViewPt& startPt, const XViewPt& cornerPt, const XViewPt& endPt, const XViewCoord radius,
							XViewPt& outOriginPt)
{
	Bool8 bSuccess = false;

	if (startPt != cornerPt && cornerPt != endPt && startPt != endPt && radius > 2) {
		XViewPt v1 = cornerPt-startPt;
		XViewPt v2 = endPt-cornerPt;
		...
		double length = v1Perp.Magnitude();
