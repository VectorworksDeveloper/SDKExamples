...
#if BUG && __has_include("Kernel/ExternalUser.h")
// look for optional kMyProgConst in optional ExternalUser.h
...
TProgrammer TProgrammer::operator &(const TProgrammer& a) const
{
	...
	// Combine the active user flag
	if (a.fOnlyIfActiveUser) dest.fOnlyIfActiveUser= true;
...
#if defined(kMyProgConst)
	// if you say who you are at compiletime, registration doesn't matter
	if (IsIn(kMyProgConst))
		return true;
#endif
	for (Sint32 i = 0; i < fMaxProgrammers; ++i) {
		if (fRegistered[i].fID1 == 0 && fRegistered[i].fID2 == 0 && fRegistered[i].fID3 == 0 && fRegistered[i].fID4 == 0)
			break;	// we've reached the end of the set registered array, shortcut out.
		if (_IsUser(fRegistered[i].fName)) {
			if (IsIn(fRegistered[i].fID4, fRegistered[i].fID3, fRegistered[i].fID2, fRegistered[i].fID1))
				return true;
			break;
		}
	}
...
bool TProgrammer::IsEveryone() const
{
	return this->MatchId( kEveryone );
}

const Sint32 TProgrammer::fMaxProgrammers = 256;
TProgrammer::TRegistered TProgrammer::fRegistered[fMaxProgrammers];

bool TProgrammer::IsIn(Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1) const
{
	Uint64 thisID4 = fID4;
	Uint64 thisID3 = fID3;
	Uint64 thisID2 = fID2;
	...
	bool result = (thisID4 & id4) == thisID4 && (thisID3 & id3) == thisID3 && (thisID2 & id2) == thisID2 && (thisID1 & id1) == thisID1;

	return result;
}


void TProgrammer::Register(const char *name, Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1, Uint64 flags, bool onlyIfActiveUser)
{
	// Skip registering things like kEveryone because they prevent anyone after them from registering (returns at // reject multiples)
#if TEST
	if ( ! (id4 == 0xFFFFFFFFFFFFFFFFULL && id3 == 0xFFFFFFFFFFFFFFFFULL && id2 == 0xFFFFFFFFFFFFFFFFULL && id1 == 0xFFFFFFFFFFFFFFFFULL) ) {
#else
	if ( ! (id4 == 0 && id3 == 0 && id2 == 0 && id1 == 0) ) {
#endif
		// Find available slot.
		Sint32 i;
		...
		{
			if (
					(fRegistered[i].fID4 & id4) == id4
				&&	(fRegistered[i].fID3 & id3) == id3
				&&	(fRegistered[i].fID2 & id2) == id2
				&&	(fRegistered[i].fID1 & id1) == id1
				)
			{
					return; // reject multiples
			}
			if (
					fRegistered[i].fID4 == 0
				&&	fRegistered[i].fID3 == 0
				&&	fRegistered[i].fID2 == 0
				&&	fRegistered[i].fID1 == 0
				)
				break;
		}
		// Fill it in.
		...
		{
			fRegistered[i].fID4 = id4;
			fRegistered[i].fID3 = id3;
			fRegistered[i].fID2 = id2;
			fRegistered[i].fID1 = id1;
			fRegistered[i].fFlags = flags;
			fRegistered[i].fOnlyIfActiveUser = onlyIfActiveUser;
			strcpy(fRegistered[i].fName, name);
		}
	}
}

bool TProgrammer::GetProgrammerName(Uint64 id4, Uint64 id3, Uint64 id2, Uint64 id1, char *name)
{
	for (Sint32 i=0; i<fMaxProgrammers; ++i) {
		if (	
				fRegistered[i].fID4 == id4
		...
		{
			strncpy(name, fRegistered[i].fName, kDebugStringLength);
			return true;
		}
	}
	return false;
...
{
	Uint64 id=1;
	if (who.fID1 != 0 || who.fID2 != 0 || who.fID3 != 0|| who.fID4 != 0) // Make sure we actually have something to search for.
	{
		for (Uint32 i=0; i<fMaxProgrammers; ++i, id*=2)
		{
			if (
					(who.fID4 == 0 || who.fID4 & fRegistered[i].fID4)
				&&	(who.fID3 == 0 || who.fID3 & fRegistered[i].fID3)
				&&	(who.fID2 == 0 || who.fID2 & fRegistered[i].fID2)
				&&	(who.fID1 == 0 || who.fID1 & fRegistered[i].fID1)
				)
			{
				one.Set(fRegistered[i].fName, fRegistered[i].fID4, fRegistered[i].fID3, fRegistered[i].fID2, fRegistered[i].fID1, fRegistered[i].fFlags, fRegistered[i].fOnlyIfActiveUser, who.fSendOnOwn);
				who.fID4 &= ~fRegistered[i].fID4;
				who.fID3 &= ~fRegistered[i].fID3;
...
//---------------------------------------------------------------------------------------
#if BUG
static bool gWantBackgroundToBreak = false; // for the moment, too annoying to default true?
#else
static bool gWantBackgroundToBreak = false;
#endif

bool GSDebug::WantBackgroundToBreak()
{
	return gWantBackgroundToBreak;
}

bool GSDebug::WantBackgroundToBreak(bool bWantToBreak)
{
	const bool savedWantToBreak = gWantBackgroundToBreak;
	gWantBackgroundToBreak = bWantToBreak;
	return savedWantToBreak;
}
	...
	public:
		OutputMessage(bool sendToFile)
		{
			...
			if (sendToFile) {
				...
				debugPath = szDebugPath;
				#endif	
				debugPath << "VW debug output.txt";

				theFile = vw_fopen(debugPath, "a+");
			}
			else
				theFile = nil;
		#else
...
#if BUG && !_MINICAD_EXTERNAL_ && !_GSESDK_
		extern bool IsThisMainThread();
		extern bool ModalFeedbackIsAllowed();
		if ( bDoBreak && (!IsThisMainThread() || !ModalFeedbackIsAllowed()))
		{
			// Allow background threads to stop even if they can't alert.
			// The output went to the console anyway.
			bDoBreak = GSDebug::WantBackgroundToBreak();
		}
		else 
#endif
		if ( bDoBreak ) {
			static char	fileText[ kBuf_size ];
			size_t attempted_len = snprintf( fileText, kBuf_size, "%s(%d)", szFile, line );
			DSTOPIF(attempted_len > kBuf_size, (kEveryone, "Assertion message is too big; it is being truncated."));
		...
		#endif
		{
			...
			if (bDoTheMessage) {
				...
				// Send message to server in network debugging.
					if (bFile) {						// Check if message is destined for the server.
						...
						if (bToDebugger && gDStopLineNumber > 0) {
							...
							snprintf(msg2, kBuf_size + 128, "DSTOP: File %s, Line %d\n  %s", gDStopFileName, gDStopLineNumber, szMsg);
				...
				// Send message to user.
					...
					#if GS_MAC && !Autobuild
					if (bToDebugger)
						os_log_fault(OS_LOG_DEFAULT, "%s%s", msg2.c_str(), szMsg);
					else {
					#endif
						OutputMessage out(true);
						out.SendString(msg2.c_str());
					...
					if (bToDebugger && doStop && who.ShouldBreak()) {
						...
						Sint32 attempted_len = 0;
						if (who.GetName() != nullptr && szFuncName && szFuncName[0])
							attempted_len = snprintf(szAlertText, kBuf_size, (const char*) szMessage1, (const char*) who, szFuncName, szMsg);
						else if (who.GetName() != nullptr)
							attempted_len = snprintf(szAlertText, kBuf_size, (const char*) szMessage2, (const char*) who, szMsg );
						else if (szFuncName && szFuncName[0])
							attempted_len = snprintf(szAlertText, kBuf_size, (const char*) szMessage2, szFuncName, szMsg );
						else
							attempted_len = snprintf(szAlertText, kBuf_size, (const char*) szMessage3, szMsg );
                        
						DSTOPIF(attempted_len > kBuf_size, (who, "Assertion message is to big. It has been truncated."));
...
Sint32 _AssertValidHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, ConstHandle h, size_t lCount)
{
	ValidateParameters vp;
	Sint32 result = 0L;
...
Bool8 _VerifyValidHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, ConstHandle h, size_t lCount)
{
...
{
	static Sint32 attempted_len = 0;
	char buf[kBuf_size];

	#ifdef _WINDOWS
		if (message)
            attempted_len = snprintf(buf, kBuf_size, "%s\r  file %s && line %d\r", message, file, line);
		else
            attempted_len = snprintf(buf, kBuf_size, "  file %s && line %d\r", file, line);
	#else
		if (message)
            attempted_len = snprintf(buf, kBuf_size, "%s\n  file %s && line %d\n", message, file, line);
		else
            attempted_len = snprintf(buf, kBuf_size, "  file %s && line %d\n", file, line);
	#endif
	#if TEST
	if (attempted_len > kBuf_size-1)
		DSTOP((toWhom, "Assertion message was too big; it has been truncated."));
	#endif
...
Sint32 _FileDebugMessage(const TProgrammer& toWhom, const char *file, Sint32 line, const char *message)
{
	...
	if (!toWhom.IsActLikeRelease()) {
		...
		Sint32 attempted_len = 0;
		if (toWhom.GetName() != nullptr && message != nullptr)
            attempted_len = snprintf(szMsg, kBuf_size, (const char*) szMessage1, (const char*) toWhom, (const char*) message);
		else if (message != nullptr)
            attempted_len = snprintf(szMsg, kBuf_size, (const char*) szMessage2, (const char*) message);
		else if (message == nullptr && toWhom.GetName() != nullptr)
            attempted_len = snprintf(szMsg, kBuf_size, (const char*) szMessage3, (const char*) toWhom);
		else
            attempted_len = snprintf(szMsg, kBuf_size, "%s", (const char*) szMessage4 );
		DSTOPIF(attempted_len > 511, (toWhom, "Assertion message is too big. It has been truncated."));

		// Write to debug window or serial port.		
#if GS_MAC && !Autobuild
		os_log_fault(OS_LOG_DEFAULT, "%s, %s\n", szMsg, szTitle);
#else
		OutputMessage out(true);
		out.SendString(szMsg);
		out.SendString(", ");
		...
		Sint32 inAssertLine, const char * inMessage, ...)
{
	...
	len = vsnprintf(buf, kBufSize, inMessage, args);;
	if( len >= kBufSize )
		DSTOP((inProgrammer, "Assertion message is too big. It has been truncated."));
...
static UserError IsValidHandle(GSConstHandle h, size_t lCount, Bool8 bReadOnly)
...
{
	return ! IsValidHandle((GSConstHandle)ptr, lCount, false).IsError();
}
#endif
...
static Sint32 _SendErrorMessage(const TProgrammer& toWhom, const char* file, Sint32 line, const char* theAssert, const char* itemName, UserError theErr, const void* theData, SintptrT lCount)
{
    ...
    const size_t kReason_size = 512;
	char reason[kReason_size];
	...
	if (specialError) {
		switch (theErr.myErr) {
			...
			case kVPMemErrorReturnedError:	{
				...
				attempted_len = snprintf(reason, kReason_size, "MemError returned error %d\n   %s: (0x%p)->(0x%p)->data", theErr.sysErr, itemName, theData, theMaster); //-V111
				break;
			}
			case kVPHandleBlockTooSmall: {
				size_t theSize = gVWMM->GetHandleSize((ConstHandle) theData);
				attempted_len = snprintf(reason, kReason_size, "Handle %s is too small\n   actual: %" PRIuPTR " bytes  specified: %" PRIdPTR " bytes", itemName, theSize, lCount); //-V111
				break;
			}
#endif
			case kVPPStringTooLong: {
				attempted_len = snprintf(reason, kReason_size, "String max size param (cbMax == %" PRIdPTR ") too big", lCount); //-V111
				break;
			}
			case kVPStringMinSizeTooLong: {
				attempted_len = snprintf(reason, kReason_size, "String min size param (cbMin) too big ( > cbMax (%" PRIdPTR "))", lCount); //-V111
				break;
			}
			case kVPStringBadLength: {
				attempted_len = snprintf(reason, kReason_size, "String length out of bounds (cbMin to cbMax (%" PRIdPTR "))", lCount); //-V111
				break;
			}
			case kVPNegativeCount: {
				attempted_len = snprintf(reason, kReason_size, "Pointer size param (lCount = %" PRIdPTR ") is negative", lCount); //-V111
				break;
			}

			default:
				attempted_len = snprintf(reason, kReason_size, "UNKNOWN ERROR");
		}
	}
		...
		attempted_len = snprintf(reason, kReason_size, "%s\n   %s: (0x%p)->(0x%p)->data", errString, itemName, theData, theMaster); //-V111
	}
	else if (displayPtr) {
		attempted_len = snprintf(reason, kReason_size, "%s\n   %s: (0x%p)->", errString, itemName, theData); //-V111
	}
	else {
		attempted_len = snprintf(reason, kReason_size, "%s", errString);
	}

	if (attempted_len > kReason_size - 1) // size of reason buffer - 1
		DSTOP((kPaul, "_SendErrorMessage -  reason message is too big. It has been truncated."));

	attempted_len = snprintf(buf, kBuf_size, "%s(%s) failed!\n%s", theAssert, itemName, reason);
	if (attempted_len > kBuf_size-1)
		DSTOP((kPaul, "_SendErrorMessage -  buf message is too big. It has been truncated."));
...
//---------------------------------------------------------------------------------------
Bool8 _VerifyValidHandle(GSConstHandle h, size_t lCount)
{
	return !IsValidHandle(h, lCount, false).IsError(false);
}

//---------------------------------------------------------------------------------------
Sint32 _AssertValidHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, GSConstHandle h, size_t lCount)
{
	if (toWhom.IsActLikeRelease()) {
...
//---------------------------------------------------------------------------------------
Bool8 _VerifyValidHandle(const TProgrammer& toWhom, const char* file, Sint32 line, const char* itemName, GSConstHandle h, size_t lCount)
{
	if (toWhom.IsActLikeRelease()) {
...
//---------------------------------------------------------------------------------------
Sint32 _AssertValidHandle(GSConstHandle h, size_t lCount)
{
...
Sint32 _AssertValidHandle(ConstHandle h, size_t lCount)
{
	ValidateParameters vp;
	Sint32 result = 0L;
