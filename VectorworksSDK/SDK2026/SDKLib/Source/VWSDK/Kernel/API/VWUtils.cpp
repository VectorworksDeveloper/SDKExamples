...
//
{
	...
	CFBooleanRef localeIsMetric = (CFBooleanRef)CFLocaleGetValue(locale, kCFLocaleUsesMetricSystem);
	isMetric = CFBooleanGetValue(localeIsMetric);
	
	CFRelease(localeIsMetric);
	CFRelease(locale);
#endif
	return isMetric;
}
		...
		if(!fLogFile){
			TFolderIdentifier logFolder;
				if ( GetAppDataFolder(logFolder) ) {
					...
					#if GS_MAC
					TXString logFileName = logFile.GetPosixFilePath();
					#else
					TXString logFileName = logFile.GetFilePath();
					#endif

					fLogFile = vw_fopen(logFileName, "a+");
				}
		}
        if(fLogFile) {
...
rapidjson::Document GetMetaDataDocument(Uint16 level) {
	...
	std::string serialNumber = TAppProtection::GetMaskedSerialNumberCache().GetStdString();
...
Sint32 LegacyLocalLogTiming(Uint16 level, double timing, const char *format, ...)
{
	...
	snprintf(modifiedFormat, kBuf_size, "TIMER: %f sec | %s", timing, format);
...
TLogTimer::TLogTimer(Uint16 level, const char* context, const char* category, double logThreshhold)
	: fLevel(level), fCategory(category), fContext(context), fLogThreshhold(logThreshhold),
	timer(std::make_unique<TFractionalTimer>())
{
}
...
TXString TSha256Hash::GetAsTXString() const
{
	...
	for (size_t i = 0; i < 32; i++) {
		snprintf(s, sizeof(s), "%02x", fHashData[i]);
		outStr += s;
	}
