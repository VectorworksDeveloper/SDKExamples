...
//=======================================================================================
// Move constructor
TXString::TXString(TXString&& src) noexcept
...
TXString::TXString(StdUStr&& src) noexcept
...
TXString::TXString(const std::string_view& sv, ETXEncoding e /*= ETXEncoding::eUTF8*/)
	: charPtr(nullptr)
	, charBufSize(0)
{
	if (sv.empty())
	...
	SetStdUStrFromCharBuffer(sv.data(), sv.size(), e);
}
	...
	, charBufSize(0)
{
	...
	static_assert(sizeof(char16_t) == sizeof(UniChar));
	CFStringGetCharacters(src, CFRangeMake(0, len), reinterpret_cast<UniChar *>(&stdUStr[0]));
}
...
//=======================================================================================
// Construct from a char16_t buffer.
TXString::TXString(const UniChar* src)
	: charPtr(nullptr)
	, charBufSize(0)
	, stdUStr(src ? (const TXChar *)src : txu(""))
{
...
//=======================================================================================
// Construct from an untrusted UniChar buffer, which may or may not be terminated.
// maxUCChars is typically the maximum size of the buffer, but could be the string length.
// If eLengthCheck is eYes, assume the worst and search for size of the string.
// If eLengthCheck is eNo, assume the string is exactly max in length.
TXString::TXString(const UniChar* src, size_t maxUCChars, ELengthCheck eLengthCheck)
	: charPtr(nullptr)
	, charBufSize(0)
	, stdUStr(
		src && eLengthCheck == ELengthCheck::eNo ? (const TXChar *)src : txu(""),
...
//=======================================================================================
// Fill the string with "count" number of "ch"
TXString::TXString(UniChar ch, size_t count /* 1 */)
	: stdUStr(count, (const TXChar)ch)
	, charPtr(nullptr)
	, charBufSize(0)
{
}
...
TXString& TXString::operator=(TXString&& src) noexcept
{
	stdUStr = std::move(src.stdUStr);
	return *this;
}
...
//=======================================================================================
// Assignment operator from std::u16string
TXString& TXString::operator=(const std::u16string& src)
{
	stdUStr = (const TXChar *)src.data();
	return *this;
}
...
TXString& TXString::operator=(const std::string_view& sv)
{
	if (sv.empty())
	{
		stdUStr.clear();
	...
	SetStdUStrFromCharBuffer(sv.data(), sv.size());
	return *this;
}
...
//=======================================================================================
// Assignment from a const UniChar buffer. If 'src' is nullptr, the string is cleared.
TXString& TXString::operator=(const UniChar* src)
{
	if (src)
	{
		stdUStr = (const TXChar *)src;
...
TXString& TXString::operator=(const UCChar ch)
{
	stdUStr = (const TXChar) ch;
	return *this;
}

//=======================================================================================
// Assignment from a UniChar
TXString& TXString::operator=(const UniChar ch)
{
	stdUStr = (const TXChar)ch;
	return *this;
}
...
TXString& TXString::operator=(const wchar_t w)
{
#if GS_WIN
	stdUStr = w;
#else
	if((int) w <= 0xffff)
	{
		stdUStr = (const TXChar) w;
	}
	else
	{
		UCChar u[3];
		utf32ToTXCharBuffer((const char32_t)w, u);
		stdUStr = u;
	}
#endif
...
TXString& TXString::operator=(int n)
{
	...
	else
	{
		if(n <= 0xffff)
		{
			stdUStr = (const TXChar) n;
		}
		else
		{
			TXChar u[3];
			utf32ToTXCharBuffer((const char32_t)n, u);
			stdUStr = u;
		}
	}
...
//=======================================================================================
// Concatenate const char16_t buffer. If 'src' is nullptr, does nothing.
TXString& TXString::operator+=(const UniChar* src)
{
	if (src != nullptr)
	{
...
TXString& TXString::operator+=(const char ch)
{
	stdUStr += (const unsigned char)ch;	// The casting is necessary on Windows.
	return *this;
}
...
TXString& TXString::operator+=(const int n)
{
	if(n >= 0 && n <= 0x10ffff)
	{
		if(n <= 0xffff)
...
//=======================================================================================
// Concatenates UniChar buffer. If 'src' is nullptr, does nothing.
TXString& TXString::operator<<(const UniChar* src)
{
	return (*this += src);
}
...
{
	return (*this += TXString::ToStringReal(number));
}
...
//=======================================================================================
// Insert UniChar buffer at position 'pos'. If pos is larger than length, the new
// string is append to the end.
TXString& TXString::Insert(size_t pos, const UniChar* src)
{
	if (src != nullptr)
	{
		if (pos < stdUStr.length())
...
TXString& TXString::Insert(size_t pos, const char ch)
{
	if(pos < stdUStr.length())
	{
		stdUStr.insert(pos, 1, (const unsigned char) ch);	// The casting is necessary.
	}
	else
	{
		stdUStr.append(1, (const unsigned char) ch);
	}
...
// Insert one character 'ch' at position 'pos'. If pos is larger than length, the new
// character is append to the end.
TXString& TXString::Insert(size_t pos, const unsigned char ch)
{
	if(pos < stdUStr.length())
...
// Insert one character 'ch' at position 'pos'. If pos is larger than length, the new
// character is append to the end.
TXString& TXString::Insert(size_t pos, const UCChar ch)
{
	if(pos < stdUStr.length())
		stdUStr.insert(pos, 1, (const TXChar) ch);
	else
		stdUStr.append(1, (const TXChar) ch);
...
// Insert one character 'ch' at position 'pos'. If pos is larger than length, the new
// character is append to the end.
TXString& TXString::Insert(size_t pos, const UniChar ch)
{
	if (pos < stdUStr.length())
		stdUStr.insert(pos, 1, (const TXChar)ch);
	else
...
// Insert one character 'ch' at position 'pos'. If pos is larger than length, the new
// character is append to the end.
TXString& TXString::Insert(size_t pos, const wchar_t w)
{
#if GS_WIN
	if(pos < stdUStr.length())
	{
	...
	if((int) w <= 0xffff)
	{
		if(pos < stdUStr.length())
		{
			stdUStr.insert(pos, 1, (const TXChar) w);
		}
		else
		{
			stdUStr.append(1, (const TXChar) w);
		}
	}
	else
	{
...
// Insert one character 'ch' at position 'pos'. If pos is larger than length, the new
// character is append to the end.
TXString& TXString::Insert(size_t pos, const int n)
{
	if(n <= 0xffff)
	{
		if(pos < stdUStr.length())
...
TXString& TXString::Replace(const TXString& oldStr, const TXString& newStr, bool bIgnoreCase /* false */)
{
	...
	if(oldStr.IsEmpty() != true)
	{
		...
		{
			StdUStr::const_iterator current = stdUStr.begin();
			StdUStr::const_iterator end     = stdUStr.end();
			StdUStr::const_iterator next    = std::search(current, end, oldStr.stdUStr.begin(), oldStr.stdUStr.end());
			
			if (next != end) {
				StdUStr buffer;
				while (next != end) {
					buffer.append(current, next);
					buffer.append(newStr);
					current = next + oldStr.stdUStr.size();
					next = std::search(current, end, oldStr.stdUStr.begin(), oldStr.stdUStr.end());
				}
				buffer.append(current, next);
				stdUStr = std::move(buffer);
			}
		}
	}
...
TXString& TXString::MakeUpper()
{
	...
	if (cfUniStr) 
	{
		...
		CFStringAppendCharacters(cfUniStr, reinterpret_cast<UniChar *>(stdUStr.data()), stdUStr.size());
...
// It can generated a longer string when the ß converts to SS
// Windows offers a LCMapStringEx(LOCALE_NAME_INVARIANT) method, but it does not work the same as Mac
// The 'previousChar' is the character BEFORE the string, to detect if the string starts in the middle of a word.
TXString& TXString::MakeCapitalized(TXChar previousChar)
{
#if GS_WIN
	bool capNext = previousChar==0 || iswspace(previousChar) || iswpunct(previousChar);
	for (size_t i = 0; i < (int)GetLength(); i++)
	{
		TXChar curChar[2] = { GetAt(i), 0 };

		if (iswspace(curChar[0]) || iswpunct(curChar[0]))
		{
			capNext = true;
		}
		else if (capNext)
	...
	if (cfUniStr)
	{
		CFIndex skipLength = 0;
		if (previousChar)
		{
			// Find out the length of the first capitalized character
			CFStringAppendCharacters(cfUniStr, reinterpret_cast<UniChar *>(&previousChar), 1);
			CFStringCapitalize(cfUniStr, nil);
			skipLength = CFStringGetLength(cfUniStr);
			CFStringDelete(cfUniStr, CFRangeMake(0, skipLength));
			CFStringAppendCharacters(cfUniStr, reinterpret_cast<UniChar *>(&previousChar), 1);
		}
...
{
#if GS_WIN
	// If true, the string needs to be converted to the precomposed form.
	if(!IsNormalizedString(NormalizationC, stdUStr.data(), (int) stdUStr.length()))
	{	
		// Store the original string.
		StdUStr original = stdUStr;

		// Get the estimated size for the precomposed string.
		int len = NormalizeString(NormalizationC, original.data(), (int)original.length(), NULL, 0);

		// If len is greater than 0, the string can be converted to the precomposed form.
		if (VERIFYN(kPChang, len > 0))
		{
			// Reserve enough buffer to store the precomposed string.
			stdUStr.resize(len);

			// Normalize the string to the precomposed form.
			len = NormalizeString(NormalizationC, original.data(), (int)original.length(), &stdUStr[0], (int)stdUStr.size());

			// If there is any error, restore the original string.
			if (len <= 0)
			{
				stdUStr = original;
			}
...
#if BUG
		// If len is less than or equal to 0, it means the conversion failed.
		// Report the error.
		if (len <= 0)
		{
			DWORD dwError = GetLastError();
			DSTOP((kPChang, "TXString::ToPrecomposed() failed with error code %d", dwError));
		}
#endif
	}
	
#else	// Mac
	NSString *oriStr = [NSString stringWithCharacters:reinterpret_cast<const UniChar*>(stdUStr.data()) length:stdUStr.length()];
	...
	[precomposedStr getCharacters:reinterpret_cast<UniChar *>(&stdUStr[0])];
...
{
#if GS_WIN
	// If true, the string is not in the decomposed form.
	if(!IsNormalizedString(NormalizationD, stdUStr.data(), (int)stdUStr.length()))
	{	
		// Store the original string.
		StdUStr original = stdUStr;

		// Get the estimated size for the decomposed string.
		int len = NormalizeString(NormalizationD, original.data(), (int)original.length(), NULL, 0);

		// If len is greater than 0, the string can be converted to the decomposed form.
		if (VERIFYN(kPChang, len > 0))
		{
			// Reserve enough buffer to store the decomposed string. 
			stdUStr.resize(len);

			// Normalize the string to the decomposed form.
			len = NormalizeString(NormalizationD, original.data(), (int)original.length(), &stdUStr[0], (int)stdUStr.size());
		...
		{
			DWORD dwError = GetLastError();
			DSTOP((kPChang, "TXString::ToDecomposed() failed with error code %d", dwError));
		}
#endif
	}
	...
	[decomposedStr getCharacters:reinterpret_cast<UniChar *>(&stdUStr[0])];
...
//=======================================================================================
// Returns a pointer to const UniChar buffer.
TXString::operator const UniChar*() const
{
	return reinterpret_cast<const UniChar *>(stdUStr.c_str());
}
...
{
	return CFStringCreateWithCharacters(NULL, *this, GetLength());
}
#endif
...
//=======================================================================================
// Copy string data into 'dst' char16_t buffer. bufElemNum is the number of elements in
// the buffer. '\0' will be added to the end of string even if buffer is not big enough.
void TXString::CopyInto(UniChar* dst, size_t bufElemSize) const
{
	CopyInto((UCChar*)dst, bufElemSize);
}
...
//=======================================================================================
// Sets the current string to the input value
TXString& TXString::ftoa(Real64 value, Sint32 precision/*=6*/)
{
	return (*this = TXString::ToStringReal( value, precision, true ));
}
...
//=======================================================================================
/*static*/ TXString TXString::ToStringReal(Real64 value, Sint32 precision/*=6*/, bool fixed/*=true*/)
{
	// ToStringReal(Real64 value)
	//    return std::to_string(value);
	// pre C++26, to_string matches printf %f which default to 6 digits.
	//    e.g. "23.430000" for 23.43 or "1.234568" for 1.23456789555555 or "0.000010" for 1e-5
	// post C++26 it matches std::format {} which uses 'shortest length' so precision may vary
	// and it might even be exponential instead of fixed-point!
	//    e.g. "23.43" for 23.43 or "1.23456789555555" for 1.23456789555555 or "1e-5" for 1e-5
	// Lets explicitly peg our implementation to the old behavior before it changes underneath us.
	// I'm switching ftoa(value) and ToStringReal(value) from to_string to this explicit .6 default- RBerge
...
bool TXString::ToDouble(const TXString& inString, double& outDouble, bool useOSLocale /* = false */)
{
    ...
    if (useOSLocale) {
        ...
        if (VERIFYN(kSWilliams, inString.GetData())) {
			// Safe to const-cast since we're not modifying the string. This allows us to compare without copying.
			NSString* string = [[NSString alloc] initWithCharactersNoCopy:const_cast<unichar *>(inString.GetUniCharPtr()) length:inString.GetLength() freeWhenDone:NO];
...
#else
	CFStringRef cs1 = CFStringCreateWithCharactersNoCopy(kCFAllocatorDefault, *this, GetLength(), kCFAllocatorNull);
	CFStringRef cs2 = CFStringCreateWithCharactersNoCopy(kCFAllocatorDefault, str, str.GetLength(), kCFAllocatorNull);
...
void TXString::PrepareCharBuffer(ETXEncoding encoding) const
{
    ...
    else
    {
        ...
        CFStringRef cfStr = CFStringCreateWithCharactersNoCopy(kCFAllocatorDefault,
															   reinterpret_cast<const UniChar *>(stdUStr.data()),
															   stdUStr.length(),
															   kCFAllocatorNull);
...
void TXString::SetStdUStrFromCharBuffer(const char* src, size_t strLen, ETXEncoding encoding)
{
	...
	else
	{
		...
		if (cfStr)
		{
			...
			CFStringGetCharacters(cfStr, cfStrRange, reinterpret_cast<UniChar *>(&stdUStr[0]));
...
// Comparison operator
// '==' comparison operator. This comparison is case insensitive.
bool operator==(const TXStringArray& lhs, const TXStringArray& rhs)
{
	if (lhs.strVec.size() != rhs.strVec.size())
		return false;
	
	bool same = true;
	for (size_t i = 0; i < lhs.strVec.size() && same; i++) {
		same = lhs.strVec[i] == rhs.strVec[i];
	}
	return same;
}
...
TXResource::operator const TXString&()
{
	if ( fResource && fString.IsEmpty() )
	{
		if ( VERIFYN( kVStanev, fResource && *fResource ) )
		{
			...
			fString = size > 0 ? TXString( (const char*) *fResource, size ) : TXString();
		}
	}
				   ...
				   const char* resourceStringIdentifier, EEmptyHandling allowEmpty)
{
	...
	{
		resourceIdentifier.Format("%s/Strings/%s", DEFAULTVWRIDENTIFIER, fileName);
#if _MINICAD_
		::GetResourceString(fString, resourceIdentifier, resourceStringIdentifier, allowEmpty);
#else
		gSDK->GetVWResourceString(fString, resourceIdentifier, resourceStringIdentifier, allowEmpty);
#endif
	}
	...
	{
		resourceIdentifier.Format("%s/Strings/%s.vwstrings", DEFAULTVWRIDENTIFIER, fileName);
#if _MINICAD_
		::GetResourceString(fString, resourceIdentifier, resourceStringIdentifier, allowEmpty);
#else
		gSDK->GetVWResourceString(fString, resourceIdentifier, resourceStringIdentifier, allowEmpty);
	...
	{
#if _MINICAD_
		::GetResourceString(fString, fileName, resourceStringIdentifier, allowEmpty);
#else
...
TXResStr::operator const TXString&() const
{
	return fString;
}

// --------------------------------------------------------------------------------------------------
// Note - it's important that this not depend on gSDK for calls inside VW because this function
// is called at early initialization states before gSDK is set up.
//
...
#if _MINICAD_
	::GetResourceString(fString, resourceIdentifier, stringIdentifier, eAllowEmptyResult);
#else
...
void TXLegacyResource::CacheString()
{
	if (fResource && fString.IsEmpty())
	{
		if (VERIFYN(kVStanev, fResource && *fResource))
		{
			size_t	size;
			gSDK->GSGetHandleSize(fResource, size);
			fString = size > 0 ? TXString((const char*)*fResource, size, ETXEncoding::eMacEncoded) : TXString();
		}
	}

}
TXLegacyResource::operator TXString() 
{
	CacheString();
	return fString;
}
...
constexpr std::array<TXChar, 18> mapDelimiters {
	'd', 'i', 'u', 'o', 'x', 'X',
	'f', 'F', 'e', 'E', 'g', 'G',
	'a', 'A', 'c', 's', 'p', 'n'
};
...
void swap(TXString& lhs, TXString& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
	lhs.swap(rhs);
}
...
#else
        bool success = false;
		CFStringRef cfStr = CFStringCreateWithCharacters(NULL, reinterpret_cast<const UniChar *>(srcUCArray), srcLen);
        if (cfStr)
        {
                   ...
                   ConstUCCharPtr chars2, size_t len2, bool ignoreCase)
{
	...
	CFStringRef str1 = CFStringCreateWithCharacters(NULL, reinterpret_cast<const UniChar *>(chars1), len1);
	CFStringRef str2 = CFStringCreateWithCharacters(NULL, reinterpret_cast<const UniChar *>(chars2), len2);
...
#else  /* Macintosh */
    CFStringRef cfs = CFStringCreateWithCharacters(NULL, reinterpret_cast<const UniChar *>(charArray), length);
...
CFComparisonResult COSStringCompare::Compare(const CFMutableStringRef cfStr1, const CFMutableStringRef cfStr2)
{
    ...
    compareResult = CFStringCompare(cfStr1, cfStr2,
                                       kCFCompareCaseInsensitive | kCFCompareDiacriticInsensitive | kCFCompareNumerically | kCFCompareLocalized);
        
    if (compareResult == kCFCompareEqualTo)
        compareResult = CFStringCompare(cfStr1, cfStr2, kCFCompareCaseInsensitive | kCFCompareNumerically | kCFCompareLocalized);
        
    if (compareResult == kCFCompareEqualTo)
        compareResult = CFStringCompare(cfStr1, cfStr2, kCFCompareCaseInsensitive | kCFCompareLocalized);
...
#else
	NSString* str3 = [NSString stringWithCharacters:reinterpret_cast<const UniChar*>(string1.GetData()) length:string1.GetLength()];
	NSString* str4 = [NSString stringWithCharacters:reinterpret_cast<const UniChar*>(string2.GetData()) length:string2.GetLength()];
