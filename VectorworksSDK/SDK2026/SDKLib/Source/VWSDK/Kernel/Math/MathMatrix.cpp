...
#if BUG
//////////////////////////////////////////////////
void BugVerifyTransformMatrix(const TransformMatrix& matrix, bool allow2DOrtho)
//
...
//
// Parameters:
//   matrix - in - the matrix to verify
//   allow2DOrtho - in - if true, only the upper 2x2
//    	section of the matrix is checked for orthogonality.
//
{
	static bool oldMatch = false;
	static TransformMatrix oldMatchMatrix;
	if ( !oldMatch || !(matrix == oldMatchMatrix) ) {
		ASSERTN(kAndrewB, matrix.IsOrthogonal(allow2DOrtho));
...
////////////////////////////////////////////////////////////
const GS_API TransformMatrix& ZeroMatrix()
//
// This function returns a const reference to a zero
// matrix (a singleton).
//
{
	static bool setToZero = false;
	static TransformMatrix zMatrix; // static, so it lasts until shutdown
	if (!setToZero) {
		zMatrix.SetToIdentity();
		zMatrix.v1.a00 = zMatrix.v1.a11 = zMatrix.v1.a22 = 0.0;
		setToZero = true;
	}
	return zMatrix;
}
...
//
{
	...
	VerifyTransformMatrix(mat, true); // true = allow a 2D "ortho" transform
