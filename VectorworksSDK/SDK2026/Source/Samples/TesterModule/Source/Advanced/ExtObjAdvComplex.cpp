...
EObjectEvent CObjAdvComplex_EventSink::OnInitXProperties(CodeRefID objectID)
{
	...
	extProps->SetObjectProperty( objectID, kObjXPropDisableVelumOpacity, true );
...
EObjectEvent CObjAdvComplex_EventSink::Recalculate()
{
	...
	if ( hProxyParent == NULL )
	{
		...
		// kObjXPropDisableVelumOpacity - Requires setting the opacity for objects during reset.
		OpacityRef tmpFillOpacity = kMaxOpacity, tmpPenOpacity = kMaxOpacity;
		gSDK->GetOpacityN(fhObject, tmpFillOpacity, tmpPenOpacity);
		gSDK->SetOpacityN(poly2D, tmpFillOpacity, tmpPenOpacity);
