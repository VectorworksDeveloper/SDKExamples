...
#if _WINDOWS
	#include "../../Include/Kernel/QuickTimeWinRemovalFlag.h"
#endif
...
public:
	...
	bool PointerBelongsToUs(const void* mem);						// return true if the memory beloings to use. This is fairly expensive.
	bool HandleBelongsToUs(void* handle);
	bool PointerIsValid(void* mem, bool checkForOwnage);	// return true if the pointer is valid
...
#ifndef _WINDOWS
	// Declare Mac calls for thread safety
	void EnterCriticalSection(MPCriticalRegionID *id);
	void LeaveCriticalSection(MPCriticalRegionID *id);
