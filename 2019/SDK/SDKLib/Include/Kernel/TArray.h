...
//
//
// Graphsoft standard dynamic array class
//
...
#if _MINICAD_
void Mem_Move(void *pDest, const void *pSrc, size_t nBytes);
#endif
			...
			if (nItems + numCells > this->fNumAllocItems) {
				...
				switch (this->fGrowthMethod) {
					...
					case eArithmeticGrowth:
						neededSize = this->fDataSize * (nItems + numCells + (kBuffSize - (nItems + numCells) % kBuffSize)); //-V104
						break;				
						
					case eGeometricGrowth:
					{
						// 20% with 10 minimum [MAF 10/20/00] //-V201
						size_t itemsCnt	= (size_t) ((float)this->fNumAllocItems * 1.2f);
						if ( itemsCnt < (size_t)(nItems + 10) )
						{
							itemsCnt = (size_t)(nItems + 10);
						}

						// If there is not enough allocated space.
					...
					case eExponentialGrowth:
						neededSize = this->fDataSize * ((this->fNumAllocItems) * (this->fNumAllocItems)); //-V104
						break;
						
					case eOneAtATimeGrowth:
						neededSize = fDataSize * (nItems + numCells); //-V104
						break;
		...
		if (std::numeric_limits<TIndexType>::max() - nItems >= numCells) {
			if (nItems + numCells > fData.fNumAllocItems) {
				...
				switch (fData.fGrowthMethod) {
					...
					case eArithmeticGrowth:
						neededSize = fData.fDataSize * (nItems + numCells + (TXMemoryRawArray<TIndexType>::kBuffSize - (nItems + numCells) % TXMemoryRawArray<TIndexType>::kBuffSize)); //-V104
						break;				
						
					case eGeometricGrowth:
					{
						// 20% with 10 minimum [MAF 10/20/00] //-V201
						size_t itemsCnt	= (size_t) ((float)fData.fNumAllocItems * 1.2f);
						if ( itemsCnt < (size_t)(nItems + 10) )
						{
							itemsCnt = (size_t)(nItems + 10);
						}
					...
					case eExponentialGrowth:
						neededSize = fData.fDataSize * ((fData.fNumAllocItems) * (fData.fNumAllocItems)); //-V104
						break;
						
					case eOneAtATimeGrowth:
						neededSize = fData.fDataSize * (nItems + numCells); //-V104
						break;
...
//
{
	...
	if (fData.fDataHandle) {
			...
			((numItems / TXMemoryRawArray<TIndexType>::kBuffSize) != (numItems - numCells) / TXMemoryRawArray<TIndexType>::kBuffSize)) 
		{
			...
			ASSERTN(kSean, numAllocCells >= (numItems - numCells));
			ASSERTN(kSean, numAllocCells == (newHandleSize - fData.fDataOffset) / fData.fDataSize); //-V104
