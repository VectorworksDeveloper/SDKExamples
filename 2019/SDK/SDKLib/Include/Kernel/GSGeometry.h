...
//
//
//  This file contains geometry types for the GraphSoft core.
//  ViewPt and ViewRect are in here.
//

#ifndef _GS_GEOMETRY_
#define _GS_GEOMETRY_
...
typedef Sint32  ViewCoordLong;
	...
	public:
		...
		Sint32 DistanceFrom(ViewPt otherPoint) const 
		{ 
			Sint32 x = (*this - otherPoint).Magnitude(); 
			return (x > 0) ? x : -x;
		}

 		Sint32 SquaredDistanceFrom(ViewPt otherPoint) const { 
			Sint32 x = ((*this - otherPoint).MagnitudeSquared()); 
			return (x > 0) ? x : -x;
		}
	...
	public:
		...
		{ 
			XViewCoord x = (*this - otherPoint).MagnitudeSquared();
			return (x > 0) ? x : -x;
		}
		...
		{ 
			XViewCoord x = (*this - otherPoint).Magnitude();
			return (x > 0) ? x : -x;
		}
	...
	inline XViewPt operator*(const double scalar, const XViewPt thePt)
		{ return XViewPt(thePt.x * scalar, thePt.y * scalar); }

	inline XViewPt operator/(const XViewPt thePt, const double scalar)
		{ return XViewPt(thePt.x / scalar, thePt.y / scalar); }
	...
	public:
		...
		ViewRect(ViewCoord leftIn, ViewCoord topIn, ViewCoord rightIn, ViewCoord bottomIn) { top = topIn; left = leftIn; right = rightIn; bottom = bottomIn; }
		...
		void Outset(ViewCoord dx, ViewCoord dy)  { left -= dx; right += dx; top -= dy; bottom += dy; }
	...
	public:
		...
		Bool8 operator==(const XViewRect& r) const {	return (top == r.top && left == r.left && bottom == r.bottom && right == r.right); }
		...
		XViewPt Center() const { return XViewPt((left + right) / 2, (top + bottom) / 2); }
		...
		void Union(const XViewRect& src2, XViewRect& dest) const { dest.top = top < src2.top ? top : src2.top; 
																dest.bottom = bottom > src2.bottom ? bottom : src2.bottom; 
																dest.left = left < src2.left ? left : src2.left; 
																dest.right = right > src2.right ? right : src2.right; }
	...
	public:
		...
		GSRect32(const XViewRect& xvr, bool bRound = true);
		...
		Boolean		Intersects(const GSRect32& r) const;
	...
	// XPoly is the floating point version of GSPoly
	// The maximum number of poly vertices allowed in an XPoly
	const Sint32 kMaxXPolyVertices = (INT32_MAX - sizeof(Sint32) - sizeof(XViewRect))/sizeof(XViewPt); 

	struct XPoly {
	// Cross platform polygon
		Sint32		polySize;
		XViewRect	polyBBox;
		XViewPt		polyPoints[1];

		Sint32		GetNumPts() const;
		};
	typedef struct XPoly XPoly, *PXPoly, **PPXPoly;
	...
	public:
		...
		void OutSet( const Sint32& distance);
