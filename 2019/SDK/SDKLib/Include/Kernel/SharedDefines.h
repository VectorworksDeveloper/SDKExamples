...
//
//	This flag enables the Beta Alerts. It should be set to 1 right before we build the final
//  release build. It should be on for Release Candidate builds. When this flag is turned on, 
//  it should only be turned on in the branch to be released. Since we never release Mainline
//  or any .0.x branch, this flag should always be off in .0.x and Mainline.
//
...
#define DISALLOW_DUPLICATES(T) \
	T (const T &);				/* no copying allowed. */ \
	T& operator=(const T &)		/* no assignment allowed, either. */
...
#define POSTDRAWING_NXOR 1
    ...
    // At most 1 of these constants should be set to 1
    // If all are 0, you get the standard developer build.
	#define SERIES_B_DEVELOPER 0
	#define SERIES_E_DEVELOPER 0
	#define SERIES_G_DEVELOPER 0
