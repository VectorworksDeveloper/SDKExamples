...
Boolean RadiansAreNearlyEqual(Radians angle1, Radians angle2)
{
	...
	if(VERIFYN(kAyo, !::IsIllegal(angle1) && !::IsIllegal(angle2))) {
		...
		if (Abs(angle1 - kTwoPi) <= kNearlyEqualEpsilonForRadians)
			angle1 = 0;
		if (Abs(angle2 - kTwoPi) <= kNearlyEqualEpsilonForRadians)
			angle2 = 0;

		if (Abs(angle1 - angle2) <= kNearlyEqualEpsilonForRadians)
			areEqual = true;
	}
