...
{
	size_t		vlen		= m_arrValues.size();
	for(size_t i=0; i<vlen; i++) {
		delete m_arrValues[i];
	}
	m_arrValues.clear();

	size_t		glen		= m_arrChildGroups.size();
	for(size_t i=0; i<glen; i++) {
		delete m_arrChildGroups[i];
	}
...
VWDataSerializerGroup* VWDataSerializerGroup::FindChildDataGroup(TDataIdentifier id, const char* szGroupName) const
{
	...
	size_t					len			= m_arrChildGroups.size();
	for(size_t i=0; i<len; i++) {
		VWDataSerializerGroup*	pGroup	= m_arrChildGroups[ i ];
		if ( pGroup->m_identifier == id || (szGroupName && pGroup->m_szName == szGroupName) ) {
			pFound		= pGroup;
			break;
		}
	}
...
VWDataSerializerValue VWDataSerializerGroup::GetValue(size_t index, const char* szValueName) const
{
	...
	{
		for(size_t i=0; i<len; i++) {
			const VWDataSerializerValue*	pValue	= m_arrValues[ i ];
	...
	if ( index < len )
		if ( m_arrValues[ index ] != nullptr )
			return * m_arrValues[ index ];
	
	return VWDataSerializerValue( m_context, (Sint32) 0 );
}
...
bool VWDataSerializerGroup::HasValue(size_t index, const char* szValueName) const
{
	...
	if ( index < len )
		return	( m_arrValues[ index ] != nullptr );
...
bool VWDataSerializerGroup::ResetValueString(size_t index, const char* szValueName)
{
	...
	if ( szValueName )
	{
		for(size_t i=0; i<len && !result; i++) {
			VWDataSerializerValue*	pValue	= m_arrValues[ i ];
			const TXString&			name	= pValue->GetName();
			if ( ! name.IsEmpty() && name == szValueName ) {
				// STOP!
	...
	if ( !result && index < len )
	{
		VWDataSerializerValue*	pValue	= m_arrValues[ index ];
		pValue->SetValue( pValue->GetTXStringValue() );
		result = true;
	}
...
VWDataSerializerGroup* VWDataSerializerGroup::GetChildGroup(TDataIdentifier id, const char* szGroupName)
{
	...
	size_t					len			= m_arrChildGroups.size();
	for(size_t i=0; i<len; i++) {
		VWDataSerializerGroup*	pChild	= m_arrChildGroups[ i ];
		if ( pChild->GetIdentifier() == id || ( szGroupName && pChild->GetName() == szGroupName ) ) {
			pFound		= pChild;
			break;
		}
	}
