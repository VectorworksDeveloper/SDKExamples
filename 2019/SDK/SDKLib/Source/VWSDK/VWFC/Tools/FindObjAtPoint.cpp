...
void CFindObjectAtPoint::Search(EFindObjectOptions objOpt, EFindObjectTraversalOptions travOpt, const VWPoint2D& pt, double dRadius)
{
...
bool CFindObjectAtPoint::CheckObject(MCObjectHandle hObject, const VWPoint3D& pt, double dRadius)
{
	...
	if( !(refId == kPlanarRefID_ScreenPlane && isPlanarObj && fIs3DSearch) )
	{
		...
		else if( VWLocus3DObj::IsLocus3DObject( hObject ) )
		{
			...
			// enlarge with the radius
			worldBounds.Outset( dRadius, dRadius );
			if ( !worldCube.IsEmpty() ) 
			{	
				worldCube.left		-= dRadius;		worldCube.right += dRadius; 
				worldCube.bottom	-= dRadius;		worldCube.top	+= dRadius; 
				worldCube.back		-= dRadius;		worldCube.front += dRadius;
			}
