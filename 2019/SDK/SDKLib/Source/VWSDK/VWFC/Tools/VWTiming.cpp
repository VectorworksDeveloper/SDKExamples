...
void VWTiming::Initialize()
{
	VWFC_ASSERT( spInstance == NULL );
	if ( spInstance == NULL )
		spInstance	= NEW_OBJECT( VWTiming );
}
...
void VWTiming::Deinitilalize()
{
	VWFC_ASSERT( spInstance != NULL );
	if ( spInstance != NULL )
		DELETE_OBJECT( VWTiming, spInstance );
}
		...
		public:
			...
			~CTimingResultsDialog()
			{
				...
				for(size_t i=0; i<len; i++) {
					...
					DELETE_OBJECT( VWTabPaneCtrl, cat.fpTabPaneCtrl );
					DELETE_OBJECT( VWListBrowserCtrl, cat.fpListBoxCtrl );
				}
			}
			...
			virtual bool CreateDialogLayout()
			{
				...
				for(size_t i=0; i<len; i++) {
					...
					dlgCat.fpTabPaneCtrl		= NEW_OBJECT( VWTabPaneCtrl )((Sint32)  ctrlID++ ); 
					if ( dlgCat.fpTabPaneCtrl && ! dlgCat.fpTabPaneCtrl->CreateControl( this, cat.category ) )	return false;

					dlgCat.fpListBoxCtrl		= NEW_OBJECT( VWListBrowserCtrl )((Sint32)  ctrlID++ ); 
			...
			virtual void OnSetUpEvent()
			{
				...
				for(size_t i=0; i<len; i++) {
					...
					size_t	len			= timeCat.arrTimerData.size();
					...
					for(size_t i=0; i<len; i++) {
						const VWTiming::STimerCollected&	timer	= timeCat.arrTimerData[ i ];
						if ( timer.isValid ) {
