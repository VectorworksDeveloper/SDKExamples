...
{
	TXString	strThisResID( fResNameWithoutPathOrExtension );
	TXString	strThisStrID( fStringIdentifier );
	TXString	strOtherResID( other.fResNameWithoutPathOrExtension );
	TXString	strOtherStrID( other.fStringIdentifier );

	if ( strThisResID == strOtherResID ) {
		return strThisStrID < strOtherStrID;
	}
	else {
		return strThisResID < strOtherResID;
	}
}
...
VWExtensionParametric::VWExtensionParametric(CallBackPtr cbp,
	...
	, farrParamDefs			( NULL )
	, farrParamChcs			( NULL )
	, fParametersSink( NULL )
	, fParametricEventSink( NULL )
...
IEventSink* VWExtensionParametric::QueryEventSink(const TSinkIID& iid)
{
	...
	if ( IID_ParametricParamsProvider == iid ) {
		if ( fParametersSink == NULL ) {
...
{
	ASSERTN( kVStanev, paramIndex < fParent->fParamsCount );
	if ( fParent->farrParamDefs_ )
	{
...
void VWExtensionParametric::CParametersProvider::GetParamDefValueImperial(size_t paramIndex, TXString& outValue)
{
	ASSERTN( kVStanev, paramIndex < fParent->fParamsCount );
	if ( fParent->farrParamDefs_ )
	{
		...
		if ( ! fParent->GetLocalizedDefault( false, thisParam.fUniversalName, outValue ) )
		{
			this->GetDefaultValue( thisParam.fDefaultImperial, outValue );
		}
	}
	else
	{
...
void VWExtensionParametric::CParametersProvider::GetParamDefValueMetric(size_t paramIndex, TXString& outValue)
{
	ASSERTN( kVStanev, paramIndex < fParent->fParamsCount );
	if ( fParent->farrParamDefs_ )
	{
		...
		if ( ! fParent->GetLocalizedDefault( true, thisParam.fUniversalName, outValue ) )
		{
			this->GetDefaultValue( thisParam.fDefaultMetric, outValue );
		}
	}
	else
	{
...
Sint32 VWParametric_EventSink::Execute(ParametricMessage* message)
{
	...
	switch ( message->fAction )
	{
		...
		case ParametricCurosrMessage::kAction_Draw:
		case ParametricCurosrMessage::kAction_MouseDown:
		case ParametricCurosrMessage::kAction_Complete:
		case ParametricCurosrMessage::kAction_Cancel: 
		case ParametricCurosrMessage::kAction_MouseMove:
