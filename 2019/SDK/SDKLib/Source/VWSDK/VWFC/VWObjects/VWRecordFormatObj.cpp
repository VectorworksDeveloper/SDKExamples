...
VCOMPtr<IEventSink> VWRecordFormatObj::GetParametricEventSink(const TSinkIID& sindIID)
{
	...
	if ( ! this->IsIfc() )
	{
		...
		if ( fh.IsParametric() )
		{
			const TXString& name = fh.GetNameOfFormat();
			VCOMPtr<IExtendedProps>	extProps( IID_ExtendedProps );
			result = extProps->QueryExtensionEventSink( name, sindIID );
		}
	}
...
size_t VWRecordFormatObj::PopupGetChoicesCount(const TXString& univParamName)
{
	size_t result = 0;
...
void VWRecordFormatObj::PopupGetChoice(const TXString& univParamName, size_t index, TXString& outKey, TXString& outValue)
{
	VWFC_ASSERT( fhObject );
	if ( VWFC_VERIFY( ! this->IsIfc() ) )
	{
...
void VWRecordFormatObj::PopupGetChoices(const TXString& univParamName, std::vector<std::pair<TXString, TXString>>& outKeyValuePairs)
{
...
size_t VWRecordFormatObj::PopupGetChoicesCount(size_t paramIndex)
{
	size_t result = 0;
...
void VWRecordFormatObj::PopupGetChoice(size_t paramIndex, size_t index, TXString& outKey, TXString& outValue)
{
	VWFC_ASSERT( fhObject );
	if ( VWFC_VERIFY( ! this->IsIfc() ) )
	{
...
{
	return ::GS_GetNamedObject( gCBP, name );
}
