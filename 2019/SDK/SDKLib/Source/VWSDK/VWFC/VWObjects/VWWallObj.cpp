...
VWWallObj::VWWallObj(MCObjectHandle h)
{
	...
	VWFC_ASSERT( type == kWallNode || type == kRoundWallNode);
	if ( type != kWallNode && type != kRoundWallNode )
...
VWArc2D VWWallObj::GetRoundWall(bool negativeSweepForCWWall) const
{
	...
	VWFC_ASSERT( fhObject );
	short type	= gSDK->GetObjectTypeN( fhObject );
	VWFC_ASSERT( type == kRoundWallNode );
	if ( type == kRoundWallNode ) {
		WorldPt			cenPt;
		double			radius;
...
SWallComponentInfo VWWallObj::GetComponentInfo(size_t index) const
{
	...
	info.fillColorFore.SetColor( (Uint8) fillFore );
	info.fillColorBack.SetColor( (Uint8) fillBack );
	...
	info.penLeftColorFore.SetColor( (Uint8) penLeftFore );
	info.penLeftColorBack.SetColor( (Uint8) penLeftBack );
	info.penRightColorFore.SetColor( (Uint8) penRightFore );
	info.penRightColorBack.SetColor( (Uint8) penRightBack );
...
bool VWWallObj::InsertNewComponent(size_t beforeIndex, const SWallComponentInfo& info)
{
	...
	return gSDK->InsertNewComponentN(fhObject, (short) beforeIndex, info.width, info.fillPat, (CBSignedByte)info.penLeftWeight, (CBSignedByte)info.penRightWeight, info.penLeftStyle, info.penRightStyle );
}
...
bool VWWallObj::IsWallObject(MCObjectHandle hObj)
{
	...
	return type == kWallNode || type == kRoundWallNode;
}
