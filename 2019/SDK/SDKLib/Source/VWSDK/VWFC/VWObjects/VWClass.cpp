...
TXString VWClass::GetClassName() const
{
	VERIFYN( kVStanev, ::GS_ValidClass( gCBP, fClassIndex ) );
...
TXString VWClass::GetClassDescription()
{
	...
	if ( classDef && ( gSDK->GetObjectType( classDef ) == kClassDefNode ) )
	{
...
{
	short type = gSDK->GetObjectType( h );
...
InternalIndex VWClass::GetClassFromHandle(MCObjectHandle hClass)
{
	...
	short type = gSDK->GetObjectType( hClass );
