...
bool VWPolygon3D::EqualishTo(const VWPolygon3D& poly) const
{
	...
	if ( thisCnt == polyCnt )
	{
		...
		for ( size_t i = 0 ; i < thisCnt ; ++ i )
		{
			...
			{
				const VWPoint3D&	pt1	= this->GetVertexAt( i );
				const VWPoint3D&	pt2	= poly.GetVertexAt( j );

				if ( MathUtils::Equalish( pt1, pt2, VWPoint3D::sEpsilon ) )
				{
					thisSupl		= i;
					polySupl		= j;
					bPosCoincide	= true;
		...
		if ( bPosCoincide && thisCnt > 0 )
		{
			for ( size_t i = 0 ; i < thisCnt ; ++ i )
			{
				...
				// check positions.
				if ( ! MathUtils::Equalish( this->GetVertexAt( thisIndex ), poly.GetVertexAt( polyIndex ), VWPoint3D::sEpsilon ) )
				{
					bEqual	= false;
					break;
				}
