...
void VWPolygon2D::CleanUp_FeaturePreserving(bool doClosenessCheck, double epsCollinear, double epsCloseness)
{
	...
	if ( vertCnt > 2 )			// has at least 3 vertices (could clean it up).
	{
		...
		ptrdiff_t				i			= startFrom;
		while ( i >= endTo )	// loop backwards.
		{
			size_t				thisInx		= i;	// this index.

			size_t				prevInx;	// index of previous poly point.
			if ( isClosed )		prevInx		= ( thisInx < startFrom ) ? ( thisInx + 1 ) : 0;
			else				prevInx		= ( thisInx + 1 );

			size_t				nextInx;	// index of next poly point.
			if ( isClosed )		nextInx		= ( thisInx > 0 ) ? ( thisInx - 1 ) : startFrom;
			...
			const VWPoint2D&	vecToPrev	= ( prevPt - thisPt ).Normalize();
			const VWPoint2D&	vecToNext	= ( nextPt - thisPt ).Normalize();
			...
			if ( doRemove )
			{
				farrVertices.erase( farrVertices.begin() + i );
				-- startFrom;	// update last checkable index.

				VWFC_ASSERT( startFrom != -1 );		// not expected, must not happen.

				// don't allow 'i' be > 'startFrom'.
				if ( thisInx /* i */ > startFrom )
				{
					-- i;
				}
			}
...
bool VWPolygon2D::GetPtOnPoly(double offset, VWPoint2D& outPt) const
{
	...
	// use polygonization to find the point on the poly.

	bool		result		= false;
	double		length		= 0;
	size_t		vertsCnt	= polygonized.GetVertexCount();

	for ( size_t i = 1 ; i < vertsCnt ; ++ i )
	{
		const VWPoint2D&	prevPt		= polygonized.GetVertexAt( i - 1 );
		const VWPoint2D&	thisPt		= polygonized.GetVertexAt( i );
...
bool VWPolygon2D::GetPtOnPoly(double offset, VWPoint2D& outPt, VWPoint2D& outNormal, size_t* outOnSegIndex) const
{
	bool		result		= false;
	double		length		= 0;
	size_t		vertsCnt	= this->GetVertexCount();
	for(size_t i=0; i<vertsCnt;) {
		...
		if ( vertType == eVertexType_Corner ) {
			if ( i > 0 ) {
				...
				const VWPoint2D&	prevPt		= this->GetVertexAt( prevIndex );
				const VWPoint2D&	thisPt		= this->GetVertexAt( i );
				...
				if ( MathUtils::GEish( newLen, offset, VWPoint2D::sEpsilon ) ) {
					...
					if ( atStart ||	atEnd )
					{
						...
						if ( atStart )
						{
							const VWPoint2D&	prevPrevPt	= this->GetVertexAt( (prevIndex > 0) ? (prevIndex - 1) : (vertsCnt - 1) );
						...
						{
							const VWPoint2D&	nextPt	= this->GetVertexAt( (prevIndex > 0) ? (prevIndex - 1) : (vertsCnt - 1) );
							v1							= (thisPt - prevPt).Normalize();
							v2							= (nextPt - thisPt).Normalize();
							ang							= Abs( (prevPt - thisPt).CalcAng180() - (nextPt - thisPt).CalcAng180() );
						}

						if ( ! this->IsClosed() && atStart && prevIndex == 0 )
						{
							outNormal		= - v2.Perp();
						}
						else if ( ! this->IsClosed() && atEnd && prevIndex == vertsCnt - 2 )
						{
							outNormal		= - v1.Perp();
						}
						else
...
VWPoint2D VWPolygon2D::GetClosestPtOnPoly(const VWPoint2D& pt, double& outDist, Sint32& outOnSegIndex) const
{
	...
	else				// poly has at least two points.
	{
		...
		for ( size_t i = 0 ; i < procCnt ; ++ i )
		{
			...
			else if ( vertType == eVertexType_Arc )	// the segment is an arc.
			{
...
bool VWPolygon2D::GetClosestPtOnPolygon(const VWPoint2D& pt, double& outSqrDist, Sint32& outOnSegIndex, VWPoint2D& outClosestPt) const
{
	// get closest projection.
	bool		isFound		= this->GetClosestProjPtOnPoly( pt, outSqrDist, outOnSegIndex, outClosestPt );

	if ( ! isFound )		// projection not found.
	{
		// get the closest polygon vertex.
		outOnSegIndex		= (Sint32) this->GetClosestPolyVertex( pt, outSqrDist );

		// validate.
		VWFC_ASSERT( outOnSegIndex != -1 );	// not expected.

		if ( outOnSegIndex != -1 )
		{
			outClosestPt	= farrVertices[ outOnSegIndex ].pt;

			// correct 'outOnSegIndex' without changing the logic.
			if ( outOnSegIndex == farrVertices.size() - 1 )
			{
				-- outOnSegIndex;
			}
...
bool VWPolygon2D::EqualishTo(const VWPolygon2D& poly) const
{
	size_t	thisCnt	= this->GetVertexCount();
	size_t	polyCnt	= poly.GetVertexCount();

	bool	bEqual	= true;
	if ( thisCnt == polyCnt ) {

		// find positions coincidence
		bool	bPosCoincide	= false;
		size_t	thisSupl, polySupl;
		for ( size_t i = 0 ; i < thisCnt ; i ++ ) {
			for ( size_t j = 0 ; j < thisCnt ; j ++ ) {

				const VWPoint2D&	pt1	= this->GetVertexAt( i );
				const VWPoint2D&	pt2	= poly.GetVertexAt( j );
				if ( MathUtils::Equalish( pt1, pt2, VWPoint2D::sEpsilon ) ) {
					thisSupl		= i;
					polySupl		= j;
					bPosCoincide	= true;
					break;
		...
		if ( bPosCoincide && thisCnt > 0 ) {
			for ( size_t i = 0 ; i < thisCnt ; i ++ ) {

				// corresponding indices
				size_t	thisIndex	= ( i + thisSupl ) % thisCnt;
				size_t	polyIndex	= ( i + polySupl ) % thisCnt;

				// check types
				EVertexType	vType1	= this->GetVertexType( thisIndex );
				EVertexType	vType2	= poly.GetVertexType( polyIndex );
				if ( vType1 != vType2 ) {
					bEqual	= false;
					break;
				}
				...
				if ( vType1 == eVertexType_Arc && 
					this->GetArcVertexRadius( thisIndex ) != poly.GetArcVertexRadius( polyIndex ) ) {
					bEqual	= false;
					break;
				}
				// check positions
				if ( ! MathUtils::Equalish( this->GetVertexAt( thisIndex ), poly.GetVertexAt( polyIndex ), VWPoint2D::sEpsilon ) ) {
					bEqual	= false;
					break;
				}
			}
	...
	if ( bboxPoly.IsPtInside( inPt, tolerance ) )
	{
		...
		if ( polyPtCnt > 1 )		// polygon has at least two points (no-empty).
		{
			...
			if ( ! isInOrNear )					// still not known.
			{
				...
				if ( outWithinPoly )			// point is within and not near the polygon.
				{
					isInOrNear					= true;

					// find the closest projection on the polygon.
					double			sqrDistToProj;
					if ( ! this->GetClosestProjPtOnPoly( inPt, sqrDistToProj, outOnSegIndex, outClosestPtOnPoly ) )
					{
						// to be here means that the point (which is within the polygon) cannot be projected. 
						ASSERTN( kVStanev, false );			// not expected. check !

						isInOrNear				= false;	// we can't rely on the results.
						outWithinPoly			= false;
					}
				}
			}
					...
					for ( size_t j = 1 ; j <= len ; ++ j )	// loop 'this poly' segments.
					{
						...
						if ( onSegments && ! parallel )		// intersection between non-parallel line segments.
						{
							...
							else
							{
								...
								{
									if ( ! fbIsClosed && ( j == len ) )		// touch is at the last vertex in 'this poly'.
									{
										// XXX Bobi: special case - touch is at the last vertex of an open poly.
										isTouching			= true;

										isDecided			= true;			// estimation is made.
									}
									else									// touch is a vertex in 'this poly'.
									{
								...
								{
									if ( ! isPolyClosed && ( i == polyLen - 1 ) )	// touch is at the last vertex in 'poly'.
									{
										// XXX Bobi: special case - touch is at the last vertex of an open poly.
										isTouching			= true;
									...
									else									// touch is a vertex in 'poly'.
									{
										// collect the suspect touch as it might not be caught at the next step due to imperfect epsilon check.
...
void VWPolygon2D::GetPolygonized(	VWPolygon2D& outPolygon,
									...
									std::function<void(size_t inpuIndex, size_t outputIndexStart, size_t outputIndexEnd)> callbackAddSegment) const
{
	...
	{
		// validate: coincident points must have same vertex types.
		ASSERTN( kVStanev, ( arrVertices[ 0 ].type == arrVertices[ ptsCnt - 1 ].type ) );
	...
	if ( isClosed && ( thisVert.type != eVertexType_Corner ) )	// the polyline is closed and start vertex is not corner.
	{
		...
		this->PolygonizeSection( lastVert, thisVert, nextVert, outPolygon, interpolate, splineRecusLevel, arcDegRes );
...
void VWPolygon2D::PolygonizeSection(	const SExtVertex& lastVert, const SExtVertex& thisVert, const SExtVertex& nextVert,
										...
										bool	interpolate		/*=false*/,
										short	splineRecusLevel/*=kPolygonize_MaxRecursLevel*/,
										double	arcDegRes		/*=kPolygonize_ArcResolution*/) const
{
		...
		case eVertexType_Radius:
			{
				...
				{
					// polygonize the arc.
					this->PolygonizeArc( lastPt, thisVert.pt, nextPt, thisVert.dArcRadius, outPolygon );
				}
				else if ( thisVert.type == eVertexType_Radius )
				{
					...
					arc.GetPolygon( arcPolygon, arcDegRes );			// polygonize.
...
void VWPolygon2D::PolygonizeArc(const VWPoint2D& lastPt, const VWPoint2D& thisPt, const VWPoint2D& nextPt, double radius, VWPolygon2D& outPolygon) const
{
	VWPoint2D	lastCam;
	VWPoint2D	thisCam;
	VWPoint2D	center;
	...
	{
		this->PolygonizeArc( lastPt, nextPt, radius, lastCam, thisCam, center, theStart, theArc, outPolygon );
	}
}

void VWPolygon2D::PolygonizeArc(const VWPoint2D &lastPt, const VWPoint2D& nextPt, double radius, VWPoint2D& lastCam, VWPoint2D& thisCam, VWPoint2D& center, double theStart, double theArc, VWPolygon2D& outPolygon) const
{
	if ( lastPt != lastCam )
	{
	...
	if ( theArc )
	{
		this->PolygonizeArcSegment( theStart, theArc, center, radius, outPolygon );
	}
...
void VWPolygon2D::PolygonizeArcSegment(double inStart, double inArc, const VWPoint2D& cen, double radius, VWPolygon2D& outPolygon) const
{
	if ( inArc < 0 )
	{
		inStart			+= 360.0;
												...
												VWPoint2D& outIntersPt, bool& outCanMove)
{
	...
	const VWPoint2D&		vecP1P0		= ( p0 - p1 ).Normalize();
...
bool VWPolygon2D::GetSegment(size_t segIndex, VWLine2D& outSegLine, VWArc2D& outSegArc, bool& outIsArc, bool& outIsLine, bool recursion)
{
	bool				isFound		= false;
...
bool VWPolygon2D::GetArcFromArcPolyAt(size_t at, VWArc2D& outArc)
{
	bool				isArc		= false;
