...
//	fOnState_Slab2ObjIndex		= pContext->AcceptState( "Slab" );
	...
	pContext->AcceptStateInternal( "SampleUpdaterObj" );
...
void CUpdateSupport::OnStateInternal(IObjUpdateSupportContext* theContext, const ObjectState& state)
{
	MCObjectHandle			hObject		= theContext->GetObject();

	TXString	msg;
	msg.Format( "SampleUpdaterObj: state INTERNAL: %d  h: 0x%X", state.fSpecifier, hObject );
	gSDK->AlertInform( "CUpdateSupport::OnStateInternal", msg, false, "sample", "CUpdateSupport::OnStateInternal" );
